...
| `GALAXY_ENABLE_API_ACCESS_LOG`      | Enable gathering of logs, Default: `False` |
| `CONNECTED_DISTRONODE_CONTROLLERS`      | List of controllers connected , Default: `[]` |
...
| `DISTRONODE_API_HOSTNAME`      | Hostname for the Distronode API, Default: `same as PULP_CONTENT_ORIGIN` |
...
- `DISTRONODE_URL_NAMESPACE` (defaults to fixed value `"galaxy:api:v3:"`)
- `DISTRONODE_DEFAULT_DISTRIBUTION_PATH` (defaults to value of `GALAXY_API_DEFAULT_DISTRIBUTION_BASE_PATH`)

### Enable collection download log

To log collection download set `DISTRONODE_COLLECT_DOWNLOAD_LOG=True`. Logged downloads can be viewed with `pulpcore-manager download-log`
