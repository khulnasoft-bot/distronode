...
When a collection is imported, a row is created in the distronode_collectionversion table with metadata about the artifact.

After import, pulp_distronode runs a function to determine the highest semantic version for the related collection versions. Related rows are then updated with is_hight=True|False.
...
https://github.com/pulp/pulp_distronode/blob/main/pulp_distronode/app/migrations/0046_add_fulltext_search_fix.py


```sql
galaxy_ng=# select namespace,name,version,search_vector from distronode_collectionversion where namespace='zabbix';
...
https://github.com/pulp/pulp_distronode/blob/93e330dca8accfa438a615b6be2194c169655e7c/pulp_distronode/app/galaxy/v3/filters.py#L152-L161
