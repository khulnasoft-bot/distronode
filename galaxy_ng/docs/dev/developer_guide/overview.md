...
Pulp is a pluggable library for managing software packages. It has the ability to store software (such as distronode collections and container images), sync them from remote sources (such as galaxy.distronode.com and quay.io) and organize them into repositories. We make use of the following pulp projects:
...
- [pulp_distronode](https://github.com/pulp/pulp_distronode/): this is a pulp plugin that adds support for distronode collections and roles. This is the primary plugin that galaxy_ng makes use of.
...
Galaxy NG itself is just another pulp plugin, albeit a strange one. Rather than providing any content types of it's own, like all the other pulp plugins do, Galaxy NG integrates existing pulp plugins into a more cohesive Distronode user experience.
