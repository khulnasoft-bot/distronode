...
- Provide interfaces for content consumption. Some clients, such as `distronode-galaxy`, require a
  ...
  pull together data from several pulp models.
    - Examples:
        ...
        - Collection search
        - Collection downloads - provides the APIs needed to drive the `distronode-galaxy` CLI.
- Provide client operations.
    - Examples:
        - Token generation. Pulpcore doesn't support API tokens, but they are needed by `distronode-galaxy`.
