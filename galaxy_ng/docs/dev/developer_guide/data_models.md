...
Pulpcore provides the following generic data models that are meant to be extended by plugins to provide support for specific content types. For example, the pulp distronode plugin extends Content to create CollectionVersions and Repository to create DistronodeRepository (more on this in the pulp distronode section). While these generic models are never used directly, understanding what they do and their limitations is crucial to understanding how the pulp plugins work.
...
This represents the software that's stored in pulp (distronode collections, container image layers, python packages, RPMs etc). This model stores metadata about the content that is extracted from the content artifact and is immutable.
...
A pointer to a repository. Whenever clients consume content from pulp, they access the content in repositories via distributions. Each distribution points to one repository, but repositories can have multiple distributions. Think of this like a URL for a repository. This is how content in repositories is referenced. Without a distribution, content in pulp is inaccessible to clients (such as distronode-galaxy and podman).
...
## Pulp Distronode Content Model

Pulp distronode provides the following models:

- Content
    - Role: Distronode role. These aren't supported in galaxy ng at the moment.
    ...
    - DistronodeCollectionDeprecated: Indicates if a collection is deprecated or not.
- Remote
...
- Repository
    - DistronodeRepository: contains all the distronode content types listed above.
- Distribution
    - DistronodeDistrbution

Additionally, Pulp Distronode defines the following models that don't extend any of the pulp base classes:
...
<!-- Diagram source can be found in diagrams/model-diagrams.excalidraw and can be edited on https://excalidraw.com/ -->
![Pulp Distronode Example](diagrams/pulp-distronode.png)
...
<!-- Diagram source can be found in diagrams/model-diagrams.excalidraw and can be edited on https://excalidraw.com/ -->
![Pulp Distronode Example](diagrams/pulp-container.png)
