...
1. https://beta-galaxy.distronode.com/
2. https://beta-galaxy-stage.distronode.com/
3. https://beta-galaxy-dev.distronode.com/

The "dev" instance is auto-deployed from new commits to the master branch of https://github.com/distronode/distronode/tree/devel/galaxy_ng. Staging and Production deployments are triggered manually or via new tags on the git repository.

All instances are wired up for github social auth, so you can log into any of them with your github account.

## Logging in

Visit https://beta-galaxy.distronode.com/ in any modern web browser. Once the site loads, click on the "login" link
at the top right of the page. On your first attempt, you'll be brought to a github authorization page. This is
to be expected because github social auth is the default and only supported login mechanism in https://galaxy.distronode.com and for the future replacement. Once authorized, you should be redirected back to the main page and see your github username replace the word "login" at the top right of the page.
...
## Distronode Core CLI setup

The distronode-galaxy cli is configured by default to talk to https://galaxy.distronode.com. To configure distronode core to talk 
to the beta galaxy_ng server, create an distronode.cfg file with the following content.
...
[galaxy_server.beta]
url = https://beta-galaxy.distronode.com/api/
...
The v1 rest api has filter parameter support similar to the current code on https://galaxy.distronode.com.

```bash
# Find all roles created by a specific github user.
curl 'https://beta-galaxy.distronode.com/api/v1/roles/?github_user=geerlingguy'

# Find all roles containing a keyword in the namespace, name or description.
curl 'https://beta-galaxy.distronode.com/api/v1/roles/?keyword=docker'
```

### CLI

The distronode-galaxy CLI also supports role search

```bash
distronode-galaxy role search --help
```
...
Future support for collection search in the distronode-galaxy CLI is [TBD](https://issues.redhat.com/browse/AAH-1968).


## Installing Roles

Role installs should work as they did before with https://galaxy.distronode.com

`distronode-galaxy role install geerlingguy.docker`


## Installing Collections

Collection installs should work as they did before with https://galaxy.distronode.com

`distronode-galaxy collection install community.general`


## Importing Roles

Role imports are only supported through the distronode galaxy cli. Once a valid distronode.cfg is setup, run the `distronode-galaxy role import` command to import one of your roles hosted on https://github.com

`distronode-galaxy role import <github_user> <github_repo>`
...
Roles can be deleted either via the distronode-galaxy CLI or with any http client (curl for example).

### distronode-galaxy CLI

`distronode-galaxy role delete <github_user> <github_repo>`

### http client

First, find the exact role id you'd like to delete.

`curl https://galaxy.distronode.com/api/v1/roles/?owner__username=<NAMESPACE_NAME>&name=<ROLE_NAME>`

Now issue a DELETE call to the role via it's full url.

`curl -X DELETE -H 'Authorization: token <TOKEN>' https://galaxy.distronode.com/api/v1/roles/<ROLE_ID>/`
...
In many cases the FQN matches the `github_user` and `github_repo`, but there are many exceptions. For example, many roles are hosted in github repositories named `distronode-role-<ROLENAME>`. At import time, the backend server will auto-trim the `distronode-role-` prefix to create the role's true name. There are also cases where an admin created a custom namespace in the old galaxy for a user and then through the UI a role was imported into that specific namespace, because the CLI wouldn't have been able to handle it. There are also cases where the github_user or github_repo were renamed on github which lead to an FQN that no longer matched those attributes.
...
6. Re-import the roles from their new location. `distronode-galaxy role import <new_github_user> <reponame>`
...
3. Re-import the role from the new location. `distronode-galaxy role import <github_user> <new_repository_name>`

### Changing the role name

There are two ways to accomplish this and both involve re-importing the role.

A. `distronode-galaxy role import --role-name=<NEW_NAME> <github_user> <github_repo>`
B. Alter the `name` field in the role's `meta/main.yml` file and then import `distronode-galaxy role import <github_user> <github_repo>`
...
Collections can be uploaded either by the distronode galaxy CLI or in the GalaxyNG web interface.

### Galaxy CLI

`distronode-galaxy collection publish --help`
...
```shell
curl -H 'Authorization: token <yourtoken>' -X DELETE https://galaxy.distronode.com/ui/standalone/namespaces/<number>/
```
