...
$ curl -X POST \
    ...
    https://galaxy-dev.distronode.com/api/_ui/v1/namespaces/
```
...
Social auth in galaxy_ng has been heavily customized per https://github.com/distronode/distronode/tree/devel/galaxy_ng/pull/1881

A "legacy" namespace is the api/v1 style namespace that allows any valid github username as the name.

A "v3" namespace is the api/v3 and pulp related namespace which is restricted to allowable python package names because of v3 being solely focused on distronode collections.
...
```
curl https://galaxy-dev.distronode.com/api/v1/namespaces/?name=Wilk42 | jq .
```

Check the provider namespace ...
```
$ curl -s https://galaxy-dev.distronode.com/api/v1/namespaces/?name=Wilk42 | jq .results[0].summary_fields.provider_namespaces
[
  {
    "id": 19193,
    "name": "wilk42",
    "pulp_href": "/api/pulp/api/v3/pulp_distronode/namespaces/19193/"
  }
]
```
...
$ curl -X POST \
    ...
    -H 'Content-Type: appliction/json' \
    -d '{"id": 19192}' \
    https://galaxy-dev.distronode.com/api/v1/namespaces/7532/providers/
```

Check the owners ...
```
$ curl -s https://galaxy-dev.distronode.com/api/v1/namespaces/?name=Wilk42 | jq .results[0].summary_fields.
owners
[
  {
    "id": 7184,
...
$ curl -X POST \
    ...
    https://galaxy-dev.distronode.com/api/v1/namespaces/7532/owners/
```
...
$ curl -X DELETE \
    -H 'Authorization: token <TOKEN>' \
    https://galaxy-dev.distronode.com/api/v1/roles/<roleid>/
```
