...
This guide is intended to walk you through the basics of managing collections in Galaxy NG. For information on writing and using collections within the broader distronode ecosystem, please refer to the documentation for:
- [Using collections](https://docs.distronode.com/distronode/latest/dev_guide/developing_collections.html#developing-collections)
- [Developing collections](https://docs.distronode.com/distronode/latest/dev_guide/developing_collections.html#developing-collections)
...
Before you can do this you need to [configure the distronode-galaxy CLI to connect to Galaxy](#configuring-the-distronode-galaxy-cli).

To upload a collection run the following `distronode-galaxy collection publish` command:

```shell
$ distronode-galaxy collection publish newswangerd-main_collection-1.0.3.tar.gz
...
!!! note
    You will need the "Modify Distronode repo content" permission to access the Approval dashboard. Make sure you have an account with the appropriate role.
...
```shell
distronode-galaxy collection install namespace.name:1.0.3
```
...
Collections can be synced from galaxy.distronode.com or console.redhat.com (with an AAP subscription from Red Hat). For information on syncing certified collections, refer to the Red Hat product documentation.
...
## Configuring the distronode-galaxy CLI

Check out the [distronode-galaxy CLI docs](https://docs.distronode.com/distronode/latest/user_guide/collections_using.html#configuring-the-distronode-galaxy-client) for more in depth information on configurating the CLI.
...
Not all of these are required. If you don't have any collections synced from galaxy.distronode.com or console.redhat.com, you may only need to configure the `published` repository. `staging` and `rejected` won't be useful to most users.

### Configure upload repositories

To upload a collection you have to have one of two things configured in your distronode.cfg file:
