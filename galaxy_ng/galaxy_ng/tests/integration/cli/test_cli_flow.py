...
from ..utils import distronode_galaxy
...
def test_publish_newer_version_collection(galaxy_client, uncertifiedv2):
    ...
    install_pid = distronode_galaxy(
        ...
        # distronode_config=distronode_config("basic_user"),
        galaxy_client=gc,
        cleanup=False,
        check_retcode=False
    )
...
def test_publish_same_collection_version(distronode_config, galaxy_client):
    ...
    distronode_galaxy(
        ...
        distronode_config=distronode_config("admin")
    )
    p = distronode_galaxy(
        f"collection publish {collection.filename}",
        check_retcode=1,
