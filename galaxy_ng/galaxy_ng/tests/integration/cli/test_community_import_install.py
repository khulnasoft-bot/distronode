...
def test_import_install_role_as_admin(distronode_config):
    ...
    cleanup_social_user('jctannerTEST', distronode_config)
    cleanup_social_user('jctannertest', distronode_config)
...
@pytest.mark.deployment_community
def test_import_install_role_as_not_admin(distronode_config):
    """Tests installing a role that was imported by an owner that is
    not the same as the role's github_user."""
    ...
    test_config = distronode_config(github_user)
    ...
    user_1_config = distronode_config("github_user_1")
    ...
    with tempfile.TemporaryDirectory() as roles_path:
        install_pid = distronode_galaxy(
            f"role install -p {roles_path} {github_user}.{role_name}",
            distronode_config=user_1_config,
            token=token,
            force_token=True,
            cleanup=False,
            check_retcode=False
...
@pytest.mark.deployment_community
def test_import_owner_not_github_user(distronode_config):
    ...
    # Login as github_user_1.
    gh1_config = distronode_config("github_user_1")
