...
from .constants import USERNAME_PUBLISHER, GALAXY_STAGE_DISTRONODE_PROFILES
from .utils import (
    distronode_galaxy,
    build_collection,
    get_client,
...
from .utils.iqe_utils import (
    ...
    is_standalone,
    is_ephemeral_env,
    galaxy_stage_distronode_user_cleanup, remove_from_cache,
    get_distronode_config, get_galaxy_client, DistronodeConfigFixture, get_hub_version, aap_gateway,
...
cli: Tests that shell out to the real distronode-galaxy cli.
...
galaxy_stage_distronode: tests that run against galaxy-stage.distronode.com
...
def distronode_config():
    return get_distronode_config()


@pytest.fixture
def published(distronode_config, artifact, galaxy_client):
    ...
    # certify
    hub_4_5 = is_hub_4_5(distronode_config)
    set_certification(distronode_config(), gc, artifact, hub_4_5=hub_4_5)

    return artifact


@pytest.fixture
def certifiedv2(distronode_config, artifact, galaxy_client):
    ...
    set_certification(distronode_config(), gc, artifact2, hub_4_5=hub_4_5)

    return (artifact, artifact2)


@pytest.fixture
def uncertifiedv2(distronode_config, artifact, settings, galaxy_client):
    ...
    dest_url = (
        f"v3/plugin/distronode/content/staging/collections/index/"
...
@pytest.fixture
def auto_approved_artifacts(distronode_config, artifact, galaxy_client):
    """ Create and publish collection version N and N+1"""

    # make sure the expected namespace exists ...
    config = distronode_config("partner_engineer")
    gc = galaxy_client("partner_engineer")
    create_namespace(gc, artifact.namespace, "")

    # publish
    config = distronode_config("basic_user")
    distronode_galaxy(
        f"collection publish {artifact.filename}",
        distronode_config=config
    )

    dest_url = (
        f"v3/plugin/distronode/content/published/collections/index/"
...
def cleanup_collections():
    ...
    path = os.path.expanduser(f"~/.distronode/collections/distronode_collections/{USERNAME_PUBLISHER}/")
    if os.path.exists(path):
        shutil.rmtree(path)


@pytest.fixture(scope="session")
def galaxy_client(distronode_config):
    return get_galaxy_client(distronode_config)


def pytest_sessionstart(session):
    distronode_config = get_distronode_config()
    hub_version = get_hub_version(distronode_config)
    if not is_standalone() and not is_ephemeral_env() and not is_dev_env_standalone():
        set_test_data(distronode_config, hub_version)
...
def pytest_runtest_setup(item):
    ...
    hub_version = get_hub_version(get_distronode_config())
...
def sync_instance_crc():
    ...
    - contain the test fixture credentials from DistronodeConfigFixture
    ...
    config = DistronodeConfigFixture(url=url, auth_url=auth_url, profile="org_admin")
    manifest = []
...
def set_test_data(distronode_config, hub_version):
    role = "admin"
    gc = GalaxyKitClient(distronode_config).gen_authorized_client(role)
    if not aap_gateway():
        ...
        if parse_version(hub_version) < parse_version('4.6'):
            pe_permissions = ["galaxy.view_group",
                              ...
                              "galaxy.add_user",
                              "galaxy.change_user",
                              "distronode.delete_collection",
                              "galaxy.delete_namespace",
                              "galaxy.add_namespace",
                              "distronode.modify_distronode_repo_content",
                              "distronode.view_distronoderepository",
...
@pytest.fixture(scope="session")
def hub_version(distronode_config):
    return get_hub_version(distronode_config)


@pytest.fixture
def gh_user_1_post(distronode_config):
    """
    ...
    after the test
    """
    gc = get_galaxy_client(distronode_config)
    yield gc("github_user", github_social_auth=True)
    galaxy_stage_distronode_user_cleanup(gc, "github_user")
    remove_from_cache("github_user")


@pytest.fixture
def gh_user_1(distronode_config):
    """
    Returns a galaxy kit client with a GitHub user logged into beta galaxy stage
    """
    gc = get_galaxy_client(distronode_config)
...
@pytest.fixture
def gh_user_2(distronode_config):
    """
    Returns a galaxy kit client with a GitHub user logged into beta galaxy stage
    """
    gc = get_galaxy_client(distronode_config)
...
@pytest.fixture
def gh_user_1_pre(distronode_config):
    """
...
@pytest.fixture
def gw_user_1(distronode_config):
    """
    Returns a galaxy kit client with a GitHub user logged into beta galaxy stage
    """
    gc = get_galaxy_client(distronode_config)
...
@pytest.fixture
def generate_test_artifact(distronode_config):
    """
    Generates a test artifact and deletes it after the test
    """
    github_user_username = GALAXY_STAGE_DISTRONODE_PROFILES["github_user"]["username"]
...
def min_hub_version(distronode_config, spec):
    version = get_hub_version(distronode_config)
    return Requirement.parse(f"galaxy_ng<{spec}").specifier.contains(version)


def max_hub_version(distronode_config, spec):
    version = get_hub_version(distronode_config)
    return Requirement.parse(f"galaxy_ng>{spec}").specifier.contains(version)


def is_hub_4_5(distronode_config):
    hub_version = get_hub_version(distronode_config)
    return parse_version(hub_version) < parse_version('4.6')


def is_hub_4_7_or_higher(distronode_config):
    hub_version = get_hub_version(distronode_config)
...
@pytest.fixture(scope="session")
def skip_if_ldap_disabled(distronode_config):
    config = distronode_config("admin")
