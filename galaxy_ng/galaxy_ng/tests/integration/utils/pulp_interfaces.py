...
class DistronodeDistroAndRepo(PulpObjectBase):
    ...
    def create(self):
        self._repo = self.client(
            f"{self.api_prefix}pulp/api/v3/repositories/distronode/distronode/",
            args=self._repo_body,
            method="POST"
        )

        resp = self.client(
            f"{self.api_prefix}pulp/api/v3/distributions/distronode/distronode/",
            args={
