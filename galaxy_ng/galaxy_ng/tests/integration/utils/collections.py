...
from distronode.galaxy.api import _urljoin
...
from .iqe_utils import get_distronode_config, get_galaxy_client
...
def build_collection(
    ...
    Use the distronode-galaxy cli to init and build a collection artifact.
    ...
    Args:
        ...
        requires_distronode: this yaml'ized string will go into meta/requirements.yml
    ...
    with tempfile.TemporaryDirectory(prefix='collection-build-') as tdir:
        ...
        # init the skeleton ...
        cmd = f"distronode-galaxy collection init {config['namespace']}.{config['name']}"
        pid = subprocess.run(
            cmd,
            shell=True,
            cwd=tdir,
        ...
        if roles:
            for role_name in roles:
                cmd = f"distronode-galaxy role init {role_name}"
                pid2 = subprocess.run(
                    cmd,
                    shell=True,
                    cwd=rolesdir,
        ...
        # build it
        cmd = "distronode-galaxy collection build ."
        pid3 = subprocess.run(
            cmd,
            shell=True,
            cwd=basedir,
...
def upload_artifact(
    ...
    :param config: The distronodeconfig object.
...
def get_collections_namespace_path(namespace):
    ...
    return os.path.expanduser(f"~/.distronode/collections/distronode_collections/{namespace}/")
...
def set_certification(config, gc, collection, level="published", hub_4_5=False):
    ...
    if config["upload_signatures"]:
        ...
        rep_obj_url = "pulp/api/v3/repositories/distronode/distronode/?name=staging"
        ...
        artifact_pulp_href = (
            ...
            + _urljoin(
                ...
                "pulp/api/v3/content/distronode/collection_versions/",
                artifact_pulp_id,
            )
            + "/"
        )
        ...
        sig_url = "pulp/api/v3/content/distronode/collection_signatures/"
    ...
    dest_url = (
        f"v3/plugin/distronode/content/{level}/collections/index/"
        f"{collection.namespace}/{collection.name}/versions/{collection.version}/"
    )
    return wait_for_url(gc, dest_url)
...
def copy_collection_version(client, collection, src_repo_name, dest_repo_name):
    ...
    # callers expect response as part of this method, ensure artifact is there
    dest_url = (
        f"v3/plugin/distronode/content/{dest_repo_name}/collections/index/"
        f"{collection.namespace}/{collection.name}/versions/{collection.version}/"
    )
    return wait_for_url(gc, dest_url)
...
def get_all_repository_collection_versions(gc):
    ...
    for repo in repositories:
        next_page = (f'{gc.galaxy_root}content/{repo}/'
                     f'v3/plugin/distronode/content/{repo}/collections/index/')
        while next_page:
            resp = gc.get(next_page)
    ...
    while True:
        ...
        for delkey in delkeys:
            ...
            with contextlib.suppress(GalaxyError):
                resp = api_client(
                    (f'{api_prefix}/v3/plugin/distronode/content'
