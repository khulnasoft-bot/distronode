...
from .client_distronode_lib import get_client
...
def clean_all_roles(distronode_config):
...
def cleanup_social_user(username, distronode_config):
    ...
    cleanup_namespace(namespace_name, api_client=get_client(config=distronode_config("admin")))

    # cleanup the group
    delete_group(username, api_client=get_client(config=distronode_config("admin")))
    delete_group(
        'namespace:' + namespace_name,
        api_client=get_client(config=distronode_config("admin"))
    )

    # cleanup the user
    delete_user(username, api_client=get_client(config=distronode_config("admin")))
...
class LegacyRoleGitRepoBuilder:
    ...
    def role_init(self):
        cmd = f'distronode-galaxy role init {self.namespace}.{self.name}'
