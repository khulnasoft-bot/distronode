...
from galaxy_ng.tests.integration.constants import GALAXY_STAGE_DISTRONODE_PROFILES, \
...
from distronode.galaxy.token import BasicAuthToken
from distronode.galaxy.token import GalaxyToken
from distronode.galaxy.token import KeycloakToken
...
class GalaxyKitClient:
    def __init__(self, distronode_config, custom_config=None, basic_token=None):
        self.config = custom_config if custom_config else distronode_config
    ...
    def gen_authorized_client(
        ...
        *,
        ...
        # role can be either be the name of a user (like `distronode_insights`)
    ...
    if cache_key not in token_cache or ignore_cache:
        ...
        if token_value:
            ...
            if basic_token:
                ...
                with patch("distronode.context.CLIARGS", {"ignore_certs": True}):
...
def is_galaxy_stage():
    return "galaxy-stage.distronode" in os.getenv(
        "HUB_API_ROOT", "http://localhost:5001/api/automation-hub/"
    )
...
def is_ocp_env():
    ...
    return ("ocp4.testing.distronode.com" in os.getenv(
        "HUB_API_ROOT", "http://localhost:5001/api/automation-hub/")
...
def galaxy_stage_distronode_user_cleanup(gc, u):
    gc_admin = gc("admin")
    github_user_username = GALAXY_STAGE_DISTRONODE_PROFILES[u]["username"]
...
def get_distronode_config():
    return DistronodeConfigFixture


def get_galaxy_client(distronode_config):
    """
    ...
    """
    galaxy_kit_client = GalaxyKitClient(distronode_config)
    return galaxy_kit_client.gen_authorized_client


class DistronodeConfigFixture(dict):
    ...
    def __init__(self, profile=None, namespace=None, url=None, auth_url=None):
        ...
        if profile:
            if is_sync_testing():
                ...
                self.PROFILES = GALAXY_STAGE_DISTRONODE_PROFILES
            else:
        ...
        galaxy_token_fn = os.path.expanduser('~/.distronode/galaxy_token')
    ...
    def __hash__(self):
        # To avoid TypeError: unhashable type: 'DistronodeConfigFixture'
    ...
    def __repr__(self):
        return f'<DistronodeConfigFixture: {self.namespace}>'
...
@lru_cache
def get_hub_version(distronode_config):
    if aap_gateway():
        cfg = distronode_config("admin")
