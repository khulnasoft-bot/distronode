...
from ..utils import (
    ...
    get_client,
    CollectionInspector,
    wait_for_all_tasks, distronode_galaxy
)
...
def _upload_test_common(config, client, artifact, base_path, dest_base_path=None, gc=None):
    ...
    with tempfile.TemporaryDirectory() as dir_:
        ...
        url = (
            f"{api_root}v3/plugin/distronode/content/"
...
def test_publish_to_custom_staging_repo(distronode_config, artifact, settings, galaxy_client):
    if settings.get("GALAXY_REQUIRE_CONTENT_APPROVAL") is not True:
        pytest.skip("GALAXY_REQUIRE_CONTENT_APPROVAL must be true")
    config = distronode_config(profile="admin")
...
@pytest.mark.min_hub_version("4.7dev")
def test_publish_to_custom_repo(distronode_config, artifact, settings, galaxy_client):
    config = distronode_config(profile="admin")
    client = get_client(
        config=config
    )
    repo = DistronodeDistroAndRepo(
        client,
...
@pytest.mark.min_hub_version("4.7dev")
def test_publish_and_auto_approve(distronode_config, artifact, settings, galaxy_client):
...
@pytest.mark.auto_approve
@pytest.mark.min_hub_version("4.7dev")
def test_auto_approve_multiple(distronode_config, artifact, settings, galaxy_client):
    if settings.get("GALAXY_REQUIRE_CONTENT_APPROVAL"):
        pytest.skip("GALAXY_REQUIRE_CONTENT_APPROVAL must be false")
    config = distronode_config(profile="admin")
    api_prefix = config.get("api_prefix")
    ...
    custom_published_repo = DistronodeDistroAndRepo(
        client,
        f"repo-test-{generate_random_string()}",
