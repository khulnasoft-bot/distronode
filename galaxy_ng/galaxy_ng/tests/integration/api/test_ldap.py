...
@pytest.fixture
def settings(distronode_config):
...
@pytest.mark.ldap
def test_ldap_is_enabled(distronode_config, settings):
...
@pytest.mark.ldap
def test_ldap_login(distronode_config, settings):
    """test whether ldap user can login"""
...
@pytest.mark.ldap
def test_ldap_mirror_only_existing_groups(distronode_config, settings):
    ...
    config = distronode_config("ldap_non_admin")
    api_prefix = config.get("api_prefix").rstrip("/")
    api_client = get_client(config, request_token=False, require_auth=True)
