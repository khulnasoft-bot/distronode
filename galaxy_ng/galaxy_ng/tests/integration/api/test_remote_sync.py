...
def test_api_ui_v1_remote_sync(galaxy_client):
    ...
    try:
        if "Internal Server Error" in resp["error"]["description"]:
            pytest.skip("Server error on https://beta-galaxy.distronode.com/. Skipping test.")
    except TypeError:
        pass
...
def test_sync_community_with_no_requirements_file(galaxy_client):
    gc = galaxy_client("admin")
    remote = gc.get("pulp/api/v3/remotes/distronode/collection/?name=community")["results"][0]
    resp = gc.patch(
        remote["pulp_href"],
        body={
    ...
    repo = gc.get("pulp/api/v3/repositories/distronode/distronode/?name=community")["results"][0]
