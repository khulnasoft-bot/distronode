...
class TestLoadData:
    ...
    @pytest.mark.min_hub_version("4.6")
    @pytest.mark.load_data
    def test_load_collections(self, galaxy_client, data, distronode_config):
        gc = galaxy_client("admin")

        for collection in data["collections"]:
            if (collection["repository"] != "published"
                    and not is_hub_4_7_or_higher(distronode_config)):
                continue
            ...
            try:
                ...
                if collection["signed"] and not is_ocp_env():
                    ...
                    sign_collection_on_demand(
                        gc, "distronode-default", collection["repository"],
