...
def test_api_publish(artifact, use_distribution, hub_version, galaxy_client):
    ...
    if use_distribution:
        ...
        distros = gc.get("pulp/api/v3/distributions/"
                         "distronode/distronode/?name=inbound-{artifact.namespace}")

        if distros["count"] == 0:
            repo = gc.get("pulp/api/v3/repositories/distronode/distronode/?name=staging")["results"][0]
            try:
                r = gc.post("pulp/api/v3/distributions/distronode/distronode/", body={
...
@pytest.mark.all
def test_validated_publish(distronode_config, artifact, galaxy_client):
    """
    ...
    set_certification(distronode_config(), gc, artifact, level="validated")
...
@pytest.mark.skip
@pytest.mark.all
def test_api_publish_bad_hash(distronode_config, artifact, upload_artifact):
    ...
    with pytest.raises(CapturingGalaxyError) as excinfo:  # noqa: SIM117
        with patch("distronode.galaxy.api.GalaxyError", CapturingGalaxyError):
...
def test_api_publish_invalid_filename(galaxy_client, artifact, wrong_name):
    ...
    with pytest.raises(GalaxyClientError) as excinfo:  # noqa: SIM117
        with patch("distronode.galaxy.api.GalaxyError", GalaxyClientError):
...
@pytest.mark.min_hub_version("4.6dev")
@pytest.mark.all
def test_distronode_requires(distronode_config, spec, galaxy_client,
                          skip_if_require_signature_for_approval):
    """
    Test handling of POSTs to the artifact endpoint neglecting to submit a file.

    Also verifies that the collections endpoint properly returns a `requires_distronode` field,
    and that the returned field matches the collection metadata.
    """
    # GALAXY_SIGNATURE_UPLOAD_ENABLED="false" in ephemeral env
    gc = galaxy_client("partner_engineer")
    _, requires_distronode, result = spec
    artifact = build_collection(
        "skeleton",
        config={"namespace": USERNAME_PUBLISHER},
        extra_files={"meta/runtime.yml": {"requires_distronode": requires_distronode}},
    )
    ...
    if result == "completed":
        set_certification(distronode_config(), gc, artifact)
        collection_resp = get_collection(gc, artifact.namespace, artifact.name, "1.0.0")
        assert collection_resp["requires_distronode"] == requires_distronode
        ui_collection_resp = get_ui_collection(gc, "published", artifact.namespace,
                                               artifact.name, "1.0.0")
        assert ui_collection_resp["latest_version"]["requires_distronode"] == requires_distronode


@pytest.mark.stage_health
@pytest.mark.importer
@pytest.mark.all
def test_distronode_lint_exception(galaxy_client, hub_version):
    """
    Ensure that:
        * distronode-lint runs against our uploaded collection
        * the bug in https://github.com/distronode/galaxy-importer/pull/115 remains fixed.
    """
    gc = galaxy_client("admin")
    ...
    pattern = "Linting .* via distronode-lint"
    ...
    # distronode-lint stderr has a variety of unstructured output, most of which
...
@pytest.mark.importer
@pytest.mark.all
def test_distronode_lint_exception_AAH_2606(galaxy_client, hub_version):
    """
    https://issues.redhat.com/browse/AAH-2609
        - distronode-lint output is missing.
    """
    gc = galaxy_client("admin")
    ...
    expected = [
        ...
        (
            ...
            (
                "meta/runtime.yml:1: meta-runtime[unsupported-version]:"
                + " 'requires_distronode' key must refer to a currently supported version such as:"
            ),
        ),
    ...
    artifact = bc(
        ...
        extra_files={
            "meta/runtime.yml": "requires_distronode: \">=2.10\"",
...
def test_api_publish_log_missing_ee_deps(galaxy_client):
    ...
    artifact = build_collection(
        ...
        extra_files={
            "meta/runtime.yml": {"requires_distronode": ">=2.10,<2.11"},
...
def test_api_publish_ignore_files_logged(galaxy_client):
    """
    Test that galaxy-importer logs when distronode-test sanity ignore files are present.
    """
    gc = galaxy_client("admin")
    ...
    ignorefile_re = re.compile(
        "Ignore files skip distronode-test sanity tests, found ignore-2.10.txt with 2 statement"
    )
