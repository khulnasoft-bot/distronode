...
from .rbac_actions.collections import (
    ...
    # distronode repository
    view_distronode_repository,
    add_distronode_repository,
    modify_distronode_repository,
    rebuild_metadata_distronode_repository,
    sign_distronode_repository,
    sync_distronode_repository,
    delete_distronode_repository,
    ...
    # distronode repository version
    view_distronode_repository_version,
    rebuild_metadata_distronode_repository_version,
    repair_distronode_repository_version,
    delete_distronode_repository_version,

    # distronode distribution
    view_distronode_distribution,
    add_distronode_distribution,
    change_distronode_distribution,
    delete_distronode_distribution,

    # distronode collection remote
    view_distronode_remote,
    add_distronode_remote,
    change_distronode_remote,
    delete_distronode_remote,
...
OBJECT_ROLES_TO_TEST = {
    ...
    "galaxy.distronode_repository_owner": {
        # distronode repository
        view_distronode_repository,
        add_distronode_repository,
        modify_distronode_repository,
...
def _get_reusable_extras(gc):
    ...
    if len(REUSABLE_EXTRA) == 0:
        ...
        REUSABLE_EXTRA = {
            ...
            "custom_staging_repo": ReusableDistronodeRepository(
                f"repo-test-{generate_random_string()}", is_staging=True),
            "custom_repo": ReusableDistronodeRepository(
                f"repo-test-{generate_random_string()}", is_staging=False),
            "private_repo": ReusableDistronodeRepository(
...
def test_object_role_actions(role, subtests, galaxy_client):
    ...
    def _apply_roles():
        ...
        if role == "galaxy.distronode_repository_owner":
