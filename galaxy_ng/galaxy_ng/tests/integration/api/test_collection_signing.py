...
def test_collection_auto_sign_on_approval(distronode_config, flags, galaxy_client, settings):
    ...
    if settings.get("GALAXY_REQUIRE_CONTENT_APPROVAL"):
        ...
        cert_result = set_certification(distronode_config(), gc, artifact)
...
@pytest.mark.collection_move
@pytest.mark.deployment_standalone
def test_collection_move_with_signatures(distronode_config, flags, galaxy_client, settings):
...
def test_upload_signature(require_auth, flags, galaxy_client, settings):
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        os.system(
            ...
            "--import dev/common/distronode-sign.key"
        )
        ...
        with open(signature_filename, "rb") as signature_file:
            response = requests.post(
                gc.galaxy_root + "pulp/api/v3/content/distronode/collection_signatures/",
                verify=False,
                ...
                data={
                    ...
                    "signed_collection": (
                        ...
                        f"content/distronode/collection_versions/{collection_version_pk}/"
                    ),
                },
                auth=("admin", "admin"),
            )
...
def test_move_with_no_signing_service_not_superuser_signature_required(
    ...
    signature_upload_response = requests.post(
        gc_admin.galaxy_root + "pulp/api/v3/content/distronode/collection_signatures/",
        verify=False,
...
def test_move_with_signing_service(flags, galaxy_client, settings, artifact,
    ...
    signing_service = settings.get("GALAXY_COLLECTION_SIGNING_SERVICE") or "distronode-default"
    upload_task = upload_artifact(None, gc, artifact)
    wait_for_task(gc, upload_task)
