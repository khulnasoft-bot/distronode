...
# this is to be enabled when https://github.com/distronode/distronode/tree/devel/galaxy_ng/pull/1627
# is merged
...
def test_delete_ee_and_content(distronode_config, galaxy_client):
...
@pytest.mark.deployment_standalone
@pytest.mark.min_hub_version("4.7dev")
def test_shared_content_is_not_deleted(distronode_config, galaxy_client):
    gc = galaxy_client("admin")
    config = distronode_config("admin")
    container_engine = config["container_engine"]
    container_registry = config["container_registry"]
