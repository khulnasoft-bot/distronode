...
def test_logging_cid_value_in_task(galaxy_client, distronode_config):
    gc = galaxy_client("admin")
    config = distronode_config("admin")
    ans_repo = gc.get(
        "pulp/api/v3/repositories/distronode/distronode/?name=rh-certified"
    )['results'][0]

    # extract pulp_id from pulp_href
    pulp_id = ans_repo["pulp_href"].split('/distronode/distronode/')[1].rstrip('/')

    # use UIClient to get Correlation-ID from req headers
    with UIClient(config=config) as uic:
        sync_req = uic.post(
            f"pulp/api/v3/repositories/distronode/distronode/{pulp_id}/sync/",
            payload={})
...
def test_task_delete(galaxy_client):
    ...
    remote = gc.post("pulp/api/v3/remotes/distronode/collection/", body={
        "name": generate_random_string(),
        "url": "https://galaxy.distronode.com",
        "requirements_file": REQUIREMENTS_YAML
    })

    repo = gc.post("pulp/api/v3/repositories/distronode/distronode/", body={
