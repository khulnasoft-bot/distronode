...
See: https://github.com/distronode/galaxy-dev/issues/149
...
def test_token_auth(profile, galaxy_client, distronode_config):
...
@pytest.mark.galaxyapi_smoke
@pytest.mark.skip_in_gw
def test_auth_admin(galaxy_client, distronode_config):
...
@pytest.mark.galaxyapi_smoke
@pytest.mark.skip_in_gw
def test_auth_exception(galaxy_client, distronode_config):
...
def test_gateway_auth_admin_gateway_sessionid(galaxy_client):
    ...
    remove_from_cache("admin")
    with pytest.raises(GalaxyClientError) as ctx:
        gc.get("v3/plugin/distronode/content/published/collections/index/", relogin=False)
