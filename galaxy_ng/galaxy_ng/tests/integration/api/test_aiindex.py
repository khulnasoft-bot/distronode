...
@pytest.fixture
def namespace(distronode_config, galaxy_client) -> str:
...
@pytest.fixture
def pe_namespace(distronode_config, galaxy_client) -> str:
...
@pytest.fixture
def legacy_namespace(distronode_config):
...
def test_legacy_namespace_add_list_remove_aiindex(distronode_config, legacy_namespace, flags):
...
@pytest.mark.deployment_community
def test_namespace_add_list_remove_aiindex(distronode_config, namespace, pe_namespace, flags):
    """Test the whole workflow for AIindex.
    ...
    with UIClient(config=distronode_config("admin")) as client:
        # 2. Add namespace to AIIndex
        assert (
            client.post(
    ...
    # 7. Repeat step 2 with a basic user
    with UIClient(config=distronode_config("basic_user")) as uclient:  # noqa: SIM117
        # 8. Assert permission error raises
    ...
    with UIClient(config=distronode_config("partner_engineer")) as uclient:
