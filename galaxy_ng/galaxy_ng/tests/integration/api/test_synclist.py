...
def test_synclist_object_get(distronode_config):
    """Validate user can read the endpoints with synclist objects."""

    config = distronode_config("org_admin")
...
@pytest.mark.synclist
@pytest.mark.deployment_cloud
def test_synclist_object_edit(distronode_config, upload_artifact):
...
def test_edit_synclist_see_in_excludes(distronode_config, upload_artifact, settings,
...
@pytest.mark.deployment_cloud
@pytest.mark.slow_in_cloud
def test_synclist_distributions(distronode_config, upload_artifact):
    """
    ...
    results = api_client(
        "pulp/api/v3/distributions/distronode/distronode/?name__icontains=synclist")
