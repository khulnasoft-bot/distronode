...
def test_x_repo_search_acl_basic_user(uncertifiedv2, galaxy_client,
    ...
    # /api/automation-hub/v3/plugin/distronode/search/collection-versions/
    namespace = ckeys[0][0]
    name = ckeys[0][1]
...
@pytest.mark.min_hub_version("4.7dev")
def test_x_repo_search_acl_anonymous_user(distronode_config, auto_approved_artifacts):
    ...
    search_url = (
        api_prefix
        + '/v3/plugin/distronode/search/collection-versions/'
...
@pytest.mark.deployment_community
@pytest.mark.min_hub_version("4.7dev")
def test_x_repo_search_acl_social_user(distronode_config, auto_approved_artifacts):
