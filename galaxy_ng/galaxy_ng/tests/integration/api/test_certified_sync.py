...
def _assert_sync(manifest, client):
    ...
    synced_collections = {(x["namespace"], x["name"]) for x in iterate_all(
        client,
        "v3/plugin/distronode/content/rh-certified/collections/index/"
    )}
...
def _assert_namespace_sync(pah_client, crc_client, namespace):
    ...
    pah_ns = pah_client(f"v3/plugin/distronode/content/rh-certified/namespaces/{namespace['name']}")
...
@pytest.mark.sync
def test_basic_sync(sync_instance_crc, distronode_config):
...
@pytest.mark.sync
def test_synclist_sync(sync_instance_crc, distronode_config):
...
@pytest.mark.sync
def test_signed_only_sync(sync_instance_crc, distronode_config):
...
@pytest.mark.sync
def test_namespace_sync(sync_instance_crc, distronode_config):
    pah_config = distronode_config(profile="admin")
    manifest, crc_config = sync_instance_crc
