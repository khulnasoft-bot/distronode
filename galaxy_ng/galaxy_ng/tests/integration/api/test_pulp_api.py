...
def test_pulp_api_redirect(galaxy_client):
    ...
    response = gc.get("pulp/api/v3/distributions/distronode/distronode/")
    assert response["count"] > 0

    # verify query params work
    response = gc.get("pulp/api/v3/distributions/distronode/distronode/?name=published")
...
@pytest.mark.parametrize(
    "url",
    [
        "pulp/api/v3/repositories/distronode/distronode/",
        "pulp/api/v3/roles/",
    ],
)
@pytest.mark.pulp_api
...
def test_gw_pulp_task_endpoint(galaxy_client, distronode_config):

    gc = galaxy_client("ee_admin")

    name = create_emtpy_local_image_container(distronode_config("admin"), gc)
...
def test_pulp_task_endpoint(distronode_config, local_container, require_auth):
    name = local_container.get_container()['name']
    config = distronode_config("ee_admin")
