...
def _create_distronode_repo_common(user, password, expect_pass):
    response = requests.post(
        f"{PULP_API_ROOT}repositories/distronode/distronode/",
        json={
            "pulp_labels": {},
            "name": f"repo_distronode-{gen_string()}",
            "description": "foobar",
            "gpgkey": "foobar"
        },
        auth=(user, password),
...
def _create_distronode_distro_common(user, password, expect_pass):
    task_response = requests.post(
        f"{PULP_API_ROOT}distributions/distronode/distronode/",
        {
...
def _create_distronode_remote_common(user, password, expect_pass):
    response = requests.post(
        f"{PULP_API_ROOT}remotes/distronode/collection/",
        json={
...
def delete_collection(user, password, expect_pass, extra):
    ...
    response = requests.delete(
        f"{API_ROOT}v3/plugin/distronode/content/staging/collections/index/{ns}/{name}/",
        auth=(user['username'], password),
    )
...
def deprecate_collections(user, password, expect_pass, extra):
    ...
    response = requests.patch(
        (
            f"{API_ROOT}v3/plugin/distronode/content/staging/collections"
...
def view_distronode_repository(user, password, expect_pass, extra):
    response = requests.get(
        f"{PULP_API_ROOT}repositories/distronode/distronode",
        auth=(user['username'], password),
    )
...
def collection_repo_add_role(user, password, expect_pass, extra):
    pulp_href = extra["custom_repo"].get_repo()['pulp_href']
    add_role_common(user, password, expect_pass, pulp_href, "galaxy.distronode_repository_owner")


def collection_repo_remove_role(user, password, expect_pass, extra):
    pulp_href = extra["custom_repo"].get_repo()['pulp_href']
    remove_role_common(user, password, expect_pass, pulp_href, "galaxy.distronode_repository_owner")


def add_distronode_repository(user, password, expect_pass, extra):
    _create_distronode_repo_common(user['username'], password, expect_pass)


def modify_distronode_repository(user, password, expect_pass, extra):
    distronode_repo = _create_distronode_repo_common(ADMIN_USER, ADMIN_PASSWORD, True).json()
    base_distronode_repo = _create_distronode_repo_common(ADMIN_USER, ADMIN_PASSWORD, True).json()

    base_distronode_repo_version = f"{base_distronode_repo['pulp_href']}versions/0/"

    response = requests.post(
        f"{SERVER}{distronode_repo['pulp_href']}modify/",
        json={"base_version": base_distronode_repo_version},
        auth=(user['username'], password),
    )
    assert_pass(expect_pass, response.status_code, 202, 403)


def rebuild_metadata_distronode_repository(user, password, expect_pass, extra):
    distronode_repo = _create_distronode_repo_common(ADMIN_USER, ADMIN_PASSWORD, True).json()

    response = requests.post(
        f"{SERVER}{distronode_repo['pulp_href']}rebuild_metadata/",
        json={
            "namespace": "foo",
            "name": "bar",
            "version": "123"
...
def sign_distronode_repository(user, password, expect_pass, extra):
    distronode_repo = _create_distronode_repo_common(ADMIN_USER, ADMIN_PASSWORD, True).json()

    sign_serv_response = requests.get(
        f"{PULP_API_ROOT}signing-services/?name=distronode-default",
    ...
    response = requests.post(
        f"{SERVER}{distronode_repo['pulp_href']}sign/",
        json={
            "content_units": ["*"],
...
def sync_distronode_repository(user, password, expect_pass, extra):
    distronode_repo = _create_distronode_repo_common(ADMIN_USER, ADMIN_PASSWORD, True).json()
    remote_response = _create_distronode_remote_common(ADMIN_USER, ADMIN_PASSWORD, True).json()

    response = requests.post(
        f"{SERVER}{distronode_repo['pulp_href']}sync/",
...
def delete_distronode_repository(user, password, expect_pass, extra):
    repo = _create_distronode_repo_common(ADMIN_USER, ADMIN_PASSWORD, True)
...
def view_distronode_repository_version(user, password, expect_pass, extra):
...
def rebuild_metadata_distronode_repository_version(user, password, expect_pass, extra):
    distronode_repo = _create_distronode_repo_common(ADMIN_USER, ADMIN_PASSWORD, True).json()

    response = requests.post(
        f"{SERVER}{distronode_repo['latest_version_href']}rebuild_metadata/",
        auth=(user['username'], password),
    )
    assert_pass(expect_pass, response.status_code, 202, 403)


def repair_distronode_repository_version(user, password, expect_pass, extra):
    repo = _create_distronode_repo_common(ADMIN_USER, ADMIN_PASSWORD, True).json()
...
def delete_distronode_repository_version(user, password, expect_pass, extra):
    repo = _create_distronode_repo_common(ADMIN_USER, ADMIN_PASSWORD, True).json()
...
def view_distronode_distribution(user, password, expect_pass, extra):
    response = requests.get(
        f"{PULP_API_ROOT}distributions/distronode/distronode/",
        auth=(user['username'], password),
    )
...
def add_distronode_distribution(user, password, expect_pass, extra):
    response = requests.post(
        f"{PULP_API_ROOT}distributions/distronode/distronode/",
        {
            "name": f"dist-test-{gen_string()}",
...
def change_distronode_distribution(user, password, expect_pass, extra):
    distronode_distro = _create_distronode_distro_common(ADMIN_USER, ADMIN_PASSWORD, True).json()

    response = requests.put(
        f"{SERVER}{distronode_distro['pulp_href']}",
        {
            "name": f"dist-test-{gen_string()}",
            "base_path": f"dist-test-{gen_string()}"
        },
...
def delete_distronode_distribution(user, password, expect_pass, extra):
    distronode_distro = _create_distronode_distro_common(ADMIN_USER, ADMIN_PASSWORD, True).json()
...
def view_distronode_remote(user, password, expect_pass, extra):
    response = requests.get(
        f"{PULP_API_ROOT}remotes/distronode/collection/",
        auth=(user['username'], password),
    )
...
def add_distronode_remote(user, password, expect_pass, extra):
    _create_distronode_remote_common(user['username'], password, expect_pass)


def change_distronode_remote(user, password, expect_pass, extra):
    distronode_remote = _create_distronode_remote_common(ADMIN_USER, ADMIN_PASSWORD, True).json()
    response = requests.put(
        f"{SERVER}{distronode_remote['pulp_href']}",
        json={
            "name": f"dist-test-{gen_string()}",
            "url": "baz.qux/api/"
        },
...
def delete_distronode_remote(user, password, expect_pass, extra):
    distronode_remote = _create_distronode_remote_common(ADMIN_USER, ADMIN_PASSWORD, True).json()
    response = requests.delete(
        f"{SERVER}{distronode_remote['pulp_href']}",
        auth=(user['username'], password),
...
def collection_remote_list_roles(user, password, expect_pass, extra):
    pulp_href = requests.get(
        f"{PULP_API_ROOT}remotes/distronode/collection/?name=community",
        auth=ADMIN_CREDENTIALS
...
def private_repo_list(user, password, expect_pass, extra):
    ...
    response = requests.get(
        f"{PULP_API_ROOT}repositories/distronode/distronode/?name={repo['name']}",
        auth=(user['username'], password),
    )
...
def private_distro_list(user, password, expect_pass, extra):
    ...
    response = requests.get(
        f"{PULP_API_ROOT}distributions/distronode/distronode/?name={distro['name']}",
        auth=(user['username'], password),
    )
...
def private_collection_version_list(user, password, expect_pass, extra):
    repo = extra["private_repo"].get_repo()
    response = requests.get(
        f"{API_ROOT}v3/plugin/distronode/search/collection-versions/?repository_name={repo['name']}",
        auth=(user['username'], password),
    )
...
def copy_collection_version(user, password, expect_pass, extra):
    ...
    collection_version_response = requests.get(
        f"{PULP_API_ROOT}content/distronode/collection_versions/?name={collection['name']}",
        auth=ADMIN_CREDENTIALS,
    ).json()
    ...
    staging_repo_resp = requests.get(
        f"{PULP_API_ROOT}repositories/distronode/distronode/?name=staging",
        auth=ADMIN_CREDENTIALS,
    ).json()
