...
@pytest.mark.all
def test_move_collection_version(distronode_config, galaxy_client):
    ...
    dest_url = (f"content/staging/v3/plugin/distronode/content/staging/collections/"
    ...
    cert_result = set_certification(distronode_config(), gc_admin, artifact, hub_4_5=hub_4_5)
...
@pytest.mark.all
def test_copy_collection_version(distronode_config, galaxy_client):
...
def test_copy_associated_content(
    ...
    # TODO(jerabekjiri): add check for distronode namespace metadata
    ...
    collection = (f'content/community/v3/plugin/distronode/content/staging/'
