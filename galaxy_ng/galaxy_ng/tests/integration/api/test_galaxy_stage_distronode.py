"""test_galaxy_stage_distronode.py - Tests that run against https://galaxy-stage.distronode.com/
"""
import logging
import subprocess
import tempfile
...
from ..utils import distronode_galaxy, wait_for_url, CollectionInspector
from ..constants import GALAXY_STAGE_DISTRONODE_PROFILES
...
from ..utils.iqe_utils import galaxy_stage_distronode_user_cleanup
from ..utils.rbac_utils import create_test_user

logger = logging.getLogger(__name__)


@pytest.mark.galaxy_stage_distronode
...
def test_me_social_with_precreated_user(galaxy_client):
    ...
    galaxy_stage_distronode_user_cleanup(galaxy_client, "github_user")
...
def test_social_auth_creates_v3_namespace_upload_cli(gh_user_1, galaxy_client,
    ...
    gc_admin = galaxy_client("admin")
    url = f"v3/plugin/distronode/content/" \
...
def test_social_download_artifact(gh_user_1, generate_test_artifact):
    ...
    with tempfile.TemporaryDirectory() as dir_:
        ...
        url = f"{gh_user_1.galaxy_root}v3/plugin/" \
              f"distronode/content/published/collections/artifacts/{filename}"
...
def test_social_auth_deprecate_collection(gh_user_1, generate_test_artifact, galaxy_client):
    ...
    r = gc.get(f"v3/plugin/distronode/content/published/collections/index/"
