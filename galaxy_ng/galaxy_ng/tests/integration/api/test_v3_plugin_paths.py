...
def test_api_v3_plugin_execution_environments_repositories_content_history(
    ...
    name = create_local_image_container(distronode_config("admin"), gc)
...
@pytest.mark.deployment_standalone
@pytest.mark.min_hub_version("4.7dev")
def test_api_v3_plugin_execution_environments_repositories(distronode_config, galaxy_client):
    ns_name = "ns_name_test_ch"
    name = "ee_name_test_ch"

    gc = galaxy_client("admin")
    name = create_local_image_container(distronode_config("admin"), gc, name=f"{ns_name}/{name}")
