...
def extract_default_config(distronode_config):
    base_cfg = distronode_config('github_user_1')
    cfg = {}
    cfg['token'] = None
...
def test_admin_can_import_legacy_roles(distronode_config):

    github_user = 'jctannerTEST'
    github_repo = 'role1'
    cleanup_social_user(github_user, distronode_config)
    cleanup_social_user(github_user.lower(), distronode_config)
...
@pytest.mark.deployment_community
def test_social_auth_v3_rbac_workflow(distronode_config):
...
@pytest.mark.deployment_community
def test_social_user_with_reclaimed_login(distronode_config):
    ...
    cleanup_social_user('Wilk42', distronode_config)
    cleanup_social_user('sean-m-sullivan', distronode_config)
    cleanup_social_user('sean_m_sullivan', distronode_config)
    cleanup_social_user('sean-m-sullivan@redhat.com', distronode_config)

    default_cfg = extract_default_config(distronode_config)
    ...
    for nsname in nsmap:
        if not nsname.lower().startswith('sean_m') and not nsname.lower().startswith('wilk42'):
            continue
        cleanup_social_user(nsname, distronode_config)
...
@pytest.mark.deployment_community
def test_social_user_sync_with_changed_login(distronode_config):

    # https://galaxy.distronode.com/api/v1/users/?username=Wilk42
    #   github_id:30054029

    # https://galaxy.distronode.com/api/v1/roles/21352/
    #   Wilk42/kerb_ldap_setup
    #   Wilk42.kerb_ldap_setup
    # https://galaxy.distronode.com/api/v1/namespaces/1838/
    #   owner: 33901:Wilk42
    ...
    cleanup_social_user('Wilk42', distronode_config)
    cleanup_social_user('wilk42', distronode_config)
    cleanup_social_user('wilk420', distronode_config)
    cleanup_social_user('Wilk420', distronode_config)
    cleanup_social_user('wilk421', distronode_config)
    cleanup_social_user('Wilk421', distronode_config)
    cleanup_social_user('sean-m-sullivan', distronode_config)
    cleanup_social_user('sean-m-sullivan@redhat.com', distronode_config)
    cleanup_social_user('30054029@GALAXY.GITHUB.UNVERIFIED.COM', distronode_config)
    ...
    #   galaxy.distronode.com shows the role as wilk42/kerb_ldap_setup
...
@pytest.mark.deployment_community
def test_rbac_utils_get_owned_v3_namespaces(distronode_config):
    pass


@pytest.mark.skip(reason='this should be unit tested')
@pytest.mark.deployment_community
def test_community_tools_urls(distronode_config):
    pass


@pytest.mark.deployment_community
def test_social_auth_no_duplicated_namespaces(distronode_config):
    ...
    cleanup_social_user('Wilk42', distronode_config)
    cleanup_social_user('wilk42', distronode_config)
    cleanup_social_user('sean-m-sullivan@findme.net', distronode_config)
    cleanup_social_user('30054029@GALAXY.GITHUB.UNVERIFIED.COM', distronode_config)
...
@pytest.mark.skip(reason='this should be unit tested')
@pytest.mark.deployment_community
def test_community_social_v3_namespace_sorting(distronode_config):
    # https://issues.redhat.com/browse/AAH-2729
...
@pytest.mark.deployment_community
def test_social_auth_access_api_ui_v1_users(distronode_config):
