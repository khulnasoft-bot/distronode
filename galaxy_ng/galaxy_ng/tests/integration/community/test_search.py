...
from ..utils import distronode_galaxy, get_client, upload_artifact, wait_for_task


pytest.skip(reason="Enable when community search feature is finished.", allow_module_level=True)


NAMESPACE_NAME = "distronode"
...
ROLE_NAME = "distronode-kubernetes-modules"
ROLE_DESCRIPTION = "Pre-release Kubernetes modules"
ROLE_TAGS = ["k8s", "kubernetes", "openshift", "modules", "api"]
GITHUB_USER = "distronode"
GITHUB_REPO = "distronode-kubernetes-modules"
BRANCH = "v0.0.1"


@pytest.fixture(autouse=True, scope="module")
def my_setup_module(distronode_config):
    """Arrange"""
    ...
    """
    {'name': 'galaxy_fake_collection', 'namespace': 'distronode', 'description':
    ...
    """
    {'name': 'distronode-kubernetes-modules', 'namespace': 'distronode',
...
def admin_client(distronode_config):
    admin_config = distronode_config("admin")
...
def test_namespace_with_sql_search(admin_client):
    ...
    namespace = admin_client("/api/_ui/v1/search/?namespace=distronode")
...
def test_name_with_sql_search(admin_client):
    """Test search."""
    name = admin_client(
        f"/api/_ui/v1/search/?search_type=sql&namespace=distronode&name={COLLECTION_NAME}"
    )
...
def test_tags_with_sql_search(admin_client):
    """Test search."""
    tag_url = "/api/_ui/v1/search/?search_type=sql&namespace=distronode"
...
def test_type_with_sql_search(admin_client):
    """Test search."""
    content_type = admin_client("/api/_ui/v1/search/?search_type=sql&namespace=distronode&type=role")
...
def test_platform_with_sql_search(admin_client):
    """Test search."""
    platform = admin_client("/api/_ui/v1/search/?search_type=sql&namespace=distronode&platform=fedora")
...
def test_deprecated_with_sql_search(admin_client):
    ...
    deprecated = admin_client(
        "/api/_ui/v1/search/?search_type=sql&namespace=distronode&deprecated=true"
    )
...
def test_keywords_with_sql_search(admin_client):
    ...
    keywords = admin_client(
        "/api/_ui/v1/search/?search_type=sql&namespace=distronode&keywords=infinidash"
    )
...
def test_sorting_with_sql_search(admin_client):
    ...
    sorting = admin_client(
        "/api/_ui/v1/search/?search_type=sql&namespace=distronode&order_by=-last_updated"
    )
...
def test_name_with_web_search(admin_client):
    """Search using vector websearch"""
    name = admin_client(f"/api/_ui/v1/search/?namespace=distronode&name={COLLECTION_NAME}")
    assert name["meta"]["count"] == 1
    assert name["data"][0]["name"] == COLLECTION_NAME
    assert name["data"][0]["namespace"] == NAMESPACE_NAME
    assert name["data"][0]["tags"] == COLLECTION_TAGS
...
def test_tags_with_web_search(admin_client):
    """Search using vector websearch"""
    tag_url = "/api/_ui/v1/search/?namespace=distronode"
    for tag in COLLECTION_TAGS:
        tag_url += f"&tags={tag}"
    tags = admin_client(tag_url)
    assert tags["meta"]["count"] == 1
...
def test_type_with_web_search(admin_client):
    """Search using vector websearch"""
    content_type = admin_client("/api/_ui/v1/search/?namespace=distronode&type=role")
    assert content_type["meta"]["count"] == 1
    assert content_type["data"][0]["name"] == ROLE_NAME
    assert content_type["data"][0]["namespace"] == NAMESPACE_NAME
    assert content_type["data"][0]["tags"] == ROLE_TAGS
...
def test_platform_with_web_search(admin_client):
    """Search using vector websearch"""
    platform = admin_client("/api/_ui/v1/search/?namespace=distronode&platform=fedora")
    assert platform["meta"]["count"] == 1
    assert platform["data"][0]["name"] == ROLE_NAME
    assert platform["data"][0]["namespace"] == NAMESPACE_NAME
    assert platform["data"][0]["tags"] == ROLE_TAGS
...
def test_deprecated_with_web_search(admin_client):
    """Search using vector websearch"""
    deprecated = admin_client("/api/_ui/v1/search/?namespace=distronode&deprecated=true")
    assert deprecated["meta"]["count"] == 0
...
def test_keywords_with_web_search(admin_client):
    """Search using vector websearch"""
    keywords = admin_client("/api/_ui/v1/search/?namespace=distronode&keywords=infinidash")
    assert keywords["meta"]["count"] == 1
    assert keywords["data"][0]["name"] == COLLECTION_NAME
    assert keywords["data"][0]["namespace"] == NAMESPACE_NAME
    assert keywords["data"][0]["tags"] == COLLECTION_TAGS
...
def test_sorting_with_web_search(admin_client):
    """Search using vector websearch"""
    sorting = admin_client("/api/_ui/v1/search/?namespace=distronode&order_by=-last_updated")
    assert sorting["meta"]["count"] == 2
    assert sorting["data"][0]["type"] == "role"
    assert sorting["data"][1]["type"] == "collection"
...
def test_compound_query_with_web_search(admin_client):
    ...
    for term in [
        "infinidash",
        "galaxy%20AND%20distronode",
        "infinidash%20OR%20java",
        "api%20-kubernetes",
    ]:
        websearch = admin_client(f"/api/_ui/v1/search/?namespace=distronode&keywords={term}")
...
def test_relevance_with_web_search(admin_client):
    ...
    keywords = admin_client(
        "/api/_ui/v1/search/?namespace=distronode" "&keywords=api%20AND%20fedora&order_by=-relevance"
    )
