...
def test_community_settings(distronode_config):
    """Tests settings are correct"""

    config = distronode_config("anonymous_user")
...
@pytest.mark.deployment_community
def test_community_feature_flags(distronode_config):
...
@pytest.mark.deployment_community
def test_me_anonymous(distronode_config):
...
@pytest.mark.deployment_community
def test_me_social(distronode_config):
    """ Tests a social authed user can see their user info """

    cleanup_social_user('gh01', distronode_config)

    cfg = distronode_config('github_user_1')
...
@pytest.mark.deployment_community
def test_social_redirect(distronode_config):
...
@pytest.mark.deployment_community
def test_me_social_with_precreated_user(distronode_config):
...
@pytest.mark.deployment_community
def test_me_social_with_v1_synced_user(distronode_config):
    """ Make sure social auth associates to the correct username """

    username = 'geerlingguy'
    real_email = 'geerlingguy@nohaxx.me'
    unverified_email = '481677@GALAXY.GITHUB.UNVERIFIED.COM'
    cleanup_social_user(username, distronode_config)
    cleanup_social_user(unverified_email, distronode_config)
    ...
    pargs = json.dumps({
        ...
        "limit": 1,
        "baseurl": "https://old-galaxy.distronode.com"
    }).encode('utf-8')
    ...
    # set the social config ...
    cfg = distronode_config(username)
...
@pytest.mark.deployment_community
def test_social_auth_creates_group(distronode_config):
...
@pytest.mark.deployment_community
def test_social_auth_creates_v3_namespace(distronode_config):
    ...
    with SocialGithubClient(config=cfg) as client:
        ...
        distronode_galaxy(
            ...
            distronode_config=distronode_config("github_user_1"),
            force_token=True,
...
@pytest.mark.deployment_community
def test_social_auth_delete_collection(distronode_config):
    ...
    with SocialGithubClient(config=cfg) as client:
        ...
        for _ in range(20):
            exists_resp = client.get(
                f"v3/plugin/distronode/content/published/collections/index/{namespace}/{name}/"
            )
...
@pytest.mark.deployment_community
def test_social_auth_deprecate_collection(distronode_config):
...
@pytest.mark.deployment_community
def test_social_auth_creates_legacynamespace(distronode_config):
...
@pytest.mark.deployment_community
def test_update_legacynamespace_owners(distronode_config):

    cleanup_social_user('gh01', distronode_config)

    # make sure user2 is created
    cfg = distronode_config('github_user_2')
    with SocialGithubClient(config=cfg) as client:
        resp = client.get('_ui/v1/me/')
        uinfo2 = resp.json()
...
@pytest.mark.deployment_community
def test_list_collections_anonymous(distronode_config):
...
@pytest.mark.deployment_community
def test_list_collections_social(distronode_config):
...
@pytest.mark.deployment_community
def test_v1_sync_with_user_and_limit(distronode_config):
...
@pytest.mark.deployment_community
def test_v1_autocomplete_search(distronode_config):
    """" Tests if v1 sync accepts a user&limit arg """
    ...
    github_user2 = '030'
    cleanup_social_user(github_user, distronode_config)
    cleanup_social_user(github_user2, distronode_config)
...
@pytest.mark.deployment_community
def test_v1_username_autocomplete_search(distronode_config):
    """"
        Tests v1 roles filtering by 'username_autocomplete' and
        distronode-galaxy role search --author works as expected.
    """
    ...
    # test correct results with distronode-galaxy role search
    cmd = [
        "distronode-galaxy"
        + " role search"
        + " --author "
...
@pytest.mark.deployment_community
def test_v1_role_pagination(distronode_config):
...
@pytest.mark.deployment_community
def test_v1_role_tag_filter(distronode_config):
    """" Tests if v1 roles are auto-sorted by created """
...
@pytest.mark.deployment_community
def test_legacy_roles_ordering(distronode_config):
...
@pytest.mark.deployment_community
def test_v1_role_versions(distronode_config):
    """
    ...
    # start the sync
    pargs = json.dumps({"github_user": "geerlingguy", "role_name": "distronode"}).encode('utf-8')
    resp = api_client('/api/v1/sync/', method='POST', args=pargs)
    assert isinstance(resp, dict)
    assert resp.get('task') is not None
    wait_for_v1_task(resp=resp, api_client=api_client)

    resp = api_client('/api/v1/roles/?username=geerlingguy&name=distronode')
    assert resp['count'] == 1
    ...
    with pytest.raises(DistronodeError) as html:
