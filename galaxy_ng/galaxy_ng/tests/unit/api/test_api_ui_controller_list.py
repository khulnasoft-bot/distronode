...
class TestUiFeatureFlagsView(BaseTestCase):
    def setUp(self):
        ...
        settings.CONNECTED_DISTRONODE_CONTROLLERS = [
    ...
    def tearDown(self):
        super().tearDown()
        settings.CONNECTED_DISTRONODE_CONTROLLERS = self.original_setting
    ...
    def test_feature_flags_url(self):
        ...
        self.assertEqual(len(controller_list), len(settings.CONNECTED_DISTRONODE_CONTROLLERS))
        for controller in controller_list:
            self.assertTrue(controller['host'] in settings.CONNECTED_DISTRONODE_CONTROLLERS)
