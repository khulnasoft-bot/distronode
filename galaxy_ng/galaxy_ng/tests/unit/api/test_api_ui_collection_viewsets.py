...
def _create_repo(name, **kwargs):
    ...
    DistronodeDistribution.objects.create(name=name, base_path=name, repository=repo)
    return repo
...
class TestUiCollectionRemoteViewSet(BaseTestCase):
    def setUp(self):
        ...
        self.repository = DistronodeRepository.objects.get(name=self.remote_data["name"])
