...
from pulp_distronode.app.models import DistronodeRepository, DistronodeDistribution
...
class TestV3NamespaceViewSet(BaseTestCase):
    ...
    def test_namespace_api_doesnt_create_delete_repo(self):
        ...
        with self.settings(GALAXY_DEPLOYMENT_MODE=self.deployment_mode):
            ...
            self.assertNotIn(created_ns_name, DistronodeRepository.objects.filter(name=ns1_name))
            self.assertNotIn(created_ns_name, DistronodeDistribution.objects.filter(name=ns1_name))
    ...
    def test_unauthorized_user_cant_delete_namespace(self):
        ...
        num_existing_repos = len(DistronodeRepository.objects.all())
        num_existing_distros = len(DistronodeDistribution.objects.all())
        ...
        with self.settings(GALAXY_DEPLOYMENT_MODE=self.deployment_mode):
            ...
            self.assertEqual(num_existing_repos, len(DistronodeRepository.objects.all()))
            self.assertEqual(num_existing_distros, len(DistronodeDistribution.objects.all()))
            self.assertEqual(num_existing_distros, num_existing_repos)
