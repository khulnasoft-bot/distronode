...
from pulp_distronode.app.models import DistronodeRepository, Collection
...
class TestSignalCreateRepository(TestCase):
    def test_create_repository_ensure_retain_repo_versions(self):
        ...
        repo_name = "test"
        repository = DistronodeRepository.objects.create(name=repo_name)
    ...
    def test_when_set_not_changed_retain_repo_versions(self):
        ...
        repo_name = "test2"
        repository = DistronodeRepository.objects.create(name=repo_name, retain_repo_versions=99)
        self.assertEqual(repository.name, repo_name)
    ...
    def test_update_do_not_change_retain_repo_versions(self):
        ...
        DistronodeRepository.objects.filter(pk=repository.pk).update(name="test3_2")
        updated = DistronodeRepository.objects.get(pk=repository.pk)
