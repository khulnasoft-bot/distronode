...
class TestCreateRemoteCommand(TestCase):
    ...
    def test_remote_created(self):
        ...
        self.assertTrue(DistronodeRepository.objects.filter(name='new-remote').exists())
        self.assertTrue(DistronodeDistribution.objects.filter(base_path='new-remote').exists())

    def test_associate_existing_entities(self):
        out = StringIO()

        existing_remote = CollectionRemote.objects.create(name='existing-remote')
        existing_repository = DistronodeRepository.objects.create(
            name='existing-repo', remote=existing_remote
        )
        DistronodeDistribution.objects.create(
        ...
        repo = DistronodeRepository.objects.get(name='existing-repo')
        distro = DistronodeDistribution.objects.get(name='existing-distro')
    ...
    def test_cannot_create_community_wo_requirements_file(self):
        with self.assertRaisesMessage(
            ...
            call_command('create-remote', 'community', 'https://galaxy.distronode.com/v3/collections/')
    ...
    def test_create_community_remote_with_requirements_file_str(self):
        ...
        call_command(
            'create-remote',
            'My New Remote',
            'https://galaxy.distronode.com/v3/collections/',
