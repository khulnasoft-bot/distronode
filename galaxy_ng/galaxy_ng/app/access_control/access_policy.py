...
from pulp_distronode.app import models as distronode_models

from pulp_container.app import models as container_models
from pulp_distronode.app.serializers import CollectionVersionCopyMoveSerializer
...
class AccessPolicyBase(AccessPolicyFromDB):
    ...
    normally be loaded in pulp distronode.
    ...
    def scope_by_view_repository_permissions(self, view, qs, field_name="", is_generic=True):
        ...
        object and False when it's a FK to DistronodeRepository
        """
        user = view.request.user
        if user.has_perm("distronode.view_distronoderepository"):
            return qs
        view_perm = Permission.objects.get(
            content_type__app_label="distronode", codename="view_distronoderepository")
        ...
        if is_generic:
            private_q = Q(**{f"{field_name}distronode_distronoderepository__private": False})
            qs = qs.select_related(f"{field_name}distronode_distronoderepository")
    ...
    def v3_can_view_repo_content(self, request, view, action):
        ...
        if path:
            distro = distronode_models.DistronodeDistribution.objects.get(base_path=path)
            repo = distro.repository
            repo = repo.cast()

            if repo.private:
                perm = "distronode.view_distronoderepository"
    ...
    def v3_can_destroy_collections(self, request, view, action):
        ...
        for delete_permission in ["galaxy.change_namespace", "distronode.delete_collection"]:
        ...
        if user.has_perm("distronode.delete_collection", collection):  # noqa: SIM103
            return True
    ...
    def has_distronode_repo_perms(self, request, view, action, permission):
        """
        ...
        if isinstance(obj, distronode_models.DistronodeRepository):
            repo = obj
        ...
        if permission == "distronode.view_distronoderepository" and not repo.private:
            return True
    ...
    def can_create_collection(self, request, view, permission):
        ...
        path = view._get_path()
        try:
            repo = distronode_models.DistronodeDistribution.objects.get(base_path=path).repository.cast()
        ...
        except distronode_models.DistronodeDistribution.DoesNotExist:
    ...
    def can_sign_collections(self, request, view, permission):
        ...
        can_modify_repo = request.user.has_perm('distronode.modify_distronode_repo_content', repository)
    ...
    def signatures_not_required_for_repo(self, request, view, action):
        ...
        for cv in data["collection_versions"]:
            sig_exists = repo_version.get_content(
                distronode_models.CollectionVersionSignature.objects
    ...
    def is_not_protected_base_path(self, request, view, action):
        ...
        if isinstance(obj, core_models.Repository):
            if distronode_models.DistronodeDistribution.objects.filter(
                repository=obj,
    ...
    def require_requirements_yaml(self, request, view, action):
        ...
        if remote := request.data.get("remote"):
            with contextlib.suppress(distronode_models.CollectionRemote.DoesNotExist):
                remote = distronode_models.CollectionRemote.objects.get(pk=extract_pk(remote))
        if not remote:
            obj = view.get_object()
        ...
        if not remote.requirements_file and any(
            ...
            raise ValidationError(
                detail={
                    'requirements_file':
                        _('Syncing content from galaxy.distronode.com without specifying a '
