...
DECLARE
    ...
    to_version   UUID;
BEGIN
    IF NOT EXISTS(SELECT 1 FROM distronode_collectionversion cv WHERE cv.certification = filter) THEN
    ...
    WHERE version_removed_id IS NULL AND content_id IN (
        ...
        FROM distronode_collectionversion cv
