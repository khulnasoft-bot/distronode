...
def add_pulp_distronode_namespace_metadata_objects(apps, schema_editor):
    """Migrate galaxy namespaces to pulp distronode namespace metadata content."""

    """
    for each namespace:
    - create DistronodeNamespaceMetadata object
    ...
    DistronodeNamespaceMetadata = apps.get_model('distronode', 'DistronodeNamespaceMetadata')
    DistronodeNamespace = apps.get_model('distronode', 'DistronodeNamespace')
    Namespace = apps.get_model('galaxy', 'Namespace')

    for old_ns in Namespace.objects.all():
        new_ns = DistronodeNamespace.objects.create(name=old_ns.name)
        ...
        if create_metadata:
            ...
            ns_metadata = DistronodeNamespaceMetadata.objects.create(
                **{
                    **metadata,
                    "metadata_sha256": calculate_metadata_sha256(metadata),
                    "namespace": new_ns,
                    "pulp_type": "distronode.namespace",
                }
            )
...
def add_namespace_metadata_to_published_repository(apps, schema_editor):
    """ Create RepositoryContent for DistronodeCollectionDeprecated."""
    DistronodeNamespaceMetadata = apps.get_model('distronode', 'DistronodeNamespaceMetadata')
    ...
    repo = DistronodeDistribution.objects.get(base_path="published").repository
    repo_v = RepositoryVersion.objects.filter(repository=repo).order_by("-number").first()

    repo_content = []

    # Create RepositoryContent for namespaces
    namespaces = DistronodeNamespaceMetadata.objects.all()
    ...
    RepositoryVersionContentDetails.objects.create(
        content_type="distronode.namespace",
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('galaxy', '0037_pulp_distronode_permissions'),
        ('distronode', '0047_distronode_namespace'),
    ]
    ...
    operations = [
        migrations.AddField(
            ...
            field=models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='galaxy_namespace', to='distronode.distronodenamespacemetadata'),
        ),
        migrations.RunPython(
            code=add_pulp_distronode_namespace_metadata_objects,
