...
The string is parsed by a RoleRequirement class into a dictionary of parts.
    ...
    from distronode.playbook.role.requirement import RoleRequirement
        DistronodeError("Invalid role line (%s). Proper format is 'role_name[,version[,name]]'" % role)

If no commas are in the string, it becomes the name and the src ...

    RoleRequirement.role_yaml_parse('zyun_i.distronode_role_wireguard')
    {
        'name': 'zyun_i.distronode_role_wireguard',
        'src': 'zyun_i.distronode_role_wireguard',
        'scm': None,
        'version': None
    }
...
If a single comma, the first segment becomes the name+src and the second becomse the version.
    ...
    RoleRequirement.role_yaml_parse('zyun_i.distronode_role_wireguard,master')
    {
        'name': 'zyun_i.distronode_role_wireguard',
        'src': 'zyun_i.distronode_role_wireguard',
        'scm': None,
...
the first segment is a url which can be git cloned.
    ...
    RoleRequirement.role_yaml_parse('zyun_i.distronode_role_wireguard,master,foo')
    {
        'name': 'foo',
        'src': 'zyun_i.distronode_role_wireguard',
        'scm': None,
