...
from pulp_distronode.app.models import Tag
from pulp_distronode.app.serializers import TagSerializer
...
class CollectionTagFilterOrdering(filters.OrderingFilter):
    def filter(self, qs, value):
        if value is not None and any(v in ["count", "-count"] for v in value):
            ...
            return qs.filter(
                distronode_collectionversion__distronode_crossrepositorycollectionversionindex__is_highest=True  # noqa: E501
            ).annotate(count=Count('distronode_collectionversion')).order_by(f"{order}count")
...
class CollectionsTagsViewSet(
    ...
    def get_queryset(self):
        ...
        return qs.annotate(count=Count("distronode_collectionversion"))
