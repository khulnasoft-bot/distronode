...
from pulp_distronode.app import viewsets as pulp_viewsets
...
class DistronodeRepositorySerializer(serializers.ModelSerializer):
    ...
    @extend_schema_field(DistronodeDistributionSerializer(many=True))
    def get_distributions(self, obj):
        return [
            DistronodeDistributionSerializer(distro).data
...
class CollectionRemoteSerializer(pulp_viewsets.CollectionRemoteSerializer):
    ...
    @extend_schema_field(DistronodeRepositorySerializer(many=True))
    def get_repositories(self, obj):
        return [
            DistronodeRepositorySerializer(repo).data
