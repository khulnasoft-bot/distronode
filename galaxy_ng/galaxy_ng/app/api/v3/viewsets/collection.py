...
from pulp_distronode.app.galaxy.v3 import views as pulp_distronode_views
from pulp_distronode.app.models import DistronodeDistribution
from pulp_distronode.app.models import CollectionImport as PulpCollectionImport
from pulp_distronode.app.models import (
    CollectionVersion,
...
from pulp_distronode.app.tasks.copy import copy_collection
...
from galaxy_ng.app.common.parsers import DistronodeGalaxy29MultiPartParser
...
class CollectionUploadViewSet(api_base.LocalSettingsMixin,
                              pulp_distronode_views.CollectionUploadViewSet):
    permission_classes = [access_policy.CollectionAccessPolicy]
    parser_classes = [DistronodeGalaxy29MultiPartParser]
...
class CollectionArtifactDownloadView(api_base.APIView):
    ...
    def _get_distronode_distribution(self, base_path):
        return DistronodeDistribution.objects.get(base_path=base_path)
    ...
    def get(self, request, *args, **kwargs):
        ...
        distribution = self._get_distronode_distribution(distro_base_path)

        if settings.DISTRONODE_COLLECT_DOWNLOAD_LOG:
            pulp_distronode_views.CollectionArtifactDownloadView.log_download(
                request, filename, distro_base_path
            )

        if settings.get("DISTRONODE_COLLECT_DOWNLOAD_COUNT", False):
            pulp_distronode_views.CollectionArtifactDownloadView.count_download(filename)
...
class CollectionRepositoryMixing:
    ...
    def get_repos(self):
        ...
        try:
            src_repo = DistronodeDistribution.objects.get(
                base_path=self.kwargs['source_path']).repository
            dest_repo = DistronodeDistribution.objects.get(
    ...
    def move_content(self, request, *args, **kwargs):
        ...
        if auto_sign and dest_repo.name == golden_repo:
            ...
            signing_service_name = settings.get(
                "GALAXY_COLLECTION_SIGNING_SERVICE", "distronode-default"
            )
            try:
        ...
        if settings.GALAXY_DEPLOYMENT_MODE == DeploymentMode.INSIGHTS.value:  # noqa: SIM300
            golden_repo = DistronodeDistribution.objects.get(
