...
from pulp_distronode.app.urls import (
    v3_urls
)
...
plugin_paths = [
    ...
    path(
        "distronode/content/<path:distro_base_path>/collections/artifacts/<str:filename>",
    ...
    path(
        "distronode/content/<path:distro_base_path>/collections/artifacts/",
...
urlpatterns = [
    # >>>> OVERRIDDEN PULP DISTRONODE ENDPOINTS <<<<<

    # Some pulp distronode endpoints have to be overridden because we have special logic
    # that is specific to galaxy_ng's business needs. Until
    # https://github.com/pulp/pulp_distronode/issues/835 is resolved, the only solution
    ...
    # redirects break on collection publish.
    path(
        "plugin/distronode/content/<path:distro_base_path>/collections/artifacts/",
        viewsets.CollectionUploadViewSet.as_view({"post": "create"}),
        name="collection-artifact-upload",
    ),

    # >>>> END OVERRIDDEN PULP DISTRONODE ENDPOINTS <<<<<

    # TODO(newswangerd): Endpoints that have not been moved to pulp distronode yet
    path(
