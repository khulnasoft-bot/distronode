...
from distronode_base.rbac.validators import validate_permissions_for_model
from distronode_base.rbac.models import (
    RoleDefinition,
    RoleUserAssignment,
    RoleTeamAssignment,
)
...
@receiver(post_save, sender=DistronodeRepository)
...
@receiver(post_save, sender=DistronodeDistribution)
...
def create_namespace_if_not_present(sender, instance, created, **kwargs):
    ...
    django signal for pulp_distronode Collection model, so that whenever a
    Collection object is created or saved, it will create a Namespace object
    if the Namespace does not already exist.
    Supports use case: In pulp_distronode sync, when a new collection is sync'd
...
@receiver(post_save, sender=DistronodeNamespaceMetadata)
def associate_namespace_metadata(sender, instance, created, **kwargs):
    """
    Update the galaxy namespace when a new pulp distronode namespace
