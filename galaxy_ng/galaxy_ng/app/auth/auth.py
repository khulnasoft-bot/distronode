...
class RHIdentityAuthentication(BaseAuthentication):
    ...
    def _get_upstream_repo(group, repository_name):
        try:
            upstream_repo = DistronodeRepository.objects.get(name=repository_name)
        except DistronodeRepository.DoesNotExist as exc:
            log.exception(exc)
            raise
    ...
    def _get_or_create_synclist_distribution(distro_name, upstream_repository):
        ...
        # is populated.
        distro, _ = DistronodeDistribution.objects.get_or_create(
