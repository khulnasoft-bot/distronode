...
from pulp_distronode.app.models import DistronodeNamespaceMetadata, DistronodeNamespace
...
def _create_pulp_namespace(galaxy_ns_pk, download_logo):
    ...
    namespace, created = DistronodeNamespace.objects.get_or_create(name=namespace_data["name"])
    metadata = DistronodeNamespaceMetadata(namespace=namespace, **namespace_data)
    metadata.calculate_metadata_sha256()
    content = DistronodeNamespaceMetadata.objects.filter(
    ...
    if content:
        ...
        repo_content_qs = (
            ...
            .select_related("content__distronode_collectionversion")
            .order_by("repository__pk")
            .filter(
                repository__remote=None,
                content__distronode_collectionversion__namespace=galaxy_ns.name,
