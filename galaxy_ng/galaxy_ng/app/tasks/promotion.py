...
from pulp_distronode.app.tasks.signature import sign
from pulp_distronode.app.tasks.copy import move_collection

from pulpcore.plugin.models import TaskGroup, SigningService


SIGNING_SERVICE_NAME = settings.get("GALAXY_COLLECTION_SIGNING_SERVICE", "distronode-default")
AUTO_SIGN = settings.get("GALAXY_AUTO_SIGN_COLLECTIONS", False)


def auto_approve(src_repo_pk, cv_pk, ns_pk=None):
    published_repos = DistronodeRepository.objects.filter(pulp_labels__pipeline="approved")
    published_pks = list(published_repos.values_list("pk", flat=True))

    staging_repos = DistronodeRepository.objects.filter(pulp_labels__pipeline="staging")
    staging_pks = list(staging_repos.values_list("pk", flat=True))

    source_repo = DistronodeRepository.objects.get(pk=src_repo_pk)
