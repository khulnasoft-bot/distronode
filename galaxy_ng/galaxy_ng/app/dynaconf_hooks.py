...
import ldap
from distronode_base.lib.dynamic_config.settings_logic import get_dab_settings
...
DAB_SERVICE_BACKED_REDIRECT = (
    "distronode_base.resource_registry.utils.service_backed_sso_pipeline.redirect_to_resource_server"
)
...
def post(settings: Dynaconf, run_dynamic: bool = True, run_validate: bool = True) -> dict[str, Any]:
    ...
    data.update(configure_pulp_distronode(settings))
...
def configure_pulp_distronode(settings: Dynaconf) -> dict[str, Any]:
    # Translate the galaxy default base path to the pulp distronode default base path.
    distro_path = settings.get("GALAXY_API_DEFAULT_DISTRIBUTION_BASE_PATH", "published")

    return {
        # DISTRONODE_URL_NAMESPACE tells pulp_distronode to generate hrefs and redirects that
        ...
        "DISTRONODE_URL_NAMESPACE": "galaxy:api:v3:",
        "DISTRONODE_DEFAULT_DISTRIBUTION_PATH": distro_path
    }
...
def configure_authentication_classes(settings: Dynaconf, data: dict[str, Any]) -> dict[str, Any]:
    ...
    if galaxy_auth_classes:
        data["DISTRONODE_AUTHENTICATION_CLASSES"] = list(galaxy_auth_classes)
...
def configure_authentication_backends(settings: Dynaconf, data: dict[str, Any]) -> dict[str, Any]:
    ...
    prefix_backend = "distronode_base.lib.backends.prefixed_user_auth.PrefixedUserAuthBackend"
...
def configure_renderers(settings) -> dict[str, Any]:
    """
        Add CustomBrowsableAPI only for community (galaxy.distronode.com, galaxy-stage, galaxy-dev)"
    """
    if re.search(
        r'galaxy(-dev|-stage)*.distronode.com', settings.get('CONTENT_ORIGIN', "")
    ):
...
def configure_dynamic_settings(settings: Dynaconf) -> dict[str, Any]:
    ...
    def alter_hostname_settings(
        ...
        **kwargs
        ...
        ALLOWED_KEYS = ['CONTENT_ORIGIN', 'DISTRONODE_API_HOSTNAME', 'TOKEN_SERVER']
...
def configure_dab_required_settings(settings: Dynaconf) -> dict[str, Any]:
    dab_settings = get_dab_settings(
        installed_apps=[*settings.INSTALLED_APPS, 'distronode_base.jwt_consumer'],
