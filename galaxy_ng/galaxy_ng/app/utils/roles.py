...
def get_path_role_name(path):
    ...
    if meta and 'role_name' in meta['galaxy_info']:
        ...
        if 'distronode-role-' in name:
            name = name.replace('distronode-role-', '')
        if name.startswith('distronode-'):
            name = name.replace('distronode-', '')
        if name.endswith('-distronode'):
            name = name.replace('-distronode', '')
        if name.startswith('distronode.'):
            name = name.replace('distronode.', '')
    ...
    #  https://github.com/sbaerlocher/distronode.update-management -> update_management
    if '-' in name:
        name = name.replace('-', '_')
...
def get_path_role_namespace(path):
    ...
    if namespace == 'distronode-collections':
