...
from pulp_distronode.app.models import CollectionVersion
from pulp_distronode.app.models import CollectionRemote
from pulp_distronode.app.models import DistronodeRepository
from pulp_distronode.app.tasks.collections import sync
from pulp_distronode.app.tasks.collections import rebuild_repository_collection_versions_metadata
...
class Command(BaseCommand):
    ...
    help = 'Sync upstream namespaces+owners from [old-]galaxy.distronode.com'

    def add_arguments(self, parser):
        parser.add_argument("--baseurl", default="https://old-galaxy.distronode.com")
    ...
    def handle(self, *args, **options):
        ...
        repo = DistronodeRepository.objects.filter(name=options['repository']).first()
        if not repo:
        ...
        for namespace_info, collection_info, collection_versions in upstream_collection_iterator(
            ...
            # pulp_distronode sync isn't smart enough to do this ...
            should_sync = False
            ...
            for cvdata in collection_versions:
                ...
                if not cv:
                    should_sync = True
                    should_rebuild = True
                elif not cv.contents or not cv.requires_distronode:
                    should_rebuild = True
    ...
    def do_sync_dispatch(self, remote, repository, namespace, name):
        ...
        # dispatch the real pulp_distronode sync code
        task = dispatch(
            sync,
            kwargs={
