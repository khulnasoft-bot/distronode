...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            repo = DistronodeRepository.objects.get(name=repository)
        ...
        if publickey:
            ...
            certs_dir = settings.get("DISTRONODE_CERTS_DIR", "/etc/pulp/certs")
        ...
        if DistronodeRepository.objects.get(pk=repo.pk).gpgkey == pubkey:
...
def set_repo_gpgkey(repo_id, gpgkey):
    ...
    with transaction.atomic():
        repo = DistronodeRepository.objects.get(pk=repo_id)
        repo.gpgkey = gpgkey
        repo.save()
