...
# Collection
HAS_COLLECTION_SIGNING=$(pulpcore-manager shell -c 'from pulpcore.app.models import SigningService;print(SigningService.objects.filter(name="distronode-default").count())' 2>/dev/null || true)
if [[ "$HAS_COLLECTION_SIGNING" -eq "0" ]]; then
    pulpcore-manager add-signing-service distronode-default /var/lib/pulp/scripts/collection_sign.sh F37575C52D4F16F3
else
