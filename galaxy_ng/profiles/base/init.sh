...
setup_signing_keyring() {
    ...
    for KEY_FINGERPRINT in $(gpg --show-keys --with-colons --with-fingerprint /src/galaxy_ng/dev/common/distronode-sign.key | awk -F: '$1 == "fpr" {print $10;}')
    do
        gpg --batch --no-default-keyring --keyring /etc/pulp/certs/galaxy.kbx --import /src/galaxy_ng/dev/common/distronode-sign.key
...
setup_repo_keyring() {
    ...
    STAGING_KEYRING=$(django-admin shell -c "from pulp_distronode.app.models import DistronodeRepository;print(DistronodeRepository.objects.get(name='staging').gpgkey)" || true)
    ...
    PUBLISHED_KEYRING=$(django-admin shell -c "from pulp_distronode.app.models import DistronodeRepository;print(DistronodeRepository.objects.get(name='published').gpgkey)" || true)
...
setup_collection_signing_service() {
    ...
    export KEY_FINGERPRINT=$(gpg --show-keys --with-colons --with-fingerprint /src/galaxy_ng/dev/common/distronode-sign.key | awk -F: '$1 == "fpr" {print $10;}' | head -n1)
    export KEY_ID=${KEY_FINGERPRINT: -16}
    gpg --batch --import /src/galaxy_ng/dev/common/distronode-sign.key
    echo "${KEY_FINGERPRINT}:6:" | gpg --import-ownertrust

    HAS_SIGNING=$(django-admin shell -c 'from pulpcore.app.models import SigningService;print(SigningService.objects.filter(name="distronode-default").count())' || true)
    if [[ "$HAS_SIGNING" -eq "0" ]]; then
        log_message "Creating signing service. using key ${KEY_ID}"
        django-admin add-signing-service distronode-default /src/galaxy_ng/dev/common/collection_sign.sh ${KEY_ID} || true
    else
