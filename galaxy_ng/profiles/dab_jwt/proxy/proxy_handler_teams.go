		...
		*/
		...
		responseTeam := TeamResponse{
			...
			SummaryFields: struct {
				Resource struct {
					DistronodeID string `json:"distronode_id"`
				} `json:"resource"`
			}{Resource: struct {
				DistronodeID string `json:"distronode_id"`
			}{DistronodeID: teamdata.DistronodeId}},
		}
...
func addTeam(w http.ResponseWriter, r *http.Request) {
	...
	newTeam.Id = newId
	newDistronodeID := uuid.NewString()
	newTeam.DistronodeId = newDistronodeID
...
func deleteTeam(w http.ResponseWriter, r *http.Request) {
	...
	user, _ := GetRequestUser(r)
	client := NewServiceIndexClient()
	if err := client.Delete(user, distronodeId); err != nil {
