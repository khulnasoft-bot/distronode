...
func generateHmacSha256SharedSecret(nonce *string) (string, error) {
	...
	//const DISTRONODE_BASE_SHARED_SECRET = "redhat1234"
	var SharedSecretNotFound = errors.New("the setting DISTRONODE_BASE_SHARED_SECRET was not set, some functionality may be disabled")

	if DISTRONODE_BASE_SHARED_SECRET == "" {
		log.Println("The setting DISTRONODE_BASE_SHARED_SECRET was not set, some functionality may be disabled.")
	...
	message := map[string]string{
		"nonce":         *nonce,
		"shared_secret": DISTRONODE_BASE_SHARED_SECRET,
	}
	...
	mac := hmac.New(sha256.New, []byte(DISTRONODE_BASE_SHARED_SECRET))
...
func GenerateUserClaims(user User) (UserClaims, error) {
	...
	for _, team := range adminTeams {
		...
		teamObj := TeamObject{
			DistronodeId: team.DistronodeId,
			Name:      team.Name,
			Org:       orgIx,
		}
