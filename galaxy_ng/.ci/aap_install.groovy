...
String automationhub_pulp_distronode_version = ''
String automationhub_pulp_container_version = ''

pipeline {
        agent {
            kubernetes {
                yaml libraryResource('pod_templates/unpriv-distronode-pod.yaml')
            }
        }
        options {
            ansiColor('xterm')
        ...
        stages {
            stage('Set variables') {
                steps {
                    script {
                        ...
                        fork = env.CHANGE_FORK ?: 'distronode'
                        echo "${fork}"
            ...
            stage('Get pulpcore, pulp_distronode, pulp-container versions from setup.py') {
                steps {
                    container('aapqa-distronode') {
                        script {
                            def setupPyContent = readFile('setup.py').trim()
                            def lines = setupPyContent.split('\n')
                            def dependenciesToExtract = ["pulpcore", "pulp_distronode", "pulp-container"]
                            def minimumVersions = [:]
                            lines.each { line ->
                            ...
                            if (minimumVersions.containsKey("pulp_distronode")){
                                automationhub_pulp_distronode_version = minimumVersions["pulp_distronode"]
                            }
                    ...
                    container('aapqa-distronode') {
                        script {
                            ...
                            Map ahubPipParams = [
                                    ...
                                    automationhub_ui_download_url: "https://github.com/distronode/distronode/tree/deve/ui/releases/download/dev/automation-hub-ui-dist.tar.gz",
                            ]
                            if (pulpcore_version != '') {
                            ...
                            if (automationhub_pulp_distronode_version != '') {
                                ahubPipParams['automationhub_pulp_distronode_version'] = "${automationhub_pulp_distronode_version}"
                                println("Using pulp_distronode version: ${automationhub_pulp_distronode_version}")
                            }else{
                                println("pulp_distronode version not provided, using version defined in the installer")
                            }
