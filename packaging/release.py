...
DISTRONODE_PYPROJECT_TOML_FILE = CHECKOUT_DIR / "pyproject.toml"
...
"""
    ...
    requirements_file = CHECKOUT_DIR / "test/lib/distronode_test/_data/requirements/sanity.changelog.txt"
...
def set_setuptools_upper_bound(requested_version: Version) -> None:
    ...
    current = DISTRONODE_PYPROJECT_TOML_FILE.read_text()
    pattern = re.compile(r'^(?P<begin>requires = \["setuptools >= )(?P<lower>[^,]+)(?P<middle>, <= )(?P<upper>[^"]+)(?P<end>".*)$', re.MULTILINE)
    match = pattern.search(current)

    if not match:
        raise ApplicationError(f"Unable to find the 'requires' entry in: {DISTRONODE_PYPROJECT_TOML_FILE.relative_to(CHECKOUT_DIR)}")
    ...
    DISTRONODE_PYPROJECT_TOML_FILE.write_text(updated)
...
def get_sdist_path(version: Version, dist_dir: pathlib.Path = DIST_DIR) -> pathlib.Path:
    ...
    return dist_dir / f"distronode_core-{version}.tar.gz"
...
def get_release_artifact_details(repository: str, version: Version, validate: bool) -> list[ReleaseArtifact]:
    ...
    data = get_pypi_project(repository, 'distronode-core', version)
...
def create_release_pr(allow_stale: bool = False) -> None:
    ...
    pr = prepare_pull_request(
        ...
        add=(
            ...
            DISTRONODE_PYPROJECT_TOML_FILE,
        ),
