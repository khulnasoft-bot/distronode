# API Clustering Overview

API supports multi-node configurations. Here is an example configuration with two control plane nodes.
...
There are two main deployment types, virtual machines (VM) or K8S. Distronode Automation Platform (AAP) can be installed via VM or K8S deployments. The upstream API project can only be installed via a K8S deployment. Either deployment type supports cluster scaling.
...
- For K8S deployments, these background processes are containerized
  - `api-ee`: receptor
  - `api-web`: uwsgi, daphne, wsbroadcast, rsyslog
  - `api-task`: dispatcher, callback receiver
  - `redis`: redis
...
2. `inspect_execution_nodes` looks at each execution node
  ...
  d. run `execution_node_health_check`
    - This is an async task submitted to the dispatcher and attempts to run `distronode-runner --worker-info` against that node
    - This command will return important information about the node's hardware resources like CPU cores, total memory, and distronode-runner version
...
4. Determine if *this* node is lost and run `local_health_check`
  a. call `get_cpu_count` and `get_mem_in_bytes` directly from distronode-runner, which is what `distronode-runner --worker-info` calls under the hood
5. If *this* instance was not found in the database, register it
6. Compare *this* node's distronode-runner version with that of other instances
...
API `Instances` can be configured to automatically join `Instance Groups` when they come online by defining a policy. These policies are evaluated for
every new Instance that comes online.

Instance Group Policies are controlled by three optional fields on an `Instance Group`:

- `policy_instance_percentage`: This is a number between 0 - 100. It guarantees that this percentage of active API instances will be added to this `Instance Group`. As new instances come online, if the number of Instances in this group relative to the total number of instances is fewer than the given percentage, then new ones will be added until the percentage condition is satisfied.
...
API itself reports as much status as it can via the API at `/api/v2/ping` in order to provide validation of the health of the cluster. This includes:
...
Ideally a regular user of API should not notice any semantic difference to the way jobs are run and reported. Behind the scenes it is worth pointing out the differences in how the system behaves.

When a job is submitted from the API interface, it gets pushed into the dispatcher queue via postgres notify/listen (https://www.postgresql.org/docs/10/sql-notify.html), and the task is handled by the dispatcher process running on that specific API node.  If an instance fails while executing jobs, then the work is marked as permanently failed.
...
It is important that project updates run on the instance which prepares the distronode-runner private data directory.
