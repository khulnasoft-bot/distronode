...
If an API resource is able to be copied, all of its object detail API views will have a related URL field
...
from `api.api.generics.CopyAPIView`. Here is an example:
```python
...
Lastly, unit test copy behavior of the new endpoint in `/api/main/tests/functional/test_copy.py` and
