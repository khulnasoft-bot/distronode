...
API uses websockets to update the UI in realtime as events happen within the
...
Since the [web/task split](https://github.com/distronode/distronode/tree/devel/api/pull/13423) landed,
...
in contrast to WSGI which we currently use for the rest of API. That is *only*
...
* `api/main/consumers.py` - the django-channels "consumers" where websocket
...
* `api/main/wsrelay.py` (formerly `api/main/wsbroadcast.py`) - an asyncio
...
* `api/main/management/commands/run_ws_heartbeat.py` - discussed below, used to
...
Consider a Kubernetes deployment of API. Before the web task split, each pod had
...
You can connect to the API channels implementation using any standard websocket
