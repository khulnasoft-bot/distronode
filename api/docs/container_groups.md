...
In a traditional API installation, jobs (distronode-playbook runs) are
...
this is is done by manually creating an instance through the API API or UI. 
...
`api/main/models/credential/__init__.py` where `kind='kubernetes'`.
