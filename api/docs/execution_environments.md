...
- [Distronode Builder documentation](https://distronode.readthedocs.io/projects/builder/en/latest/)
...
Installers should run the `api-manage register_default_execution_environments` command to pre-populate
...
These EEs are critical for system function, so this command must be ran for API to be functional.
...
dependencies out of the venvs and into EEs.
 ...
 - `api-manage list_custom_venvs`
 - `api-manage custom_venv_associations`
 - `api-manage export_custom_venv`

Follow those in order, and read the help text to see what arguments are necessary.

Output from the `api-manage export_custom_venv -q ..` command can
be a starting point for writing an `distronode-builder` definition file.
