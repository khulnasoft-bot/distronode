...
a project as an Distronode inventory source.
...
all paths that could be used as an Distronode inventory because of the wide
...
> Any Inventory Distronode supports should be supported by this feature.

This is accomplished by making use of the `distronode-inventory` command.

When running an inventory update, the `distronode-inventory` command is invoked,
passing the location specified by `source_path` to the `-i` CLI option.
Then `distronode-inventory` writes JSON data to a file, which is read, parsed,
...
on the same instance (inside of the API cluster) as the project update.
