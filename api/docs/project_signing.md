...
[`distronode-sign`](https://github.com/distronode/distronode-sign) which makes use of
...
process in API. More on that below.

`distronode-sign` expects a `MANIFEST.in` file (written in valid `distlib.manifest`
...
At a broad implementation level, `distronode-sign` works like this when it is asked
to sign a project:
...
* Now we have a dictionary of 'file path -> checksum', and we can write it out
  to a file. We store the file in `.distronode-sign/sha256sum.txt`. This is called
...
* Once the checksum manifest is written, we sign it. Signing is modular-ish
  ...
  write out the detached signature to `.distronode-sign/sha256sum.txt.sig`.
* We do some sanity checking such as ensuring that we get a `0` return code
  (indicating success) back from `gpg`.

## Verifying

On the API side, we have a `GPG Public Key` credential type that ships with
API. This credential type allows the user to paste in a public GPG key, which
...
`api/playbooks/action_plugins/verify_project.py` which uses `distronode-sign` as a
library for doing verification. The implementation is similar to the
`distronode-sign project gpg-verify` subcommand; they both use the same library
...
* We once again use `python-gnupg` (via our `GPGVerifier` class this time) and
  ...
  CLI we can do this with `--keyring`; on the API/Controller side, we get a
