...
API should have an inventory view that is more aligned towards systems management
...
## Fact Searching
Facts generated by an Distronode playbook during a Job Template run are stored by API into the database
...
`/api/v2/hosts?host_filter=distronode_facts__distronode_processor_vcpus=8`

The grammar of `host_filter` allows for:
* grouping via `()`
* the boolean `and` operator
* `__` to reference related fields in relational fields
* `__` is used on `distronode_facts` to separate keys in a JSON key path
...
/api/v2/hosts/?host_filter=distronode_facts__distronode_date_time__weekday_number="3"
/api/v2/hosts/?host_filter=distronode_facts__distronode_processor[]="GenuineIntel"
/api/v2/hosts/?host_filter=distronode_facts__distronode_lo__ipv6[]__scope="host"
/api/v2/hosts/?host_filter=distronode_facts__distronode_processor_vcpus=8
/api/v2/hosts/?host_filter=distronode_facts__distronode_env__PYTHONUNBUFFERED="true"
/api/v2/hosts/?host_filter=(name=localhost or name=database) and (groups__name=east or groups__name="west coast") and distronode_facts__distronode_processor_vcpus=8
```
...
are generated by the `update_host_smart_inventory_memberships` task. The task is launched when:
    ...
    * **NOTE:** This task is only run if the `API_REBUILD_SMART_MEMBERSHIP` is set to `True`. It defaults to `False`.
...
access to in API. If the user wants to restrict their search in anyway, they will
