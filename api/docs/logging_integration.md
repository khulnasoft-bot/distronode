...
data feed should be useful in order to gain insights into API usage
...
from the API. These data loggers are the following:
 ...
 - `api.analytics.job_events` - Data returned from the Distronode callback module
 - `api.analytics.activity_stream` - Record of changes to the objects within the API app
 - `api.analytics.system_tracking` - Data gathered by Distronode scan modules ran by scan job templates

These loggers only use log-level of `INFO`. Additionally, the standard API logs are deliverable through this
...
up, also refer to instructions in [`/tools/elastic/README.md`](https://github.com/distronode/distronode/tree/devel/api/blob/devel/tools/elastic/README.md).
...
| Field | Information |
|-----------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `cluster_host_id` | (string) Unique identifier of the host within the API cluster |
| `level` | (choice of `DEBUG`, `INFO`, `WARNING`, `ERROR`, etc.) Standard python log level, roughly reflecting the significance of the event; all of the data loggers (as a part of this feature) use `INFO` level, but the other API logs will use different levels as appropriate |
| `logger_name` | (string) Name of the logger we use in the settings, *e.g.*, "`api.analytics.activity_stream`" |
...
| changes           | (string) Unique identifier of the host within the API cluster                                                         |
...
settings, as documented, 2) taking some action in API that causes a log
...
**API logs:** Formatting of Traceback message is a known issue in several
...
not cause API operations to hang.
