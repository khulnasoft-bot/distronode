...
API supports the use of Distronode Collections. This section will give ways to use Collections in job runs.
...
If you specify a collections requirements file in SCM at `collections/requirements.yml` of a project,
 then API will install collections from that file to a special cache folder in project updates.
...
```
distronode-galaxy collection install -r requirements.yml -p <project cache location>/_<project #>__<project name>/stage/requirements_collections
```
...
That means that the `distronode-galaxy` commands are ran to download content
...
```
/var/lib/api/projects/.__api_cache/_42__project_name/stage
```
...
```
/var/lib/api/projects/.__api_cache/_42__project_name/63
```
...
For details on how Galaxy servers are configured in Distronode in general see:

https://docs.distronode.com/distronode/devel/user_guide/collections_using.html
(if "devel" link goes stale in the future, it is for Distronode 2.9)

You can specify a list of zero or more servers to download roles and
collections from for API Project Updates.  This is done by associating Galaxy
credentials (in sequential order) via the API at
`/api/v2/organizations/N/galaxy_credentials/`.  Authentication
via an API token is optional (i.e., https://galaxy.distronode.com/), but other
content sources (such as Red Hat Distronode Automation Hub) require proper
...
requirement by using the `source:` option, as described in Distronode documentation.
