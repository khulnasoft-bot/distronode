...
API has added support for assigning zero or more credentials to Job Templates and Inventory Updates via a singular, unified interface.

Background
----------

Prior to API (Tower 3.2), Job Templates had a certain set of requirements surrounding their relation to Credentials:
...
This model lacked the ability associate multiple Vault credentials with a playbook run, a use case supported by Distronode core from Distronode 2.4 onwards.
...
This specific use case covers Distronode's support for multiple Vault passwords for
a playbook run (since Distronode 2.4):
http://docs.distronode.com/distronode/latest/vault.html#vault-ids-and-multiple-vault-passwords

Vault credentials in API now have an optional field, `vault_id`, which is
analogous to the `--vault-id` argument to `distronode-playbook`.  To run
a playbook which makes use of multiple Vault passwords:

1.  Make a Vault credential in API for each Vault password; specify the Vault
...
2.  Assign multiple Vault credentials to the job template via the new
    ...
    ```
3.  Launch the Job Template, and `distronode-playbook` will be invoked with
