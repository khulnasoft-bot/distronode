...
keys, API tokens for cloud services, etc.) in API are stored in the API database
after being encrypted with a symmetric encryption cipher utilizing AES-256 in
CBC mode alongside a SHA-256 HMAC.

Alternatively, API supports retrieving secret values from third-party secret
...
When configuring API to pull a secret from a third party system, there are
generally three steps.

Here is an example of creating an (1) API Machine Credential with
...
Credential Plugins in API are just importable Python functions that are
registered using setuptools entrypoints
(https://setuptools.readthedocs.io/en/latest/setuptools.html#dynamic-discovery-of-services-and-plugins)

Example plugins officially supported in API can be found in the source code at
`api.main.credential_plugins`.

For instructions on writing and installing your own custom credential plugin, see: https://github.com/distronode/distronode/tree/devel/api-custom-credential-plugin-example

Programmatic Secret Fetching
----------------------------
If you want to programmatically fetch secrets from a supported external secret
management system (for example, if you wanted to compose an API database connection
...
```python
from api.main.credential_plugins import hashivault
...
API supports retrieving secret values from HashiCorp Vault KV
...
```shell
~ curl -sik "https://api.example.org/api/v2/credential_types/?name=Machine" \
...
```shell
~ curl -sik "https://api.example.org/api/v2/credential_types/?name__startswith=HashiCorp" \
    -H "Authorization: Bearer <token>"
HTTP/1.1 200 OK
{
    "results": [
...
```shell
~ curl -sik "https://api.example.org/api/v2/credentials/" \
    -H "Authorization: Bearer <token>" \
...
```shell
~ curl -sik "https://api.example.org/api/v2/credentials/1/input_sources/" \
    -H "Authorization: Bearer <token>" \
    -H "Content-Type: application/json" \
    -X POST \
...
API supports signing public keys via HashiCorp Vault's SSH Secrets Engine
...
4. Associate the Machine Credential with a Job Template.  When the Job Template
   is run, API will use the provided HashiCorp URL and token to sign the
   unsigned public key data using the HashiCorp Vault SSH Secrets API.
   API will generate an `id_rsa` and `id_rsa-cert.pub` on the fly and
