...
API allows jobs to run on a schedule (with optional recurrence rules) via
...
[RFC5545](https://www.rfc-editor.org/rfc/rfc5545.txt) RRULE string and within API [RRULE Limitations](#rrule-limitations).
...
`DTSTART` values provided to API _must_ provide timezone information (they may
not be naive dates).
...
API provides an endpoint for previewing the future dates and times for
...
API implements the following constraints on top of the `RFC5545` specification:
...
Any time an `api.model.Schedule` is saved with a valid `rrule` value, the
...
API makes use of [Celery Periodic Tasks
