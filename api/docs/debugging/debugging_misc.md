...
SQL Debugging
-------------
API includes a powerful tool for tracking slow queries across all of its Python processes.
As the API user, run:

```
$ api-manage profile_sql --threshold 2 --minutes 5
```

...where threshold is the max query time in seconds, and minutes it the number of minutes to record.
For the next five minutes (in this example), any API Python process that generates a SQL query
...
We've made it easier to correlate postgres connections <--> the processes that is doing the query. For example, the following line is a log entry from a slow running `api-manage` command. 

```
< 2021-04-21 12:42:10.307 UTC api_1-1540765-/bin/api-manage gather_analytics --dry-run -v 3 >LOG:  duration: 1211.270 ms  statement: SELECT MIN("main_jobevent"."id") AS "pk__min", MAX("main_jobevent"."id") AS "pk__max" FROM "main_jobevent" WHERE ("main_jobevent"."modified" >= '2021-03-24T12:42:08.846790+00:00'::timestamptz AND "main_jobevent"."modified" <= '2021-04-21T12:42:08.846790+00:00'::timestamptz)
```

The above entry was found in the log file `/var/lib/pgsql/data/pg_log/postgresql-Wed.log`

Note the `application_name` portion. This allows us to trace the query to the node `api_1` with processes id `1540765`. The full task command line string gives us context for each long-running query that we need to find the needle in the hay stack without having to go to each individual API node and query Linux by the pid to understand what work is being done by each pid.
```
api_1-1540765-/bin/api-manage gather_analytics --dry-run -v 3
...
-----------------+---------------------------------------------------------------------------------------------------------------+--------+-----------------------------------------------------------------
 00:13:13.125703 | COPY (SELECT main_jobevent.id,                                                                               +| active | api_1-1540765-/bin/api-manage gather_analytics --dry-run -v 3
...
Remote Debugging
----------------
Python processes in API's development environment are kept running in the
...
can use it to set remote breakpoints in API code and debug interactively over
a telnet session:

```python
    # api/main/tasks.py

    class SomeTask(api.main.tasks.jobs.BaseTask):
        ...
        def run(self, pk, **kwargs):
            ...
            # port will be reported as a warning in the API logs, e.g.,
            #
...
To simplify remote debugging session management, API's development
...
Graph Jobs
----------
The `api-manage graph_jobs` can be used to visualize how Jobs progress from
pending to waiting to running.

```
api-manage graph_jobs --help
usage: api-manage graph_jobs [-h] [--refresh REFRESH] [--width WIDTH]
...
```
/var/lib/api/venv/api/bin/pip3 install gprof2dot
```
...
```
from api.main.utils.profiling import profile
...
total 24
drwxr-xr-x 2 api  root 4096 Oct 15 13:23 .
drwxrwxr-x 1 root root 4096 Oct 15 13:23 ..
-rw-r--r-- 1 api  root  635 Oct 15 13:23 2.001s-task_manager_profile-2303-272858af-3bda-45ec-af9e-7067aa86e4f3.dot
-rw-r--r-- 1 api  root  587 Oct 15 13:23 2.001s-task_manager_profile-2303-272858af-3bda-45ec-af9e-7067aa86e4f3.pstats
-rw-r--r-- 1 api  root  632 Oct 15 13:23 2.002s-task_manager_profile-2303-4cdf4660-3ef4-4238-8164-33611822d9e3.dot
-rw-r--r-- 1 api  root  587 Oct 15 13:23 2.002s-task_manager_profile-2303-4cdf4660-3ef4-4238-8164-33611822d9e3.pstats
```
...
```
from api.main.utils.profiling import timing
...
```
from api.main.utils.profiling import APIProfiler

prof = APIProfiler("hello_world")
prof.start()
'''
