...
To enable DDT, you need to set your `INTERNAL_IPS` to the IP address of your load balancer.  This can be overridden by creating a new settings file beginning with `local_` in `api/settings/` (e.g. `local_overrides.py`).
...
```
api_1        | 14:42:08 uwsgi.1     | 172.18.0.1 GET /api/v2/tokens/ - HTTP/1.1 200
```
...
> Note that enabling DDT is detrimental to the performance of API and adds overhead to every API request.  It is
