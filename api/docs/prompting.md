...
Admins of templates in API have the option to allow fields to be overwritten
...
provided for `credentials`, which is otherwise not possible in API API design.
...
fields, here is what happens in this action:
 ...
 - `limit` of the job takes the value of `""`, which means that Distronode will
...
that become `distronode-playbook` options.
