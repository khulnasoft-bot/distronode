...
Making individual API calls in rapid succession or at high concurrency can overwhelm API's ability to serve web requests. When the application's ability to serve is exhausted, clients often receive 504 timeout errors.
...
Following is an example of a post request at the /api/v2/bulk/host_create:
    ...
    {
        "inventory": 1,
        "hosts": [{"name": "host1", "variables": "distronode_connection: local"}, {"name": "host2"}, {"name": "host3"}, {"name": "host4"}, {"name": "host5"}, {"name": "host6"}]
    }
