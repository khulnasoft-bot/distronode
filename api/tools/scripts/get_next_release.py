...
def getCurrentVersions():
    ...
    for repo in product_repos:
        response = session.get('https://api.github.com/repos/distronode/{}/releases'.format(repo))
...
def getNextReleases():
    ...
    #
    for repo in product_repos:
        response = session.get('https://api.github.com/repos/distronode/{}/compare/{}...devel'.format(repo, versions['current'][repo]))
        ...
        for commit in commit_data['commits']:
            response = session.get('https://api.github.com/repos/distronode/{}/commits/{}/pulls'.format(repo, commit['sha']))
            prs = response.json()
            for a_pr in prs:
        ...
        print("https://github.com/distronode/{}/compare/{}...devel".format(repo, versions['current'][repo]))
...
product_repos = ['api', 'api-operator']
...
@newsbot We're happy to announce that the next release of API, version {0} is now available!
Some notable features include:
{2}

In addition API Operator version {1} has also been released!
Some notable features include:
{3}

Please see the releases pages for more details:
API: [https://github.com/distronode/distronode/tree/devel/api/releases/tag/{0}](https://github.com/distronode/distronode/tree/devel/api/releases/tag/{0})
Operator: [https://github.com/distronode/distronode/tree/devel/api-operator/releases/tag/{1}](https://github.com/distronode/distronode/tree/devel/api-operator/releases/tag/{1})

{4}
'''.format(
        versions['next']['api'],
        versions['next']['api-operator'],
        '\n'.join(versions['release_notes']['api']),
        '\n'.join(versions['release_notes']['api-operator']),
        display_known_issues,
    )
)
...
Subject: Announcing API {0} and API-Operator {1}
Body:
Hi all,

We're happy to announce that the next release of API, version {0} is now available!
Some notable features include:
{2}
...
Please see the releases pages for more details:
API: https://github.com/distronode/distronode/tree/devel/api/releases/tag/{0}
Operator: https://github.com/distronode/distronode/tree/devel/api-operator/releases/tag/{1}
