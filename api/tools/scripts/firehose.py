#! /usr/bin/env api-python

#
# !!! READ BEFORE POINTING THIS AT YOUR FOOT !!!
#
# This script attempts to connect to an API database and insert (by default)
...
db = json.loads(subprocess.check_output(['api-manage', 'print_settings', 'DATABASES', '--format', 'json']))
...
def generate_jobs(jobs, batch_size, time_delta):
    ...
    from api import prepare_env

    prepare_env()
    setup_django()

    from api.main.models import UnifiedJob, Job, JobTemplate
    ...
    from api.main.models import JobEvent
    from api.main.utils.common import create_partition
    ...
    try:
        ...
        for conname, contype, condef in constraints:
            if contype == 'c':
                ...
                # worthless, because Distronode doesn't emit counters, line
