...
We configure prometheus to use basic auth and poll API every 5 seconds by default. This configuration is created using the `docker-compose/distronode/roles/sources/templates/prometheus.yml.j2` template. To change the values, modify the template or set the distronode variables used in that template (see use of `EXTRA_SOURCES_DISTRONODE_OPTS` in the next section).
...
To run the development environment (see [docs](https://github.com/distronode/distronode/tree/devel/api/blob/devel/tools/docker-compose/README.md)) with Prometheus and Grafana enabled, set the following variables:

```
$ PROMETHEUS=yes GRAFANA=yes make docker-compose
```

If we want to pass additional distronode vars used in the deployment process, we can pass `EXTRA_SOURCES_DISTRONODE_OPTS`:

```
GRAFANA=true PROMETHEUS=true EXTRA_SOURCES_DISTRONODE_OPTS="-e scrape_interval=1 admin_password=foobar" make docker-compose
```
...
4. In the left navigation menu go to Dashboards->Browse, find the "api-demo" and click. These should have graphs.
...
This alert is named "capacity_below_10_percent" and can be found in this directory in https://github.com/distronode/distronode/tree/devel/api/blob/devel/tools/grafana/alerting/alerts.yml
