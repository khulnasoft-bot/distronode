...
main() {
  ...
  _tmp=$(python -c "import tempfile; print(tempfile.mkdtemp(suffix='.api-requirements', dir='/tmp'))")
  ...
  if [[ "$NEEDS_HELP" == "1" ]] ; then
    ...
    echo "It should be run from within the api container"
    echo ""
  ...
  if [[ ! -d /api_devel ]] ; then
      echo "This script should be run inside the api container"
      exit
  fi
  ...
  echo "Changing $base_dir to /api_devel/requirements"
  cat requirements.txt | sed "s:$base_dir:/api_devel/requirements:" > "${dest_requirements}"
