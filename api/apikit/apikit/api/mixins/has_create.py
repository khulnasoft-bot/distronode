...
from apikit.utils import get_class_if_instance, class_name_to_kw_arg, is_proper_subclass, super_dir_set
from apikit.utils.toposort import toposort
...
class DSAdapter(object):
    ...
    ex:
    ```
    base_sc = HasCreate().create(inventory=apikit.api.Inventory)
...
class HasCreate(object):
    ...
    def create_and_update_dependencies(self, *provided_and_desired_dependencies):
        ...
        ***
        ...
        ```
        ex:
        self.dependencies = [apikit.api.pages.Inventory]
        self.create_and_update_dependencies()
        inventory = self._dependency_store[apikit.api.pages.Inventory]

        ex:
        self.dependencies = [apikit.api.pages.Inventory]
        self.create_and_update_dependencies((apikit.api.pages.Inventory, dict(attr_one=1, attr_two=2)))
        inventory = self._dependency_store[apikit.api.pages.Inventory]
        ...
        ex:
        self.dependencies = []
        self.optional_dependencies = [apikit.api.pages.Organization]
        self.create_and_update_dependencies(apikit.api.pages.Organization)
        organization = self._dependency_store[apikit.api.pages.Organization]
