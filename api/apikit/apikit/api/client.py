...
from apikit import exceptions as exc
...
class Connection(object):
    """A requests.Session wrapper for establishing connection w/ API instance"""

    def __init__(self, server, verify=False):
        self.server = server
        self.verify = verify
        # Note: We use the old sessionid here incase someone is trying to connect to an older API version
        # There is a check below so that if API returns an X-API-Session-Cookie-Name we will grab it and
