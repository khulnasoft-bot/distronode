...
from apikit.cli.utils import HelpfulArgumentParser, cprint, disable_color, colored
from apikit.api.utils import uses_sessions  # noqa


__version__ = _get_version('apikit')


class CLI(object):
    """A programmatic HTTP OPTIONS-based CLI for API/Distronode Tower.

    This CLI works by:

    - Configuring CLI options via Python's argparse (authentication, formatting
      options, etc...)
    - Discovering API API endpoints at /api/v2/ and mapping them to _resources_
    ...
    def connect(self):
        ...
        if remainder and remainder[0] == 'config':
            ...
            # API connectivity
            return
        # ...otherwise, set up a apikit connection because we're
        ...
        try:
            ...
            if self.help:
                ...
                if len(unknown) == 1 and os.path.basename(unknown[0]) == 'api':
                    return
            raise
    ...
    def parse_action(self, page, from_sphinx=False):
        ...
        :param page: a apikit.api.pages.TentativePage object representing the
        ...
        # command line so we can pass them on to the underlying apikit call
        ...
        # determine the apikit method to call
    ...
    def parse_args(self, argv, env=None):
        ...
        self.parser.add_argument(
            '--help',
            action='store_true',
            help='prints usage information for the api tool',
        )
        self.parser.add_argument('--version', dest='conf.version', action='version', help='display api CLI version', version=__version__)
