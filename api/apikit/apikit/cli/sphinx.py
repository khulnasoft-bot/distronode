...
class CustomAutoprogramDirective(AutoprogramDirective):
    def run(self):
        ...
        nodes[0].children.insert(1, paragraph(text=('This is an exhaustive guide of every available command in the api CLI tool.')))
        disclaimer = (
            'The commands and parameters documented here can (and will) '
            'vary based on a variety of factors, such as the API API '
            'version, API settings, and access level of the authenticated '
            'user.  For the most accurate view of available commands, '
            'invoke the api CLI using the --help flag.'
        )
