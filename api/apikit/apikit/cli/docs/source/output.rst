...
By default, api prints valid JSON for successful commands.  The ``-f`` (or
...
.. code:: bash
    ...
    api jobs list -f human
    api jobs list -f human --filter name,created,status
    api jobs list -f human --filter *
