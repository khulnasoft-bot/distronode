    ...
    if page:
        ...
        if results.count > 1:
            raise argparse.ArgumentTypeError(
                'Multiple {0} exist with that {1}. To look up an ID, run:\napi {0} list --{1} "{2}" -f human'.format(model_name, identity, value)
            )
...
class ResourceOptionsParser(object):
    ...
    def __init__(self, v2, page, resource, parser):
        ...
        :param v2: a apikit.api.pages.page.TentativePage instance
        :param page: a apikit.api.pages.page.TentativePage instance
    ...
    def build_query_arguments(self, method, http_method):
        ...
        for k, param in self.options.get(http_method, {}).items():
            ...
            if method == 'list':
                if k == 'id':
                    # don't allow `api <resource> list` to filter on `--id`
                    # it's weird, and that's what api <resource> get is for
                    continue
            ...
            # see: https://github.com/distronode/distronode/tree/devel/api/issues/2371
