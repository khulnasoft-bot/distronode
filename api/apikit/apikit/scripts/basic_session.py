...
from apikit import api, config, utils, exceptions, WSClient  # noqa
from apikit.api.utils import check_related, delete_all, get_all, uses_sessions  # noqa
from apikit.api.utils import as_user as _as_user

if str(os.getenv('APIKIT_DEBUG', 'false')).lower() in ['true', '1']:
...
def parse_args():
    ...
    parser.add_argument(
        '--base-url',
        dest='base_url',
        default=os.getenv('APIKIT_BASE_URL', 'http://127.0.0.1:8013'),
        help='URL for API.  Defaults to env var APIKIT_BASE_URL or http://127.0.0.1:8013',
    )
    parser.add_argument(
        '-c',
        '--credential-file',
        dest='credential_file',
        default=os.getenv('APIKIT_CREDENTIAL_FILE', utils.not_provided),
        help='Path for yml credential file.  If not provided or set by APIKIT_CREDENTIAL_FILE, set '
        'APIKIT_USER and APIKIT_USER_PASSWORD env vars for api user credentials.',
    )
    parser.add_argument(
        '-p',
        '--project-file',
        dest='project_file',
        default=os.getenv('APIKIT_PROJECT_FILE'),
        help='Path for yml project config file.If not provided or set by APIKIT_PROJECT_FILE, projects will not have default SCM_URL',
    )
    ...
    try:
        ...
        if akit_args.credential_file != utils.not_provided:
            ...
            config.credentials = utils.PseudoNamespace(
                {'default': {'username': os.getenv('APIKIT_USER', 'admin'), 'password': os.getenv('APIKIT_USER_PASSWORD', 'password')}}
            )
