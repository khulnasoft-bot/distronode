...
`host_filter` can also be used to query JSON data in the related `distronode_facts`. `__` may be used to traverse JSON dictionaries. `[]` may be used to traverse JSON arrays.

    ?host_filter=distronode_facts__distronode_processor_vcpus=8
    ?host_filter=distronode_facts__distronode_processor_vcpus=8 and name="my_host" and distronode_facts__distronode_lo__ipv6[]__scope=host
