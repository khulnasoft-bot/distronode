...
from distronode_base.lib.utils.models import get_type_for_model
from distronode_base.rbac.models import RoleEvaluation, ObjectRole
from distronode_base.rbac import permission_registry

# API
from api.main.access import get_user_capabilities
from api.main.constants import ACTIVE_STATES, org_role_to_permission
...
from api.main.models.base import VERBOSITY_CHOICES, NEW_JOB_TYPE_CHOICES
from api.main.models.rbac import role_summary_fields_generator, give_creator_permissions, get_role_codenames, to_permissions, get_role_from_object_role
from api.main.fields import ImplicitRoleField
from api.main.utils import (
    get_model_for_type,
...
from api.main.utils.filters import SmartFilter
from api.main.utils.plugins import load_combined_inventory_source_options
from api.main.utils.named_url_graph import reset_counters
from api.main.scheduler.task_manager_models import TaskManagerModels
from api.main.redact import UriCleaner, REPLACE_STR
from api.main.signals import update_inventory_computed_fields


from api.main.validators import vars_validate_or_raise

from api.api.versioning import reverse
from api.api.fields import BooleanNullField, CharNullField, ChoiceNullField, VerbatimField, DeprecatedCredentialField

# API Utils
from api.api.validators import HostnameRegexValidator
...
SUMMARIZABLE_FK_FIELDS = {
    ...
    'resource': ('distronode_id', 'resource_type'),
}
        ...
        if host_filter:
            try:
                for match in models.JSONField.get_lookups().keys():
                    ...
                    if re.match('distronode_facts[^=]+{}='.format(match), host_filter):
                        raise models.base.ValidationError({'host_filter': 'distronode_facts does not support searching with {}'.format(match)})
...
class HostSerializer(BaseSerializerWithVariables):
    ...
    class Meta:
        ...
        fields = (
            ...
            'distronode_facts_modified',
        )
        read_only_fields = ('last_job', 'last_job_host_summary', 'distronode_facts_modified')
    ...
    def get_related(self, obj):
        ...
        res.update(
            dict(
                ...
                distronode_facts=self.reverse('api:host_distronode_facts_detail', kwargs={'pk': obj.pk}),
            )
        )
        if obj.inventory.kind == 'constructed':
            res['original_host'] = self.reverse('api:host_detail', kwargs={'pk': obj.instance_id})
            res['distronode_facts'] = self.reverse('api:host_distronode_facts_detail', kwargs={'pk': obj.instance_id})
        if obj.inventory:
    ...
    def validate(self, attrs):
        ...
        if port:
            ...
            vars_dict['distronode_ssh_port'] = port
...
class DistronodeFactsSerializer(BaseSerializer):
    class Meta:
        model = Host

    def to_representation(self, obj):
        return obj.distronode_facts
...
class ResourceAccessListElementSerializer(UserSerializer):
    ...
    def to_representation(self, user):
        ...
        if settings.DISTRONODE_BASE_ROLE_SYSTEM_ACTIVATED:
...
class AdHocCommandSerializer(UnifiedJobSerializer):
    ...
    def validate_extra_vars(self, value):
        redacted_extra_vars, removed_vars = extract_distronode_vars(value)
        if removed_vars:
...
class ProjectUpdateEventSerializer(JobEventSerializer):
    ...
    def get_event_data(self, obj):
        ...
        if obj.event_data.get('task_action') in ('git', 'svn', 'distronode.builtin.git', 'distronode.builtin.svn'):
            try:
...
class NotificationTemplateSerializer(BaseSerializer):
            ...
            for event in messages:
                ...
                if body:
                    try:
                        ...
                        # https://github.com/distronode/distronode/tree/devel/api/issues/14410
    ...
    def validate(self, attrs):
        from api.api.views import NotificationTemplateDetail
...
class ScheduleSerializer(LaunchConfigurationBaseSerializer, SchedulePreviewSerializer):
    ...
    def validate(self, attrs):
        ...
        # see: https://github.com/distronode/distronode/tree/devel/api/issues/8641
...
class ActivityStreamSerializer(BaseSerializer):
    ...
    def get_object_association(self, obj):
        if not obj.object_relationship_type:
            ...
            # "api.main.models.inventory.Inventory.admin_role"
            # due to historical reasons the UI expects just "role" here
            return "role"
        # default case: these values look like
        # "api.main.models.organization.Organization_notification_templates_success"
