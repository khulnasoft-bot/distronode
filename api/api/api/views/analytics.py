...
from api.api.permissions import AnalyticsPermission
...
API_ANALYTICS_API_PREFIX = 'analytics'
...
logger = logging.getLogger('api.api.views.analytics')
...
class AnalyticsGenericView(APIView):
    ...
    def _request_headers(request):
        ...
        headers['X-Rh-Analytics-Source-Version'] = get_api_version()
    ...
    def _get_analytics_path(request_path):
        parts = request_path.split(f'{API_ANALYTICS_API_PREFIX}/')
    ...
    def _update_response_links(json_response):
        ...
        for key, value in json_response['links'].items():
            if value:
                json_response['links'][key] = value.replace(AUTOMATION_ANALYTICS_API_URL_PATH, f"/api/v2/{API_ANALYTICS_API_PREFIX}")
