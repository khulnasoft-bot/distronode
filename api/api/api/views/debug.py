...
class TaskManagerDebugView(APIView):
    ...
    def get(self, request):
        ...
        if not settings.API_DISABLE_TASK_MANAGERS:
            msg = f"Running {self.prefix} manager. To disable other triggers to the {self.prefix} manager, set API_DISABLE_TASK_MANAGERS to True"
        else:
            msg = f"API_DISABLE_TASK_MANAGERS is True, this view is the only way to trigger the {self.prefix} manager"
        return Response(msg)
