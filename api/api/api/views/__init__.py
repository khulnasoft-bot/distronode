...
from distronode_base.lib.utils.requests import get_remote_hosts
...
# API
from api.main.tasks.system import send_notifications, update_inventory_computed_fields
from api.main.access import get_user_queryset
from api.api.generics import (
    APIView,
    BaseUsersList,
    CopyAPIView,
    GenericCancelView,
...
from api.api.views.labels import LabelSubListCreateAttachDetachView
from api.api.versioning import reverse
from api.main import models
from api.main.models.rbac import get_role_definition
...
from api.main.utils.encryption import encrypt_value
from api.main.utils.filters import SmartFilter
from api.main.utils.plugins import compute_cloud_inventory_sources
from api.main.redact import UriCleaner
...
from api.api import renderers
from api.api import serializers
from api.api.metadata import RoleMetadata
from api.main.constants import ACTIVE_STATES, SURVEY_TYPE_MAPPING
from api.main.scheduler.dag_workflow import WorkflowDAG
from api.api.views.mixin import (
...
from api.api.pagination import UnifiedJobEventPagination
from api.main.utils import set_environ

logger = logging.getLogger('api.api.views')
...
def api_exception_handler(exc, context):
    ...
    if isinstance(exc, APIException):
        ...
        if 'api.named_url_rewritten' in req.environ and not str(getattr(exc, 'status_code', 0)).startswith('2'):
            ...
            req.path = req.environ['api.named_url_rewritten']
...
class InstanceHealthCheck(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if obj.node_type == models.Instance.Types.EXECUTION:
            from api.main.tasks.system import execution_node_health_check
...
class CredentialExternalTest(SubDetailAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            with set_environ(**settings.API_TASK_ENV):
...
class HostRelatedSearchMixin(object):
    ...
    def related_search_fields(self):
        # Edge-case handle: https://github.com/distronode/distronode-tower/issues/7712
        ret = super(HostRelatedSearchMixin, self).related_search_fields
        ret.append('distronode_facts')
        return ret
...
class HostDistronodeFactsDetail(RetrieveAPIView):
    model = models.Host
    serializer_class = serializers.DistronodeFactsSerializer
    ...
    def get(self, request, *args, **kwargs):
        ...
        if obj.inventory.kind == 'constructed':
            ...
            return HttpResponseRedirect(reverse('api:host_distronode_facts_detail', kwargs={'pk': obj.instance_id}, request=self.request))
...
class InventorySourceCredentialsList(SubListAttachDetachAPIView):
    ...
    def is_valid_relation(self, parent, sub, created=False):
        ...
        # subject to change for https://github.com/distronode/distronode/tree/devel/api/issues/277
        # or https://github.com/distronode/distronode/tree/devel/api/issues/223
...
class JobTemplateCallback(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if extra_vars is not None and job_template.ask_variables_on_launch:
            extra_vars_redacted, removed = extract_distronode_vars(extra_vars)
...
class JobJobEventsChildrenSummary(APIView):
    ...
    def get(self, request, **kwargs):
        ...
        # hierarchy. If distronode is ran with a strategy like free or host_pinned, then
