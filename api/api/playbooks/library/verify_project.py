DISTRONODE_METADATA = {"metadata_version": "1.0", "status": ["stableinterface"], "supported_by": "community"}
...
description:
  - Makes use of the 'distronode-sign' project as a library for ensuring that an
    Distronode project has not been tampered with.
  ...
  project_path:
    description:
      - Directory of the project being verified. Expected to contain a
        C(.distronode-sign) directory with a generated checksum manifest file and a
        detached signature for it. These files are produced by the
        C(distronode-sign) command-line utility.
    required: true
  validation_type:
    description:
  ...
  gpg_pubkey:
    description:
      - The public key to validate a checksum manifest against. Must match the
        detached signature in the project's C(.distronode-sign) directory.
