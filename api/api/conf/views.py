...
# API
from api.api.generics import APIView, GenericAPIView, ListAPIView, RetrieveUpdateDestroyAPIView
from api.api.permissions import IsSystemAdminOrAuditor
from api.api.versioning import reverse
from api.main.utils import camelcase_to_underscore
from api.main.tasks.system import clear_setting_cache
from api.conf.models import Setting
from api.conf.serializers import SettingCategorySerializer, SettingSingletonSerializer
from api.conf import settings_registry
from api.main.utils.external_logging import reconfigure_rsyslog
...
class SettingLoggingTest(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if default_logger != 'api':
                default_logger = f'api.analytics.{default_logger}'
        except IndexError:
            default_logger = 'api'
        logging.getLogger(default_logger).error('API Connection Test Message')

        hostname = getattr(settings, 'LOG_AGGREGATOR_HOST', None)
        protocol = getattr(settings, 'LOG_AGGREGATOR_PROTOCOL', None)

        try:
            subprocess.check_output(['rsyslogd', '-N1', '-f', '/var/lib/api/rsyslog/rsyslog.conf'], stderr=subprocess.STDOUT)
