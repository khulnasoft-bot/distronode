...
from api.conf.fields import PrimaryKeyRelatedField
...
logger = logging.getLogger('api.conf.settings')
...
class SettingsWrapper(UserSettingsHolder):
    ...
    def initialize(cls, cache=None, registry=None):
        ...
        ``api.conf.settings_registry`` is used by default.
        """
        if not getattr(settings, '_api_conf_settings', False):
    ...
    def __init__(self, default_settings, cache, registry):
        ...
        self.__dict__['_api_conf_settings'] = self
        self.__dict__['_api_conf_preload_expires'] = None
        self.__dict__['_api_conf_preload_lock'] = threading.RLock()
        self.__dict__['_api_conf_init_readonly'] = False
        self.__dict__['cache'] = EncryptedCacheProxy(cache, registry)
        self.__dict__['registry'] = registry
        self.__dict__['_api_conf_memoizedcache'] = cachetools.TTLCache(maxsize=2048, ttl=SETTING_MEMORY_TTL)
        self.__dict__['_api_conf_memoizedcache_lock'] = threading.Lock()
    ...
    def _preload_cache(self):
        ...
        with self._api_conf_preload_lock:
            # If local preload timeout has not expired, skip preloading.
            if self._api_conf_preload_expires and self._api_conf_preload_expires > time.time():
                return
            # Otherwise update local preload timeout.
            self.__dict__['_api_conf_preload_expires'] = time.time() + SETTING_CACHE_TIMEOUT
            ...
            if not self._api_conf_init_readonly:
                ...
                for key in get_writeable_settings(self.registry):
                    ...
                    self.__dict__['_api_conf_init_readonly'] = True
        ...
        if self.cache.get('_api_conf_preload_expires', default=empty) is not empty:
            return
        ...
        settings_to_cache['_api_conf_preload_expires'] = self._api_conf_preload_expires
    ...
    @cachetools.cachedmethod(
        cache=lambda self: self.__dict__['_api_conf_memoizedcache'],
        key=lambda *args, **kwargs: SettingsWrapper.hashkey(*args, **kwargs),
        lock=lambda self: self.__dict__['_api_conf_memoizedcache_lock'],
    )
...
def __getattr_without_cache__(self, name):
    ...
    # change in-process (the entire point of api's custom settings implementation)
