...
from api.conf import models, fields
from api.conf.settings import SettingsWrapper, EncryptedCacheProxy, SETTING_CACHE_NOTSET
from api.conf.registry import SettingsRegistry

from api.main.utils import encrypt_field, decrypt_field
...
def settings(request):
    ...
    `api.conf.settings.SettingsWrapper` and passes it as an argument into the
    test function.

    This mimics the work done by `api.conf.settings.SettingsWrapper.initialize`
...
def test_read_only_setting(settings):
    settings.registry.register('API_READ_ONLY', field_class=fields.CharField, category=_('System'), category_slug='system', default='NO-EDITS', read_only=True)
    assert settings.API_READ_ONLY == 'NO-EDITS'
    ...
    assert settings == ['API_READ_ONLY']


@pytest.mark.defined_in_file(API_SOME_SETTING='DEFAULT')
@pytest.mark.parametrize('read_only', [True, False])
def test_setting_defined_in_file(settings, read_only):
    kwargs = {'read_only': True} if read_only else {}
    settings.registry.register('API_SOME_SETTING', field_class=fields.CharField, category=_('System'), category_slug='system', **kwargs)
    assert settings.API_SOME_SETTING == 'DEFAULT'
    assert len(settings.registry.get_registered_settings(read_only=False)) == 0
    settings = settings.registry.get_registered_settings(read_only=True)
    assert settings == ['API_SOME_SETTING']


@pytest.mark.defined_in_file(API_SOME_SETTING='DEFAULT')
def test_setting_defined_in_file_with_empty_default(settings):
    settings.registry.register('API_SOME_SETTING', field_class=fields.CharField, category=_('System'), category_slug='system', default='')
    assert settings.API_SOME_SETTING == 'DEFAULT'
    assert len(settings.registry.get_registered_settings(read_only=False)) == 0
    settings = settings.registry.get_registered_settings(read_only=True)
    assert settings == ['API_SOME_SETTING']
...
def test_setting_defined_in_file_with_specific_default(settings):
    settings.registry.register('API_SOME_SETTING', field_class=fields.CharField, category=_('System'), category_slug='system', default=123)
    assert settings.API_SOME_SETTING == 'DEFAULT'
    assert len(settings.registry.get_registered_settings(read_only=False)) == 0
    settings = settings.registry.get_registered_settings(read_only=True)
    assert settings == ['API_SOME_SETTING']
...
def test_read_only_defaults_are_cached(settings):
    ...
    settings.registry.register('API_SOME_SETTING', field_class=fields.CharField, category=_('System'), category_slug='system')
    assert settings.API_SOME_SETTING == 'DEFAULT'
    assert settings.cache.get('API_SOME_SETTING') == 'DEFAULT'
...
def test_cache_respects_timeout(settings):
    ...
    assert settings.API_SOME_SETTING == 'DEFAULT'
    cache_expiration = settings.cache.get('_api_conf_preload_expires')
    assert cache_expiration > time.time()

    assert settings.API_SOME_SETTING == 'DEFAULT'
    assert settings.cache.get('_api_conf_preload_expires') == cache_expiration


def test_default_setting(settings, mocker):
    "settings that specify a default are inserted into the cache"
    settings.registry.register('API_SOME_SETTING', field_class=fields.CharField, category=_('System'), category_slug='system', default='DEFAULT')

    settings_to_cache = mocker.Mock(**{'order_by.return_value': []})
    mocker.patch('api.conf.models.Setting.objects.filter', return_value=settings_to_cache)
    assert settings.API_SOME_SETTING == 'DEFAULT'
    assert settings.cache.get('API_SOME_SETTING') == 'DEFAULT'
...
def test_setting_is_from_setting_file(settings, mocker):
    settings.registry.register('API_SOME_SETTING', field_class=fields.CharField, category=_('System'), category_slug='system')
    assert settings.API_SOME_SETTING == 'DEFAULT'
    assert settings.registry.get_setting_field('API_SOME_SETTING').defined_in_file is True
...
def test_setting_is_not_from_setting_file(settings, mocker):
    ...
    mocker.patch('api.conf.models.Setting.objects.filter', return_value=settings_to_cache)
    assert settings.API_SOME_SETTING == 'DEFAULT'
    assert settings.registry.get_setting_field('API_SOME_SETTING').defined_in_file is False
...
def test_empty_setting(settings, mocker):
    ...
    mocker.patch('api.conf.models.Setting.objects.filter', return_value=mocks)
    with pytest.raises(AttributeError):
        settings.API_SOME_SETTING
    assert settings.cache.get('API_SOME_SETTING') == SETTING_CACHE_NOTSET
...
def test_setting_from_db(settings, mocker):
    ...
    setting_from_db = mocker.Mock(key='API_SOME_SETTING', value='FROM_DB')
    ...
    mocker.patch('api.conf.models.Setting.objects.filter', return_value=mocks)
    assert settings.API_SOME_SETTING == 'FROM_DB'
    assert settings.cache.get('API_SOME_SETTING') == 'FROM_DB'
...
def test_read_only_setting_assignment(settings):
    ...
    with pytest.raises(ImproperlyConfigured):
        settings.API_SOME_SETTING = 'CHANGED'
    assert settings.API_SOME_SETTING == 'DEFAULT'
...
def test_db_setting_create(settings, mocker):
    ...
    with apply_patches(
        [
            mocker.patch('api.conf.models.Setting.objects.filter', return_value=setting_list),
            mocker.patch('api.conf.models.Setting.objects.create', mocker.Mock()),
        ]
    ):
        settings.API_SOME_SETTING = 'NEW-VALUE'

    models.Setting.objects.create.assert_called_with(key='API_SOME_SETTING', user=None, value='NEW-VALUE')
...
def test_db_setting_update(settings, mocker):
    ...
    existing_setting = mocker.Mock(key='API_SOME_SETTING', value='FROM_DB')
...
def test_db_setting_deletion(settings, mocker):
    ...
    existing_setting = mocker.Mock(key='API_SOME_SETTING', value='FROM_DB')
    mocker.patch('api.conf.models.Setting.objects.filter', return_value=[existing_setting])
    del settings.API_SOME_SETTING
...
def test_charfield_properly_sets_none(settings, mocker):
    "see: https://github.com/distronode/distronode-tower/issues/5322"
    settings.registry.register('API_SOME_SETTING', field_class=fields.CharField, category=_('System'), category_slug='system', allow_null=True)
    ...
    with apply_patches(
        ...
        settings.API_SOME_SETTING = None

    models.Setting.objects.create.assert_called_with(key='API_SOME_SETTING', user=None, value=None)


def test_settings_use_cache(settings, mocker):
    settings.registry.register('API_VAR', field_class=fields.CharField, category=_('System'), category_slug='system')
    settings.cache.set('API_VAR', 'foobar')
    settings.cache.set('_api_conf_preload_expires', 100)
    # Will fail test if database is used
    getattr(settings, 'API_VAR')


def test_settings_use_an_encrypted_cache(settings, mocker):
    settings.registry.register('API_ENCRYPTED', field_class=fields.CharField, category=_('System'), category_slug='system', encrypted=True)
    ...
    settings.cache.set('API_ENCRYPTED_ID', 402)
    settings.cache.set('API_ENCRYPTED', 'foobar')
    settings.cache.set('_api_conf_preload_expires', 100)
    # Will fail test if database is used
    getattr(settings, 'API_ENCRYPTED')
...
def test_sensitive_cache_data_is_encrypted(settings, mocker):
    ...
    setting_from_db = mocker.Mock(pk=123, key='API_ENCRYPTED', value='SECRET!')
    mocks = mocker.Mock(**{'order_by.return_value': mocker.Mock(**{'__iter__': lambda self: iter([setting_from_db]), 'first.return_value': setting_from_db})})
    mocker.patch('api.conf.models.Setting.objects.filter', return_value=mocks)
    cache.set('API_ENCRYPTED', 'SECRET!')
    assert cache.get('API_ENCRYPTED') == 'SECRET!'
    assert native_cache.get('API_ENCRYPTED') == 'FRPERG!'
...
def test_readonly_sensitive_cache_data_is_encrypted(settings):
    ...
    settings.registry.register('API_ENCRYPTED', field_class=fields.CharField, category=_('System'), category_slug='system', read_only=True, encrypted=True)
...
@pytest.mark.defined_in_file(API_VAR='DEFAULT')
def test_in_memory_cache_only_for_registered_settings(settings):
    ...
    settings._api_conf_memoizedcache.clear()
    settings.MIDDLEWARE
    assert len(settings._api_conf_memoizedcache) == 0  # does not cache MIDDLEWARE
    settings.registry.register('API_VAR', field_class=fields.CharField, category=_('System'), category_slug='system')
    settings._wrapped.__dict__['all_supported_settings'] = ['API_VAR']  # because it is cached_property
    settings._api_conf_memoizedcache.clear()
    assert settings.API_VAR == 'DEFAULT'
    assert len(settings._api_conf_memoizedcache) == 1  # caches registered settings
...
def test_in_memory_cache_works(settings):
    settings._api_conf_memoizedcache.clear()
    settings.registry.register('API_VAR', field_class=fields.CharField, category=_('System'), category_slug='system')
    settings._wrapped.__dict__['all_supported_settings'] = ['API_VAR']

    settings._api_conf_memoizedcache.clear()

    with mock.patch('api.conf.settings.SettingsWrapper._get_local', return_value='DEFAULT') as mock_get:
        assert settings.API_VAR == 'DEFAULT'
        mock_get.assert_called_once_with('API_VAR')
...
@pytest.mark.defined_in_file(API_VAR=[])
def test_getattr_with_database_error(settings):
    ...
    """
    settings.registry.register('API_VAR', field_class=fields.StringListField, default=[], category=_('System'), category_slug='system')
    settings._api_conf_memoizedcache.clear()
