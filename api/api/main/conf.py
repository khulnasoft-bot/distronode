...
# API
from api.conf import fields, register, register_validate
from api.main.models import ExecutionEnvironment
from api.main.constants import SUBSCRIPTION_USAGE_MODEL_UNIQUE_HOSTS

logger = logging.getLogger('api.main.conf')
...
register(
    ...
    help_text=_('Paths where Tower will look for custom virtual environments (in addition to /var/lib/api/venv/). Enter one path per line.'),
    category=_('System'),
    category_slug='system',
    default=[],
)
...
register(
    'AD_HOC_COMMANDS',
    field_class=fields.StringListField,
    label=_('Distronode Modules Allowed for Ad Hoc Jobs'),
...
register(
    ...
    help_text=_(
        'Distronode allows variable substitution via the Jinja2 templating '
...
register(
    'API_RUNNER_KEEPALIVE_SECONDS',
    field_class=fields.IntegerField,
    label=_('K8S Distronode Runner Keep-Alive Message Interval'),
...
register(
    ...
    help_text=_(
        'Additional environment variables set for invocations of distronode-galaxy within project updates. '
        'Useful if you must use a proxy server for distronode-galaxy but not git.'
    ),
    category=_('Jobs'),
    category_slug='jobs',
    placeholder={'HTTP_PROXY': 'myproxy.local:8080'},
...
register(
    ...
    help_text=_('Adds the CLI -vvv flag to distronode-playbook runs of project_update.yml used for project updates.'),
    category=_('Jobs'),
    category_slug='jobs',
)
...
register(
    'API_MOUNT_ISOLATED_PATHS_ON_K8S',
    field_class=fields.BooleanField,
    default=False,
...
register(
    ...
    field_class=fields.BooleanField,
    default=False,
    label=_('Ignore Distronode Galaxy SSL Certificate Verification'),
...
register(
    ...
    field_class=fields.StringListField,
    required=False,
    label=_('Distronode Callback Plugins'),
...
register(
    ...
    help_text=_(
        'If no output is detected from distronode in this number of seconds the execution will be terminated. '
...
register(
    'DISTRONODE_FACT_CACHE_TIMEOUT',
    field_class=fields.IntegerField,
    min_value=0,
    default=0,
    label=_('Per-Host Distronode Fact Cache Timeout'),
    help_text=_(
        'Maximum time, in seconds, that stored Distronode facts are considered valid since '
        'the last time they were modified. Only valid, non-stale, facts will be accessible by '
        'a playbook. Note, this does not influence the deletion of distronode_facts from the database. '
...
register(
    ...
    field_class=fields.StringListField,
    default=['api', 'activity_stream', 'job_events', 'system_tracking', 'broadcast_websocket', 'job_lifecycle'],
    ...
    help_text=_(
        ...
        'job_events - callback data from Distronode job events\n'
...
register(
    ...
    help_text=_(
        ...
        'api.anlytics ignore this setting)'
    ),
    category=_('Logging'),
    category_slug='logging',
)
...
register(
    ...
    help_text=_(
        ...
        'of the external log aggregator (defaults to /var/lib/api). '
