...
from api.main.utils import memoize
...
class RecordedQueryLog(object):
    def __init__(self, log, db, dest='/var/log/tower/profile'):
        ...
        self.dest = dest
        try:
            self.threshold = cache.get('api-profile-sql-threshold')
        except Exception:
...
class DatabaseWrapper(BaseDatabaseWrapper):
    ...
    def force_debug_cursor(self):
        ...
        try:
            threshold = cache.get('api-profile-sql-threshold')
        except Exception:
            # if we can't reach the cache, just assume profiling's off
            threshold = None
