...
# API
from api.main.dispatch.reaper import reap_job
...
from api.main.utils.common import task_manager_bulk_reschedule, is_testing
from api.main.signals import disable_activity_stream
from api.main.constants import ACTIVE_STATES
from api.main.scheduler.dependency_graph import DependencyGraph
from api.main.scheduler.task_manager_models import TaskManagerModels
import api.main.analytics.subsystem_metrics as s_metrics
...
class TaskBase:
    def __init__(self, prefix=""):
        ...
        self.control_task_impact = settings.API_CONTROL_NODE_TASK_IMPACT
        ...
        for workflow_job in self.all_tasks:
            ...
            if workflow_job.status == 'running':
                ...
                for spawn_node in spawn_nodes:
                    ...
                    # TODO: should we emit a status on the socket here similar to tasks.py api_periodic_scheduler() ?
...
class TaskManager(TaskBase):
    ...
    def reap_jobs_from_orphaned_instances(self):
        ...
        # for example, SQL backup an api install with running jobs and restore it
        # elsewhere
    ...
    def process_tasks(self):
        ...
        if self.pre_start_failed:
            from api.main.tasks.system import handle_failure_notifications
