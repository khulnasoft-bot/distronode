...
from api.main.utils.common import parse_yaml_or_json, deepmerge
...
class PodManager(object):
    ...
    def list_active_jobs(self, instance_group):
        ...
        pods = {}
        try:
            for pod in pm.kube_api.list_namespaced_pod(pm.namespace, label_selector='distronode-api={}'.format(settings.INSTALL_UUID)).to_dict().get('items', []):
                job = pod['metadata'].get('labels', {}).get('distronode-api-job-id')
                if job:
                    try:
    ...
    def pod_definition(self):
        ...
        if self.task:
            pod_spec['metadata'] = deepmerge(
                pod_spec.get('metadata', {}), dict(name=self.pod_name, labels={'distronode-api': settings.INSTALL_UUID, 'distronode-api-job-id': str(self.task.id)})
            )
