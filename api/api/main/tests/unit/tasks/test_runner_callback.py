from api.main.tasks.callback import RunnerCallback
from api.main.constants import DISTRONODE_RUNNER_NEEDS_UPDATE_MESSAGE
...
def test_special_distronode_runner_message(mock_me):
    rc = RunnerCallback()
    ...
    assert rc.get_delayed_update_fields().get('result_traceback') == (
        'Traceback:\ngot an unexpected keyword argument\nFile: foo.py\n'
        'Traceback:\ngot an unexpected keyword argument\nFile: bar.py\n'
        f'{DISTRONODE_RUNNER_NEEDS_UPDATE_MESSAGE}'
    )
