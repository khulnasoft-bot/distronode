...
from api.api.views import JobTemplateDetail
...
@mock.patch('api.api.serializers.JobOptionsSerializer.get_related', lambda x, y: {})
...
class TestJobTemplateSerializerGetSummaryFields:
    ...
    def test_copy_edit_standard(self, mocker, job_template_factory):
        ...
        mocker.patch("api.api.serializers.role_summary_fields_generator", return_value='Can eat pie')
        mocker.patch("api.main.access.JobTemplateAccess.can_change", return_value='foobar')
        mocker.patch("api.main.access.JobTemplateAccess.can_copy", return_value='foo')
