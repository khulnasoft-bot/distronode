...
from api.main.utils.external_logging import construct_rsyslog_conf_template
from api.main.tests.functional.api.test_settings import _mock_logging_defaults
    ...
    [
        ...
        (
            ...
            '\n'.join(
                [
                    'template(name="api" type="string" string="%rawmsg-after-pri%")\nmodule(load="omhttp")',
                    'action(type="omhttp" server="yoursplunk" serverport="80" usehttps="off" allowunsignedcerts="off" skipverifyhost="off" action.resumeRetryCount="-1" template="api" action.resumeInterval="5" queue.spoolDirectory="/var/lib/api" queue.filename="api-external-logger-action-queue" queue.maxDiskSpace="1g" queue.maxFileSize="100m" queue.type="LinkedList" queue.saveOnShutdown="on" queue.syncqueuefiles="on" queue.checkpointInterval="1000" queue.size="131072" queue.highwaterMark="98304" queue.discardMark="117964" queue.discardSeverity="5" errorfile="/var/log/tower/rsyslog.err" restpath="services/collector/event")',  # noqa
                ]
            ),
        ),
        (
            ...
            '\n'.join(
                [
                    'template(name="api" type="string" string="%rawmsg-after-pri%")\nmodule(load="omhttp")',
                    'action(type="omhttp" server="yoursplunk" serverport="8088" usehttps="on" allowunsignedcerts="off" skipverifyhost="off" action.resumeRetryCount="-1" template="api" action.resumeInterval="5" queue.spoolDirectory="/var/lib/api" queue.filename="api-external-logger-action-queue" queue.maxDiskSpace="1g" queue.maxFileSize="100m" queue.type="LinkedList" queue.saveOnShutdown="on" queue.syncqueuefiles="on" queue.checkpointInterval="1000" queue.size="131072" queue.highwaterMark="98304" queue.discardMark="117964" queue.discardSeverity="5" errorfile="/var/log/tower/rsyslog.err" restpath="services/collector/event")',  # noqa
                ]
            ),
        ),
        (
        ...
        (
            ...
            '\n'.join(
                [
                    'template(name="api" type="string" string="%rawmsg-after-pri%")\nmodule(load="omhttp")',
                    'action(type="omhttp" server="yoursplunk.org" serverport="8088" usehttps="on" allowunsignedcerts="off" skipverifyhost="off" action.resumeRetryCount="-1" template="api" action.resumeInterval="5" queue.spoolDirectory="/var/lib/api" queue.filename="api-external-logger-action-queue" queue.maxDiskSpace="1g" queue.maxFileSize="100m" queue.type="LinkedList" queue.saveOnShutdown="on" queue.syncqueuefiles="on" queue.checkpointInterval="1000" queue.size="131072" queue.highwaterMark="98304" queue.discardMark="117964" queue.discardSeverity="5" errorfile="/var/log/tower/rsyslog.err" restpath="services/collector/event")',  # noqa
                ]
            ),
        ),
        (
            ...
            '\n'.join(
                [
                    'template(name="api" type="string" string="%rawmsg-after-pri%")\nmodule(load="omhttp")',
                    'action(type="omhttp" server="yoursplunk.org" serverport="8088" usehttps="off" allowunsignedcerts="off" skipverifyhost="off" action.resumeRetryCount="-1" template="api" action.resumeInterval="5" queue.spoolDirectory="/var/lib/api" queue.filename="api-external-logger-action-queue" queue.maxDiskSpace="1g" queue.maxFileSize="100m" queue.type="LinkedList" queue.saveOnShutdown="on" queue.syncqueuefiles="on" queue.checkpointInterval="1000" queue.size="131072" queue.highwaterMark="98304" queue.discardMark="117964" queue.discardSeverity="5" errorfile="/var/log/tower/rsyslog.err" restpath="services/collector/event")',  # noqa
                ]
            ),
        ),
        (
...
def test_rsyslog_conf_template(enabled, log_type, host, port, protocol, errorfile, expected_config):
    ...
    setattr(mock_settings, 'LOGGING["handlers"]["external_logger"]["address"]', '/var/run/api-rsyslog/rsyslog.sock')
