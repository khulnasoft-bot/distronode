...
from api.main.fields import JSONSchemaField, ImplicitRoleField, ImplicitRoleDescriptor
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({'file': {'template.cert': '{{api_secret}}'}, 'env': {'FROM_FILE': "{{tower.filename.cert}}"}}, True),
        # With named files, `tower.filename` is another namespace, so it cannot be referenced
        ({'file': {'template.cert': '{{api_secret}}'}, 'env': {'FROM_FILE': "{{tower.filename}}"}}, False),
        # With an unnamed file, `tower.filename` is just the filename
        ({'file': {'template': '{{api_secret}}'}, 'env': {'THE_FILENAME': "{{tower.filename}}"}}, True),
        ({'file': {'foo': 'bar'}}, False),
        ({'env': 123}, False),
        ({'env': {}}, True),
        ({'env': {'API_SECRET': '{{api_secret}}'}}, True),
        ({'env': {'API_SECRET_99': '{{api_secret}}'}}, True),
        ({'env': {'99': '{{api_secret}}'}}, False),
        ({'env': {'API_SECRET=': '{{api_secret}}'}}, False),
        ({'env': {'DISTRONODE_SETTING': '{{api_secret}}'}}, False),
        ({'env': {'DRAGON': u'🐉'}}, False),
        ({'env': {u'🐉': 'DRAGON'}}, False),
    ...
    type_ = CredentialType(
        ...
        inputs={
            'fields': [
                ...
                {'id': 'api_secret', 'type': 'string', 'label': '_'},
