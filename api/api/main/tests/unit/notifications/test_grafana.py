...
import api.main.notifications.grafana_backend as grafana_backend


def test_send_messages():
    with mock.patch('api.main.notifications.grafana_backend.requests') as requests_mock:
...
def test_send_messages_with_tags():
    with mock.patch('api.main.notifications.grafana_backend.requests') as requests_mock:
        ...
        m['finished'] = dt.datetime.utcfromtimestamp(120).isoformat()
        m['subject'] = "test subject"
        backend = grafana_backend.GrafanaBackend("testapikey", dashboardId=None, panelId=None, annotation_tags=["distronode"])
        message = EmailMessage(
            m['subject'],
            {"started": m['started'], "finished": m['finished']},
            [],
        ...
        requests_mock.post.assert_called_once_with(
            'https://example.com/api/annotations',
            headers={'Content-Type': 'application/json', 'Authorization': 'Bearer testapikey'},
            json={'tags': ['distronode'], 'text': 'test subject', 'isRegion': True, 'timeEnd': 120000, 'time': 60000},
            verify=True,
        )
        assert sent_messages == 1
