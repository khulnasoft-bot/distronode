...
import api.main.notifications.rocketchat_backend as rocketchat_backend


def test_send_messages():
    with mock.patch('api.main.notifications.rocketchat_backend.requests') as requests_mock, mock.patch(
        'api.main.notifications.rocketchat_backend.get_api_http_client_headers'
    ) as version_mock:
        requests_mock.post.return_value.status_code = 201
        version_mock.return_value = {'Content-Type': 'application/json', 'User-Agent': 'API 0.0.1.dev (open)'}
        ...
        requests_mock.post.assert_called_once_with(
            ...
            headers={'Content-Type': 'application/json', 'User-Agent': 'API 0.0.1.dev (open)'},
            verify=True,
        )
...
def test_send_messages_with_username():
    with mock.patch('api.main.notifications.rocketchat_backend.requests') as requests_mock:
        requests_mock.post.return_value.status_code = 201
