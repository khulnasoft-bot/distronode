...
def hosts(ref_time):
    ...
    return [
        Host(name='host1', distronode_facts={"a": 1, "b": 2}, distronode_facts_modified=ref_time, inventory=inventory),
        Host(name='host2', distronode_facts={"a": 1, "b": 2}, distronode_facts_modified=ref_time, inventory=inventory),
        Host(name='host3', distronode_facts={"a": 1, "b": 2}, distronode_facts_modified=ref_time, inventory=inventory),
        Host(name=u'Iñtërnâtiônàlizætiøn', distronode_facts={"a": 1, "b": 2}, distronode_facts_modified=ref_time, inventory=inventory),
    ]
...
def test_start_job_fact_cache(hosts, tmpdir):
    ...
    for host in hosts:
        ...
        with open(filepath, 'r') as f:
            assert f.read() == json.dumps(host.distronode_facts)
...
def test_finish_job_fact_cache_with_existing_data(hosts, mocker, tmpdir, ref_time):
    ...
    distronode_facts_new = {"foo": "bar"}
    filepath = os.path.join(fact_cache, hosts[1].name)
    with open(filepath, 'w') as f:
        f.write(json.dumps(distronode_facts_new))
        f.flush()
    ...
    for host in (hosts[0], hosts[2], hosts[3]):
        assert host.distronode_facts == {"a": 1, "b": 2}
        assert host.distronode_facts_modified == ref_time
    assert hosts[1].distronode_facts == distronode_facts_new
    assert hosts[1].distronode_facts_modified > ref_time
    bulk_update.assert_called_once_with([hosts[1]], ['distronode_facts', 'distronode_facts_modified'])
...
def test_finish_job_fact_cache_clear(hosts, mocker, ref_time, tmpdir):
    ...
    for host in (hosts[0], hosts[2], hosts[3]):
        assert host.distronode_facts == {"a": 1, "b": 2}
        assert host.distronode_facts_modified == ref_time
    assert hosts[1].distronode_facts == {}
    assert hosts[1].distronode_facts_modified > ref_time
    bulk_update.assert_called_once_with([hosts[1]], ['distronode_facts', 'distronode_facts_modified'])
