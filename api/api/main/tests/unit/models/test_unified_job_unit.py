...
from api.main.models import UnifiedJob, UnifiedJobTemplate, WorkflowJob, WorkflowJobNode, WorkflowApprovalTemplate, Job, User, Project, JobTemplate, Inventory
...
def test_cancel(unified_job):
    with mock.patch('api.main.models.unified_jobs.connection.on_commit', wraps=mock_on_commit):
        unified_job.cancel()
...
def test_cancel_job_explanation(unified_job):
    ...
    with mock.patch('api.main.models.unified_jobs.connection.on_commit'):
...
class TestMetaVars:
    ...
    def test_job_metavars(self):
        ...
        assert (
            Job(name='fake-job', pk=42, id=42, launch_type='manual', created_by=maker, inventory=inv, execution_node='example-exec-node').api_meta_vars()
            == result_hash
        )
