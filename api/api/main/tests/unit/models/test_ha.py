...
def test_cleanup_params_defaults():
    ...
    assert inst.get_cleanup_task_kwargs(exclude_strings=['api_423_']) == {'exclude_strings': ['api_423_'], 'file_pattern': '/tmp/api_*_*', 'grace_period': 60}


def test_cleanup_params_for_image_cleanup():
    inst = Instance(hostname='foobar')
    # see CLI conversion in api.main.tests.unit.utils.test_receptor
