...
from api_plugins.interfaces._temporary_private_container_api import CONTAINER_ROOT
...
from api.main.models.credential import HIDDEN_PASSWORD, ManagedCredentialType

from api.main.tasks import jobs, system, receptor
from api.main.utils import encrypt_field, encrypt_value
from api.main.utils.safe_yaml import SafeLoader

from api.main.utils.licensing import Licenser
from api.main.constants import JOB_VARIABLE_PREFIXES
...
def private_data_dir():
    private_data = tempfile.mkdtemp(prefix='api_')
...
def mock_create_partition():
    with mock.patch('api.main.tasks.jobs.create_partition') as cp_mock:
        yield cp_mock
...
def test_send_notifications_job_id(mocker):
    mocker.patch('api.main.models.UnifiedJob.objects.get')
    system.send_notifications([], job_id=1)
    assert UnifiedJob.objects.get.called
    assert UnifiedJob.objects.get.called_with(id=1)


@mock.patch('api.main.models.UnifiedJob.objects.get')
@mock.patch('api.main.models.Notification.objects.filter')
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('API_SECRET', 'SECRET'),
        ('DISTRONODE_GALAXY_SERVER_PRIMARY_GALAXY_TOKEN', 'SECRET'),
    ],
)
...
class TestGenericRun:
    def test_generic_failure(self, patch_Job, execution_environment, mock_me, mock_create_partition):
        ...
        with mock.patch('api.main.tasks.jobs.shutil.copytree'):
    ...
    def test_api_task_env(self, patch_Job, private_data_dir, execution_environment, mock_me):
        ...
        with mock.patch('api.main.tasks.jobs.settings.API_TASK_ENV', {'FOO': 'BAR'}):
...
class TestAdhocRun(TestJobExecution):
    def test_options_jinja_usage(self, adhoc_job, adhoc_update_model_wrapper, mock_me, mock_create_partition):
        ...
        adhoc_job.module_args = '{{ distronode_ssh_pass }}'
...
class TestJobCredentials(TestJobExecution):
    ...
    def test_username_jinja_usage(self, job, private_data_dir, mock_me):
        ...
        credential = Credential(pk=1, credential_type=ssh, inputs={'username': '{{ distronode_ssh_pass }}'})
    ...
    def test_become_jinja_usage(self, job, private_data_dir, flag, mock_me):
        task = jobs.RunJob()
        ssh = CredentialType.defaults['ssh']()
        credential = Credential(pk=1, credential_type=ssh, inputs={'username': 'joe', flag: '{{ distronode_ssh_pass }}'})
        job.credentials.add(credential)
    ...
    def test_net_credentials(self, authorize, expected_authorize, job, private_data_dir, mock_me):
        ...
        assert env['DISTRONODE_NET_USERNAME'] == 'bob'
        assert env['DISTRONODE_NET_PASSWORD'] == 'secret'
        assert env['DISTRONODE_NET_AUTHORIZE'] == expected_authorize
        if authorize:
            assert env['DISTRONODE_NET_AUTH_PASS'] == 'authorizeme'
        with open(env['DISTRONODE_NET_SSH_KEYFILE'], 'r') as f:
            assert f.read() == self.EXAMPLE_PRIVATE_KEY
        assert safe_env['DISTRONODE_NET_PASSWORD'] == HIDDEN_PASSWORD
    ...
    def test_api_task_env(self, settings, private_data_dir, job, mock_me):
        settings.API_TASK_ENV = {'FOO': 'BAR'}
        task = jobs.RunJob()
        task.instance = job
        env = task.build_env(job, private_data_dir)
...
class TestProjectUpdateGalaxyCredentials(TestJobExecution):
    ...
    def test_galaxy_credentials_ignore_certs(self, private_data_dir, project_update, ignore, mock_me):
        ...
        if ignore:
            assert env['DISTRONODE_GALAXY_IGNORE'] == 'True'
        else:
            assert 'DISTRONODE_GALAXY_IGNORE' not in env
    ...
    def test_galaxy_credentials_empty(self, private_data_dir, project_update, mock_me):
        ...
        for k in env:
            assert not k.startswith('DISTRONODE_GALAXY_SERVER')
    ...
    def test_single_public_galaxy(self, private_data_dir, project_update, mock_me):
        ...
        assert sorted([(k, v) for k, v in env.items() if k.startswith('DISTRONODE_GALAXY')]) == [
            ('DISTRONODE_GALAXY_SERVER_LIST', 'server0'),
            ('DISTRONODE_GALAXY_SERVER_SERVER0_URL', 'https://galaxy.distronode.com/'),
        ]
    ...
    def test_multiple_galaxy_endpoints(self, private_data_dir, project_update, mock_me):
        ...
        env = task.build_env(project_update, private_data_dir)
        assert sorted([(k, v) for k, v in env.items() if k.startswith('DISTRONODE_GALAXY')]) == [
            ('DISTRONODE_GALAXY_SERVER_LIST', 'server0,server1'),
            ('DISTRONODE_GALAXY_SERVER_SERVER0_URL', 'https://galaxy.distronode.com/'),
            ('DISTRONODE_GALAXY_SERVER_SERVER1_AUTH_URL', 'https://sso.redhat.com/example/openid-connect/token/'),  # noqa
            ('DISTRONODE_GALAXY_SERVER_SERVER1_TOKEN', 'secret123'),
            ('DISTRONODE_GALAXY_SERVER_SERVER1_URL', 'https://cloud.redhat.com/api/automation-hub/'),
        ]
...
class TestProjectUpdateCredentials(TestJobExecution):
    ...
    def test_api_task_env(self, project_update, settings, private_data_dir, scm_type, execution_environment, mock_me):
        project_update.execution_environment = execution_environment
        settings.API_TASK_ENV = {'FOO': 'BAR'}
        task = jobs.RunProjectUpdate()
        task.instance = project_update
...
class TestInventoryUpdateCredentials(TestJobExecution):
    ...
    def test_api_task_env(self, inventory_update, private_data_dir, settings, mocker, mock_me):
        task = jobs.RunInventoryUpdate()
        task.instance = inventory_update
        gce = CredentialType.defaults['gce']()
        inventory_update.source = 'gce'
...
def test_managed_injector_redaction(injector_cls):
    """See api.main.models.inventory.PluginFileInjector._get_shared_env
    The ordering within api.main.tasks.jobs.BaseTask and contract with build_env
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            # Extra data is a dict but RemoteWorkType is not distronode-runner
            {
                "y4NgMKKW": {
                    'ExtraData': {
                        'RemoteWorkType': 'not-distronode-runner',
                    },
                }
            },
            False,
        ...
        [
            # Extra data is a dict and its an distronode-runner but we have no params
            {
                'zpdFi4BX': {
                    'ExtraData': {
                        'RemoteWorkType': 'distronode-runner',
                    },
                }
            },
            False,
        ...
        [
            # Extra data is a dict and its an distronode-runner but params is not --worker-info
            {
                'zpdFi4BX': {
                    'ExtraData': {'RemoteWorkType': 'distronode-runner', 'RemoteParams': {'params': '--not-worker-info'}},
                }
            },
            False,
        ],
        [
            # Extra data is a dict and its an distronode-runner but params starts without cleanup
            {
                'zpdFi4BX': {
                    'ExtraData': {'RemoteWorkType': 'distronode-runner', 'RemoteParams': {'params': 'not cleanup stuff'}},
                }
            },
            False,
        ],
        [
            # Extra data is a dict and its an distronode-runner w/ params but still running
            {
                'zpdFi4BX': {
                    'ExtraData': {'RemoteWorkType': 'distronode-runner', 'RemoteParams': {'params': '--worker-info'}},
                    "StateName": "Running",
                }
            },
            False,
        ...
        [
            # Extra data is a dict and its an distronode-runner w/ params and completed
            {
                'zpdFi4BX': {
                    'ExtraData': {'RemoteWorkType': 'distronode-runner', 'RemoteParams': {'params': '--worker-info'}},
                    "StateName": "Succeeded",
...
def test_administrative_workunit_reaper(work_unit_data, expected_function_call):
    ...
    with mock.patch('api.main.tasks.receptor.get_receptor_ctl') as mock_get_receptor_ctl:
        mock_get_receptor_ctl.return_value = ReceptorControl('/var/run/api-receptor/receptor.sock')
