from api.main.tasks.receptor import _convert_args_to_cli


def test_file_cleanup_scenario():
    args = _convert_args_to_cli({'exclude_strings': ['api_423_', 'api_582_'], 'file_pattern': '/tmp/api_*_*'})
    assert ' '.join(args) == 'cleanup --exclude-strings "api_423_" "api_582_" --file-pattern=/tmp/api_*_*'


def test_image_cleanup_scenario():
    # See input dict in api.main.tests.unit.models.test_ha
