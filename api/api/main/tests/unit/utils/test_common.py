...
from api.main.utils import common
from api.api.validators import HostnameRegexValidator

from api.main.models import Job, AdHocCommand, InventoryUpdate, ProjectUpdate, SystemJob, WorkflowJob, Inventory, JobTemplate, UnifiedJobTemplate, UnifiedJob
...
def memoized_function(mocker, mock_cache):
    with mock.patch('api.main.utils.common.get_memoize_cache', return_value=mock_cache):
...
def test_memoize_delete(memoized_function, mock_cache):
    ...
    with mock.patch('api.main.utils.common.memoize_delete', side_effect=mock_cache.delete):
...
def test_extract_distronode_vars():
    my_dict = {"foobar": "baz", "distronode_connetion_setting": "1928"}
    redacted, var_list = common.extract_distronode_vars(json.dumps(my_dict))
    assert var_list == set(['distronode_connetion_setting'])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('git', 'notgit@github.com:distronode/distronode/tree/devel/api.git', True, True, True, False, ValueError('Username must be "git" for SSH access to github.com.')),
        (
            'git',
        ...
        ('git', 'git:password@github.com:distronode/distronode/tree/devel/api.git', True, True, True, False, 'git+ssh://git@github.com/distronode/distronode/tree/devel/api.git'),
        # Disabling the special handling should not raise an error
        ('git', 'notgit@github.com:distronode/distronode/tree/devel/api.git', True, True, False, False, 'git+ssh://notgit@github.com/distronode/distronode/tree/devel/api.git'),
