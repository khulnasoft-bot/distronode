...
Several of the subfolders of `api/main/tests/` indicate a different required _environment_
where you can run the tests. Those folders are:

 - `functional/` - requires a test database and no other services running
 - `live/` - must run in `tools_api_1` container launched by `make docker-compose`
...
These tests are ran from the root API folder.
