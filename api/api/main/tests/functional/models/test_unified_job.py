...
# API
from api.main.models import UnifiedJobTemplate, Job, JobTemplate, WorkflowJobTemplate, Project, WorkflowJob, Schedule, Credential
...
class TestCreateUnifiedJob:
    ...
    def test_many_to_many(self, mocker, job_template_labels):
        ...
        _get_unified_job_field_names = mocker.patch('api.main.models.jobs.JobTemplate._get_unified_job_field_names', return_value=['labels'])
...
class TestMetaVars:
    ...
    def test_deleted_user(self, admin_user):
        ...
        user_vars = ['_'.join(x) for x in itertools.product(['tower', 'api'], ['user_name', 'user_id', 'user_email', 'user_first_name', 'user_last_name'])]

        for key in user_vars:
            assert key in job.api_meta_vars()

        # deleted user is hard to simulate as this test occurs within one transaction
        job = Job.objects.get(pk=job.id)
        job.created_by_id = 999999999
        for key in user_vars:
            assert key not in job.api_meta_vars()
    ...
    def test_scheduled_workflow_job_node_metavars(self, workflow_job_template):
        ...
        assert job.api_meta_vars() == result_hash
