...
# API models
from api.main.models import ActivityStream, Organization, JobTemplate, Credential, CredentialType, Inventory, InventorySource, Project, User

# other API
from api.main.utils import model_to_dict, model_instance_diff
from api.main.utils.common import get_allowed_fields
from api.main.signals import model_serializer_mapping
...
def test_anon_user_action():
    with mock.patch('api.main.signals.get_current_user') as u_mock:
...
def test_cluster_node_recorded(inventory, project):
    ...
    with mock.patch('api.main.models.activity_stream.settings.CLUSTER_HOST_ID', 'foo_host'):
...
def test_cluster_node_long_node_name(inventory, project):
    jt = JobTemplate.objects.create(name='testjt', inventory=inventory, project=project)
    with mock.patch('api.main.models.activity_stream.settings.CLUSTER_HOST_ID', 'f' * 700):
        job = jt.create_unified_job()
...
def test_credential_defaults_idempotency():
    ...
    old_inputs = CredentialType.objects.get(name='Red Hat Distronode Automation Platform', kind='cloud').inputs
    ...
    CredentialType.setup_tower_managed_defaults()
    assert CredentialType.objects.get(name='Red Hat Distronode Automation Platform', kind='cloud').inputs == old_inputs
