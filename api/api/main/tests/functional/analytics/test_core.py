...
from api.main.analytics import gather, register, ship
...
def mock_valid_license():
    with mock.patch('api.main.analytics.core._valid_license') as license:
...
def test_gather(mock_valid_license):
    ...
    with tarfile.open(tgzfiles[0], "r:gz") as archive:
        ...
        assert json.loads(files['./example.json'].read()) == {'api': 123}
...
def mock_analytic_post():
    ...
    with mock.patch('api.main.analytics.core.requests.Session.post', return_value=mock.Mock(status_code=200)) as mock_post:
        yield mock_post
