...
def test_ask_at_runtime_xfail(get, post, admin):
    ...
    response = post(
        reverse('api:credential_type_list'),
        {
            ...
            'injectors': {'env': {'DISTRONODE_MY_CLOUD_TOKEN': '{{api_token}}'}},
        },
        admin,
    )
    assert response.status_code == 400
...
def test_create_with_valid_injectors(get, post, admin):
    response = post(
        reverse('api:credential_type_list'),
        {
            ...
            'name': 'MyCloud',
            'inputs': {'fields': [{'id': 'api_token', 'label': 'API Token', 'type': 'string', 'secret': True}]},
            'injectors': {'env': {'API_MY_CLOUD_TOKEN': '{{api_token}}'}},
        },
        admin,
        expect=201,
    )
    ...
    assert injectors['env'] == {'API_MY_CLOUD_TOKEN': '{{api_token}}'}
...
def test_create_with_undefined_template_variable_xfail(post, admin):
    response = post(
        reverse('api:credential_type_list'),
        {
            ...
            'name': 'MyCloud',
            'inputs': {'fields': [{'id': 'api_token', 'label': 'API Token', 'type': 'string', 'secret': True}]},
            'injectors': {'env': {'API_MY_CLOUD_TOKEN': '{{api_tolkien}}'}},
        },
        admin,
    )
    assert response.status_code == 400
