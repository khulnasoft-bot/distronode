...
from api.api.views import RelatedJobsPreventDeleteMixin, UnifiedJobDeletionMixin
from api.main.models import JobTemplate, User, Job, AdHocCommand, ProjectUpdate, InstanceGroup, Label, Organization
...
def test_job_relaunch_permission_denied_response_other_user(get, post, inventory, project, alice, bob, survey_spec_factory):
    ...
    api/main/tests/functional/test_rbac_job.py::TestJobRelaunchAccess::test_other_user_prompts
    """
...
def test_block_unprocessed_events(delete, admin_user, mocker):
    ...
    with mock.patch('api.api.views.mixin.now', lambda: time_of_request):
