...
def test_jobs_settings(get, put, patch, delete, admin):
    ...
    patch(url, user=admin, data={'API_ISOLATION_SHOW_PATHS': ['/home']}, expect=200)
    response = get(url, user=admin, expect=200)
    assert response.data['API_ISOLATION_SHOW_PATHS'] == ['/home']
    data.pop('API_ISOLATION_SHOW_PATHS')
    data.pop('API_DISTRONODE_CALLBACK_PLUGINS')
    put(url, user=admin, data=data, expect=200)
    response = get(url, user=admin, expect=200)
    assert response.data['API_ISOLATION_SHOW_PATHS'] == []
    assert response.data['API_DISTRONODE_CALLBACK_PLUGINS'] == []
...
def test_api_task_env_validity(get, patch, admin, value, expected):
    url = reverse('api:setting_singleton_detail', kwargs={'category_slug': 'jobs'})
    patch(url, user=admin, data={'API_TASK_ENV': value}, expect=expected)

    resp = get(url, user=admin)
    if expected == 200:
        assert resp.data['API_TASK_ENV'] == dict((k, str(v)) for k, v in value.items())
    else:
        assert resp.data['API_TASK_ENV'] == dict()
