...
# API
from api.main.models import ProjectUpdate, CredentialType, Credential
...
def test_galaxy_credential_association_forbidden(alice, organization, post):
    ...
    cred = Credential.objects.create(credential_type=galaxy, name='Public Galaxy', organization=organization, inputs={'url': 'https://galaxy.distronode.com/'})
...
def test_galaxy_credential_association(alice, admin, organization, post, get):
    ...
    for i in range(5):
        cred = Credential.objects.create(
            credential_type=galaxy, name=f'Public Galaxy {i + 1}', organization=organization, inputs={'url': 'https://galaxy.distronode.com/'}
        )
        url = reverse('api:organization_galaxy_credentials_list', kwargs={'pk': organization.id})
...
def test_org_admin_credential_count(org_admin, admin, organization, post, get):
    ...
    for i in range(3):
        cred = Credential.objects.create(credential_type=galaxy, name=f'test_{i}', inputs={'url': 'https://galaxy.distronode.com/'})
