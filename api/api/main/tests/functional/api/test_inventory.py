...
from api.main.models import InventorySource, Inventory, ActivityStream
...
def test_urlencode_host_filter(post, admin_user, organization):
    ...
    inventory.hosts.create(
        distronode_facts={"distronode_distribution_version": "7.4"}
    )
    """
    ...
    post(
        ...
        data={
            ...
            'host_filter': 'distronode_facts__distronode_distribution_version=%227.4%22',
        },
        user=admin_user,
        expect=201,
    )
    ...
    assert si.host_filter == 'distronode_facts__distronode_distribution_version="7.4"'
...
def test_host_filter_unicode(post, admin_user, organization):
    post(
        reverse('api:inventory_list'),
        data={'name': 'smart inventory', 'kind': 'smart', 'organization': organization.pk, 'host_filter': u'distronode_facts__distronode_distribution=レッドハット'},
        user=admin_user,
        expect=201,
    )
    si = Inventory.objects.get(name='smart inventory')
    assert si.host_filter == u'distronode_facts__distronode_distribution=レッドハット'


@pytest.mark.django_db
@pytest.mark.parametrize("lookup", ['icontains', 'has_keys'])
def test_host_filter_invalid_distronode_facts_lookup(post, admin_user, organization, lookup):
    resp = post(
        reverse('api:inventory_list'),
        data={
            'name': 'smart inventory',
            'kind': 'smart',
            'organization': organization.pk,
            'host_filter': u'distronode_facts__distronode_distribution__{}=cent'.format(lookup),
        },
        user=admin_user,
        expect=400,
    )
    assert 'distronode_facts does not support searching with __{}'.format(lookup) in json.dumps(resp.data)


@pytest.mark.django_db
def test_host_filter_distronode_facts_exact(post, admin_user, organization):
    post(
        reverse('api:inventory_list'),
        data={
            'name': 'smart inventory',
            'kind': 'smart',
            'organization': organization.pk,
            'host_filter': 'distronode_facts__distronode_distribution__exact="CentOS"',
        },
        user=admin_user,
        expect=201,
    )
...
def test_create_inventory_host_with_limits(post, admin_user, inventory, hosts, expected_status_code):
    # The per-Organization host limits functionality should be a no-op on API.
...
def test_inventory_source_vars_prohibition(post, inventory, admin_user):
    with mock.patch('api.api.serializers.settings') as mock_settings:
...
class TestInventorySourceCredential:
    ...
    def test_validating_credential_type(self, organization, inventory, admin_user, post):
        ...
        from api.main.models.credential import Credential, CredentialType
