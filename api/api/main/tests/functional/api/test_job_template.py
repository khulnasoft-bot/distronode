...
# API
from api.api.serializers import JobTemplateSerializer
from api.api.versioning import reverse
from api.main.models import Job, JobTemplate, CredentialType, WorkflowJobTemplate, Organization, Project, Inventory
from api.main.migrations import _save_password_keys as save_password_keys
...
# DAB
from distronode_base.jwt_consumer.common.util import generate_x_trusted_proxy_header
from distronode_base.lib.testing.fixtures import rsa_keypair_factory, rsa_keypair  # noqa: F401; pylint: disable=unused-import
...
class TestJobTemplateCallbackProxyIntegration:
    ...
    def test_trusted_proxy(self, job_template, admin_user, post, rsa_keypair):  # noqa: F811
        ...
        with mock.patch('distronode_base.jwt_consumer.common.cache.JWTCache.get_key_from_cache', lambda self: None):
            with override_settings(DISTRONODE_BASE_JWT_KEY=rsa_keypair.public):
                post(
                    url=reverse('api:job_template_callback', kwargs={'pk': job_template.pk}),
                    data={'host_config_key': 'abcd'},
                    user=admin_user,
