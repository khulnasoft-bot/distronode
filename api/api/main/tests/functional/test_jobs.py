...
from api.main.tasks.system import cluster_node_heartbeat
from api.main.tasks.facts import update_hosts
...
@pytest.mark.django_db
@mock.patch('api.main.tasks.system.inspect_execution_and_hop_nodes', lambda *args, **kwargs: None)
@mock.patch('api.main.models.ha.get_cpu_effective_capacity', lambda cpu, is_control_node: 8)
@mock.patch('api.main.models.ha.get_mem_effective_capacity', lambda mem, is_control_node: 62)
...
@pytest.mark.django_db
@mock.patch('api.main.models.ha.get_cpu_effective_capacity', lambda cpu: 8)
@mock.patch('api.main.models.ha.get_mem_effective_capacity', lambda mem: 62)
...
class TestDistronodeFactsSave:
    ...
    def test_update_hosts_deleted_host(self, inventory):
        ...
        for host in inventory.hosts.all():
            ...
            assert host.distronode_facts == {'foo': 'bar'}

    def test_update_hosts_forever_deadlock(self, inventory, mocker):
        hosts = [Host.objects.create(inventory=inventory, name=f'foo{i}') for i in range(3)]
        for host in hosts:
            host.distronode_facts = {'foo': 'bar'}
        db_mock = mocker.patch('api.main.tasks.facts.Host.objects.bulk_update')
    ...
    def fake_bulk_update(self, host_list):
        if self.current_call > 2:
            return Host.objects.bulk_update(host_list, ['distronode_facts', 'distronode_facts_modified'])
        self.current_call += 1
    ...
    def test_update_hosts_resolved_deadlock(self, inventory, mocker):
        ...
        for host in hosts:
            host.distronode_facts = {'foo': 'bar'}
        self.current_call = 0
        mocker.patch('api.main.tasks.facts.raw_update_hosts', new=self.fake_bulk_update)
        update_hosts(hosts)
        for host in inventory.hosts.all():
            host.refresh_from_db()
            assert host.distronode_facts == {'foo': 'bar'}
