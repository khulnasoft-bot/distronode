...
# API
from api.main.management.commands import inventory_import
from api.main.models import Inventory, Host, Group, InventorySource
from api.main.utils.mem_inventory import MemGroup
...
TEST_INVENTORY_CONTENT = {
    ...
    "others": {
        "hosts": {
            "10.11.12.13": {},
            "10.12.14.16": {"distronode_port": 8022},
            "::1": {},
            "fe80::1610:9fff:fedd:654b": {},
            "fe80::1610:9fff:fedd:b654": {"distronode_port": 1022},
        }
    },
    "servers": {"children": ["dbservers", "webservers"], "vars": {"varb": "B"}},
    "ungrouped": {},
    "webservers": {
        "hosts": {"web1.example.com": {"distronode_ssh_host": "w1.example.net"}, "web2.example.com": {}, "web3.example.com": {"distronode_port": 1022}},
        "vars": {"webvar": "blah"},
    },
}
...
class TestINIImports:
    @mock.patch.object(inventory_import, 'DistronodeInventoryLoader', MockLoader)
    def test_inventory_single_ini_import(self, inventory, capsys):
        inventory_import.DistronodeInventoryLoader._data = TEST_INVENTORY_CONTENT
        cmd = inventory_import.Command()
        ...
        assert Host.objects.get(name='web1.example.com').variables_dict == {'distronode_ssh_host': 'w1.example.net'}
        # Old version uses `distronode_ssh_port` but new version uses `distronode_port`
        assert Host.objects.get(name='web3.example.com').variables_dict == {'distronode_port': 1022}
        assert Host.objects.get(name='fe80::1610:9fff:fedd:b654').variables_dict == {'distronode_port': 1022}
        assert Host.objects.get(name='10.12.14.16').variables_dict == {'distronode_port': 8022}
    ...
    def test_hostvars_are_saved(self, inventory):
        inventory_import.DistronodeInventoryLoader._data = {
...
class TestEnabledVar:
    ...
    def test_enabled_var_not_present(self, cmd):
        assert cmd._get_enabled({'distronode_connection': 'local'}) is None
...
def test_tower_version_compare():
    ...
    # mimic example from https://github.com/distronode/distronode/pull/52747
    # until that is merged, this is the best testing we can do
    cmd.all_group.variables = {'tower_metadata': {"distronode_version": "2.7.5", "license_type": "open", "version": "2.0.1-1068-g09684e2c41"}}
