...
from api.main.models import AdHocCommand, InventoryUpdate, JobTemplate, Job
from api.main.models.activity_stream import ActivityStream
...
def test_get_cleanup_task_kwargs_active_jobs(node_type, active):
    ...
    if active:
        assert kwargs['exclude_strings'] == [f'api_{job.pk}_']
    else:
...
class TestPolicyTaskScheduling:
    ...
    def test_policy_task_ran_for_ig_when_needed(self, instance_group_factory, field, value, expect):
        ...
        with mock.patch('api.main.models.ha.schedule_policy_task') as mock_policy:
...
def test_health_check_clears_errors():
    ...
    data = dict(version='distronode-runner-4.2', cpu=782, memory=int(39e9), uuid='asdfasdfasdfasdfasdf', errors='')
