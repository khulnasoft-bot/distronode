...
from api.main.tasks.system import execution_node_health_check, _cleanup_images_and_files
from api.main.models import Instance, Job
...
def test_no_worker_info_on_API_nodes(node_type):
...
def job_folder_factory(request):
    def _rf(job_id='1234'):
        pdd_path = tempfile.mkdtemp(prefix=f'api_{job_id}_')
...
def test_does_not_run_reaped_job(mocker, mock_me):
    ...
    mock_run = mocker.patch('api.main.tasks.jobs.distronode_runner.interface.run')
    try:
        RunJob().run(job.id)
    except Exception:
        pass
