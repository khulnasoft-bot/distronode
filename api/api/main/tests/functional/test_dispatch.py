...
from api.main.models import Job, WorkflowJob, Instance
from api.main.dispatch import reaper
from api.main.dispatch.pool import StatefulPoolWorker, WorkerPool, AutoscalePool
from api.main.dispatch.publish import task
from api.main.dispatch.worker import BaseWorker, TaskWorker
from api.main.dispatch.periodic import Scheduler
...
def _disable_database_settings(mocker):
    m = mocker.patch('api.conf.settings.SettingsWrapper.all_supported_settings', new_callable=mock.PropertyMock)
    m.return_value = []
...
class TestTaskDispatcher:
    ...
    def test_function_dispatch(self):
        result = self.tm.perform_work({'task': 'api.main.tests.functional.test_dispatch.add', 'args': [2, 2]})
        assert result == 4

    def test_function_dispatch_must_be_decorated(self):
        result = self.tm.perform_work({'task': 'api.main.tests.functional.test_dispatch.restricted', 'args': [2, 2]})
        assert isinstance(result, ValueError)
        assert str(result) == 'api.main.tests.functional.test_dispatch.restricted is not decorated with @task()'  # noqa

    def test_method_dispatch(self):
        result = self.tm.perform_work({'task': 'api.main.tests.functional.test_dispatch.Adder', 'args': [2, 2]})
        assert result == 4
    ...
    def test_python_function_cannot_be_imported(self):
        ...
        assert isinstance(result, ValueError)
        assert str(result) == 'os.system is not a valid api task'  # noqa

    def test_undefined_function_cannot_be_imported(self):
        result = self.tm.perform_work({'task': 'api.foo.bar'})
        assert isinstance(result, ModuleNotFoundError)
        assert str(result) == "No module named 'api.foo'"  # noqa
...
class TestTaskPublisher:
    ...
    def test_function_apply_async(self):
        ...
        assert message['task'] == 'api.main.tests.functional.test_dispatch.add'
        assert queue == 'foobar'
    ...
    def test_method_apply_async(self):
        ...
        assert message['args'] == [2, 2]
        assert message['kwargs'] == {}
        assert message['task'] == 'api.main.tests.functional.test_dispatch.Adder'
        assert queue == 'foobar'

    def test_apply_async_queue_required(self):
        with pytest.raises(ValueError) as e:
            message, queue = add.apply_async([2, 2])
        assert "api.main.tests.functional.test_dispatch.add: Queue value required and may not be None" == e.value.args[0]
...
class TestJobReaper(object):
    @pytest.mark.parametrize(
        ...
        [
            ...
            ('running', 'api', '', None, True),  # running, has the instance as its execution_node
            ('running', '', 'api', None, True),  # running, has the instance as its controller_node
            ('waiting', '', '', None, False),  # waiting, not assigned to the instance
            ('waiting', 'api', '', None, False),  # waiting, was edited less than a minute ago
            ('waiting', '', 'api', None, False),  # waiting, was edited less than a minute ago
            ('waiting', 'api', '', yesterday, False),  # waiting, managed by another node, ignore
            ('waiting', '', 'api', yesterday, True),  # waiting, assigned to the controller_node, stale
        ],
    )
    def test_should_reap(self, status, fail, execution_node, controller_node, modified):
        i = Instance(hostname='api')
        i.save()
        j = Job(
            status=status,
    ...
    def test_workflow_does_not_reap(self):
        i = Instance(hostname='api')
        i.save()
        j = WorkflowJob(status='running', execution_node='api')
        j.save()
        reaper.reap(i)
...
class TestScheduler:
    ...
    def test_missed_schedule(self, mocker):
        ...
        mocker.patch('api.main.dispatch.periodic.time.time', return_value=scheduler.global_start + 50)
    ...
    def test_advance_schedule(self, mocker):
        ...
        for job in scheduler.jobs:
            ...
            job.offset = 0.0
        mocker.patch('api.main.dispatch.periodic.time.time', return_value=scheduler.global_start + 29)
        to_run = scheduler.get_and_mark_pending()
        assert set(job.name for job in to_run) == set(['joba', 'jobb'])
        mocker.patch('api.main.dispatch.periodic.time.time', return_value=scheduler.global_start + 39)
        to_run = scheduler.get_and_mark_pending()
        assert len(to_run) == 1
    ...
    def test_scheduler_debug(self, mocker):
        ...
        mocker.patch('api.main.dispatch.periodic.time.time', return_value=current_time - 1.0e-8)
