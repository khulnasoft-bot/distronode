...
from api_plugins.interfaces._temporary_private_container_api import get_incontainer_path

from api.main.tasks.jobs import RunInventoryUpdate
from api.main.models import InventorySource, Credential, CredentialType, UnifiedJob, ExecutionEnvironment
from api.main.constants import STANDARD_INVENTORY_UPDATE_ENV
from api.main.tests import data
from api.main.utils.plugins import discover_available_cloud_provider_plugin_names
...
def read_content(private_data_dir, raw_env, inventory_update):
    ...
    cache_file_regex = re.compile(r'/tmp/api_{0}_[a-zA-Z0-9_]+/{1}_cache[a-zA-Z0-9_]+'.format(inventory_update.id, inventory_update.source))
...
@mock.patch('api_plugins.interfaces._temporary_private_licensing_api.detect_server_product_name', return_value='NOT-API')
...
def test_inventory_update_injected_content(product_name, this_kind, inventory, fake_credential_factory, mock_me):
    ...
    def substitute_run(api_receptor_job):
        ...
        """
        envvars = api_receptor_job.runner_params['envvars']

        private_data_dir = envvars.pop('API_PRIVATE_DATA_DIR')
        assert envvars.pop('DISTRONODE_INVENTORY_ENABLED') == 'auto'
        ...
        env.pop('DISTRONODE_COLLECTIONS_PATHS', None)  # collection paths not relevant to this test
    ...
    with mock.patch('api.main.queue.CallbackQueueDispatcher.dispatch', lambda self, obj: None):
        ...
        with mock.patch.object(UnifiedJob, 'websocket_emit_status', mock.Mock()):
            ...
            with mock.patch('api.main.tasks.receptor.APIReceptorJob.run', substitute_run):
                with mock.patch('api.main.tasks.jobs.create_partition'):
