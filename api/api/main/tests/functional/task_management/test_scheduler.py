...
from api.main.models import WorkflowJobTemplate, JobTemplate, Job, Project, InventorySource, Inventory
from api.main.models.ha import Instance
...
def test_single_job_scheduler_launch(hybrid_instance, controlplane_instance_group, job_template_factory, mocker):
    ...
    mocker.patch("api.main.scheduler.TaskManager.start_task")
...
class TestJobLifeCycle:
    def run_tm(self, tm, expect_channel=None, expect_schedule=None, expect_commit=None):
        ...
        """
        with mock.patch('api.main.models.unified_jobs.UnifiedJob.websocket_emit_status') as mock_channel:
            with mock.patch('api.main.utils.common.ScheduleManager._schedule') as tm_sch:
    ...
    def test_hybrid_capacity(self, job_template, hybrid_instance):
        enough_capacity = job_template.create_unified_job()
        insufficient_capacity = job_template.create_unified_job()
        expected_task_impact = enough_capacity.task_impact + settings.API_CONTROL_NODE_TASK_IMPACT
        all_ujs = [enough_capacity, insufficient_capacity]
        for uj in all_ujs:
            uj.signal_start()
    ...
    def test_project_update_capacity(self, project, hybrid_instance, instance_group_factory, controlplane_instance_group):
        ...
        expected_task_impact = pu.task_impact + settings.API_CONTROL_NODE_TASK_IMPACT
        pu.signal_start()
...
def test_multi_jt_capacity_blocking(hybrid_instance, job_template_factory, mocker):
    ...
    tm = TaskManager()
    with mock.patch('api.main.models.Job.task_impact', new_callable=mock.PropertyMock) as mock_task_impact:
...
def test_single_job_dependencies_project_launch(controlplane_instance_group, job_template_factory, mocker):
    ...
    p.scm_type = "git"
    p.scm_url = "http://github.com/distronode/distronode.git"
    p.save(skip_update=True)
    with mock.patch("api.main.scheduler.TaskManager.start_task"):
