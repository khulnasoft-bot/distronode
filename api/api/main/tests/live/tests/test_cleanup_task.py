...
from api.main.models import Instance, JobTemplate
...
def test_folder_cleanup_multiple_running_jobs_execution_node(request):
    ...
    job_patterns = []
    for job in jobs:
        job_pattern = f'api_{job.id}_1234'
    ...
    subprocess.call('distronode-runner worker ' + remote_command, shell=True)
    print('distronode-runner worker ' + remote_command)
