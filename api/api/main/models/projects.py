...
from api.main.models.base import PROJECT_UPDATE_JOB_TYPE_CHOICES, PERM_INVENTORY_DEPLOY
from api.main.models.events import ProjectUpdateEvent, UnpartitionedProjectUpdateEvent
from api.main.models.notifications import (
    NotificationTemplate,
    JobNotificationMixin,
)
from api.main.models.unified_jobs import (
    UnifiedJob,
    UnifiedJobTemplate,
)
from api.main.models.jobs import Job
from api.main.models.mixins import ResourceMixin, TaskManagerProjectUpdateMixin, CustomVirtualEnvMixin, RelatedJobsMixin
from api.main.utils import update_scm_url, polymorphic
from api.main.utils.distronode import skip_directory, could_be_inventory, could_be_playbook
...
logger = logging.getLogger('api.main.models.projects')
...
class ProjectOptions(models.Model):
    ...
    def get_cache_path(self):
        local_path = os.path.basename(self.local_path)
        if local_path:
            return os.path.join(settings.PROJECTS_ROOT, '.__api_cache', local_path)
    ...
    def playbooks(self):
        ...
        if project_path:
            for dirpath, dirnames, filenames in os.walk(smart_str(project_path), followlinks=settings.API_SHOW_PLAYBOOK_LINKS):
...
class Project(UnifiedJobTemplate, ProjectOptions, ResourceMixin, CustomVirtualEnvMixin, RelatedJobsMixin):
    ...
    inventory_files = models.JSONField(
        ...
        help_text=_('Suggested list of content that could be Distronode inventory in the project'),
    )
    ...
    def delete(self, *args, **kwargs):
        ...
        for path_to_delete in paths_to_delete:
            if self.scm_type and path_to_delete:  # non-manual, concrete path
                from api.main.tasks.system import delete_project_files
...
class ProjectUpdate(UnifiedJob, ProjectOptions, JobNotificationMixin, TaskManagerProjectUpdateMixin):
    ...
    def _get_task_class(cls):
        from api.main.tasks.jobs import RunProjectUpdate
