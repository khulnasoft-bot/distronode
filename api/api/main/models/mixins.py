...
from api.main.models.rbac import Role, RoleAncestorEntry, to_permissions
from api.main.utils import parse_yaml_or_json, get_custom_venv_choices, get_licenser, polymorphic
from api.main.utils.execution_environments import get_default_execution_environment
from api.main.utils.encryption import decrypt_value, get_encryption_key, is_encrypted
from api.main.utils.polymorphic import build_polymorphic_ctypes_map
from api.main.fields import AskForField
from api.main.constants import ACTIVE_STATES, org_role_to_permission


logger = logging.getLogger('api.main.models.mixins')
...
class WebhookMixin(models.Model):
    ...
    def update_webhook_status(self, status):
        ...
        status_api = self.extra_vars_dict.get('api_webhook_status_api')
        if not status_api:
        ...
        try:
            ...
            if self.webhook_service == 'bitbucket_dc':
                data = {
                    ...
                    'key': 'distronode/distronode/tree/devel/api' if license_type == 'open' else 'distronode/tower',
                    'url': self.get_ui_url(),
                }
            else:
                data = {
                    'state': statuses[status],
                    'context': 'distronode/distronode/tree/devel/api' if license_type == 'open' else 'distronode/tower',
