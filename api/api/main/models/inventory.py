...
from distronode_base.lib.utils.models import prevent_search

# API
from api.api.versioning import reverse
from api.main.utils.plugins import discover_available_cloud_provider_plugin_names, compute_cloud_inventory_sources
from api.main.consumers import emit_channel_notification
from api.main.fields import (
    ImplicitRoleField,
    SmartFilterField,
    OrderedManyToManyField,
)
from api.main.managers import HostManager, HostMetricActiveManager
from api.main.models.base import BaseModel, CommonModelNameNotUnique, VarsDictProperty, accepts_json
from api.main.models.events import InventoryUpdateEvent, UnpartitionedInventoryUpdateEvent
from api.main.models.unified_jobs import UnifiedJob, UnifiedJobTemplate
from api.main.models.mixins import (
    ResourceMixin,
    TaskManagerInventoryUpdateMixin,
    RelatedJobsMixin,
    CustomVirtualEnvMixin,
)
from api.main.models.notifications import (
    NotificationTemplate,
    JobNotificationMixin,
)
from api.main.utils import _inventory_updates
from api.main.utils.safe_yaml import sanitize_jinja
from api.main.utils.execution_environments import get_control_plane_execution_environment
...
logger = logging.getLogger('api.main.models.inventory')
...
class Inventory(CommonModelNameNotUnique, ResourceMixin, RelatedJobsMixin):
    ...
    def get_absolute_url(self, request=None):
        if request is not None:
            # circular import
            from api.api.urls.inventory import constructed_inventory_urls
    ...
    def schedule_deletion(self, user_id=None):
        from api.main.tasks.system import delete_inventory
        from api.main.signals import activity_stream_delete
    ...
    def _update_host_smart_inventory_memeberships(self):
        if self.kind == 'smart' and settings.API_REBUILD_SMART_MEMBERSHIP:

            def on_commit():
                from api.main.tasks.system import update_host_smart_inventory_memberships
...
class Host(CommonModelNameNotUnique, RelatedJobsMixin):
    ...
    distronode_facts = models.JSONField(
        blank=True,
        default=dict,
        help_text=_('Arbitrary JSON structure of most recent distronode_facts, per-host.'),
    )
    distronode_facts_modified = models.DateTimeField(
        default=None,
        editable=False,
        null=True,
        help_text=_('The date and time distronode_facts was last modified.'),
    )
    ...
    def get_effective_host_name(self):
        """
        Return the name of the host that will be used in actual distronode
        command run.
        """
        host_name = self.name
        if 'distronode_ssh_host' in self.variables_dict:
            host_name = self.variables_dict['distronode_ssh_host']
        if 'distronode_host' in self.variables_dict:
            host_name = self.variables_dict['distronode_host']
        return host_name

    def _update_host_smart_inventory_memeberships(self):
        if settings.API_REBUILD_SMART_MEMBERSHIP:
...
class Group(CommonModelNameNotUnique, RelatedJobsMixin):
    ...
    def delete_recursive(self):
        from api.main.utils import ignore_inventory_computed_fields
        from api.main.tasks.system import update_inventory_computed_fields
        from api.main.signals import disable_activity_stream, activity_stream_delete
    ...
    def job_host_summaries(self):
        from api.main.models.jobs import JobHostSummary

        return JobHostSummary.objects.filter(host__in=self.all_hosts)

    @property
    def job_events(self):
        from api.main.models.jobs import JobEvent

        return JobEvent.objects.filter(host__in=self.all_hosts)

    @property
    def ad_hoc_commands(self):
        from api.main.models.ad_hoc_commands import AdHocCommand
...
class HostMetricSummaryMonthly(models.Model):
    """
    HostMetric summaries computed by scheduled task 'api.main.tasks.system.host_metric_summary_monthly' monthly
    """
...
class InventoryUpdate(UnifiedJob, InventorySourceOptions, JobNotificationMixin, TaskManagerInventoryUpdateMixin, CustomVirtualEnvMixin):
    ...
    def _get_task_class(cls):
        from api.main.tasks.jobs import RunInventoryUpdate
