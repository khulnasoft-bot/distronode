...
from api.main.utils import decrypt_field, classproperty, set_environ
from api.main.validators import validate_ssh_private_key
from api.main.models.base import CommonModelNameNotUnique, PasswordFieldsModel, PrimordialModel
from api.main.models.mixins import ResourceMixin
from api.main.models.rbac import (
    ROLE_SINGLETON_SYSTEM_ADMINISTRATOR,
    ROLE_SINGLETON_SYSTEM_AUDITOR,
)
from api.main.models import Team, Organization
from api.main.utils import encrypt_field

# DAB
from distronode_base.resource_registry.tasks.sync import get_resource_server_client
from distronode_base.resource_registry.utils.settings import resource_server_defined


__all__ = ['Credential', 'CredentialType', 'CredentialInputSource', 'build_safe_env']

logger = logging.getLogger('api.main.models.credential')
credential_plugins = {entry_point.name: entry_point.load() for entry_point in entry_points(group='api_plugins.credentials')}
...
def build_safe_env(env):
    ...
    for k, v in safe_env.items():
        if k == 'AWS_ACCESS_KEY_ID':
            continue
        elif k.startswith('DISTRONODE_') and not k.startswith('DISTRONODE_NET') and not k.startswith('DISTRONODE_GALAXY_SERVER'):
            continue
...
def check_resource_server_for_user_in_organization(user, organization, requesting_user):
    ...
    client = get_resource_server_client(settings.RESOURCE_SERVICE_PATH, jwt_user_id=str(requesting_user.resource.distronode_id), raise_if_bad_request=False)
    # need to get the organization object_id in resource server, by querying with distronode_id
    response = client._make_request(path=f'resources/?distronode_id={str(organization.resource.distronode_id)}', method='GET')
    ...
    response = client._make_request(
        path=f'role_user_assignments/?role_definition__name__in=Organization Member,Organization Admin&user__resource__distronode_id={str(user.resource.distronode_id)}&object_id={org_id_in_resource_server}',
        method='GET',
    )
    response_json = response.json()
    if response.status_code != 200:
...
class CredentialType(CommonModelNameNotUnique):
    ...
    def inject_credential(self, credential, env, safe_env, args, private_data_dir):
        from api_plugins.interfaces._temporary_private_inject_api import inject_credential
...
from api_plugins.credentials.plugins import *  # noqa
