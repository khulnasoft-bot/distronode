...
from distronode_base.lib.utils.models import prevent_search, get_type_for_model
from distronode_base.rbac import permission_registry

# API
from api.main.models.base import CommonModelNameNotUnique, PasswordFieldsModel, NotificationFieldsModel
from api.main.dispatch import get_task_queuename
from api.main.dispatch.control import Control as ControlDispatcher
from api.main.registrar import activity_stream_registrar
from api.main.models.mixins import TaskManagerUnifiedJobMixin, ExecutionEnvironmentMixin
from api.main.models.rbac import to_permissions
...
from api.main.utils.encryption import encrypt_dict, decrypt_field
from api.main.utils import polymorphic
from api.main.constants import ACTIVE_STATES, CAN_CANCEL, JOB_VARIABLE_PREFIXES
from api.main.redact import UriCleaner, REPLACE_STR
from api.main.consumers import emit_channel_notification
from api.main.fields import AskForField, OrderedManyToManyField

__all__ = ['UnifiedJobTemplate', 'UnifiedJob', 'StdoutMaxBytesExceeded']

logger = logging.getLogger('api.main.models.unified_jobs')
logger_job_lifecycle = logging.getLogger('api.analytics.job_lifecycle')
...
class UnifiedJobTemplate(PolymorphicModel, CommonModelNameNotUnique, ExecutionEnvironmentMixin, NotificationFieldsModel):
    ...
    def accessible_pk_qs(cls, accessor, role_field):
        ...
        from distronode_base.rbac.models import RoleEvaluation
    ...
    def notification_templates(self):
        ...
        from api.main.models.notifications import NotificationTemplate
    ...
    def create_unified_job(self, instance_groups=None, **kwargs):
        ...
        from api.main.signals import disable_activity_stream, activity_stream_create
...
class UnifiedJob(
    ...
    distronode_version = models.CharField(
        max_length=255,
        blank=True,
        default='',
        editable=False,
        help_text=_("The version of Distronode Core installed in the execution environment."),
    )
    host_status_counts = models.JSONField(
        ...
        default=None,
        editable=False,
        help_text=_("Playbook stats from the Distronode playbook_on_stats event."),
    )
    ...
    def result_stdout_raw_handle(self, enforce_max_bytes=True):
        ...
        if enforce_max_bytes:
            ...
            from api.main.tasks.system import purge_old_stdout_files  # circular import

            purge_old_stdout_files.apply_async()

        # Before the addition of event-based stdout, older versions of
        # api stored stdout as raw text blobs in a certain database column
    ...
    def signal_start(self, **kwargs):
        ...
        # from api.main.tasks import notify_task_runner
    ...
    def control_plane_instance_group(self):
        from api.main.models.ha import InstanceGroup
    ...
    def api_meta_vars(self):
        """
        The result of this method is used as extra_vars of a job launched
        by API, for purposes of client playbook hooks
        """
        r = {}
    ...
    def launched_by(self):
        ...
        if ancestor_job.launch_type == "dependency":
            return {'id': None, 'name': 'Generated by API', 'type': 'Dependency', 'url': None}
