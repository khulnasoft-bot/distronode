...
from api.main.constants import HOST_FACTS_FIELDS
from api.main.models.base import (
    BaseModel,
...
from api.main.models.events import JobEvent, UnpartitionedJobEvent, UnpartitionedSystemJobEvent, SystemJobEvent
from api.main.models.unified_jobs import UnifiedJobTemplate, UnifiedJob
from api.main.models.notifications import (
    NotificationTemplate,
    JobNotificationMixin,
)
from api.main.utils import parse_yaml_or_json, getattr_dne, NullablePromptPseudoField, polymorphic
from api.main.fields import ImplicitRoleField, AskForField, JSONBlob, OrderedManyToManyField
...
class JobOptions(BaseModel):
    ...
    use_fact_cache = models.BooleanField(
        default=False,
        help_text=_(
            "If enabled, the service will act as an Distronode Fact Cache Plugin; persisting "
            "facts at the end of a playbook run to the database and caching facts for use by Distronode."
        ),
    )
...
class JobTemplate(UnifiedJobTemplate, JobOptions, SurveyJobTemplateMixin, ResourceMixin, CustomVirtualEnvMixin, RelatedJobsMixin, WebhookTemplateMixin):
    ...
    def create_unified_job(self, **kwargs):
        ...
        if slice_event:
            ...
            from api.main.models.workflow import WorkflowJobTemplate, WorkflowJobNode
...
class Job(UnifiedJob, JobOptions, SurveyJobMixin, JobNotificationMixin, TaskManagerJobMixin, CustomVirtualEnvMixin, WebhookMixin):
    ...
    credential.  It represents a single invocation of distronode-playbook with the
    given parameters.
    """
    ...
    def _get_task_class(cls):
        from api.main.tasks.jobs import RunJob
    ...
    def retry_qs(self, status):
        ...
        if status == 'all':
            ...
            # Special case for parity with Distronode .retry files
    ...
    def display_artifacts(self):
        ...
        if artifacts.get('_distronode_no_log', False):
            return "$hidden due to Distronode no_log flag$"
        return artifacts
    ...
    def api_meta_vars(self):
        r = super(Job, self).api_meta_vars()
        if self.project:
...
class SystemJob(UnifiedJob, SystemJobOptions, JobNotificationMixin):
    ...
    @classmethod
    def _get_task_class(cls):
        from api.main.tasks.jobs import RunSystemJob
