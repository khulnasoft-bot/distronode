...
from api.main.utils import is_testing
from api.api.versioning import reverse
from api.main.fields import ImplicitRoleField
from api.main.managers import InstanceManager, UUID_DEFAULT
from api.main.constants import JOB_FOLDER_PREFIX
from api.main.models.base import BaseModel, HasEditsMixin
from api.main.models.rbac import (
    ROLE_SINGLETON_SYSTEM_ADMINISTRATOR,
    ROLE_SINGLETON_SYSTEM_AUDITOR,
)
from api.main.models.unified_jobs import UnifiedJob
from api.main.utils.common import get_corrected_cpu, get_cpu_effective_capacity, get_corrected_memory, get_mem_effective_capacity
from api.main.models.mixins import RelatedJobsMixin, ResourceMixin
from api.main.models.receptor_address import ReceptorAddress

# distronode-runner
from distronode_runner.utils.capacity import get_cpu_count, get_mem_in_bytes

__all__ = ('Instance', 'InstanceGroup', 'InstanceLink', 'TowerScheduleState')

logger = logging.getLogger('api.main.models.ha')
...
class Instance(HasPolicyEditsMixin, BaseModel):
    """A model representing an API instance running against this database."""
    ...
    def consumed_capacity(self):
        ...
        if self.node_type in ('hybrid', 'control'):
            capacity_consumed += (
                settings.API_CONTROL_NODE_TASK_IMPACT * UnifiedJob.objects.filter(controller_node=self.hostname, status__in=('running', 'waiting')).count()
            )
    ...
    def get_cleanup_task_kwargs(self, **kwargs):
        """
        Produce options to use for the command: distronode-runner worker cleanup
        ...
        if settings.API_CLEANUP_PATHS:
            vargs['file_pattern'] = os.path.join(settings.API_ISOLATION_BASE_PATH, JOB_FOLDER_PREFIX % '*') + '*'
        vargs.update(kwargs)
    ...
    def local_health_check(self):
        ...
        self.save_health_data(api_application_version, get_cpu_count(), get_mem_in_bytes(), update_last_seen=True, errors=errors)


class InstanceGroup(HasPolicyEditsMixin, BaseModel, RelatedJobsMixin, ResourceMixin):
    """A model representing a Queue/Group of API Instances."""
...
def schedule_policy_task():
    from api.main.tasks.system import apply_cluster_membership_policies
...
def schedule_write_receptor_config(broadcast=True):
    from api.main.tasks.receptor import write_receptor_config  # prevents circular import
...
def on_instance_saved(sender, instance, created=False, raw=False, **kwargs):
    ...
    if settings.IS_K8S and instance.node_type in [Instance.Types.HOP, Instance.Types.EXECUTION]:
        if instance.node_state == Instance.States.DEPROVISIONING:
            from api.main.tasks.receptor import remove_deprovisioned_node  # prevents circular import
