...
from api.main import consumers
from api.main.fields import JSONBlob
from api.main.managers import DeferJobCreatedManager
from api.main.constants import MINIMAL_EVENTS
from api.main.models.base import CreatedModifiedModel
from api.main.utils import ignore_inventory_computed_fields, camelcase_to_underscore

analytics_logger = logging.getLogger('api.analytics.job_events')

logger = logging.getLogger('api.main.models.events')
...
class BasePlaybookEvent(CreatedModifiedModel):
    ...
    def get_event_display2(self):
        ...
        if self.event == 'runner_on_async_ok':
            try:
                ...
                pass
        # Runner events with distronode_job_id are part of async starting/polling.
        if self.event in ('runner_on_ok', 'runner_on_failed'):
            try:
                module_name = res['invocation']['module_name']
                job_id = res['distronode_job_id']
    ...
    def create_from_data(cls, **kwargs):
        ...
        # event* emitted by Distronode as a playbook runs.  That means that
...
class JobEvent(BasePlaybookEvent):
    ...
    def _update_host_summary_from_stats(self, hostnames):
        with ignore_inventory_computed_fields():
            ...
            from api.main.models import Host, JobHostSummary  # circular import
    ...
    def _update_host_metrics(updated_hosts_list):
        from api.main.models import HostMetric  # circular import
...
class AdHocCommandEvent(BaseCommandEvent):
    ...
    #   to hosts through adhoc_events. But in https://github.com/distronode/distronode/tree/devel/api/pull/8236/ we
