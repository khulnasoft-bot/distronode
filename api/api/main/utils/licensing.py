...
class Licenser(object):
    ...
    UNLICENSED_DATA = dict(
        ...
        product_name="Red Hat Distronode Automation Platform",
        valid_key=False,
    )
    ...
    def get_satellite_subs(self, host, user, pw):
        ...
        for org in orgs.json()['results']:
            resp = requests.get(
                '/'.join([host, '/katello/api/organizations/{}/subscriptions/?search=Red Hat Distronode Automation'.format(org['id'])]),
                verify=verify,
                auth=(user, pw),
            )
            resp.raise_for_status()
            ...
            if results != []:
                for sub in results:
                    ...
                    license['productName'] = "Red Hat Distronode Automation"
    ...
    def is_appropriate_sat_sub(self, sub):
        if 'Red Hat Distronode Automation' not in sub['subscription_name']:
            return False
        return True
    ...
    def is_appropriate_sub(self, sub):
        if sub['activeSubscription'] is False:
            return False
        # Products that contain Distronode Tower
    ...
    def generate_license_options_from_entitlements(self, json):
        ...
        if valid_subs:
            ...
            for sub in valid_subs:
                license = self.__class__(subscription_name='Red Hat Distronode Automation Platform')
        ...
        raise ValueError('No valid Red Hat Distronode Automation subscription could be found for this account.')  # noqa
    ...
    def validate(self):
        ...
        from api.main.models import Host, HostMetric, Instance
...
def get_licenser(*args, **kwargs):
    from api.main.utils.licensing import Licenser, OpenLicense

    try:
        if detect_server_product_name() == 'API':
            return OpenLicense()
        else:
