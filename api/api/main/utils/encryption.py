...
logger = logging.getLogger('api.main.utils.encryption')
...
def encrypt_field(instance, field_name, ask=False, subfield=None, secret_key=None):
    ...
    # API for symmetric encryption - generally it's used to encrypt sensitive
    # values that we store in the API database (such as SSH private keys for
    # credentials).
    #
    # If you're reading this function's code because you're thinking about
    # using it to encrypt *something new*, please remember that API has
    # official support for *regenerating* the SECRET_KEY (on which the
    # symmetric key is based):
    #
    # $ api-manage regenerate_secret_key
    # $ setup.sh -k
    #
    # ...so you'll need to *also* add code to support the
    # migration/re-encryption of these values (the code in question lives in
    # `api.main.management.commands.regenerate_secret_key`):
    #
    ...
    # places, you would also need to update the `api-manage regenerate_secret_key`
