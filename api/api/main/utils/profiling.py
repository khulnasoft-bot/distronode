...
class APITiming(APIProfileBase):
...
def timing(name, *init_args, **init_kwargs):
    def decorator_profile(func):
        ...
        def wrapper_profile(*args, **kwargs):
            timing = APITiming(name, *init_args, **init_kwargs)
            timing.start()
...
class APIProfiler(APIProfileBase):
    ...
    def stop(self):
        if self.started:
            ...
            print("APIProfiler::stop() called without calling start() first", file=sys.stderr)
            return None
...
def profile(name, *init_args, **init_kwargs):
    def decorator_profile(func):
        @functools.wraps(func)
        def wrapper_profile(*args, **kwargs):
            prof = APIProfiler(name, *init_args, **init_kwargs)
            prof.start()
            res = func(*args, **kwargs)
            prof.stop()
            return res
