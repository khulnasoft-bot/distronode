...
logger = logging.getLogger('api.main.utils.distronode')


__all__ = ['skip_directory', 'could_be_playbook', 'could_be_inventory']


valid_playbook_re = re.compile(r'^\s*?-?\s*?(?:hosts|(distronode\.builtin\.)?include|(distronode\.builtin\.)?import_playbook):\s*?.*?$')
...
def could_be_playbook(project_path, dir_path, filename):
    ...
    try:
        for n, line in enumerate(codecs.open(playbook_path, 'r', encoding='utf-8', errors='ignore')):
            ...
            elif n == 0 and line.startswith('$DISTRONODE_VAULT;'):
                matched = True
                break
    except IOError:
        return None
...
def read_distronode_config(project_path, variables_of_interest):
    fnames = ['/etc/distronode/distronode.cfg']
    if project_path:
        fnames.append(os.path.join(project_path, 'distronode.cfg'))
    values = {}
    try:
        parser = ConfigParser()
        parser.read(fnames)
        ...
        logger.exception('Failed to read distronode configuration(s) {}'.format(fnames))
    return values
