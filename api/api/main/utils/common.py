...
logger = logging.getLogger('api.main.utils')
...
def is_testing(argv=None):
    ...
    if os.environ.get('DJANGO_SETTINGS_MODULE') == 'api.main.tests.settings_for_test':
        return True
...
def get_api_version():
    """
    Return API version as reported by setuptools.
    """
    from api import __version__

    try:
        return _get_version('api')
    except Exception:
        return __version__


def get_api_http_client_headers():
    license = get_license().get('license_type', 'UNLICENSED')
    headers = {
        'Content-Type': 'application/json',
        'User-Agent': '{} {} ({})'.format('API' if license == 'open' else 'Red Hat Distronode Automation Platform', get_api_version(), license),
    }
    return headers
...
class ScheduleTaskManager(ScheduleManager):
    def __init__(self):
        from api.main.scheduler.tasks import task_manager
...
class ScheduleDependencyManager(ScheduleManager):
    def __init__(self):
        from api.main.scheduler.tasks import dependency_manager
...
class ScheduleWorkflowManager(ScheduleManager):
    def __init__(self):
        from api.main.scheduler.tasks import workflow_manager
    ...
    for venv_path in all_venv_paths:
        if os.path.exists(venv_path):
            for d in os.listdir(venv_path):
                if venv_path == settings.BASE_VENV_PATH and d == 'api':
                    continue
...
def is_distronode_variable(key):
    return key.startswith('distronode_')


def extract_distronode_vars(extra_vars):
    extra_vars = parse_yaml_or_json(extra_vars)
    distronode_vars = set([])
    for key in list(extra_vars.keys()):
        if is_distronode_variable(key):
            extra_vars.pop(key)
            distronode_vars.add(key)
    return (extra_vars, distronode_vars)
...
def truncate_stdout(stdout, size):
    from api.main.constants import ANSI_SGR_PATTERN
...
def cleanup_new_process(func):
    ...
    def wrapper_cleanup_new_process(*args, **kwargs):
        from api.conf.settings import SettingsWrapper  # noqa
...
def load_all_entry_points_for(entry_point_subsections: list[str], /) -> dict[str, EntryPoint]:
    return {ep.name: ep for entry_point_category in entry_point_subsections for ep in entry_points(group=f'api_plugins.{entry_point_category}')}
