...
from api.main.utils.reload import supervisor_service_command
...
def construct_rsyslog_conf_template(settings=settings):
    ...
    spool_directory = getattr(settings, 'LOG_AGGREGATOR_MAX_DISK_USAGE_PATH', '/var/lib/api').rstrip('/')
    error_log_file = getattr(settings, 'LOG_AGGREGATOR_RSYSLOGD_ERROR_LOG_FILE', '')

    queue_options = [
        f'queue.spoolDirectory="{spool_directory}"',
        'queue.filename="api-external-logger-action-queue"',
    ...
    parts.extend(
        [
            '$WorkDirectory /var/lib/api/rsyslog',
            f'$MaxMessageSize {max_bytes}',
            '$IncludeConfig /var/lib/api/rsyslog/conf.d/*.conf',
...
def reconfigure_rsyslog():
    ...
    with tempfile.TemporaryDirectory(dir='/var/lib/api/rsyslog/', prefix='rsyslog-conf-') as temp_dir:
        path = temp_dir + '/rsyslog.conf.temp'
        with open(path, 'w') as f:
            os.chmod(path, 0o640)
            f.write(tmpl + '\n')
        shutil.move(path, '/var/lib/api/rsyslog/rsyslog.conf')
    supervisor_service_command(command='restart', service='api-rsyslogd')
