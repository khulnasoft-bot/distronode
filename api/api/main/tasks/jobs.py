...
# Shared code for the API platform
from api_plugins.interfaces._temporary_private_container_api import CONTAINER_ROOT, get_incontainer_path
...
from api.main.constants import (
...
from api.main.tasks.callback import (
    RunnerCallback,
...
from api.main.tasks.signals import with_signal_handling, signal_callback
from api.main.tasks.receptor import APIReceptorJob
from api.main.tasks.facts import start_fact_cache, finish_fact_cache
from api.main.exceptions import ApiTaskError, PostRunError, ReceptorNodeNotFound
from api.main.utils.distronode import read_distronode_config
from api.main.utils.safe_yaml import safe_dump, sanitize_jinja
from api.main.utils.common import (
    update_scm_url,
    extract_distronode_vars,
    get_api_version,
    create_partition,
    ScheduleWorkflowManager,
    ScheduleTaskManager,
)
from api.conf.license import get_license
from api.main.utils.handlers import SpecialInventoryHandler
from api.main.tasks.system import update_smart_memberships_for_inventory, update_inventory_computed_fields
from api.main.utils.update_model import update_model
from rest_framework.exceptions import PermissionDenied
from django.utils.translation import gettext_lazy as _

logger = logging.getLogger('api.main.tasks.jobs')
...
class BaseTask(object):
    ...
    def build_execution_environment_params(self, instance, private_data_dir):
        ...
        if settings.API_ISOLATION_SHOW_PATHS:
            params['container_volume_mounts'] = []
            for this_path in settings.API_ISOLATION_SHOW_PATHS:
    ...
    def build_private_data_dir(self, instance):
        ...
        """
        path = tempfile.mkdtemp(prefix=JOB_FOLDER_PREFIX % instance.pk, dir=settings.API_ISOLATION_BASE_PATH)
        os.chmod(path, stat.S_IRUSR | stat.S_IWUSR | stat.S_IXUSR)
        if settings.API_CLEANUP_PATHS:
    ...
    def build_project_dir(self, instance, private_data_dir):
        """
        Create the distronode-runner project subdirectory. In many cases this is the source checkout.
    ...
    def build_private_data_files(self, instance, private_data_dir):
        ...
        {
            'credentials': {
                <api.main.models.Credential>: '/path/to/decrypted/data',
                <api.main.models.Credential>: '/path/to/decrypted/data',
                ...
            },
            'certificates': {
                <api.main.models.Credential>: /path/to/signed/ssh/certificate,
                <api.main.models.Credential>: /path/to/signed/ssh/certificate,
                ...
            }
        }
        ...
        if private_data is not None:
            for credential, data in private_data.get('credentials', {}).items():
                ...
                if credential and credential.credential_type.namespace in ('ssh', 'scm'):
                    ssh_key_data = data
                # Distronode network modules do not yet support ssh-agent.
    ...
    def build_extra_vars_file(self, instance, private_data_dir):
        """
        Build distronode yaml file filled with extra vars to be passed via -e@file.yml
        """
    ...
    def build_env(self, instance, private_data_dir, private_data_files=None):
        """
        Build environment dictionary for distronode-playbook.
        """
        env = {}
        # Add DISTRONODE_* settings to the subprocess environment.
        for attr in dir(settings):
            if attr == attr.upper() and attr.startswith('DISTRONODE_') and not attr.startswith('DISTRONODE_BASE_'):
                env[attr] = str(getattr(settings, attr))
        # Also set environment variables configured in API_TASK_ENV setting.
        for key, value in settings.API_TASK_ENV.items():
            env[key] = str(value)

        env['API_PRIVATE_DATA_DIR'] = private_data_dir
    ...
    def write_args_file(self, private_data_dir, args):
        return self.write_private_data_file(private_data_dir, 'cmdline', distronode_runner.utils.args2cmdline(*args), sub_dir='env')
    ...
    def final_run_hook(self, instance, status, private_data_dir):
        ...
        distronode_version_file = os.path.join(artifact_dir, 'distronode_version.txt')
        ...
        if os.path.exists(distronode_version_file):
            with open(distronode_version_file) as ee_distronode_info:
                distronode_version_info = ee_distronode_info.readline()
                instance.distronode_version = distronode_version_info
                instance.save(update_fields=['distronode_version'])
    ...
    def run(self, pk, **kwargs):
        ...
        try:
            ...
            if not os.path.exists(settings.API_ISOLATION_BASE_PATH):
                raise RuntimeError('API_ISOLATION_BASE_PATH=%s does not exist' % settings.API_ISOLATION_BASE_PATH)
            ...
            params = {
                ...
                'suppress_env_files': getattr(settings, 'API_RUNNER_OMIT_ENV_FILES', True),
                'envvars': env,
            }
            ...
            if self.should_use_fact_cache():
                # Enable Distronode fact cache.
                params['fact_cache_type'] = 'jsonfile'
            else:
                # Disable Distronode fact cache.
            ...
            runner_settings = {
                ...
                'suppress_distronode_output': True,
                'suppress_output_file': getattr(settings, 'API_RUNNER_SUPPRESS_OUTPUT_FILE', True),
            }
            ...
            if isinstance(self.instance, SystemJob):
                res = distronode_runner.interface.run(
                ...
                receptor_job = APIReceptorJob(self, params)
            ...
            if status in ('timeout', 'error'):
                ...
                if (cancel_flag_value is False) and signal_callback():
                    ...
                    self.runner_callback.delay_update(skip_if_already_set=True, job_explanation="The running distronode process received a shutdown signal.")
                    status = 'failed'
        ...
        if status != 'successful':
            if status == 'canceled':
                raise ApiTaskError.TaskCancel(self.instance, rc)
            else:
                raise ApiTaskError.TaskError(self.instance, rc)
...
class SourceControlMixin(BaseTask):
    ...
    def get_sync_needs(self, project, scm_branch=None):
        ...
        # Determine whether or not this project sync needs to populate the cache for Distronode content, roles and collections
...
class RunJob(SourceControlMixin, BaseTask):
    """
    Run a job using distronode-playbook.
    """
    ...
    def build_private_data(self, job, private_data_dir):
        ...
        {
            'credentials': {
                <api.main.models.Credential>: <credential_decrypted_ssh_key_data>,
                <api.main.models.Credential>: <credential_decrypted_ssh_key_data>,
                ...
            },
            'certificates': {
                <api.main.models.Credential>: <signed SSH certificate data>,
                <api.main.models.Credential>: <signed SSH certificate data>,
                ...
            }
        }
    ...
    def build_env(self, job, private_data_dir, private_data_files=None):
        ...
        env['DISTRONODE_RETRY_FILES_ENABLED'] = "False"
        env['MAX_EVENT_RES'] = str(settings.MAX_EVENT_RES_DATA)
        if hasattr(settings, 'API_DISTRONODE_CALLBACK_PLUGINS') and settings.API_DISTRONODE_CALLBACK_PLUGINS:
            env['DISTRONODE_CALLBACK_PLUGINS'] = ':'.join(settings.API_DISTRONODE_CALLBACK_PLUGINS)
        env['API_HOST'] = settings.TOWER_URL_BASE
        ...
        env['DISTRONODE_SSH_CONTROL_PATH_DIR'] = '/runner/cp'
        ...
        for network_cred in job.network_credentials:
            env['DISTRONODE_NET_USERNAME'] = network_cred.get_input('username', default='')
            env['DISTRONODE_NET_PASSWORD'] = network_cred.get_input('password', default='')

            ssh_keyfile = cred_files.get(network_cred, '')
            if ssh_keyfile:
                env['DISTRONODE_NET_SSH_KEYFILE'] = ssh_keyfile

            authorize = network_cred.get_input('authorize', default=False)
            env['DISTRONODE_NET_AUTHORIZE'] = str(int(authorize))
            if authorize:
                env['DISTRONODE_NET_AUTH_PASS'] = network_cred.get_input('authorize_password', default='')

        path_vars = (
            ('DISTRONODE_COLLECTIONS_PATHS', 'collections_paths', 'requirements_collections', '~/.distronode/collections:/usr/share/distronode/collections'),
            ('DISTRONODE_ROLES_PATH', 'roles_path', 'requirements_roles', '~/.distronode/roles:/usr/share/distronode/roles:/etc/distronode/roles'),
            ('DISTRONODE_COLLECTIONS_PATH', 'collections_path', 'requirements_collections', '~/.distronode/collections:/usr/share/distronode/collections'),
        )

        config_values = read_distronode_config(os.path.join(private_data_dir, 'project'), list(map(lambda x: x[1], path_vars)))
    ...
    def build_args(self, job, private_data_dir, passwords):
        """
        Build command line argument list for running distronode-playbook,
        ...
        # it doesn't make sense to rely on distronode-playbook's default of using
        # the current user.
    ...
    def build_extra_vars_file(self, job, private_data_dir):
        ...
        extra_vars.update(job.api_meta_vars())
    ...
    def pre_run_hook(self, job, private_data_dir):
        ...
        # where distronode expects to find it
        if self.should_use_fact_cache():
    ...
    def post_run_hook(self, job, status):
        ...
        private_data_dir = job.job_env.get('API_PRIVATE_DATA_DIR')
...
class RunProjectUpdate(BaseTask):
    ...
    def build_env(self, project_update, private_data_dir, private_data_files=None):
        ...
        env['DISTRONODE_RETRY_FILES_ENABLED'] = str(False)
        env['DISTRONODE_ASK_PASS'] = str(False)
        env['DISTRONODE_BECOME_ASK_PASS'] = str(False)
        env['DISPLAY'] = ''  # Prevent stupid password popup when running tests.
        # give distronode a hint about the intended tmpdir to work around issues
        # like https://github.com/distronode/distronode/issues/30064
        env['TMP'] = settings.API_ISOLATION_BASE_PATH
        env['PROJECT_UPDATE_ID'] = str(project_update.pk)
        if settings.GALAXY_IGNORE_CERTS:
            env['DISTRONODE_GALAXY_IGNORE'] = str(True)
        ...
        if project_update.project.organization:
            for i, cred in enumerate(project_update.project.organization.galaxy_credentials.all()):
                env[f'DISTRONODE_GALAXY_SERVER_SERVER{i}_URL'] = cred.get_input('url')
                auth_url = cred.get_input('auth_url', default=None)
                token = cred.get_input('token', default=None)
                if token:
                    env[f'DISTRONODE_GALAXY_SERVER_SERVER{i}_TOKEN'] = token
                if auth_url:
                    env[f'DISTRONODE_GALAXY_SERVER_SERVER{i}_AUTH_URL'] = auth_url
                galaxy_server_list.append(f'server{i}')

        if galaxy_server_list:
            env['DISTRONODE_GALAXY_SERVER_LIST'] = ','.join(galaxy_server_list)
    ...
    def build_extra_vars_file(self, project_update, private_data_dir):
        ...
        if not galaxy_creds_are_defined and (settings.API_ROLES_ENABLED or settings.API_COLLECTIONS_ENABLED):
        ...
        extra_vars.update(
            {
                ...
                'api_license_type': get_license().get('license_type', 'UNLICENSED'),
                'api_version': get_api_version(),
                'scm_url': scm_url,
                'scm_branch': scm_branch,
                'scm_clean': project_update.scm_clean,
                'scm_track_submodules': project_update.scm_track_submodules,
                'roles_enabled': galaxy_creds_are_defined and settings.API_ROLES_ENABLED,
                'collections_enabled': galaxy_creds_are_defined and settings.API_COLLECTIONS_ENABLED,
        ...
        if project_update.scm_type == 'archive':
            ...
            extra_vars['distronode_remote_tmp'] = os.path.join(project_update.get_project_path(check_if_exists=False), '.distronode_api', 'tmp')
    ...
    def build_project_dir(self, instance, private_data_dir):
        ...
        # to be consistent with the distronode-runner model,
        # that is moved into the runner project folder here
        api_playbooks = self.get_path_to('../../', 'playbooks')
        shutil.copytree(api_playbooks, os.path.join(private_data_dir, 'project'))
    ...
    def make_local_copy(project, job_private_data_dir):
        ...
        :param str job_private_data_dir: The root of the target distronode-runner folder
        """
        project_path = project.get_project_path(check_if_exists=False)
        ...
        subfolders = []
        if settings.API_COLLECTIONS_ENABLED:
            subfolders.append('requirements_collections')
        if settings.API_ROLES_ENABLED:
...
class RunInventoryUpdate(SourceControlMixin, BaseTask):
    ...
    def build_env(self, inventory_update, private_data_dir, private_data_files=None):
        """Build environment dictionary for distronode-inventory.
        ...
        if inventory_update.source == 'scm' and inventory_update.source_project_update:
            env_key = 'DISTRONODE_COLLECTIONS_PATHS'
            config_setting = 'collections_paths'
            folder = 'requirements_collections'
            default = '~/.distronode/collections:/usr/share/distronode/collections'

            config_values = read_distronode_config(os.path.join(private_data_dir, 'project'), [config_setting])
            ...
            paths = [os.path.join(CONTAINER_ROOT, folder)] + paths
            env[env_key] = os.pathsep.join(paths)
        if 'DISTRONODE_COLLECTIONS_PATHS' in env:
            paths = env['DISTRONODE_COLLECTIONS_PATHS'].split(':')
        else:
            paths = ['~/.distronode/collections', '/usr/share/distronode/collections']
        paths.append('/usr/share/automation-controller/collections')
        env['DISTRONODE_COLLECTIONS_PATHS'] = os.pathsep.join(paths)
    ...
    def build_args(self, inventory_update, private_data_dir, passwords):
        ...
        args = ['distronode-inventory', '--list', '--export']
        ...
        # Added this in order to allow older versions of distronode-inventory https://github.com/distronode/distronode/pull/79596
        # limit should be usable in distronode-inventory 2.15+
    ...
    def pseudo_build_inventory(self, inventory_update, private_data_dir):
        ...
        to be "Distronode" inventory (by runner) even though it is
    ...
    def post_run_hook(self, inventory_update, status):
        ...
        private_data_dir = inventory_update.job_env['API_PRIVATE_DATA_DIR']
        ...
        # Verbosity is applied to saving process, as well as distronode-inventory CLI option
        if inventory_update.verbosity:
        ...
        inv_logger = logging.getLogger('api.main.commands.inventory_import')
        formatter = inv_logger.handlers[0].formatter
        formatter.job_start = inventory_update.started
        handler.formatter = formatter
        inv_logger.handlers[0] = handler

        from api.main.management.commands.inventory_import import Command as InventoryImportCommand
...
class RunAdHocCommand(BaseTask):
    """
    Run an ad hoc command using distronode.
    """
    ...
    def build_env(self, ad_hoc_command, private_data_dir, private_data_files=None):
        """
        Build environment dictionary for distronode.
        """
        ...
        env['DISTRONODE_LOAD_CALLBACK_PLUGINS'] = '1'
        env['DISTRONODE_SFTP_BATCH_MODE'] = 'False'

        return env

    def build_args(self, ad_hoc_command, private_data_dir, passwords):
        """
        Build command line argument list for running distronode, optionally using
        ...
        # Always specify the normal SSH user as root by default.  Since this
        # task is normally running in the background under a service account,
        # it doesn't make sense to rely on distronode's default of using the
        # current user.
        ssh_username = ssh_username or 'root'
        args = []
    ...
    def build_extra_vars_file(self, ad_hoc_command, private_data_dir):
        ...
        if ad_hoc_command.extra_vars_dict:
            redacted_extra_vars, removed_vars = extract_distronode_vars(ad_hoc_command.extra_vars_dict)
            if removed_vars:
        ...
        extra_vars.update(ad_hoc_command.api_meta_vars())
        self._write_extra_vars_file(private_data_dir, extra_vars)
...
class RunSystemJob(BaseTask):
    ...
    def build_args(self, system_job, private_data_dir, passwords):
        args = ['api-manage', system_job.job_type]
        try:
