...
# API
from api.main.models.inventory import Host


logger = logging.getLogger('api.main.tasks.facts')
system_tracking_logger = logging.getLogger('api.analytics.system_tracking')
...
def start_fact_cache(hosts, destination, log_data, timeout=None, inventory_id=None):
    ...
    if timeout is None:
        timeout = settings.DISTRONODE_FACT_CACHE_TIMEOUT
    ...
    for host in hosts:
        if (not host.distronode_facts_modified) or (timeout and host.distronode_facts_modified < now() - datetime.timedelta(seconds=timeout)):
        ...
        try:
            with codecs.open(filepath, 'w', encoding='utf-8') as f:
                ...
                json.dump(host.distronode_facts, f)
...
def raw_update_hosts(host_list):
    Host.objects.bulk_update(host_list, ['distronode_facts', 'distronode_facts_modified'])
        ...
        if os.path.exists(filepath):
            ...
            if (not facts_write_time) or modified > facts_write_time:
                with codecs.open(filepath, 'r', encoding='utf-8') as f:
                    try:
                        distronode_facts = json.load(f)
                    except ValueError:
                        continue
                    host.distronode_facts = distronode_facts
                    host.distronode_facts_modified = now()
                    ...
                    system_tracking_logger.info(
                        ...
                        extra=dict(
                            ...
                            host_name=host.name,
                            distronode_facts=host.distronode_facts,
                            distronode_facts_modified=host.distronode_facts_modified.isoformat(),
                            job_id=job_id,
                        ),
                    )
            ...
            # if the file goes missing, distronode removed it (likely via clear_facts)
            host.distronode_facts = {}
            host.distronode_facts_modified = now()
            hosts_to_update.append(host)
