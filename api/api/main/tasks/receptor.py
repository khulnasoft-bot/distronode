...
# API
from api.main.utils.execution_environments import get_default_pod_spec
from api.main.exceptions import ReceptorNodeNotFound
from api.main.utils.common import (
    deepmerge,
    parse_yaml_or_json,
    cleanup_new_process,
)
from api.main.constants import MAX_ISOLATED_PATH_COLON_DELIMITER
from api.main.tasks.signals import signal_state, signal_callback, SignalExit
from api.main.models import Instance, InstanceLink, UnifiedJob, ReceptorAddress
...
logger = logging.getLogger('api.main.tasks.receptor')
...
def administrative_workunit_reaper(work_list=None):
    ...
    for unit_id, work_data in work_list.items():
        ...
        if extra_data is None:
            continue  # if this is not distronode-runner work, we do not want to touch it
        if isinstance(extra_data, str):
            if not work_data.get('StateName', None) or work_data.get('StateName') in RECEPTOR_ACTIVE_STATES:
                continue
        else:
            if extra_data.get('RemoteWorkType') != 'distronode-runner':
                continue
...
def run_until_complete(node, timing_data=None, **kwargs):
    """
    Runs an distronode-runner work_type on remote node, waits until it completes, then returns stdout.
    """
    config_data = read_receptor_config()
    ...
    result = receptor_ctl.submit_work(worktype='distronode-runner', node=node, **kwargs)
    ...
    if state_name.lower() == 'failed':
        ...
        if work_detail:
            ...
            raise RemoteJobError(f'Unknown distronode-runner error on node {node}, stdout:\n{stdout}')

    return stdout


def worker_info(node_name, work_type='distronode-runner'):
    error_list = []
    ...
    try:
        ...
        if 'unrecognized arguments: --worker-info' in details:
            error_list.append(f'Old version (2.0.1 or earlier) of distronode-runner on node {node_name} without --worker-info')
        else:
...
def _convert_args_to_cli(vargs):
    """
    For the distronode-runner worker cleanup command
...
def worker_cleanup(node_name, vargs, timeout=300.0):
    ...
    logger.debug(f'Running command over receptor mesh on {node_name}: distronode-runner worker {remote_command}')
...
class APIReceptorJob:
    ...
    def _run_internal(self, receptor_ctl):
        ...
        if self.work_type == 'distronode-runner':
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            ...
            if res.status == 'error':
                # If distronode-runner ran, but an error occured at runtime, the traceback information
    ...
    def transmit(self, _socket):
        try:
            distronode_runner.interface.run(streamer='transmit', _output=_socket.makefile('wb'), **self.runner_params)
        finally:
    ...
    def processor(self, resultfile):
        return distronode_runner.interface.run(
            streamer='process',
            quiet=True,
            _input=resultfile,
    ...
    def receptor_params(self):
        if self.task.instance.is_container_group_task:
            ...
            receptor_params = {
                ...
                "pod_pending_timeout": getattr(settings, 'API_CONTAINER_GROUP_POD_PENDING_TIMEOUT', "5m"),
            }
            ...
            if self.work_type == 'distronode-runner' and settings.API_CLEANUP_PATHS:
    ...
    @property
    def sign_work(self):
        if self.work_type in ('distronode-runner', 'local'):
    ...
    def pod_definition(self):
        ...
        pod_spec['spec']['containers'][0]['args'] = ['distronode-runner', 'worker', '--private-data-dir=/runner']

        if settings.API_RUNNER_KEEPALIVE_SECONDS:
            pod_spec['spec']['containers'][0].setdefault('env', [])
            pod_spec['spec']['containers'][0]['env'].append({'name': 'DISTRONODE_RUNNER_KEEPALIVE_SECONDS', 'value': str(settings.API_RUNNER_KEEPALIVE_SECONDS)})
        ...
        if settings.API_MOUNT_ISOLATED_PATHS_ON_K8S and settings.API_ISOLATION_SHOW_PATHS:
            spec_volume_mounts = []
            spec_volumes = []

            for idx, this_path in enumerate(settings.API_ISOLATION_SHOW_PATHS):
                mount_option = None
        ...
        if self.task:
            pod_spec['metadata'] = deepmerge(
                ...
                dict(name=self.pod_name, labels={'distronode-api': settings.INSTALL_UUID, 'distronode-api-job-id': str(self.task.instance.id)}),
            )
...
RECEPTOR_CONFIG_STARTER = (
    ...
    {'work-command': {'worktype': 'local', 'command': 'distronode-runner', 'params': 'worker', 'allowruntimeparams': True}},
