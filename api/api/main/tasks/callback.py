...
from api.main.constants import MINIMAL_EVENTS, DISTRONODE_RUNNER_NEEDS_UPDATE_MESSAGE
from api.main.utils.update_model import update_model
from api.main.queue import CallbackQueueDispatcher

logger = logging.getLogger('api.main.tasks.callback')
...
class RunnerCallback:
    ...
    def get_delayed_update_fields(self):
        ...
        if 'got an unexpected keyword argument' in self.extra_update_fields.get('result_traceback', ''):
            self.delay_update(result_traceback=DISTRONODE_RUNNER_NEEDS_UPDATE_MESSAGE)
        return self.extra_update_fields

    def event_handler(self, event_data):
        #
        # ⚠️  D-D-D-DANGER ZONE ⚠️
        # This method is called once for *every event* emitted by Distronode
        ...
        #
        """
        Distronode runner puts a parent_uuid on each event, no matter what the type.
        API only saves the parent_uuid if the event is for a Job.
        """
        # cache end_line locally for RunInventoryUpdate tasks
        # which generate job events from two 'streams':
        # distronode-inventory and the api.main.commands.inventory_import
        # logger
        ...
        if isinstance(self, RunnerCallbackForProjectUpdate):
            ...
            # it's common for Distronode's SCM modules to print
            ...
            try:
                if task in ('git', 'svn', 'distronode.builtin.git', 'distronode.builtin.svn'):
    ...
    def finished_callback(self, runner_obj):
        """
        Distronode runner callback triggered on finished run
        """
        event_data = {
            'event': 'EOF',
    ...
    def status_handler(self, status_data, runner_config):
        """
        Distronode runner callback triggered on status transition
        """
        if status_data['status'] == 'starting':
            ...
            for k, v in self.safe_env.items():
                if k in job_env:
                    job_env[k] = v
            from api.main.signals import disable_activity_stream  # Circular import
...
class RunnerCallbackForProjectUpdate(RunnerCallback):
    ...
    def event_handler(self, event_data):
        ...
        if returned_data.get('task_action', '') in ('set_fact', 'distronode.builtin.set_fact'):
            returned_facts = returned_data.get('res', {}).get('distronode_facts', {})
