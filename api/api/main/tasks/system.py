...
# Runner
import distronode_runner.cleanup

# dateutil
from dateutil.parser import parse as parse_date

# django-distronode-base
from distronode_base.resource_registry.tasks.sync import SyncExecutor

# API
from api import __version__ as api_application_version
from api.main.access import access_registry
...
from api.main.constants import ACTIVE_STATES, ERROR_STATES
from api.main.dispatch.publish import task
from api.main.dispatch import get_task_queuename, reaper
from api.main.utils.common import ignore_inventory_computed_fields, ignore_inventory_group_removal

from api.main.utils.reload import stop_local_services
from api.main.utils.pglock import advisory_lock
from api.main.tasks.helpers import is_run_threshold_reached
from api.main.tasks.receptor import get_receptor_ctl, worker_info, worker_cleanup, administrative_workunit_reaper, write_receptor_config
from api.main.consumers import emit_channel_notification
from api.main import analytics
from api.conf import settings_registry
from api.main.analytics.subsystem_metrics import DispatcherMetrics

from rest_framework.exceptions import PermissionDenied

logger = logging.getLogger('api.main.tasks.system')
...
def dispatch_startup():
    startup_logger = logging.getLogger('api.main.tasks')
...
def profile_sql(threshold=1, minutes=1):
    if threshold <= 0:
        cache.delete('api-profile-sql-threshold')
        logger.error('SQL PROFILING DISABLED')
    else:
        cache.set('api-profile-sql-threshold', threshold, timeout=minutes * 60)
    ...
    if runner_cleanup_kwargs:
        ...
        with StringIO() as buffer:
            with redirect_stdout(buffer):
                distronode_runner.cleanup.run_cleanup(runner_cleanup_kwargs)
...
def execution_node_health_check(node):
    ...
    instance.save_health_data(
        version='distronode-runner-' + data.get('runner_version', '???'),
...
def inspect_established_receptor_connections(mesh_status):
    ...
    '''
    from api.main.models import InstanceLink
...
def cluster_node_heartbeat(dispatch_time=None, worker_tasks=None):
    ...
    if this_inst:
        ...
        if settings.API_AUTO_DEPROVISION_INSTANCES:
    ...
    for other_inst in instance_list:
        if other_inst.node_type in ('execution', 'hop'):
            continue
        if other_inst.version == "" or other_inst.version.startswith('distronode-runner'):
            continue
        if Version(other_inst.version.split('-', 1)[0]) > Version(api_application_version.split('-', 1)[0]) and not settings.DEBUG:
            logger.error(
    ...
    for other_inst in lost_instances:
        ...
        try:
            if settings.API_AUTO_DEPROVISION_INSTANCES and other_inst.node_type == "control":
...
@task(queue=get_task_queuename)
def api_receptor_workunit_reaper():
    """
    When an API job is launched via receptor, files such as status, stdin, and stdout are created
    ...
    After an API job executes, the receptor work unit directory is cleaned up by
    issuing the work release command. In some cases the release process might fail, or
    if API crashes during a job's execution, the work release command is never issued to begin with.
    As such, this periodic task will obtain a list of all receptor work units, and find which ones
    belong to API jobs that are in a completed state (status is canceled, error, or succeeded).
...
def api_k8s_reaper():
    ...
    from api.main.scheduler.kubernetes import PodManager  # prevent circular import
    ...
    for group in InstanceGroup.objects.filter(is_container_group=True).iterator():
        ...
        for job in reap_job_candidates:
            ...
            try:
                pm = PodManager(job)
                pm.kube_api.delete_namespaced_pod(name=pods[job.id], namespace=pm.namespace, _request_timeout=settings.API_CONTAINER_GROUP_K8S_API_TIMEOUT)
            except Exception:
                logger.exception("Failed to delete orphaned pod {} from {}".format(job.log_format, group))


@task(queue=get_task_queuename)
def api_periodic_scheduler():
    lock_session_timeout_milliseconds = settings.TASK_MANAGER_LOCK_TIMEOUT * 1000
    with advisory_lock('api_periodic_scheduler_lock', lock_session_timeout_milliseconds=lock_session_timeout_milliseconds, wait=False) as acquired:
...
def deep_copy_model_obj(model_module, model_name, obj_pk, new_obj_pk, user_pk, permission_check_func=None):
    ...
    from api.api.generics import CopyAPIView
    from api.main.signals import disable_activity_stream
