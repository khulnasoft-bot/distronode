...
from api.main.models import UnifiedJob
from api.main.analytics.collectors import (
    counts,
    instance_info,
    job_instance_counts,
    job_counts,
...
def metrics():
    ...
    SYSTEM_INFO = Info('api_system', 'API System Information', registry=REGISTRY)
    ORG_COUNT = Gauge('api_organizations_total', 'Number of organizations', registry=REGISTRY)
    USER_COUNT = Gauge('api_users_total', 'Number of users', registry=REGISTRY)
    TEAM_COUNT = Gauge('api_teams_total', 'Number of teams', registry=REGISTRY)
    INV_COUNT = Gauge('api_inventories_total', 'Number of inventories', registry=REGISTRY)
    PROJ_COUNT = Gauge('api_projects_total', 'Number of projects', registry=REGISTRY)
    JT_COUNT = Gauge('api_job_templates_total', 'Number of job templates', registry=REGISTRY)
    WFJT_COUNT = Gauge('api_workflow_job_templates_total', 'Number of workflow job templates', registry=REGISTRY)
    HOST_COUNT = Gauge(
        'api_hosts_total',
        'Number of hosts',
        [
    ...
    SCHEDULE_COUNT = Gauge('api_schedules_total', 'Number of schedules', registry=REGISTRY)
    USER_SESSIONS = Gauge(
        'api_sessions_total',
        'Number of sessions',
        [
    ...
    RUNNING_JOBS = Gauge('api_running_jobs_total', 'Number of running jobs on the system', registry=REGISTRY)
    PENDING_JOBS = Gauge('api_pending_jobs_total', 'Number of pending jobs on the system', registry=REGISTRY)
    STATUS = Gauge(
        'api_status_total',
    ...
    INSTANCE_LAUNCH_TYPE = Gauge(
        'api_instance_launch_type_total',
    ...
    INSTANCE_CONSUMED_CAPACITY = Gauge(
        'api_instance_consumed_capacity',
    ...
    INSTANCE_REMAINING_CAPACITY = Gauge(
        'api_instance_remaining_capacity',
    ...
    LICENSE_INSTANCE_TOTAL = Gauge('api_license_instance_total', 'Total number of managed hosts provided by your license', registry=REGISTRY)
    LICENSE_INSTANCE_FREE = Gauge('api_license_instance_free', 'Number of remaining managed hosts provided by your license', registry=REGISTRY)

    DATABASE_CONNECTIONS = Gauge('api_database_connections_total', 'Number of connections to database', registry=REGISTRY)
