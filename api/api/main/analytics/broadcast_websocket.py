...
logger = logging.getLogger('api.analytics.broadcast_websocket')
...
class RelayWebsocketStats:
    def __init__(self, local_hostname, remote_hostname):
        ...
        self._messages_received_total = Counter(
            f'api_{self.remote_name}_messages_received_total',
        ...
        self._messages_received_current_conn = Gauge(
            f'api_{self.remote_name}_messages_received_currrent_conn',
        ...
        self._connection = Enum(
            f'api_{self.remote_name}_connection', 'Websocket broadcast connection', states=['disconnected', 'connected'], registry=self._registry
        )
        self._connection.state('disconnected')
        self._connection_start = Gauge(f'api_{self.remote_name}_connection_start', 'Time the connection was established', registry=self._registry)

        self._messages_received_per_minute = Gauge(
            f'api_{self.remote_name}_messages_received_per_minute', 'Messages received per minute', registry=self._registry
        )
        self._internal_messages_received_per_minute = FixedSlidingWindow()

    def unregister(self):
        self._registry.unregister(f'api_{self.remote_name}_messages_received')
        self._registry.unregister(f'api_{self.remote_name}_connection')
