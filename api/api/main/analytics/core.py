...
from api.main.models import Job
from api.main.access import access_registry
from api.main.utils import get_api_http_client_headers, set_environ, datetime_hook
from api.main.utils.pglock import advisory_lock

__all__ = ['register', 'gather', 'ship']


logger = logging.getLogger('api.main.analytics')
...
def all_collectors():
    from api.main.analytics import collectors

    return {
        func.__api_analytics_key__: {
            'name': func.__api_analytics_key__,
            'version': func.__api_analytics_version__,
            'description': func.__api_analytics_description__ or '',
        }
        for name, func in inspect.getmembers(collectors)
        if inspect.isfunction(func) and hasattr(func, '__api_analytics_key__')
    }
...
def register(key, version, description=None, format='json', expensive=None, full_sync_interval=None):
    ...
    def decorate(f):
        f.__api_analytics_key__ = key
        f.__api_analytics_version__ = version
        f.__api_analytics_description__ = description
        f.__api_analytics_type__ = format
        f.__api_expensive__ = expensive
        f.__api_full_sync_interval__ = full_sync_interval
        return f
...
def gather(dest=None, module=None, subset=None, since=None, until=None, collection_type='scheduled'):
    ...
    :param module: the module to search for registered analytic collector
                   functions; defaults to api.main.analytics.collectors
    """
    ...
    with advisory_lock('gather_analytics_lock', wait=False) as acquired:
        ...
        from api.conf.models import Setting
        from api.main.analytics import collectors
        ...
        collector_list = [
            ...
            if inspect.isfunction(func) and hasattr(func, '__api_analytics_key__') and (not subset or name in subset)
        ]
        if not any(c.__api_analytics_key__ == 'config' for c in collector_list):
        ...
        json_collectors = [func for func in collector_list if func.__api_analytics_type__ == 'json']
        csv_collectors = [func for func in collector_list if func.__api_analytics_type__ == 'csv']

        dest = pathlib.Path(dest or tempfile.mkdtemp(prefix='api_analytics'))
        ...
        for func in json_collectors:
            key = func.__api_analytics_key__
            filename = f'{key}.json'
            try:
                last_entry = max(last_entries.get(key) or last_gather, until - timedelta(weeks=4))
                results = (func(since or last_entry, collection_type=collection_type, until=until), func.__api_analytics_version__)
        ...
        for func in csv_collectors:
            ...
            try:
                ...
                if func.__api_full_sync_interval__:
                    last_full_sync = last_entries.get(f"{key}_full")
                    full_sync_enabled = not last_full_sync or last_full_sync < now() - timedelta(days=func.__api_full_sync_interval__)

                kwargs['full_sync_enabled'] = full_sync_enabled
                if func.__api_expensive__:
                    slices = func.__api_expensive__(key, since, until, last_gather, **kwargs)
                else:
                ...
                for start, end in slices:
                    ...
                    for fpath in files:
                        payload = {filename: (fpath, func.__api_analytics_version__)}
...
def ship(path):
    ...
    with open(path, 'rb') as f:
        ...
        s = requests.Session()
        s.headers = get_api_http_client_headers()
