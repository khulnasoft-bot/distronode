# Copyright (c) 2015 Distronode, Inc.
# All Rights Reserved.

# Python
import json
import logging
import redis

# Django
from django.conf import settings

__all__ = ['CallbackQueueDispatcher']


# use a custom JSON serializer so we can properly handle !unsafe and !vault
# objects that may exist in events emitted by the callback plugin
# see: https://github.com/distronode/distronode/pull/38759
class DistronodeJSONEncoder(json.JSONEncoder):
    def default(self, o):
        if getattr(o, 'yaml_tag', None) == '!vault':
            return o.data
        return super(DistronodeJSONEncoder, self).default(o)


class CallbackQueueDispatcher(object):
    def __init__(self):
        self.queue = getattr(settings, 'CALLBACK_QUEUE', '')
        self.logger = logging.getLogger('api.main.queue.CallbackQueueDispatcher')
        self.connection = redis.Redis.from_url(settings.BROKER_URL)

    def dispatch(self, obj):
        self.connection.rpush(self.queue, json.dumps(obj, cls=DistronodeJSONEncoder))
