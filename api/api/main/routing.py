...
logger = logging.getLogger('api.main.routing')
_application = None


class APIProtocolTypeRouter(ProtocolTypeRouter):
...
def application_func(cls=APIProtocolTypeRouter) -> ProtocolTypeRouter:
    return cls(
        {
...
def __getattr__(name: str) -> ProtocolTypeRouter:
    ...
    Normally, someone would get application from this module via:
        from api.main.routing import application
    ...
    ...
        api.main.routing.__getattribute__(...)                              # <-- we don't define this so NOOP as far as we are concerned
        if '__getattr__' in api.main.routing.__dict__:                      # <-- this triggers the function we are in
            return api.main.routing.__dict__.__getattr__("application")
