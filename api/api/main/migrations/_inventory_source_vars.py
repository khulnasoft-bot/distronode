...
logger = logging.getLogger('api.main.migrations')


class PluginFileInjector(object):
    plugin_name = None  # Distronode core name used to reference plugin
...
class azure_rm(PluginFileInjector):
    ...
    def inventory_as_dict(self, inventory_source, private_data_dir):
        ...
        group_by_hostvar = {
            ...
            # Introduced with https://github.com/distronode/distronode/pull/53046
        ...
        if source_vars.get('use_private_ip', False):
            ret['hostvar_expressions']['distronode_host'] = 'private_ipv4_addresses[0]'
...
class ec2(PluginFileInjector):
    ...
    def _compat_compose_vars(self):
        return {
            ...
            'distronode_host': 'public_ip_address',
            # new with https://github.com/distronode/distronode/pull/53645
    ...
    def inventory_as_dict(self, inventory_source, private_data_dir):
        ...
        group_by_hostvar = {
            ...
            # https://github.com/distronode/distronode/pull/53812
        ...
        if source_vars.get('destination_variable') or source_vars.get('vpc_destination_variable'):
            for fd in ('destination_variable', 'vpc_destination_variable'):
                if source_vars.get(fd):
                    compose_dict['distronode_host'] = source_vars.get(fd)
                    break
...
class gce(PluginFileInjector):
    ...
    def _compat_compose_vars(self):
        ...
        # https://github.com/distronode/distronode/issues/51884
        return {
            ...
            'distronode_ssh_host': 'networkInterfaces[0].accessConfigs[0].natIP | default(networkInterfaces[0].networkIP)',
        }
    ...
    def inventory_as_dict(self, inventory_source, private_data_dir):
        ...
        keyed_groups = [
            ...
            # https://github.com/distronode/distronode/issues/51883
...
class vmware(PluginFileInjector):
    ...
    def inventory_as_dict(self, inventory_source, private_data_dir):
        ...
        # https://github.com/distronode/distronode/blob/devel/docs/docsite/rst/scenario_guides/vmware_scenarios/vmware_inventory_vm_attributes.rst
        UPPERCASE_PROPS = [
            "availableField",
            "configIssue",
            "configStatus",
        ...
        ret['compose'] = {'distronode_host': 'guest.ipAddress'}  # default value
        ret['compose']['distronode_ssh_host'] = ret['compose']['distronode_host']
        # the distronode_uuid was unique every host, every import, from the script
        ret['compose']['distronode_uuid'] = '99999999 | random | to_uuid'
        ...
        if host_pattern:
            ...
            ret['compose']['distronode_host'] = stripped_hp
            ret['compose']['distronode_ssh_host'] = stripped_hp
...
class openstack(PluginFileInjector):
    ...
    def inventory_as_dict(self, inventory_source, private_data_dir):
        ...
        #
        # These are, yet again, different from distronode_variables in script logic
...
class rhv(PluginFileInjector):
    ...
    def inventory_as_dict(self, inventory_source, private_data_dir):
        ...
        ret['compose'] = {'distronode_host': '(devices.values() | list)[0][0] if devices else None'}
        ret['keyed_groups'] = []
...
class satellite6(PluginFileInjector):
    ...
    def inventory_as_dict(self, inventory_source, private_data_dir):
        ...
        want_distronode_ssh_host = False
        want_facts = True
        ...
        for k, v in foreman_opts.items():
            ...
            elif k == 'satellite6_want_distronode_ssh_host' and isinstance(v, bool):
                want_distronode_ssh_host = v
        ...
        if want_distronode_ssh_host:
            ret['compose'] = {'distronode_ssh_host': "foreman['ip6'] | default(foreman['ip'], true)"}
