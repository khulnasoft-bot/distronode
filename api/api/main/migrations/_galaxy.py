...
from api.main.utils.encryption import encrypt_field, decrypt_field

from django.conf import settings
from django.utils.timezone import now

from api.main.models import CredentialType as ModernCredentialType
...
def migrate_galaxy_settings(apps, schema_editor):
    ...
    # by default, prior versions of API automatically pulled content
    # from galaxy.distronode.com
    ...
    try:
        ...
        public_galaxy_credential = Credential(
            created=now(), modified=now(), name='Distronode Galaxy', managed=True, credential_type=galaxy_type, inputs={'url': 'https://galaxy.distronode.com/'}
        )
    ...
    for org in Organization.objects.all():
        if private_galaxy_url and private_galaxy_url.value:
            ...
            if (username and username.value) or (password and password.value):
                logger.error(
                    f'Specifying HTTP basic auth for the Distronode Galaxy API '
            ...
            if 'cloud.redhat.com' in inputs['url']:
                name = f'Distronode Automation Hub ({private_galaxy_url.value})'
        ...
        for fallback in fallback_servers:
            ...
            cred = Credential(created=now(), modified=now(), name=f'Distronode Galaxy ({url})', organization=org, credential_type=galaxy_type, inputs=inputs)
            cred.save()
            if token:
                # encrypt based on the primary key from the prior save
                cred.inputs['token'] = encrypt_field(cred, 'token')
