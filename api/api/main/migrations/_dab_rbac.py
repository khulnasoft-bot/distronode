...
def migrate_to_new_rbac(apps, schema_editor):
    ...
    for role_definition in RoleDefinition.objects.filter(managed=True).exclude(name__in=(settings.DISTRONODE_BASE_JWT_MANAGED_ROLES)):
    ...
    from api.main.models.rbac import role_descriptions
...
def setup_managed_role_definitions(apps, schema_editor):
    ...
    Permission = apps.get_model('dab_rbac', 'DABPermission')
    RoleDefinition = apps.get_model('dab_rbac', 'RoleDefinition')
    Organization = apps.get_model(settings.DISTRONODE_BASE_ORGANIZATION_MODEL)
