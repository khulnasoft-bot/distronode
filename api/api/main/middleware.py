...
from api.main import migrations
from api.main.utils.profiling import APIProfiler
from api.main.utils.common import memoize
from api.urls import get_urlpatterns
from api.main.utils.named_url_graph import reset_counters


logger = logging.getLogger('api.main.middleware')
perf_logger = logging.getLogger('api.analytics.performance')
...
class TimingMiddleware(threading.local, MiddlewareMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.prof = APIProfiler("TimingMiddleware")

    def process_request(self, request):
        self.start_time = time.time()
        if settings.API_REQUEST_PROFILE:
            self.prof.start()
...
class URLModificationMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        if request.path_info != new_path:
            request.environ['api.named_url_rewritten'] = request.path
