...
from api.main.models.inventory import Inventory, InventorySource, InventoryUpdate, Host
from api.main.utils.mem_inventory import MemInventory, dict_to_mem_data
from api.main.utils.safe_yaml import sanitize_jinja

# other API imports
from api.main.models.rbac import batch_role_ancestor_rebuilding
from api.main.utils import ignore_inventory_computed_fields, get_licenser
from api.main.utils.execution_environments import get_default_execution_environment
from api.main.signals import disable_activity_stream
from api.main.constants import STANDARD_INVENTORY_UPDATE_ENV
from api.main.utils.pglock import advisory_lock

logger = logging.getLogger('api.main.commands.inventory_import')
...
class DistronodeInventoryLoader(object):
    """
    Given executable `source` (directory, executable, or file) this will
    use the distronode-inventory CLI utility to convert it into in-memory
    representational objects. Example:
        /usr/bin/distronode/distronode-inventory -i hosts --list
    """
    ...
    def get_base_args(self):
        ...
        bargs.extend(['distronode-inventory', '-i', self.source])
    ...
    def command_to_json(self, cmd):
        ...
        if proc.returncode != 0:
            raise RuntimeError('%s failed (rc=%d) with stdout:\n%s\nstderr:\n%s' % ('distronode-inventory', proc.returncode, stdout, stderr))
    ...
    def load(self):
        ...
        logger.info('Reading Distronode inventory source: %s', self.source)
...
class Command(BaseCommand):
    ...
    def _update_inventory(self):
        ...
        # issue for this: https://github.com/distronode/distronode/tree/devel/api/issues/11623
    ...
    def _create_update_groups(self):
        ...
        for group_name in all_group_names:
            ...
            group_desc = mem_group.variables.pop('_api_description', 'imported')
    ...
    def _create_update_hosts(self, pk_mem_host_map):
        ...
        for mem_host_name in sorted(mem_host_names_to_create):
            ...
            host_desc = import_vars.pop('_api_description', 'imported')
    ...
    def remote_tower_license_compare(self, local_license_type):
        # this requires https://github.com/distronode/distronode/pull/52747
    ...
    def handle(self, *args, **options):
        ...
        with advisory_lock('inventory_{}_import'.format(inventory_id)):
            ...
            try:
                data = DistronodeInventoryLoader(source=source, verbosity=verbosity).load()
                logger.debug('Finished loading from source: %s', source)

            except SystemExit:
                logger.debug("Error occurred while running distronode-inventory")
    ...
    def perform_update(self, options, data, inventory_update):
        """Shared method for both api-manage CLI updates and inventory updates
        ...
        # Since perform_update can be invoked either through the api-manage CLI
        ...
        with advisory_lock('inventory_{}_perform_update'.format(self.inventory.id)):
            ...
            if settings.DEBUG:
                ...
                # https://github.com/distronode/distronode-tower/issues/7414#issuecomment-321615104
