# Copyright (c) 2018 Distronode by Red Hat
# All Rights Reserved.
...
from api.main.dispatch.pool import WorkerPool
from api.main.dispatch.periodic import Scheduler
from api.main.dispatch import pg_bus_conn
from api.main.utils.db import set_connection_name
...
if 'run_callback_receiver' in sys.argv:
    ...
    logger = logging.getLogger('api.main.dispatch')
...
class APIConsumerBase(object):
    ...
    def record_statistics(self):
        if time.time() - self.last_stats > 1:  # buffer stat recording to once per second
            try:
                self.redis.set(f'api_{self.name}_statistics', self.pool.debug())
            except Exception:
...
class APIConsumerRedis(APIConsumerBase):
    def run(self, *args, **kwargs):
        super(APIConsumerRedis, self).run(*args, **kwargs)
...
class APIConsumerPG(APIConsumerBase):
    ...
    def run(self, *args, **kwargs):
        super(APIConsumerPG, self).run(*args, **kwargs)
        ...
        while True:
            try:
                with pg_bus_conn(new_connection=True) as conn:
                    ...
                    # this is the main operational loop for api-manage run_dispatcher
