...
from api.main.models import JobEvent, AdHocCommandEvent, ProjectUpdateEvent, InventoryUpdateEvent, SystemJobEvent, UnifiedJob
from api.main.constants import ACTIVE_STATES
from api.main.models.events import emit_event_detail
from api.main.utils.profiling import APIProfiler
import api.main.analytics.subsystem_metrics as s_metrics
from .base import BaseWorker

logger = logging.getLogger('api.main.commands.run_callback_receiver')
...
class CallbackBrokerWorker(BaseWorker):
    ...
    distronode-runner display callback plugin.
    """
    ...
    def __init__(self):
        ...
        self.prof = APIProfiler("CallbackBrokerWorker")
        for key in self.redis.keys('api_callback_receiver_statistics_*'):
    ...
    def record_statistics(self):
        ...
        if time.time() - self.last_stats > settings.JOB_EVENT_STATISTICS_INTERVAL:
            try:
                self.redis.set(f'api_callback_receiver_statistics_{self.pid}', self.debug())
    ...
    def work_loop(self, *args, **kw):
        if settings.API_CALLBACK_PROFILE:
    ...
    def perform_work(self, body):
        try:
            ...
            if not flush:
                ...
                if notification_trigger_event:  # if this is an Distronode stats event, ensure notifications on flush
