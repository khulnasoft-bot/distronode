...
from api.main.tasks.system import dispatch_startup, inform_cluster_of_shutdown
...
class TaskWorker(BaseWorker):
    ...
    def resolve_callable(task):
        ...
        api.main.tasks.system.delete_inventory
        api.main.tasks.jobs.RunProjectUpdate
        """
        if not task.startswith('api.'):
            raise ValueError('{} is not a valid api task'.format(task))
    ...
    def perform_work(self, body):
        ...
        body = {
            ...
            'callbacks': [{
                'args': [],
                'kwargs': {}
                'task': u'api.main.tasks.system.handle_work_success'
            }],
            'errbacks': [{
                'args': [],
                'kwargs': {},
                'task': 'api.main.tasks.system.handle_work_error'
            }],
            'kwargs': {},
            'task': u'api.main.tasks.jobs.RunProjectUpdate'
        }
        """
        ...
        try:
            ...
            try:
                if getattr(exc, 'is_api_task_error', False):
