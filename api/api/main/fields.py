...
from api.main.utils.encryption import encrypt_value, decrypt_value, get_encryption_key
from api.main.validators import validate_ssh_private_key
from api.main.constants import ENV_BLOCKLIST
from api.main import utils
...
logger = logging.getLogger('api.main.fields')
...
def is_implicit_parent(parent_role, child_role):
    ...
    from api.main.models.rbac import ROLE_SINGLETON_SYSTEM_ADMINISTRATOR, ROLE_SINGLETON_SYSTEM_AUDITOR
...
class ImplicitRoleField(models.ForeignKey):
    ...
    def bind_m2m_changed(self, _self, _role_class, cls):
        ...
        for field_name in field_names:
            ...
            # consistency is assured by unit test api.main.tests.functional
    ...
    def _post_save(self, instance, created, *args, **kwargs):
        ...
        # Avoid circular import
        from api.main.models.rbac import batch_role_ancestor_rebuilding, Role
    ...
    def _post_delete(self, instance, *args, **kwargs):
        ...
        # Avoid circular import
        from api.main.models.rbac import Role
...
class DynamicCredentialInputField(JSONSchemaField):
    ...
    `api.main.models.credential:CredentialInputSource().metadata`.
...
class CredentialInputField(JSONSchemaField):
    """
    Used to validate JSON for
    `api.main.models.credential:Credential().inputs`.
...
class CredentialTypeInputField(JSONSchemaField):
    """
    Used to validate JSON for
    `api.main.models.credential:CredentialType().inputs`.
    """
...
class CredentialTypeInjectorField(JSONSchemaField):
    """
    Used to validate JSON for
    `api.main.models.credential:CredentialType().injectors`.
    """
        ...
        return {
            ...
            'properties': {
                ...
                'extra_vars': {
                    'type': 'object',
                    'patternProperties': {
                        # http://docs.distronode.com/distronode/playbooks_variables.html#what-makes-a-valid-variable-name
    ...
    def validate_env_var_allowed(self, env_var):
        if env_var.startswith('DISTRONODE_'):
            raise django_exceptions.ValidationError(
                _('Environment variable {} may affect Distronode configuration so its use is not allowed in credentials.').format(env_var),
                code='invalid',
