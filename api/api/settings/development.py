...
# Development settings for API project.
...
# api-manage shell_plus --notebook
...
# to disable this, set `COLOR_LOGS = False` in api/settings/local_settings.py
COLOR_LOGS = True
LOGGING['handlers']['console']['()'] = 'api.main.utils.handlers.ColorHandler'  # noqa
...
# Distronode base virtualenv paths and enablement
# only used for deprecated fields and management commands for them
BASE_VENV_PATH = os.path.realpath("/var/lib/api/venv")

CLUSTER_HOST_ID = socket.gethostname()

API_CALLBACK_PROFILE = True
...
# Only works in combination with settings.SETTINGS_MODULE == 'api.settings.development'
API_DISABLE_TASK_MANAGERS = False
...
# If any local_*.py files are present in api/settings/, use them to override
# default settings for development.  If not present, we can still run using
# only the defaults.
# this needs to stay at the bottom of this file
try:
    if os.getenv('API_KUBE_DEVEL', False):
