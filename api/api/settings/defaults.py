...
DATABASES = {
    'default': {
        ...
        'TEST': {
            ...
            'NAME': os.path.join(BASE_DIR, 'api_test.sqlite3')
        },
    }
}
...
# the K8S cluster where api itself is running)
IS_K8S = False

API_CONTAINER_GROUP_K8S_API_TIMEOUT = 10
API_CONTAINER_GROUP_DEFAULT_NAMESPACE = os.getenv('MY_POD_NAMESPACE', 'default')
API_CONTAINER_GROUP_DEFAULT_JOB_LABEL = os.getenv('API_CONTAINER_GROUP_DEFAULT_JOB_LABEL', 'distronode_job')
...
API_CONTAINER_GROUP_POD_PENDING_TIMEOUT = "2h"

# How much capacity controlling a task costs a hybrid or control node
API_CONTROL_NODE_TASK_IMPACT = 1
...
STATIC_ROOT = '/var/lib/api/public/static'
...
PROJECTS_ROOT = '/var/lib/api/projects/'
...
JOBOUTPUT_ROOT = '/var/lib/api/job_status/'
...
# This list is used for creating default EEs when running api-manage create_preload_data.
# Should be ordered from highest to lowest precedence.
# The api-manage register_default_execution_environments command reads this setting and registers the EE(s)
# If a registry credential is needed to pull the image, that can be provided to the api-manage command
GLOBAL_JOB_EXECUTION_ENVIRONMENTS = [{'name': 'API EE (latest)', 'image': 'quay.io/distronode/distronode/tree/devel/api-ee:latest'}]
# This setting controls which EE will be used for project updates.
# The api-manage register_default_execution_environments command reads this setting and registers the EE
...
# If a registry credential is needed to pull the image, that can be provided to the api-manage command
CONTROL_PLANE_EXECUTION_ENVIRONMENT = 'quay.io/distronode/distronode/tree/devel/api-ee:latest'
...
# The maximum size of the distronode callback event's res data structure
...
SUBSYSTEM_METRICS_REDIS_KEY_PREFIX = "api_metrics"
...
# before the api_k8s_reaper task will tear down the pods
...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            'context_processors': [  # NOQA
                ...
                'django.template.context_processors.request',
                'api.ui.context_processors.csp',
                'api.ui.context_processors.version',
            ],
            'builtins': ['api.main.templatetags.swagger'],
            'libraries': {
                "distronode_base.lib.templatetags.requests": "distronode_base.lib.templatetags.requests",
                "distronode_base.lib.templatetags.util": "distronode_base.lib.templatetags.util",
            },
        },
        'DIRS': [
            os.path.join(BASE_DIR, 'templates'),
            os.path.join(BASE_DIR, 'ui', 'public'),
            os.path.join(BASE_DIR, 'ui', 'build', 'api'),
        ],
    },
]
...
MAX_PAGE_SIZE = 200
REST_FRAMEWORK = {
    'DEFAULT_PAGINATION_CLASS': 'api.api.pagination.Pagination',
    'PAGE_SIZE': 25,
    'DEFAULT_AUTHENTICATION_CLASSES': (
        'distronode_base.jwt_consumer.api.auth.ApiJWTAuthentication',
        'api.api.authentication.SessionAuthentication',
        'api.api.authentication.LoggedBasicAuthentication',
    ),
    'DEFAULT_PERMISSION_CLASSES': ('api.api.permissions.ModelAccessPermission',),
    'DEFAULT_PARSER_CLASSES': ('api.api.parsers.JSONParser',),
    'DEFAULT_RENDERER_CLASSES': ('api.api.renderers.DefaultJSONRenderer', 'api.api.renderers.BrowsableAPIRenderer'),
    'DEFAULT_METADATA_CLASS': 'api.api.metadata.Metadata',
    'EXCEPTION_HANDLER': 'api.api.views.api_exception_handler',
    'VIEW_DESCRIPTION_FUNCTION': 'api.api.generics.get_view_description',
...
SWAGGER_SETTINGS = {
    'DEFAULT_AUTO_SCHEMA_CLASS': 'api.api.swagger.CustomSwaggerAutoSchema',
}

AUTHENTICATION_BACKENDS = ('api.main.backends.APIModelBackend',)
...
RECEPTOR_SERVICE_ADVERTISEMENT_PERIOD = 60  # https://github.com/distronode/receptor/blob/aa1d589e154d8a0cb99a220aff8f98faf2273be6/pkg/netceptor/netceptor.go#L34
...
CELERYBEAT_SCHEDULE = {
    'tower_scheduler': {'task': 'api.main.tasks.system.api_periodic_scheduler', 'schedule': timedelta(seconds=30), 'options': {'expires': 20}},
    'cluster_heartbeat': {
        'task': 'api.main.tasks.system.cluster_node_heartbeat',
        'schedule': timedelta(seconds=CLUSTER_NODE_HEARTBEAT_PERIOD),
        'options': {'expires': 50},
    },
    'gather_analytics': {'task': 'api.main.tasks.system.gather_analytics', 'schedule': timedelta(minutes=5)},
    'task_manager': {'task': 'api.main.scheduler.tasks.task_manager', 'schedule': timedelta(seconds=20), 'options': {'expires': 20}},
    'dependency_manager': {'task': 'api.main.scheduler.tasks.dependency_manager', 'schedule': timedelta(seconds=20), 'options': {'expires': 20}},
    'k8s_reaper': {'task': 'api.main.tasks.system.api_k8s_reaper', 'schedule': timedelta(seconds=60), 'options': {'expires': 50}},
    'receptor_reaper': {'task': 'api.main.tasks.system.api_receptor_workunit_reaper', 'schedule': timedelta(seconds=60)},
    'send_subsystem_metrics': {'task': 'api.main.analytics.analytics_tasks.send_subsystem_metrics', 'schedule': timedelta(seconds=20)},
    'cleanup_images': {'task': 'api.main.tasks.system.cleanup_images_and_files', 'schedule': timedelta(hours=3)},
    'cleanup_host_metrics': {'task': 'api.main.tasks.host_metrics.cleanup_host_metrics', 'schedule': timedelta(hours=3, minutes=30)},
    'host_metric_summary_monthly': {'task': 'api.main.tasks.host_metrics.host_metric_summary_monthly', 'schedule': timedelta(hours=4)},
    'periodic_resource_sync': {'task': 'api.main.tasks.system.periodic_resource_sync', 'schedule': timedelta(minutes=15)},
}

# Django Caching Configuration
DJANGO_REDIS_IGNORE_EXCEPTIONS = True
CACHES = {'default': {'BACKEND': 'api.main.cache.APIRedisCache', 'LOCATION': 'unix:///var/run/redis/redis.sock?db=1'}}
...
# Any DISTRONODE_* settings will be passed to the task runner subprocess
# environment

# Do not want API to ask interactive questions and want it to be friendly with
# reprovisioning
DISTRONODE_HOST_KEY_CHECKING = False

# RHEL has too old of an SSH so distronode will select paramiko and this is VERY
# slow.
DISTRONODE_PARAMIKO_RECORD_HOST_KEYS = False

# Force distronode in color even if we don't have a TTY so we can properly colorize
# output
DISTRONODE_FORCE_COLOR = True

# If tmp generated inventory parsing fails (error state), fail playbook fast
DISTRONODE_INVENTORY_UNPARSED_FAILED = True

# Additional environment variables to be passed to the distronode subprocesses
API_TASK_ENV = {}

# Additional environment variables to apply when running distronode-galaxy commands
# to fetch Distronode content - roles and collections
GALAXY_TASK_ENV = {'DISTRONODE_FORCE_COLOR': 'false', 'GIT_SSH_COMMAND': "ssh -o StrictHostKeyChecking=no"}

# Rebuild Host Smart Inventory memberships.
API_REBUILD_SMART_MEMBERSHIP = False
...
# inventory scripts).
# Note: This setting may be overridden by database settings.
API_ISOLATION_BASE_PATH = tempfile.gettempdir()

# User definable distronode callback plugins
# Note: This setting may be overridden by database settings.
API_DISTRONODE_CALLBACK_PLUGINS = ""

# Automatically remove nodes that have missed their heartbeats after some time
API_AUTO_DEPROVISION_INSTANCES = False
...
# See https://github.com/distronode/distronode/tree/devel/api-facts-playbooks
...
LOG_AGGREGATOR_MAX_DISK_USAGE_PATH = '/var/lib/api'
...
ASGI_APPLICATION = "api.main.routing.application"
...
LOGGING = {
    ...
    'filters': {
        ...
        'require_debug_true_or_test': {'()': 'api.main.utils.RequireDebugTrueOrTest'},
        'external_log_enabled': {'()': 'api.main.utils.filters.ExternalLoggerEnabled'},
        'dynamic_level_filter': {'()': 'api.main.utils.filters.DynamicLevelFilter'},
        'guid': {'()': 'api.main.utils.filters.DefaultCorrelationId'},
    },
    'formatters': {
        'simple': {'format': '%(asctime)s %(levelname)-8s [%(guid)s] %(name)s %(message)s'},
        'json': {'()': 'api.main.utils.formatters.LogstashFormatter'},
        'timed_import': {'()': 'api.main.utils.formatters.TimeFormatter', 'format': '%(relativeSeconds)9.3f %(levelname)-8s %(message)s'},
    ...
    'handlers': {
        ...
        'external_logger': {
            'class': 'api.main.utils.handlers.RSysLogHandler',
            'formatter': 'json',
            'address': '/var/run/api-rsyslog/rsyslog.sock',
    ...
    'loggers': {
        ...
        'distronode_base': {'handlers': ['console', 'file', 'tower_warnings']},
        ...
        'api': {'handlers': ['console', 'file', 'tower_warnings', 'external_logger'], 'level': 'DEBUG'},
        'api.conf': {'handlers': ['null'], 'level': 'WARNING'},
        'api.conf.settings': {'handlers': ['null'], 'level': 'WARNING'},
        'api.main': {'handlers': ['null']},
        'api.main.commands.run_callback_receiver': {'handlers': ['callback_receiver'], 'level': 'INFO'},  # very noisey debug-level logs
        'api.main.dispatch': {'handlers': ['dispatcher']},
        'api.main.consumers': {'handlers': ['console', 'file', 'tower_warnings'], 'level': 'INFO'},
        'api.main.rsyslog_configurer': {'handlers': ['rsyslog_configurer']},
        'api.main.cache_clear': {'handlers': ['cache_clear']},
        'api.main.ws_heartbeat': {'handlers': ['ws_heartbeat']},
        'api.main.wsrelay': {'handlers': ['wsrelay']},
        'api.main.commands.inventory_import': {'handlers': ['inventory_import'], 'propagate': False},
        'api.main.tasks': {'handlers': ['task_system', 'external_logger', 'console'], 'propagate': False},
        'api.main.analytics': {'handlers': ['task_system', 'external_logger', 'console'], 'level': 'INFO', 'propagate': False},
        'api.main.scheduler': {'handlers': ['task_system', 'external_logger', 'console'], 'propagate': False},
        'api.main.access': {'level': 'INFO'},  # very verbose debug-level logs
        'api.main.signals': {'level': 'INFO'},  # very verbose debug-level logs
        'api.api.permissions': {'level': 'INFO'},  # very verbose debug-level logs
        'api.analytics': {'handlers': ['external_logger'], 'level': 'INFO', 'propagate': False},
        'api.analytics.broadcast_websocket': {'handlers': ['console', 'file', 'wsrelay', 'external_logger'], 'level': 'INFO', 'propagate': False},
        'api.analytics.performance': {'handlers': ['console', 'file', 'tower_warnings', 'external_logger'], 'level': 'DEBUG', 'propagate': False},
        'api.analytics.job_lifecycle': {'handlers': ['console', 'job_lifecycle', 'external_logger'], 'level': 'DEBUG', 'propagate': False},
...
# Specify 'filename' (used if the environment variable API_LOGGING_MODE is unset or 'file')
...
logging_mode = os.getenv('API_LOGGING_MODE', 'file')
if logging_mode not in ('file', 'stdout'):
    raise Exception("API_LOGGING_MODE must be 'file' or 'stdout'")
...
API_REQUEST_PROFILE = False

#
# Optionally, API can generate DOT graphs
# (http://www.graphviz.org/doc/info/lang.html) for per-request profiling
# via gprof2dot (https://github.com/jrfonseca/gprof2dot)
#
# If you set this to True, you must `/var/lib/api/venv/api/bin/pip install gprof2dot`
...
#
API_REQUEST_PROFILE_WITH_DOT = False

# Allow profiling callback workers via SIGUSR1
API_CALLBACK_PROFILE = False

# Delete temporary directories created to store playbook run-time
API_CLEANUP_PATHS = True

# Allow distronode-runner to store env folder (may contain sensitive information)
API_RUNNER_OMIT_ENV_FILES = True

# Allow distronode-runner to save distronode output
# (changing to False may cause performance issues)
API_RUNNER_SUPPRESS_OUTPUT_FILE = True

# https://github.com/distronode/distronode-runner/pull/1191/files
# Interval in seconds between the last message and keep-alive messages that
# distronode-runner will send
API_RUNNER_KEEPALIVE_SECONDS = 0

# Delete completed work units in receptor
RECEPTOR_RELEASE_WORK = True
RECEPTOR_KEEP_WORK_ON_ERROR = False

# K8S only. Use receptor_log_level on API spec to set this properly
RECEPTOR_LOG_LEVEL = 'info'

MIDDLEWARE = [
    'django_guid.middleware.guid_middleware',
    'distronode_base.lib.middleware.logging.log_request.LogTracebackMiddleware',
    'api.main.middleware.SettingsCacheMiddleware',
    'api.main.middleware.TimingMiddleware',
    'django.contrib.sessions.middleware.SessionMiddleware',
    'api.main.middleware.MigrationRanCheckMiddleware',
    'corsheaders.middleware.CorsMiddleware',
    'django.middleware.locale.LocaleMiddleware',
    'django.middleware.common.CommonMiddleware',
    'django.middleware.csrf.CsrfViewMiddleware',
    'django.contrib.auth.middleware.AuthenticationMiddleware',
    'api.main.middleware.DisableLocalAuthMiddleware',
    'django.contrib.messages.middleware.MessageMiddleware',
    'api.main.middleware.OptionalURLPrefixPath',
    'crum.CurrentRequestUserMiddleware',
    'api.main.middleware.URLModificationMiddleware',
    'api.main.middleware.SessionTimeoutMiddleware',
]
...
# Connect to other API nodes using http or https
...
API_MOUNT_ISOLATED_PATHS_ON_K8S = False
...
# django-distronode-base
DISTRONODE_BASE_TEAM_MODEL = 'main.Team'
DISTRONODE_BASE_ORGANIZATION_MODEL = 'main.Organization'
DISTRONODE_BASE_RESOURCE_CONFIG_MODULE = 'api.resource_api'
DISTRONODE_BASE_PERMISSION_MODEL = 'main.Permission'
...
# Use API base view, to give 401 on unauthenticated requests
DISTRONODE_BASE_CUSTOM_VIEW_PARENT = 'api.api.generics.APIView'

# If we have a resource server defined, apply local changes to that server
RESOURCE_SERVER_SYNC_ENABLED = True

# Settings for the distronode_base RBAC system

# This has been moved to data migration code
DISTRONODE_BASE_ROLE_PRECREATE = {}

# Name for auto-created roles that give users permissions to what they create
DISTRONODE_BASE_ROLE_CREATOR_NAME = '{cls.__name__} Creator'

# Use the new Gateway RBAC system for evaluations? You should. We will remove the old system soon.
DISTRONODE_BASE_ROLE_SYSTEM_ACTIVATED = True

# Permissions a user will get when creating a new item
DISTRONODE_BASE_CREATOR_DEFAULTS = ['change', 'delete', 'execute', 'use', 'adhoc', 'approve', 'update', 'view']

# Temporary, for old roles API compatibility, save child permissions at organization level
DISTRONODE_BASE_CACHE_PARENT_PERMISSIONS = True

# Currently features are enabled to keep compatibility with old system, except custom roles
DISTRONODE_BASE_ALLOW_TEAM_ORG_ADMIN = False
# DISTRONODE_BASE_ALLOW_CUSTOM_ROLES = True
DISTRONODE_BASE_ALLOW_CUSTOM_TEAM_ROLES = False
DISTRONODE_BASE_ALLOW_SINGLETON_USER_ROLES = True
DISTRONODE_BASE_ALLOW_SINGLETON_TEAM_ROLES = False  # System auditor has always been restricted to users
DISTRONODE_BASE_ALLOW_SINGLETON_ROLES_API = False  # Do not allow creating user-defined system-wide roles

# system username for django-distronode-base
