...
EXAMPLES = """
    - name: Create a string for a schedule
      debug:
        msg: "{{ lookup('api.api.schedule_rrule', 'none', start_date='1979-09-13 03:45:07') }}"
"""
...
class LookupModule(LookupBase):
    ...
    def get_rrule(self, frequency, kwargs):
        ...
        if frequency not in self.frequencies:
            raise DistronodeError('Frequency of {0} is invalid'.format(frequency))
        ...
        if 'start_date' in kwargs:
            try:
                ...
                raise_from(DistronodeError('Parameter start_date must be in the format YYYY-MM-DD [HH:MM:SS]'), e)
        ...
        if frequency == 'none':
            ...
            if 'end_on' in kwargs:
                ...
                if re.match(r'^\d+$', end_on):
                    ...
                    try:
                        ...
                        raise_from(DistronodeError('Parameter end_on must either be an integer or in the format YYYY-MM-DD [HH:MM:SS]'), e)
            ...
            if frequency == 'week' and 'on_days' in kwargs:
                ...
                for day in kwargs['on_days'].split(','):
                    ...
                    if day not in self.weekdays:
                        raise DistronodeError('Parameter on_days must only contain values {0}'.format(', '.join(self.weekdays.keys())))
            ...
            if frequency == 'month':
                if 'month_day_number' in kwargs and 'on_the' in kwargs:
                    raise DistronodeError('Month based frequencies can have month_day_number or on_the but not both')
                ...
                if 'month_day_number' in kwargs:
                    try:
                        ...
                        raise_from(DistronodeError('month_day_number must be between 1 and 31'), e)
                ...
                if 'on_the' in kwargs:
                    try:
                        ...
                        raise_from(DistronodeError('on_the parameter must be two words separated by a space'), e)

                    if weekday not in self.weekdays:
                        raise DistronodeError('Weekday portion of on_the parameter is not valid')
                    if occurance not in self.set_positions:
                        raise DistronodeError('The first string of the on_the parameter is not valid')
