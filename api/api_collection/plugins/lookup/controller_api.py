...
description:
  - Returns GET requests from the Automation Platform Controller API. See
    U(https://docs.distronode.com/automation-controller/latest/html/towerapi/) for API usage.
  - For use that is cross-compatible between the api.api and distronode.controller collection
...
options:
  ...
  max_objects:
    ...
    type: integer
    default: 1000
extends_documentation_fragment: api.api.auth_plugin
notes:
...
- name: Load the UI settings
  set_fact:
    controller_settings: "{{ lookup('api.api.controller_api', 'settings/ui') }}"

- name: Load the UI settings specifying the connection info
  set_fact:
    controller_settings: "{{ lookup('api.api.controller_api', 'settings/ui', host='controller.example.com',
                             username='admin', password=my_pass_var, verify_ssl=False) }}"

- name: Report the usernames of all users with admin privs
  debug:
    msg: "Admin users: {{ query('api.api.controller_api', 'users', query_params={ 'is_superuser': true }) | map(attribute='username') | join(', ') }}"
...
- name: debug all organizations in a loop  # use query to return a list
  ...
  loop: "{{ query('api.api.controller_api', 'organizations') }}"
  loop_control:
    label: "{{ item['name'] }}"
...
- name: Make sure user 'john' is an org admin of the default org if the user exists
  role:
    ...
    role: admin
    user: john
  when: "lookup('api.api.controller_api', 'users', query_params={ 'username': 'john' }) | length == 1"
...
from distronode.module_utils._text import to_native
...
class LookupModule(LookupBase):
    ...
    def handle_error(self, **kwargs):
        raise DistronodeError(to_native(kwargs.get('msg')))
    ...
    def run(self, terms, variables=None, **kwargs):
        if len(terms) != 1:
            raise DistronodeError('You must pass exactly one endpoint to query')
        ...
        if 'status_code' not in response:
            raise DistronodeError("Unclear response from API: {0}".format(response))

        if response['status_code'] != 200:
            raise DistronodeError("Failed to query the API: {0}".format(response['json'].get('detail', response['json'])))
        ...
        if self.get_option('expect_objects') or self.get_option('expect_one'):
            if ('id' not in return_data) and ('results' not in return_data):
                raise DistronodeError('Did not obtain a list or detail view at {0}, and ' 'expect_objects or expect_one is set to True'.format(terms[0]))

        if self.get_option('expect_one'):
            if 'results' in return_data and len(return_data['results']) != 1:
                raise DistronodeError('Expected one object from endpoint {0}, ' 'but obtained {1} from API'.format(terms[0], len(return_data['results'])))
