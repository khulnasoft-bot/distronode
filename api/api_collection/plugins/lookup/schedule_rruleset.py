...
EXAMPLES = """
    - name: Create a ruleset for everyday except Sundays
      set_fact:
        complex_rule: "{{ lookup(api.api.schedule_rruleset, '2022-04-30 10:30:45', rules=rrules, timezone='UTC' ) }}"
      vars:
        rrules:
          - frequency: 'day'
            interval: 1
...
class LookupModule(LookupBase):
    ...
    def __init__(self, *args, **kwargs):
        if LIBRARY_IMPORT_ERROR:
            raise_from(DistronodeError('{0}'.format(LIBRARY_IMPORT_ERROR)), LIBRARY_IMPORT_ERROR)
    ...
    def process_integer(self, field_name, rule, min_value, max_value, rule_number):
        ...
        for value in rule[field_name]:
            ...
            if not re.match(r"^\d+$", str(value)) or int(value) < min_value or int(value) > max_value:
                raise DistronodeError('In rule {0} {1} must be between {2} and {3}'.format(rule_number, field_name, min_value, max_value))
    ...
    def process_list(self, field_name, rule, valid_list, rule_number):
        ...
        for value in rule[field_name]:
            ...
            if value not in valid_list:
                raise DistronodeError('In rule {0} {1} must only contain values in {2}'.format(rule_number, field_name, ', '.join(valid_list.keys())))
            return_values.append(valid_list[value])
        return return_values

    def run(self, terms, variables=None, **kwargs):
        if len(terms) != 1:
            raise DistronodeError('You may only pass one schedule type in at a time')

        # Validate the start date
        try:
            start_date = LookupModule.parse_date_time(terms[0])
        except Exception as e:
            raise_from(DistronodeError('The start date must be in the format YYYY-MM-DD [HH:MM:SS]'), e)

        if not kwargs.get('rules', None):
            raise DistronodeError('You must include rules to be in the ruleset via the rules parameter')

        # All frequencies can use a timezone but rrule can't support the format that API uses.
        # So we will do a string manip here if we need to
        timezone = 'America/New_York'
        if 'timezone' in kwargs:
            if kwargs['timezone'] not in pytz.all_timezones:
                raise DistronodeError('Timezone parameter is not valid')
        ...
        for rule_index in range(0, len(kwargs['rules'])):
            ...
            if invalid_options:
                raise DistronodeError('Rule {0} has invalid options: {1}'.format(rule_number, ', '.join(invalid_options)))
            frequency = rule.get('frequency', None)
            if not frequency:
                raise DistronodeError("Rule {0} is missing a frequency".format(rule_number))
            if frequency not in self.frequencies:
                raise DistronodeError('Frequency of rule {0} is invalid {1}'.format(rule_number, frequency))
                ...
                if 'end_on' in rule:
                    ...
                    if re.match(r'^\d+$', end_on):
                        ...
                        try:
                            ...
                            raise_from(
                                DistronodeError('In rule {0} end_on must either be an integer or in the format YYYY-MM-DD [HH:MM:SS]'.format(rule_number)), e
                            )
            ...
            try:
                ...
                raise_from(DistronodeError('Failed to parse rrule for rule {0} {1}: {2}'.format(rule_number, str(rrule_kwargs), e)), e)

            # API requires an interval. rrule will not add interval if it's set to 1
        ...
        if not got_at_least_one_rule:
            raise DistronodeError("A ruleset must contain at least one RRULE")
        ...
        try:
            ...
            raise_from(DistronodeError("Failed to parse generated rule set via rruleset {0}".format(e)), e)
