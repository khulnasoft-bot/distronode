...
short_description: Distronode dynamic inventory plugin for the Automation Platform Controller.
description:
...
# If some of the arguments are missing, Distronode will attempt to read them from environment variables.
# distronode-inventory -i /path/to/controller_inventory.yml --list
...
# distronode-inventory -i @controller_inventory --list
'''

import os

from distronode.module_utils import six
from distronode.module_utils._text import to_text, to_native
...
def handle_error(**kwargs):
    raise DistronodeParserError(to_native(kwargs.get('msg')))


class InventoryModule(BaseInventoryPlugin):
    NAME = 'api.api.controller'  # REPLACE
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        if isinstance(inventory_id, int):
            ...
            try:
                ...
                raise_from(DistronodeOptionsError(
        ...
        if self.get_option('include_metadata'):
            ...
            for key in ('version', 'distronode_version'):
