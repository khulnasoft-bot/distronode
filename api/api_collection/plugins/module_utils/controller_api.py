...
from distronode.module_utils.urls import Request, SSLValidationError, ConnectionError
from distronode.module_utils.parsing.convert_bool import boolean as strtobool
from distronode.module_utils.six import PY2
from distronode.module_utils.six import raise_from
from distronode.module_utils.six.moves import StringIO
from distronode.module_utils.six.moves.urllib.error import HTTPError
from distronode.module_utils.six.moves.http_cookiejar import CookieJar
from distronode.module_utils.six.moves.urllib.parse import urlparse, urlencode, quote
from distronode.module_utils.six.moves.configparser import ConfigParser, NoOptionError
...
try:
    from distronode.module_utils.compat.version import LooseVersion as Version
except ImportError:
    try:
        from distutils.version import LooseVersion as Version
    except ImportError:
        raise AssertionError('To use this plugin or module with distronode-core 2.11, you need to use Python < 3.12 with distutils.version present')
...
class ControllerModule(DistronodeModule):
    url = None
    AUTH_ARGSPEC = dict(
    ...
    def build_url(self, endpoint, query_params=None, app_key=None):
        ...
        api_version = self.apps_api_versions.get(app_key, self.apps_api_versions.get("api", "v2"))
...
class ControllerAPIModule(ControllerModule):
    ...
    _COLLECTION_TYPE = "api"
    # This maps the collections type (api/tower) to the values returned by the API
    # Those values can be found in api/api/generics.py line 204
    collection_to_version = {
        'api': 'API',
        'controller': 'Red Hat Distronode Automation Platform',
    }
    session = None
    ...
    def make_request(self, method, endpoint, *args, **kwargs):
        ...
        # Authenticate to API (if not already done so)
    ...
    def api_path(self, app_key=None):
        ...
        if self._COLLECTION_TYPE != "api" or app_key is not None:
            if app_key is None:
                app_key = "controller"
    ...
    def delete_if_needed(self, existing_item, item_type=None, on_delete=None, auto_exit=True):
        ...
        #   2. The response from API from calling the delete on the endpont. It's up to you to process the response and exit from the module
        # Note: common error codes from the API API can cause the module to fail
        if existing_item:
    ...
    def create_if_needed(self, existing_item, new_item, endpoint, on_create=None, auto_exit=True, item_type='unknown', associations=None):
        ...
        #    2. The response from API from calling the patch on the endpont. It's up to you to process the response and exit from the module
        # Note: common error codes from the API API can cause the module to fail
        response = None
        if not endpoint:
        ...
        if existing_item:
            ...
            # 200 is response from approval node creation on tower 3.7.3 or api 15.0.0 or earlier.
    ...
    def update_if_needed(self, existing_item, new_item, item_type=None, on_update=None, auto_exit=True, associations=None):
        ...
        #    2. The response from API from patching to the endpoint. It's up to you to process the response and exit from the module.
        # Note: common error codes from the API API can cause the module to fail
        response = None
        if existing_item:
