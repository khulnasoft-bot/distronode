...
- name: Create a workflow job template
  workflow_job_template:
    ...
    description: created by Distronode Playbook
    organization: Default

- name: Create a workflow job template with workflow nodes in template
  api.api.workflow_job_template:
    name: example-workflow
...
def create_workflow_nodes(module, response, workflow_nodes, workflow_id):
    for workflow_node in workflow_nodes:
        ...
        # Two lookup methods are used based on a fix added in 21.11.0, and the api export model
