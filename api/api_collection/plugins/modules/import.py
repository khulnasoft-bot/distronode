...
DISTRONODE_METADATA = {'metadata_version': '1.1', 'status': ['preview'], 'supported_by': 'community'}
...
description:
    - Import assets into Automation Platform Controller. See
      U(https://www.distronode.com/tower) for an overview.
options:
    assets:
      description:
...
requirements:
  - "apikit >= 9.3.0"
extends_documentation_fragment: api.api.auth
'''
...
- name: Load data from a json file created by a command like api export --organization Default
  import:
    assets: "{{ lookup('file', 'org.json') | from_json() }}"
'''

from ..module_utils.apikit import ControllerAPIKitModule

# These two lines are not needed if apikit changes to do programatic notifications on issues
from distronode.module_utils.six.moves import StringIO
import logging

# In this module we don't use EXPORTABLE_RESOURCES, we just want to validate that our installed apikit has import/export
try:
    from apikit.api.pages.api import EXPORTABLE_RESOURCES  # noqa: F401; pylint: disable=unused-import
...
def main():
    ...
    module = ControllerAPIKitModule(argument_spec=argument_spec, supports_check_mode=False)

    assets = module.params.get('assets')

    if not HAS_EXPORTABLE_RESOURCES:
        module.fail_json(msg="Your version of apikit does not appear to have import/export")
    ...
    logger = logging.getLogger('apikit.api.pages.api')
