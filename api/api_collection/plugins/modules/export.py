...
try:
    from apikit.api.pages.api import EXPORTABLE_RESOURCES
...
def main():
    ...
    # We are not going to raise an error here because the __init__ method of ControllerAPIKitModule will do that for us
    if HAS_EXPORTABLE_RESOURCES:
        for resource in EXPORTABLE_RESOURCES:
            argument_spec[resource] = dict(type='list', elements='str')

    module = ControllerAPIKitModule(argument_spec=argument_spec)

    if not HAS_EXPORTABLE_RESOURCES:
        module.fail_json(msg="Your version of apikit does not have import/export")

    # The export process will never change the API system
    ...
    for logger_name in ['apikit.api.pages.api', 'apikit.api.pages.page']:
