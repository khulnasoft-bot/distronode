...
This Distronode collection allows for easy interaction with an API server via Distronode playbooks.

This source for this collection lives in the `api_collection` folder inside of the
API GitHub repository.
The previous home for this collection was inside the folder [lib/distronode/modules/web_infrastructure/distronode_tower](https://github.com/distronode/distronode/tree/stable-2.9/lib/distronode/modules/web_infrastructure/distronode_tower) in the Distronode repo,
...
Run `make build_collection` from the root folder of the API source tree.
This will create the `tar.gz` file inside the `api_collection` folder
with the current API version, for example: `api_collection/api-api-9.2.0.tar.gz`.
...
may require the official [API CLI](https://pypi.org/project/apikit/)
...
Notable releases of the `api.api` collection:
 ...
 - 11.0.0 has no non-deprecated modules that depend on the deprecated `tower-cli` [PyPI](https://pypi.org/project/distronode-tower-cli/).
...
The following notes are changes that may require changes to playbooks:
 ...
 - The module `tower_notification` was renamed `tower_notification_template`. In `distronode >= 2.10` there is a seamless redirect. Distronode 2.9 does not respect the redirect.
...
Tests to verify compatibility with the most recent API code are in `api_collection/test/api`.
...
Distronode source, set up a dedicated virtual environment:
...
make clean-api
pip install -e <path to your Distronode>
pip install -e .
pip install -e apikit
py.test api_collection/test/api/
```

## Running Integration Tests

The integration tests require a virtualenv with `distronode >= 2.9` and `apikit`.
The collection must first be installed, which can be done using `make install_collection`.
You also need a configuration file, as described in the [Running](https://github.com/distronode/distronode/tree/devel/api/blob/devel/api_collection/README.md#running) section.

How to run the tests:

```
# distronode-test must be run from the directory in which the collection is installed
cd ~/.distronode/collections/distronode_collections/api/api/
distronode-test integration
```

## Licensing

All content in this folder is licensed under the same license as Distronode,
