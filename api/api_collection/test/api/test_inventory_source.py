...
from api.main.models import Organization, Inventory, InventorySource, Project
...
def test_inventory_source_create(run_module, admin_user, base_inventory, project):
    source_path = '/var/lib/api/example_source_path/'
    result = run_module(
        'inventory_source',
...
def test_scm_source_needs_project(run_module, admin_user, base_inventory):
    result = run_module(
        'inventory_source',
        dict(
            name='SCM inventory without project', inventory=base_inventory.name, state='present', source='scm', source_path='/var/lib/api/example_source_path/'
        ),
        admin_user,
    )
