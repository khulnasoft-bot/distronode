...
from api.main.models import Credential, CredentialType, Organization
...
def cred_type():
    ...
    ct = CredentialType.objects.create(
        name='Distronode Galaxy Token',
        inputs={"fields": [{"id": "token", "type": "string", "secret": True, "label": "Distronode Galaxy Secret Token Value"}], "required": ["token"]},
        injectors={
            "extra_vars": {
                "galaxy_token": "{{token}}",
            }
...
def test_create_vault_credential(run_module, admin_user, organization):
    # https://github.com/distronode/distronode/issues/61324
    Organization.objects.create(name='test-org')
