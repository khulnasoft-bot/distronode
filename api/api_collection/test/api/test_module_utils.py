...
from api.main.models import Organization, Team, Project, Inventory
from requests.models import Response
from unittest import mock

api_name = 'API'
controller_name = 'Red Hat Distronode Automation Platform'
ping_version = '1.2.3'
...
def getAPIheader(self, header_name, default):
    mock_headers = {'X-API-Product-Name': api_name, 'X-API-Product-Version': ping_version}
    return mock_headers.get(header_name, default)
...
def mock_api_ping_response(self, method, url, **kwargs):
    r = Response()
    r.getheader = getAPIheader.__get__(r)
    r.read = read.__get__(r)
    r.status = status.__get__(r)
    return r
...
def test_version_warning(collection_import, silence_warning):
    ...
    cli_data = {'DISTRONODE_MODULE_ARGS': {}}
    testargs = ['module_file2.py', json.dumps(cli_data)]
    with mock.patch.object(sys, 'argv', testargs):
        with mock.patch('distronode.module_utils.urls.Request.open', new=mock_api_ping_response):
            my_module = ControllerAPIModule(argument_spec=dict())
            my_module._COLLECTION_VERSION = "2.0.0"
            my_module._COLLECTION_TYPE = "api"
            my_module.get_endpoint('ping')
    silence_warning.assert_called_once_with(
        'You are running collection version {0} but connecting to {1} version {2}'.format(my_module._COLLECTION_VERSION, api_name, ping_version)
    )


def test_version_warning_strictness_api(collection_import, silence_warning):
    ControllerAPIModule = collection_import('plugins.module_utils.controller_api').ControllerAPIModule
    cli_data = {'DISTRONODE_MODULE_ARGS': {}}
    testargs = ['module_file2.py', json.dumps(cli_data)]
...
def test_version_warning_strictness_controller(collection_import, silence_warning):
    ...
    with mock.patch.object(sys, 'argv', testargs):
        with mock.patch('distronode.module_utils.urls.Request.open', new=mock_controller_ping_response):
            my_module = ControllerAPIModule(argument_spec=dict())
            my_module._COLLECTION_VERSION = "1.2.0"
...
def test_type_warning(collection_import, silence_warning):
    ...
    with mock.patch.object(sys, 'argv', testargs):
        with mock.patch('distronode.module_utils.urls.Request.open', new=mock_api_ping_response):
            ...
            my_module.get_endpoint('ping')
    silence_warning.assert_called_once_with(
        'You are using the {0} version of this collection but connecting to {1}'.format(my_module._COLLECTION_TYPE, api_name)
    )
...
def test_no_templated_values(collection_import):
    ...
    api_collection/tools/roles/template_galaxy/tasks/main.yml
    ...
    assert InventoryModule.NAME == 'api.api.controller', (
        'The inventory plugin FQCN is templated when the collection is built ' 'and the code should retain the default of api.api.'
    )
...
def test_multiple_lookup(run_module, admin_user):
    ...
    proj1 = Project.objects.create(
        ...
        organization=org1,
        scm_type='git',
        scm_url="https://github.com/distronode/distronode-tower-samples",
    )
