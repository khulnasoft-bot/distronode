...
from distronode_base.rbac.models import RoleDefinition, DABPermission
from api.main.tests.functional.conftest import _request
from api.main.tests.functional.conftest import credentialtype_scm, credentialtype_ssh  # noqa: F401; pylint: disable=unused-import
...
HAS_TOWER_CLI = False
HAS_API_KIT = False
logger = logging.getLogger('api.main.tests')


@pytest.fixture(autouse=True)
def apikit_path_set(monkeypatch):
    """Monkey patch sys.path, insert apikit source code so that
    the package does not need to be installed.
    """
    base_folder = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir, os.pardir, os.pardir, 'apikit'))
...
def collection_import():
    """These tests run assuming that the api_collection folder is inserted
...
def run_module(request, collection_import):
    def rf(module_name, module_params, request_user):
        ...
        # distronode_collections.api.api.plugins.modules.{}
        ...
        # Distronode params can be passed as an invocation argument or over stdin
        # this short circuits within the DistronodeModule interface
        def mock_load_params(self):
            self.params = module_params

        if getattr(resource_module, 'ControllerAPIKitModule', None):
            resource_class = resource_module.ControllerAPIKitModule
        elif getattr(resource_module, 'ControllerAPIModule', None):
            resource_class = resource_module.ControllerAPIModule
        else:
            raise RuntimeError("The module has neither a ControllerAPIKitModule or a ControllerAPIModule")

        with mock.patch.object(resource_class, '_load_params', new=mock_load_params):
            # Call the test utility (like a mock server) instead of issuing HTTP requests
            with mock.patch('distronode.module_utils.urls.Request.open', new=new_open):
                if HAS_TOWER_CLI:
                    tower_cli_mgr = mock.patch('tower_cli.api.Session.request', new=new_request)
                elif HAS_API_KIT:
                    tower_cli_mgr = mock.patch('apikit.api.client.requests.Session.request', new=new_request)
                else:
                    tower_cli_mgr = suppress()
                with tower_cli_mgr:
                    try:
                        # Distronode modules return data to the mothership over stdout
...
def silence_deprecation():
    ...
    """
    with mock.patch('distronode.module_utils.basic.DistronodeModule.deprecate') as this_mock:
        yield this_mock
...
def silence_warning():
    ...
    with mock.patch('distronode.module_utils.basic.DistronodeModule.warn') as this_mock:
        yield this_mock
