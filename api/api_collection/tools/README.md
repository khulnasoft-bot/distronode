...
These instructions assume you have distronode-core and the collection installed.
To install the collection in-place (to pick up any local changes to source)
the `make symlink_collection` will symlink the `api_collection/` folder to
the appropriate place under `~/.distronode/collections`.

This is a shortcut for quick validation of tests that bypasses `distronode-test`.
...
```
distronode-playbook -i localhost, api_collection/tools/integration_testing.yml
```

To run just one module (the most common use case), use the `-e test=<name>`.

```
distronode-playbook -i localhost, api_collection/tools/integration_testing.yml -e test=host
```
...
```
distronode-playbook -i localhost, api_collection/tools/integration_testing.yml -e test=$(ls -1Am api_collection/tests/integration/targets/ | tr -d '[:space:]')
```

Depending on the module, you may need special dependencies.
For instance, the rrule lookup plugins need `pytz`.
These will be satisfied if you install requirements in `api_collection/requirements.txt`.
