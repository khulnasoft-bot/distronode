...
describe('Approval Dashboard process', () => {
  ...
  it('collection should be uploaded into different repo', () => {
    ...
    cy.intercept(
      'GET',
      `${apiPrefix}v3/plugin/distronode/search/collection-versions/?namespace=*`,
    ).as('upload');
    cy.galaxykit('-i namespace create', 'distronode');
    cy.menuGo('Collections > Namespaces');

    cy.get(`a[href="${uiPrefix}namespaces/distronode/"]`).click();
    cy.contains('Upload collection').click();
    cy.fixture('collections/distronode-posix-1.4.0.tar.gz', 'binary')
      .then(Cypress.Blob.binaryStringToBlob)
      .then((fileContent) => {
        cy.get('input[type="file"]').attachFile({
          fileContent,
          fileName: 'distronode-posix-1.4.0.tar.gz',
    ...
    cy.visit(`${uiPrefix}repo/staging2/distronode/posix/`);
    cy.contains('distronode');
    cy.contains('posix');
    cy.contains('1.4.0');
  });
});
