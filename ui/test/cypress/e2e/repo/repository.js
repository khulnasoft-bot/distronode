...
function versionCheck(version) {
  cy.login();
  cy.visit(`${uiPrefix}distronode/repositories/repo1Test/`);
  cy.contains('a', 'Versions').click();
  cy.get(
    '[data-cy="PageWithTabs-DistronodeRepositoryDetail-repository-versions"]',
  );
  cy.contains(
    '[data-cy="PageWithTabs-DistronodeRepositoryDetail-repository-versions"]',
    version + ' (latest)',
...
['with remote', 'without remote'].forEach((mode) => {
  ...
  describe(`Repository ${mode}`, () => {
    it('creates, edit and sync repository', () => {
      ...
      cy.visit(`${uiPrefix}distronode/repositories/`);
      cy.contains('Repositories');
      cy.contains('button', 'Add repository').click();
      cy.contains('Add new repository');
      cy.get('[data-cy="Page-DistronodeRepositoryEdit"] input[id="name"]').type(
        'repo1Test',
      );
      cy.get(
        '[data-cy="Page-DistronodeRepositoryEdit"] input[id="description"]',
      ...
      cy.contains(
        '[data-cy="Page-DistronodeRepositoryEdit"] button',
        'Save',
      ).click();
      ...
      cy.contains(
        '[data-cy="PageWithTabs-DistronodeRepositoryDetail-details"]',
      ...
      cy.get(
        '[data-cy="Page-DistronodeRepositoryEdit"] input[id="retain_repo_versions"]',
      )
        .clear()
        .type('5');
    ...
    it('checks there are 2 versions and collection is here', () => {
      versionCheck(1);
      cy.contains(
        '[data-cy="PageWithTabs-DistronodeRepositoryDetail-repository-versions"] a',
        1,
      ).click();
      cy.contains(
        '[data-cy="PageWithTabs-DistronodeRepositoryDetail-repository-versions"]',
    ...
    it('removes collections', () => {
      cy.login();
      cy.visit(
        `${uiPrefix}distronode/repositories/repo1Test/?tab=collection-versions`,
      );
    ...
    it('checks there are 3 versions and revert repo', () => {
      versionCheck(2);
      cy.get(
        '[data-cy="PageWithTabs-DistronodeRepositoryDetail-repository-versions"] [aria-label="Actions"]',
      )
        .eq(1)
        .click();
