...
export function repositoryBasePath(name, pulp_href?): Promise<string> {
  return Promise.all([
    pulp_href
      ...
      : DistronodeRepositoryAPI.list({ name, page_size: 1 }).then(firstResult),
    DistronodeDistributionAPI.list({ name, page_size: 1 }).then(firstResult),
    ...
    distribution = await DistronodeDistributionAPI.list({
