...
import { canEditDistronodeRemoteAccess } from 'src/permissions';
import {
  assignRoles,
  jsxErrorMessage,
  parsePulpIDFromURL,
...
export const RemoteAccessTab = ({
  ...
  const query = () => {
    ...
    DistronodeRemoteAPI.myPermissions(id)
      .then(({ data: { permissions } }) => {
        setCanEditOwners(
          canEditDistronodeRemoteAccess({
            hasPermission,
        ...
        DistronodeRemoteAPI.listRoles(id, { page_size: 100 })
          .then(({ data: { roles } }) => {
  ...
  const addUser = (user, roles) => {
    const rolePromises = roles.map((role) =>
      DistronodeRemoteAPI.addRole(id, {
        role: role.name,
        users: [user.username],
      }),
    );
  ...
  const removeUser = (user) => {
    const roles = user.object_roles.map((role) =>
      DistronodeRemoteAPI.removeRole(id, {
        role,
        users: [user.username],
      }),
    );
  ...
  const removeUserRole = (role, user) => {
    const removedRole = DistronodeRemoteAPI.removeRole(id, {
      role,
      users: [user.username],
    });
    updateRoles({
  ...
  return (
    <AccessTab
      ...
      groups={groups}
      name={name}
      pulpObjectType='remotes/distronode/collection'
      removeGroup={removeGroup}
      removeRole={removeRole}
      removeUser={removeUser}
      removeUserRole={removeUserRole}
      selectRolesMessage={t`The selected roles will be added to this specific Distronode remote.`}
      ...
      user={selectedUser}
      users={users}
      urlPrefix={formatPath(Paths.distronodeRemoteDetail, {
        name,
      })}
    />
  );
