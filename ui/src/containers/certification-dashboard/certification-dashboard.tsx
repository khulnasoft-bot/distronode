...
interface IState {
  ...
  repositories: {
    approved?: DistronodeRepositoryType;
    rejected?: DistronodeRepositoryType;
  };
}
...
class CertificationDashboard extends Component<RouteProps, IState> {
  ...
  componentDidMount() {
    ...
    if (
      !user ||
      user.is_anonymous ||
      !hasPermission('distronode.modify_distronode_repo_content')
    ) {
  ...
  private remove(collection) {
    ...
    this.setUpdatingVersion(collection);
    return DistronodeRepositoryAPI.removeContent(
