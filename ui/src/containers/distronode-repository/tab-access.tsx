...
import { canEditDistronodeRepositoryAccess } from 'src/permissions';
import {
  assignRoles,
  jsxErrorMessage,
  parsePulpIDFromURL,
...
export const RepositoryAccessTab = ({
  ...
  const query = () => {
    ...
    DistronodeRepositoryAPI.myPermissions(id)
      .then(({ data: { permissions } }) => {
        setCanEditOwners(
          canEditDistronodeRepositoryAccess({
            hasPermission,
        ...
        DistronodeRepositoryAPI.listRoles(id, { page_size: 100 })
          .then(({ data: { roles } }) => {
  ...
  const addUser = (user, roles) => {
    const rolePromises = roles.map((role) =>
      DistronodeRepositoryAPI.addRole(id, {
        role: role.name,
        users: [user.username],
      }),
    );
  ...
  const removeUser = (user) => {
    const roles = user.object_roles.map((role) =>
      DistronodeRepositoryAPI.removeRole(id, {
        role,
        users: [user.username],
      }),
    );
  ...
  const removeUserRole = (role, user) => {
    const removedRole = DistronodeRepositoryAPI.removeRole(id, {
      role,
      users: [user.username],
    });
    updateRoles({
  ...
  return (
    <AccessTab
      ...
      groups={groups}
      name={name}
      pulpObjectType='repositories/distronode/distronode'
      removeGroup={removeGroup}
      removeRole={removeRole}
      removeUser={removeUser}
      removeUserRole={removeUserRole}
      selectRolesMessage={t`The selected roles will be added to this specific Distronode repository.`}
      ...
      user={selectedUser}
      users={users}
      urlPrefix={formatPath(Paths.distronodeRepositoryDetail, {
        name,
      })}
    />
  );
