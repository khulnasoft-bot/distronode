...
import { distronodeRepositoryVersionRevertAction } from 'src/actions';
import {
  DistronodeRepositoryAPI,
  type DistronodeRepositoryType,
  type DistronodeRepositoryVersionType,
  PulpAPI,
} from 'src/api';
import {
  DateComponent,
...
export const RepositoryVersionsTab = ({
  ...
  const queryList = ({ params }) =>
    DistronodeRepositoryAPI.listVersions(pulpId, params);
  const queryDetail = ({ number }) =>
    DistronodeRepositoryAPI.listVersions(pulpId, { number });
  ...
  const renderTableRow = (
    item: DistronodeRepositoryVersionType,
    index: number,
    actionContext,
    listItemActions,
  ) => {
  ...
  return state.params.repositoryVersion ? (
    version ? (
      <>
        ...
        />
        <VersionContent
          {...version.content_summary.present['distronode.collection_version']}
          repositoryName={repositoryName}
        />
      </>
    ) : (
    ...
    <DetailList<DistronodeRepositoryVersionType>
      actionContext={{
        addAlert,
        state: modalState,
      ...
      filterConfig={null}
      listItemActions={[distronodeRepositoryVersionRevertAction]}
      noDataButton={null}
