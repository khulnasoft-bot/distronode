...
export const RoleImportForm = ({ addAlert }: IProps) => {
  ...
  const formFields = [
    ...
    },
    {
      ...
      helper:
        !data.github_repo ||
        data.github_repo.startsWith('distronode-role-') ||
        'distronode-role-'.startsWith(data.github_repo) ? null : (
          <HelperText>
            <HelperTextItem variant='warning'>
              {t`Did you mean ${`distronode-role-${data.github_repo}`}?`}{' '}
              <Button
                variant='link'
                onClick={() =>
                  updateField('github_repo', `distronode-role-${data.github_repo}`)
                }
              >{t`Change`}</Button>
            </HelperTextItem>
          </HelperText>
