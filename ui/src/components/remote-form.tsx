...
interface IProps {
  ...
  remote: RemoteType;
  remoteType: 'registry' | 'distronode-remote';
  saveRemote: () => void;
  showModal?: boolean;
  showMain?: boolean;
  title?: string;
...
export class RemoteForm extends Component<IProps, IState> {
  ...
  render() {
    ...
    const isCommunityRemote =
      ...
      remote?.url === 'https://galaxy.distronode.com/api/';

    switch (remoteType) {
      case 'distronode-remote':
  ...
  private renderForm(
    ...
    const writeOnlyFields =
      remote[
        remoteType === 'distronode-remote' ? 'hidden_fields' : 'write_only_fields'
      ];

    const docsDistronodeLink = (
      <ExternalLink href='https://docs.distronode.com/distronode/latest/user_guide/collections_using.html#install-multiple-collections-with-a-requirements-file'>
        requirements.yml
      </ExternalLink>
    );
          ...
          <FormGroup
            ...
            labelIcon={
              <HelpButton
                content={
                  <Trans>
                    This uses the same {docsDistronodeLink} format as the
                    distronode-galaxy CLI with the caveat that roles aren&apos;t
  ...
  private isValid(requiredFields) {
    ...
    if (remoteType === 'distronode-remote') {
