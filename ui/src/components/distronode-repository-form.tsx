...
import { DistronodeRemoteAPI, type DistronodeRepositoryType } from 'src/api';
import {
  FormFieldHelper,
  HelpButton,
  LazyDistributions,
...
interface IProps {
  ...
  repository: DistronodeRepositoryType;
  updateRepository: (r) => void;
}

export const DistronodeRepositoryForm = ({
  allowEditName,
  errorMessages,
  onCancel,
  onSave,
  ...
  const loadRemotes = (name?) => {
    ...
    DistronodeRemoteAPI.list({ ...(name ? { name__icontains: name } : {}) })
