...
interface IApplicationInfo {
  ...
  pulp_distronode_version?: string;
...
export const HubAboutModal = ({ isOpen, onClose, user, userName }: IProps) => {
  ...
  const {
    ...
    pulp_distronode_version, // 0.19.0
        ...
        <TextList component={TextListVariants.dl}>
          ...
          <Value>
            ...
            {galaxy_ng_version?.includes('dev') ? (
              ...
              <ExternalLink
                href={`https://github.com/distronode/galaxy_ng/releases/tag/${galaxy_ng_version}`}
              >
                {galaxy_ng_version}
              </ExternalLink>
            )}
            {galaxy_ng_commit ? (
              <>
                ...
                {galaxy_ng_sha ? (
                  <ExternalLink
                    href={`https://github.com/distronode/galaxy_ng/commit/${galaxy_ng_sha}`}
                  >
                    {galaxy_ng_commit}
                  </ExternalLink>
                ) : (
          ...
          <Label>{t`Pulp Distronode Version`}</Label>
          <Value>
            <ExternalLink
              href={`https://github.com/pulp/pulp_distronode/releases/tag/${pulp_distronode_version}`}
            >
              {pulp_distronode_version}
            </ExternalLink>
          </Value>
          ...
          <Value>
            <ExternalLink
              href={`https://github.com/distronode/galaxy-importer/releases/tag/v${galaxy_importer_version}`}
            >
          ...
          {aap_version && (
            <>
              <Label>{t`Distronode Automation Platform`}</Label>
          ...
          <Value>
            <ExternalLink
              href={`https://github.com/distronode/distronode/tree/deve/ui/commit/${ui_sha}`}
            >
              {ui_sha}
            </ExternalLink>
          </Value>
