...
import { canSyncDistronodeRepository } from 'src/permissions';
...
export const distronodeRepositorySyncAction = Action({
  condition: canSyncDistronodeRepository,
  title: msg`Sync`,
  ...
  visible: (_item, { hasPermission }) =>
    hasPermission('distronode.change_collectionremote'),
  disabled: ({ remote, last_sync_task }) => {
    ...
    if (remote && remote.url === 'https://galaxy.distronode.com/api/') {
      const name = remote.name;
      const url = formatPath(Paths.distronodeRemoteEdit, { name });
...
function syncRepository({ name, pulp_href }, { addAlert, query }, syncParams) {
  ...
  return DistronodeRepositoryAPI.sync(pulpId, syncParams || { mirror: true })
    .then(({ data }) => {
