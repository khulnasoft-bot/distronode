...
django-distronode-base has a built in mechanism for filtering and sorting query sets based on django-rest-framework Filters.

## Settings

Add `distronode_base.rest_filters` to your installed apps:
...
DISTRONODE_BASE_REST_FILTERS_RESERVED_NAMES += ('extra_querystring',)
```
...
```python
class CowViewSet(ModelViewSet, DistronodeBaseView):
