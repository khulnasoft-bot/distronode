# Distronode Resource Registry

## Setup

### App Setup
Add `distronode_base.resource_registry` to your installed apps:
...
from distronode_base.authentication.models import Authenticator
from distronode_base.resource_registry.registry import ResourceConfig, ServiceAPIConfig, SharedResource
from distronode_base.resource_registry.shared_types import TeamType, UserType
...
- `shared_resource`: `namedtuple("SharedResource", ["serializer", "is_provider"])`. `serializer` is the serializer class instance for this resource.
  It should be set to one of the serializers in distronode_base.resource_registry.shared_types. `is_provider` should be set to true if the service
...
Once this model is provided, register it in settings.py by setting `DISTRONODE_BASE_RESOURCE_CONFIG_MODULE`, ex: `DISTRONODE_BASE_RESOURCE_CONFIG_MODULE = "test_app.resource_api"`
...
```python
from distronode_base.resource_registry.urls import urlpatterns as resource_api_urls
...
```
from distronode_base.resource_registry.fields import DistronodeResourceField

class Organization(AbstractOrganization):
    resource = DistronodeResourceField(primary_key_field="id")
```
...
In [3]: Organization.objects.first().resource.distronode_id
...
In [7]: for org in Organization.objects.select_related("resource").all():
   ...:     print(org.resource.distronode_id)
...
Resources are generic foreign keys to other models in the system that are given a unique Distronode ID. These are created via a post migration signal and kept up to date via `post_delete` and `post_save` signals.

#### Distronode ID

Distronode IDs are unique identifiers for a resource. They are are made up of two parts: the first portion of the service's ID and a UUIDv4 that is generated for each resource. They follow the pattern: `SSSSSSSS:RRRRRRRR-RRRR-RRRR-RRRR-RRRRRRRRRRRR` where `S` is the service short ID and `R` is the resource UUID.
...
```json
{
    "object_id": "2",
    "name": null,
    "distronode_id": "4c4ef945:faed95ca-2cea-43d4-ae18-4a5f0782a5de",
...
```json
{
    "object_id": "1",
    "name": "admin",
    "distronode_id": "4c4ef945:57289235-e68e-4abf-8e50-f868f9e5ff04",
...
An distronode ID can also be specified when creating a resource by including it in the request body. By default a new ID will be created for new resources.
...
```csv
distronode_id,resource_hash,modified
...
As shown above, status will be printed before each resource distronode_id,
the statuses are:
...
- `ManifestItem` - Serializer for resource manifest CSV containing distronode_id and resource_hash
...
when saving.. This patching is done in `distronode_base.resource_registry.apps`. A
...
sync. Delete operations call `pre_delete` as we need the `distronode_id` before
syncing the delete.

The signals call the method:
`distronode_base.resource_server.utils.sync_to_resource_server.sync_to_resource_server()`
...
```python
from distronode_base.resource_registry.signals.handlers import no_reverse_sync
