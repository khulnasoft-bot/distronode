...
django-distronode-base has can serve as an OAuth2 provider similar to how AWX can.
...
Add `distronode_base.oauth2_provider` to your installed apps:
...
REST_FRAMEWORK = [
    'DEFAULT_AUTHENTICATION_CLASSES': [
        'distronode_base.oauth2_provider.authentication.LoggedOAuth2Authentication',
        ...
    ]
]
...
distronode_base.oauth2_provider defines a system check that checks all APIView
...
For such cases, a DISTRONODE_BASE_OAUTH2_PROVIDER_PERMISSIONS_CHECK_IGNORED_VIEWS exists
...
```python
DISTRONODE_BASE_OAUTH2_PROVIDER_PERMISSIONS_CHECK_IGNORED_VIEWS = [
    'distronode_base.resource_registry.views.ValidateLocalUserView',
...
settings logic:
```python
DISTRONODE_BASE_OAUTH2_PROVIDER_PERMISSIONS_CHECK_DEFAULT_IGNORED_VIEWS = []
```
