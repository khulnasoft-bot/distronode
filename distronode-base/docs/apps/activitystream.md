...
The django-distronode-base `activitystream` app provides an audit log of changes
that happen to models within the system.

## Consumer Docs

To make use of `activitystream` in your application, first add
`distronode_base.activitystream` to your `INSTALLED_APPS`.

Now in your models you can:

```python
from distronode_base.activitystream.models import AuditableModel
```
...
`distronode_base.activitystream.models.entry.Entry`, and changes are stored in its
`changes` field, which is a `JSONField`.

The JSON stored in this field is that of a `dict` returned by
the function `distronode_base.lib.utils.models.diff`. Ultimately what we store will
...
- Any model fields wrapped with `distronode_base.lib.utils.models.prevent_search()`
...
```
from distronode_base.activitystream import urls as activity_stream_urls
urlpatterns = [
    ...
