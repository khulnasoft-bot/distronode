...
Start with `docs/Installation.md` for the core distronode_base setup.
The app name is dab_rbac, INSTALLED_APPS path is "distronode_base.rbac".
...
because "distronode_base.rbac" needs to come later in `INSTALLED_APPS`.

Because of this, DAB RBAC sends a special signal so you can run logic after
permissions are created.

```python
from distronode_base.rbac.triggers import dab_post_migrate
...
from distronode_base.rbac.api.related import RelatedAccessMixin
from distronode_base.rbac.api.permissions import DistronodeBaseObjectPermissions
...
class MyModelViewSet(viewsets.ModelViewSet):
    permission_classes = [DistronodeBaseObjectPermissions]  # (2)
...
2. `DistronodeBaseObjectPermissions` - checks object permission for detail views and global "add" permission
...
https://github.com/distronode/distronode/tree/devel/distronode-base/issues/78
...
list in `settings.DISTRONODE_BASE_CREATOR_DEFAULTS`. Not all entries in this will apply to
all models.

```
DISTRONODE_BASE_CREATOR_DEFAULTS = ['change', 'execute', 'delete', 'view']
```
...
```
DISTRONODE_BASE_TEAM_MODEL = 'auth.Group'
...
```
DISTRONODE_BASE_BYPASS_SUPERUSER_FLAGS = ['is_superuser']
DISTRONODE_BASE_BYPASS_ACTION_FLAGS = {'view': 'is_platform_auditor'}
```
...
There are a number of settings following the naming `DISTRONODE_BASE_ALLOW_*`.
...
Apps that utilize django-distronode-base may wish to add extra validation when assigning roles to actors (users or teams).
