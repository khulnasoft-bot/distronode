...
django-distronode-base provides a plugable authentication system (see authentication.md for high level details) that are backed by individual plugin files. The plugin files are meant to provide access to specific authentication methods like SAML, ldap, OIDC, tacacs, etc. 
...
### The File
Authenticator plugins in django-distronode-base should be individual files in the distronode_base/authenticator_plugins folder. That being said, you can configure django-distronode-base to look in additional directories for the authenticator_plugins (see authentication.md). The file should be a logical name for the backend service you are communicating with; for example, saml.py houses the code for connecting to SAML. 

### The AuthenticatorPlugin Class
The primary class you need to implement in your file is called `AuthenticatorPlugin`. When django-distronode-base attempts to instantiate an authenticator from an authenticator_plugin it will be loading this class. This class must be a superclass of `AbstractAuthenticatorPlugin` but can also subclass additional classes if required. For example, the SAML authenticator class subclasses `SAMLAuth` and `SocialAuthMixin` in addition to `AbstractAuthenticatorPlugin`.

The abstract class can be found in distronode_base.authentication.authenticator_plugins.base in here you can see several fields and methods that can be set or overridden as part of the authenticator class.
...
`logger`: The plugin will default to a logger of `distronode_base.authentication.models.abstract_authenticator` but you can set logger to be more specific for your plugin. i.e. `distronode_base.authentication.authenticator_plugins.saml`. Note: this takes the logger class, not a string of the logger name.
...
The Configuration Class tells the plugin system what attributes are needed for this authentication_plugin to work with its backend and will vary based on backend implementation. This is a Serializer field and as such has several fields related to serialization. The configuration class for your authenticator_plugin must be a superclass of `distronode_base.authentication.authenticator_plugins.base.BaseAUthenticatorConfiguration`
...
#### Serializer Fields
Your configuration class can then specify 0 or more configuration fields. These fields are serializer fields but should come from `distronode_base.lib.serializers.fields`. There we subclass serializer fields to add the ui_field_label and any additional fields we might require in the future.
...
If you need to add global URLs like those needed for social-core you can either do it in the django-distronode-base URLs to specify how an app would do that in the authentication.md file. This may be a better general approach since django-distronode-base provides many functions so an app including django-distronode-base may not need URLs for an authenticator it does not use.
...
from distronode_base.authentication.utils.authenticator_lib import AbstractAuthenticatorPlugin, BaseAuthenticatorConfiguration
from distronode_base.authentication.social_auth import SocialAuthMixin
from distronode_base.lib.serializers.fields import URLField

logger = logging.getLogger('distronode_base.authentication.authenticator_plugins.keycloak')
