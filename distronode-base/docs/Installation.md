...
Currently we install django-distronode-base via a pip install from the git repository.

If you want the devel version you can simply install from the repo:
```
pip install git+https://github.com/distronode/distronode/tree/devel/distronode-base.git[all]
```

This will install django-distronode-base as well as all its optional dependencies.
...
```
pip install git+https://github.com/distronode/distronode/tree/devel/distronode-base.git[api_documentation,rest_filters]
```

If you are using django-distronode-base from another project you will likely want to install a specific version from one of the github releases.
...
|--------------------------------|-----------------------|-------------------------|
| distronode_base.authentication    | dab_authentication    | authentication          |
| distronode_base.api_documentation | dab_api_documentation | api_documentation       |
| distronode_base.rest_filters      | dab_rest_filters      | rest_filters            |
| distronode_base.resource_registry | dab_resource_registry | resource_registry       |
| distronode_base.jwt_consumer      | dab_jwt_consumer      | jwt_consumer            |
| distronode_base.rbac              | dab_rbac              | rbac                    |

## settings.py

As a convenience, we can let django-distronode-base automatically add and modify the settings it needs to function:

```
from split_settings.tools import include

from distronode_base.lib import dynamic_config
dab_settings = os.path.join(os.path.dirname(dynamic_config.__file__), 'dynamic_settings.py')
include(dab_settings)
```

You can get a list of settings that may be modified from `DISTRONODE_BASE_OVERRIDABLE_SETTINGS` after
...
found in the `DISTRONODE_BASE_OVERRIDDEN_SETTINGS` setting.

## urls.py

Please read the various sections of this documentation for what urls django-distronode-base will need for your application to function.

As a convenience, we can let django-distronode-base automatically add the urls it needs to function:
```
from distronode_base.lib.dynamic_config.dynamic_urls import api_version_urls, root_urls, api_urls
