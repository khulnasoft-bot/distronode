...
django-distronode-base can house common serializer fields. These are in `distronode_base.lib.serializers.fields`.

`distronode_base.lib.serializers.fields.URLField` can handle doing URL validation (see validation.md).



# ValidationSerializerMixin

django-distronode-base offers a `ValidationSerializerMixin` for DRF serializers. This is in `distronode_base.lib.serializers.validation`.
