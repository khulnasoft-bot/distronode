# django-distronode-base tests

We try to maintain at least an 80% test coverage of django-distronode-base. This is enforced through GitHub actions on PR creation. The GitHub actions use SonarCloud to scan the code for issus one of which is code coverage.

All tests for django-distronode-base are located in `test_app/tests`. The directory structure in that folder mimics that of distronode_base so you can easily tell which test files cover which files in distronode_base.
