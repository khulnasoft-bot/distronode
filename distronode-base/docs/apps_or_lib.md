# django-distronode-base Features
django-distronode-base provides features as combination of django applications and python libraries.

All top level folders in distronode_base are django applications for features except for the lib folder.
...
This will automatically create a folder and files for you with the name of your application in distronode_base.
...
In the `apps.py` file be sure to change the `name` of the application to be prefixed with `distronode_base.`. For example, if your called your app `testing` the generated line would be:
...
You can remove `tests.py` as we don't put tests in distronode_base. Additionally, you should create a folder matching your application name in `test_app/tests` and put all of your test files in there.
...
```
from distronode_base.<app_name>.apps import <app class>
app_name = <app class>.label
```

## Views in apps

All views in a django app should inherit by default, from `distronode_base.lib.utils.views.DistronodeBaseDjangoAppApiView`. 
