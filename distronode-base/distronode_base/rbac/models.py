...
# distronode_base lib functions
from distronode_base.lib.abstract_models.common import CommonModel, ImmutableCommonModel

# distronode_base RBAC logic imports
from distronode_base.lib.utils.models import is_add_perm
from distronode_base.rbac.permission_registry import permission_registry
from distronode_base.rbac.prefetch import TypesPrefetch
from distronode_base.rbac.validators import validate_assignment, validate_permissions_for_model

logger = logging.getLogger('distronode_base.rbac.models')
...
class RoleDefinitionManager(models.Manager):
    ...
    def give_creator_permissions(self, user, obj) -> Optional['RoleUserAssignment']:
        ...
        for super_flag in settings.DISTRONODE_BASE_BYPASS_SUPERUSER_FLAGS:
            if getattr(user, super_flag):
                return

        needed_actions = settings.DISTRONODE_BASE_CREATOR_DEFAULTS
        ...
        if set(needed_perms) - set(has_permissions):
            kwargs = {'permissions': needed_perms, 'name': settings.DISTRONODE_BASE_ROLE_CREATOR_NAME.format(obj=obj, cls=type(obj))}
...
class RoleDefinition(CommonModel):
    ...
    def give_or_remove_global_permission(self, actor, giving=True):
        ...
        if actor._meta.model_name == 'user':
            if giving and (not settings.DISTRONODE_BASE_ALLOW_SINGLETON_USER_ROLES):
            ...
            if not settings.DISTRONODE_BASE_ALLOW_SINGLETON_TEAM_ROLES:
        ...
        if actor._meta.model_name == 'user':
            ...
            from distronode_base.rbac.evaluations import bound_singleton_permissions
    ...
    def give_or_remove_permission(self, actor, content_object, giving=True, sync_action=False):
        ...
        from distronode_base.rbac.triggers import needed_updates_on_assignment, update_after_assignment
    ...
    def user_global_permissions(cls, user, permission_qs=None):
        ...
        perm_set = set()
        if settings.DISTRONODE_BASE_ALLOW_SINGLETON_USER_ROLES:
        ...
        if settings.DISTRONODE_BASE_ALLOW_SINGLETON_TEAM_ROLES:
...
class RoleTeamAssignment(AssignmentBase):
    ...
    team = models.ForeignKey(
        settings.DISTRONODE_BASE_TEAM_MODEL, on_delete=models.CASCADE, related_name='role_assignments', help_text=_("The team that receives permissions.")
    )
...
class ObjectRole(ObjectRoleFields):
    ...
    teams = models.ManyToManyField(
        to=settings.DISTRONODE_BASE_TEAM_MODEL,
    ...
    provides_teams = models.ManyToManyField(
        settings.DISTRONODE_BASE_TEAM_MODEL,
    ...
    def expected_direct_permissions(self, types_prefetch=None):
        ...
        for permission in types_prefetch.permissions_for_object_role(self):
            ...
            if is_add_perm(permission.codename) or settings.DISTRONODE_BASE_CACHE_PARENT_PERMISSIONS:
                expected_evaluations.add((permission.codename, self.content_type_id, object_id))
