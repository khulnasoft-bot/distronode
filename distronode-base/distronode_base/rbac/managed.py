...
logger = logging.getLogger('distronode_base.rbac.managed')
...
class ManagedRoleConstructor:
    ...
    def allowed_permissions(self, model: Optional[Type[Model]]) -> set[str]:
        from distronode_base.rbac.validators import combine_values, permissions_allowed_for_role
...
class OrganizationMixin:
    model_name = settings.DISTRONODE_BASE_ORGANIZATION_MODEL


class TeamMixin:
    model_name = settings.DISTRONODE_BASE_TEAM_MODEL
...
def get_managed_role_constructors(apps, setting_value: dict[str, dict]) -> dict[str, ManagedRoleConstructor]:
    ...
    from the entries in setting_value, expected to be from settings.DISTRONODE_BASE_MANAGED_ROLE_REGISTRY"""
    ret = {}
