...
from distronode_base.rbac.models import ObjectRole, RoleDefinition, RoleEvaluation, RoleEvaluationUUID
...
logger = logging.getLogger('distronode_base.rbac.caching')
...
def compute_object_role_permissions(object_roles=None, types_prefetch=None):
    ...
    if to_add:
        ...
        if to_add_int:
            RoleEvaluation.objects.bulk_create(to_add_int, ignore_conflicts=settings.DISTRONODE_BASE_EVALUATIONS_IGNORE_CONFLICTS)
        if to_add_uuid:
            RoleEvaluationUUID.objects.bulk_create(to_add_uuid, ignore_conflicts=settings.DISTRONODE_BASE_EVALUATIONS_IGNORE_CONFLICTS)
