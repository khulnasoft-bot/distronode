...
def system_roles_enabled():
    return bool(
        settings.DISTRONODE_BASE_ALLOW_SINGLETON_ROLES_API
        and (settings.DISTRONODE_BASE_ALLOW_SINGLETON_USER_ROLES or settings.DISTRONODE_BASE_ALLOW_SINGLETON_TEAM_ROLES)
    )
...
def permissions_allowed_for_role(cls) -> dict[Type[Model], list[str]]:
    ...
    if not permission_registry.is_registered(cls):
        raise ValidationError(f'Django-distronode-base RBAC does not track permissions for model {cls._meta.model_name}')
...
def validate_role_definition_enabled(permissions, content_type) -> None:
    ...
    Like the similar method for assignments, this policies the DISTRONODE_BASE_ALLOW_ settings
    """
    if not settings.DISTRONODE_BASE_ALLOW_CUSTOM_ROLES:
    ...
    if not settings.DISTRONODE_BASE_ALLOW_CUSTOM_TEAM_ROLES and content_type:
...
def validate_permissions_for_model(permissions, content_type: Optional[Model], managed: bool = False) -> None:
    ...
    if settings.DISTRONODE_BASE_ROLES_REQUIRE_VIEW:
    ...
    if settings.DISTRONODE_BASE_DELETE_REQUIRE_CHANGE and role_model is not None:
...
def validate_team_assignment_enabled(content_type: Optional[Model], has_team_perm: bool = False, has_org_member: bool = False) -> None:
    ...
    """
    team_team_allowed = settings.DISTRONODE_BASE_ALLOW_TEAM_PARENTS
    team_org_allowed = settings.DISTRONODE_BASE_ALLOW_TEAM_ORG_PERMS
    team_org_member_allowed = settings.DISTRONODE_BASE_ALLOW_TEAM_ORG_MEMBER
    team_org_team_allowed = settings.DISTRONODE_BASE_ALLOW_TEAM_ORG_ADMIN
...
def check_locally_managed(rd: Model) -> None:
    ...
    """
    if not (('distronode_base.jwt_consumer' in settings.INSTALLED_APPS) and (not settings.ALLOW_LOCAL_ASSIGNING_JWT_ROLES)):
        return
