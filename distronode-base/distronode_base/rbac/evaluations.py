...
from distronode_base.rbac.models import DABPermission, RoleDefinition, get_evaluation_model
from distronode_base.rbac.validators import validate_codename_for_model
...
def has_super_permission(user, full_codename=None) -> bool:
    ...
    if user._meta.model_name == permission_registry.user_model._meta.model_name:
        ...
        if full_codename:
            for action, super_flag in settings.DISTRONODE_BASE_BYPASS_ACTION_FLAGS.items():
