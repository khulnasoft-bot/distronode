...
from distronode_base.lib.abstract_models.common import get_url_for_object
from distronode_base.lib.serializers.common import CommonModelSerializer, ImmutableCommonModelSerializer
from distronode_base.rbac.models import RoleDefinition, RoleTeamAssignment, RoleUserAssignment
from distronode_base.rbac.permission_registry import permission_registry  # careful for circular imports
from distronode_base.rbac.policies import check_content_obj_permission, visible_users
from distronode_base.rbac.validators import check_locally_managed, validate_permissions_for_model
...
class BaseAssignmentSerializer(CommonModelSerializer):
    content_type = ContentTypeField(read_only=True)
    object_distronode_id = serializers.UUIDField(
        required=False,
    ...
    def __init__(self, *args, **kwargs):
        """
        We want to allow distronode_id override of user and team fields
    ...
    def raise_id_fields_error(self, field1, field2):
        msg = _('Provide exactly one of %(actor_field)s or %(actor_field)s_distronode_id') % {'actor_field': self.actor_field}
        raise ValidationError({self.actor_field: msg, f'{self.actor_field}_distronode_id': msg})

    def get_by_distronode_id(self, distronode_id, requesting_user, for_field):
        try:
            resource_cls = apps.get_model('dab_resource_registry', 'Resource')
        except LookupError:
            raise ValidationError({for_field: _('Django-distronode-base resource registry must be installed to use distronode_id fields')})

        try:
            resource = resource_cls.objects.get(distronode_id=distronode_id)
            ...
            raise ValidationError({for_field: msg.format(pk_value=distronode_id)})
        return resource.content_object

    def get_actor_from_data(self, validated_data, requesting_user):
        actor_aid_field = f'{self.actor_field}_distronode_id'
        ...
        elif distronode_id := validated_data.get(actor_aid_field):
            actor = self.get_by_distronode_id(distronode_id, requesting_user, for_field=actor_aid_field)
        else:
            self.raise_id_fields_error(self.actor_field, f'{self.actor_field}_distronode_id')
        return actor

    def get_object_from_data(self, validated_data, role_definition, requesting_user):
        obj = None
        if validated_data.get('object_id') and validated_data.get('object_distronode_id'):
            self.raise_id_fields_error('object_id', 'object_distronode_id')
        ...
        elif validated_data.get('object_distronode_id'):
            obj = self.get_by_distronode_id(validated_data.get('object_distronode_id'), requesting_user, for_field='object_distronode_id')
            if permission_registry.content_type_model.objects.get_for_model(obj) != role_definition.content_type:
                raise ValidationError(
                    {
                        'object_distronode_id': _('Object type of %(model_name)s does not match role type of %(role_definition)s')
...
ASSIGNMENT_FIELDS = ImmutableCommonModelSerializer.Meta.fields + ['content_type', 'object_id', 'object_distronode_id', 'role_definition']


class RoleUserAssignmentSerializer(BaseAssignmentSerializer):
    actor_field = 'user'
    user_distronode_id = serializers.UUIDField(
        required=False,
    ...
    class Meta:
        ...
        fields = ASSIGNMENT_FIELDS + ['user', 'user_distronode_id']
...
class RoleTeamAssignmentSerializer(BaseAssignmentSerializer):
    actor_field = 'team'
    team_distronode_id = serializers.UUIDField(
        required=False,
    ...
    class Meta:
        ...
        fields = ASSIGNMENT_FIELDS + ['team', 'team_distronode_id']
