...
from distronode_base.rbac.policies import can_change_user

logger = logging.getLogger('distronode_base.rbac.api.permissions')
...
class RoleDefinitionPermissions(AuthenticatedReadAdminChange):
    def has_permission(self, request, view):
        if is_cloned_request(request) and settings.DISTRONODE_BASE_ALLOW_CUSTOM_ROLES is False:
            return False
        return super().has_permission(request, view)


class DistronodeBaseObjectPermissions(DjangoObjectPermissions):
...
class DistronodeBaseUserPermissions(DistronodeBaseObjectPermissions):
    def model_is_valid(self, model_cls):
        return bool(model_cls._meta.model_name == 'user')

    def has_create_permission(self, request, model_cls):
        org_cls = apps.get_model(settings.DISTRONODE_BASE_ORGANIZATION_MODEL)
