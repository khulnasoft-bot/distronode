...
from distronode_base.rbac.managed import ManagedRoleConstructor, get_managed_role_constructors
...
from distronode_base.utils.permission_registry import permission_registry

permission_registry.register(MyModel, AnotherModel)
"""

logger = logging.getLogger('distronode_base.rbac.permission_registry')
...
class PermissionRegistry:
    ...
    def get_resource_prefix(self, cls: Type[Model]) -> str:
        ...
        if registry := self.get_resource_registry():
            # duplicates logic in distronode_base/resource_registry/apps.py
            try:
    ...
    def get_resource_registry(self):
        if 'distronode_base.resource_registry' not in settings.INSTALLED_APPS:
            return None
    ...
    def register_managed_role_constructors(self) -> None:
        """Adds the data in setting DISTRONODE_BASE_MANAGED_ROLE_REGISTRY to the managed role registry"""
        managed_defs = get_managed_role_constructors(self.apps, settings.DISTRONODE_BASE_MANAGED_ROLE_REGISTRY)
    ...
    def call_when_apps_ready(self, apps, app_config) -> None:
        from distronode_base.rbac import triggers
        from distronode_base.rbac.evaluations import bound_has_obj_perm, bound_singleton_permissions, connect_rbac_methods
        from distronode_base.rbac.management import create_dab_permissions
        ...
        for model_name, kwargs in settings.DISTRONODE_BASE_RBAC_MODEL_REGISTRY.items():
        ...
        post_migrate.connect(
            ...
            dispatch_uid="distronode_base.rbac.management.create_dab_permissions",
        )
        post_migrate.connect(
            triggers.post_migration_rbac_setup,
            sender=app_config,
            dispatch_uid="distronode_base.rbac.triggers.post_migration_rbac_setup",
        )
    ...
    @property
    def team_model(self):
        return self.apps.get_model(settings.DISTRONODE_BASE_TEAM_MODEL)
