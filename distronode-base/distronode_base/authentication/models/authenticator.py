...
from distronode_base.authentication.authenticator_plugins.utils import generate_authenticator_slug, get_authenticator_plugin
from distronode_base.lib.abstract_models.common import UniqueNamedCommonModel
    ...
    def save(self, *args, **kwargs):
        ...
        for field in getattr(authenticator, 'configuration_encrypted_fields', []):
            if field in self.configuration:
                self.configuration[field] = distronode_encryption.encrypt_string(self.configuration[field])
        ...
        try:
            ...
            for field in getattr(authenticator, 'configuration_encrypted_fields', []):
                if field in instance.configuration:
                    instance.configuration[field] = distronode_encryption.decrypt_string(instance.configuration[field])
        except ImportError:
