...
from distronode_base.lib.utils.views.permissions import IsSuperuserOrAuditor, try_add_oauth2_scope_permission

logger = logging.getLogger('distronode_base.authentication.views.authenticator_users')


def get_authenticator_user_view():
    try:
        user_viewset_name = settings.DISTRONODE_BASE_USER_VIEWSET
        ...
        logger.error("DISTRONODE_BASE_USER_VIEWSET was not an APIView")
    except AttributeError:
        logger.debug("DISTRONODE_BASE_USER_VIEWSET was not specified")
    except Http404:
