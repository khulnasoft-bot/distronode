...
from distronode_base.lib.abstract_models import AbstractOrganization, AbstractTeam, CommonModel
from distronode_base.lib.utils.auth import get_organization_model, get_team_model
from distronode_base.lib.utils.string import is_empty

from .trigger_definition import TRIGGER_DEFINITION

logger = logging.getLogger('distronode_base.authentication.utils.claims')
...
def load_reconcile_user_class():
    module_path = getattr(settings, 'DISTRONODE_BASE_AUTHENTICATOR_RECONCILE_MODULE', 'distronode_base.authentication.utils.claims')
    try:
...
class ReconcileUser:
    ...
    def reconcile_user_claims(cls, user: AbstractUser, authenticator_user: AuthenticatorUser) -> None:
        ...
        if 'distronode_base.rbac' in settings.INSTALLED_APPS:
