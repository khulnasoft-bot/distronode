...
from distronode_base.lib.abstract_models.organization import AbstractOrganization
from distronode_base.lib.abstract_models.team import AbstractTeam
...
def get_object_by_distronode_id(qs: QuerySet, distronode_id: Union[str, UUID], annotate_as: str = 'distronode_id_for_filter') -> Model:
    ...
    resource_qs = resource_cls.objects.filter(object_id=pk_reference, content_type=ct).values('distronode_id')
    return qs.annotate(**{annotate_as: resource_qs}).get(**{annotate_as: distronode_id})


def get_user_by_distronode_id(distronode_id: Union[str, UUID], annotate_as: str = 'distronode_id_for_filter') -> Model:
    return get_object_by_distronode_id(get_user_model().objects.all(), distronode_id, annotate_as=annotate_as)
