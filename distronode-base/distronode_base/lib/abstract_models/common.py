...
from distronode_base.lib.abstract_models.immutable import ImmutableModel
from distronode_base.lib.utils.encryption import distronode_encryption
from distronode_base.lib.utils.models import current_user_or_system_user, is_system_user
from distronode_base.lib.utils.response import get_relative_url

logger = logging.getLogger('distronode_base.lib.abstract_models.common')
...
class AbstractCommonModel(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        for field in self.encrypted_fields:
            ...
            setattr(self, field, distronode_encryption.encrypt_string(field_value))
    ...
    def from_db(self, db, field_names, values):
        ...
        for field in self.encrypted_fields:
            ...
            setattr(instance, field, distronode_encryption.decrypt_string(field_value))
...
class CommonModel(AbstractCommonModel, CreatableModel, ModifiableModel):
    """
    A base model for most django-distronode-base apps to extend from.
