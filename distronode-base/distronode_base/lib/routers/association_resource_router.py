...
logger = logging.getLogger('distronode_base.lib.routers.association_resource_router')
...
class RelatedListMixin:
    ...
    def check_parent_object_permissions(self, request, parent_obj: Model) -> None:
        ...
        """
        if (request.method not in SAFE_METHODS) and 'distronode_base.rbac' in settings.INSTALLED_APPS and permission_registry.is_registered(parent_obj):
            from distronode_base.rbac.policies import check_content_obj_permission
