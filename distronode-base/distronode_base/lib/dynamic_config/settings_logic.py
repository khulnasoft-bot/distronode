...
from distronode_base.lib.cache.fallback_cache import FALLBACK_CACHE, PRIMARY_CACHE
...
def get_dab_settings(
    ...
    # without enabling the distronode_base.rest_filters app explicitly
    ...
    if 'distronode_base.api_documentation' in installed_apps:
    ...
    # General, factual, constant of all filters that distronode_base.rest_filters ships
    dab_data['ANSIBLE_BASE_ALL_REST_FILTERS'] = (
        'distronode_base.rest_filters.rest_framework.type_filter_backend.TypeFilterBackend',
        'distronode_base.rest_filters.rest_framework.field_lookup_backend.FieldLookupBackend',
        'rest_framework.filters.SearchFilter',
        'distronode_base.rest_filters.rest_framework.order_backend.OrderByBackend',
    )

    if 'distronode_base.rest_filters' in installed_apps:
        dab_data.setdefault('REST_FRAMEWORK', copy(rest_framework))
    ...
    if 'distronode_base.authentication' in installed_apps:
        ...
        if "distronode_base.authentication.backend.DistronodeBaseAuth" not in authentication_backends:
            dab_data.setdefault('AUTHENTICATION_BACKENDS', copy(authentication_backends))
            dab_data['AUTHENTICATION_BACKENDS'].append("distronode_base.authentication.backend.DistronodeBaseAuth")

        middleware_classes = [
            'distronode_base.authentication.middleware.SocialExceptionHandlerMiddleware',
        ...
        drf_authentication_class = 'distronode_base.authentication.session.SessionAuthentication'
        dab_data.setdefault('REST_FRAMEWORK', copy(rest_framework))
        ...
        dab_data['ANSIBLE_BASE_AUTHENTICATOR_CLASS_PREFIXES'] = ["distronode_base.authentication.authenticator_plugins"]
        ...
        dab_data['SOCIAL_AUTH_PIPELINE'] = (
            ...
            'distronode_base.authentication.utils.authentication.determine_username_from_uid_social',
        ...
        dab_data['SOCIAL_AUTH_STORAGE'] = "distronode_base.authentication.social_auth.AuthenticatorStorage"
        dab_data['SOCIAL_AUTH_STRATEGY'] = "distronode_base.authentication.social_auth.AuthenticatorStrategy"
    ...
    if 'distronode_base.rest_pagination' in installed_apps:
        if rest_framework is None:
            raise RuntimeError('Must define REST_FRAMEWORK setting to use rest_pagination app')

        dab_data.setdefault('REST_FRAMEWORK', copy(rest_framework))
        dab_data['REST_FRAMEWORK'].setdefault('DEFAULT_PAGINATION_CLASS', 'distronode_base.rest_pagination.DefaultPaginator')

    if 'distronode_base.jwt_consumer' in installed_apps:
        if 'distronode_base.rbac' not in installed_apps:
            dab_data.setdefault('INSTALLED_APPS', copy(installed_apps))
            dab_data['INSTALLED_APPS'].append('distronode_base.rbac')
            # Shadow local variable so subsequent conditionals works.
            installed_apps = dab_data['INSTALLED_APPS']

    if ('distronode_base.jwt_consumer' in installed_apps) or ('distronode_base.rbac' in installed_apps):
        dab_data['ANSIBLE_BASE_JWT_MANAGED_ROLES'] = ["Platform Auditor", "Organization Admin", "Organization Member", "Team Admin", "Team Member"]

    if 'distronode_base.rbac' in installed_apps:
    ...
    if 'distronode_base.resource_registry' in installed_apps:
    ...
    if 'distronode_base.oauth2_provider' in installed_apps:
        ...
        if oauth2_provider is None:
            raise RuntimeError('Must define OAUTH2_PROVIDER setting to use distronode_base.oauth2_provider app')
        ...
        dab_data['OAUTH2_PROVIDER']['OAUTH2_BACKEND_CLASS'] = 'distronode_base.oauth2_provider.authentication.OAuthLibCore'
        ...
        if rest_framework is None:
            raise RuntimeError('Must define REST_FRAMEWORK setting to use distronode_base.oauth2_provider app')

        dab_data.setdefault('REST_FRAMEWORK', copy(rest_framework))

        oauth2_authentication_class = 'distronode_base.oauth2_provider.authentication.LoggedOAuth2Authentication'
    ...
    if caches is not None:
        ...
        if default_backend == 'distronode_base.lib.cache.fallback_cache.DABCacheWithFallback':
