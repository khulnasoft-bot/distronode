...
#
# Here were my steps:
#  1. Make sure 'distronode_base.oauth2_provider' is in test_app INSTALLED_APPS (this already should be)
#  2. Comment out all OAUTH2_ settings in distronode_base/lib/dynamic_config/dynamic_settings.py which reference dab_oauth2_provider.*
...
#       ('organization', ... to=settings.DISTRONODE_BASE_ORGANIZATION_MODEL)),
#     We should also add this in the migration dependencies:
#       migrations.swappable_dependency(settings.DISTRONODE_BASE_ORGANIZATION_MODEL),
#  8. Uncomment all OAUTH2_PROVIDER_* settings
#  9. Revert step 3
#  10. gateway-manage makemigrations && gateway-manage migrate distronode_base
