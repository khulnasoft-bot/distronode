...
class Resource(models.Model):
    ...
    # we're not using this as the primary key because the distronode_id can change if the object is
    # externally managed.
    distronode_id = models.UUIDField(default=uuid.uuid4, db_index=True, unique=True, help_text=_("A unique ID identifying this resource by the resource server."))
    ...
    def summary_fields(self):
        return {"distronode_id": self.distronode_id, "resource_type": self.resource_type}
    ...
    @classmethod
    def create_resource(
        cls, resource_type: ResourceType, resource_data: dict, distronode_id: Union[str, uuid.UUID, None] = None, service_id: Union[str, uuid.UUID, None] = None
    ):
        from ..signals.handlers import no_reverse_sync
        ...
        with transaction.atomic():
            ...
            if distronode_id:
                resource.distronode_id = distronode_id
            if service_id:
    ...
    def update_resource(self, resource_data: dict, distronode_id=None, is_partially_migrated=None, partial=False, service_id: Union[str, uuid.UUID, None] = None):
        from ..signals.handlers import no_reverse_sync
        ...
        with transaction.atomic():
            if distronode_id:
                self.distronode_id = distronode_id
            if service_id:
