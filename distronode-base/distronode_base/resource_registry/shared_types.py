...
from distronode_base.resource_registry.utils.resource_type_serializers import DistronodeResourceForeignKeyField, SharedResourceTypeSerializer
from distronode_base.resource_registry.utils.sso_provider import get_sso_provider_server
...
class TeamType(SharedResourceTypeSerializer):
    ...
    name = serializers.CharField()
    organization = DistronodeResourceForeignKeyField("shared.organization", required=False, allow_null=True)
    description = serializers.CharField(
        default="",
        allow_blank=True,
    )
