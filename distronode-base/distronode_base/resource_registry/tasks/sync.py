...
from distronode_base.resource_registry.rest_client import ResourceAPIClient, get_resource_server_client
...
class ManifestItem:
    ...
    def __hash__(self):
        return hash(self.distronode_id)
...
def get_orphan_resources(
    ...
    ).exclude(distronode_id__in=[item.distronode_id for item in manifest_list])
...
def get_managed_resource(manifest_item: ManifestItem) -> Resource | None:
    ...
    return Resource.objects.filter(
        distronode_id=manifest_item.distronode_id,
...
def resource_sync(
    ...
    def set_resource_local_variables():
        ...
        if resource_data is None or resource_type_name is None:
            resp = api_client.get_resource(manifest_item.distronode_id)
...
class SyncExecutor:
    ...
    def _report_manifest_item(self, result: SyncResult):
        ...
        msg = f"{result.status.value.upper()} {result.item.distronode_id}"
