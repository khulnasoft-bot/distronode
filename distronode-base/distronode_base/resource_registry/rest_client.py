...
from distronode_base.resource_registry.resource_server import get_resource_server_config, get_service_token

ResourceRequestBody = namedtuple(
    "ResourceRequestBody",
    ["distronode_id", "service_id", "is_partially_migrated", "resource_type", "resource_data"],
...
logger = logging.getLogger('distronode_base.resources_api.rest_client')
...
class ResourceAPIClient:
    """
    Client for Distronode services to interact with the service-index/ api
    """

    header_name = "X-DISTRONODE-SERVICE-AUTH"
    _jwt_timeout = None
    _jwt = None
    ...
    def __init__(
        ...
        jwt_user_id (UUID): distronode ID of the user to make the request as.
    ...
    def _get_request_dict(self, data: ResourceRequestBody):
        ...
        for k in raw_dict:
            if raw_dict[k] is not None:
                ...
                if k in ("distronode_id", "service_id"):
    ...
    def get_resource(self, distronode_id):
        return self._make_request("get", f"resources/{distronode_id}/")

    def get_additional_resource_data(self, distronode_id):
        return self._make_request("get", f"resources/{distronode_id}/additional_data/")

    def update_resource(self, distronode_id, data: ResourceRequestBody, partial=False):
        action = "patch" if partial else "put"
        return self._make_request(action, f"resources/{distronode_id}/", self._get_request_dict(data))

    def delete_resource(self, distronode_id):
        return self._make_request("delete", f"resources/{distronode_id}/")
