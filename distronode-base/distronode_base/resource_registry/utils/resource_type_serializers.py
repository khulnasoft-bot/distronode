...
class DistronodeResourceManyRelated(serializers.Field):
    ...
    def to_representation(self, value):
        ...
        return Resource.objects.filter(content_type__resource_type__name=self.resource_type, object_id__in=pks).values_list("distronode_id", flat=True)


class DistronodeResourceForeignKeyField(serializers.UUIDField):
    default_error_messages = {
        'invalid': _('Must be a valid UUID.'),
        'does_not_exist': _('Invalid distronode id "{distronode_id}" - resource does not exist.'),
    }
    ...
    # Convert distronode ID to internal object ID
    def to_internal_value(self, data):
        distronode_id = super().to_internal_value(data)

        try:
            resource = Resource.objects.get(content_type__resource_type__name=self.resource_type, distronode_id=distronode_id)
        except Resource.DoesNotExist:
            self.fail('does_not_exist', distronode_id=data)
