...
from distronode_base.lib.utils.response import CSVStreamResponse, get_relative_url
from distronode_base.lib.utils.views.django_app_api import DistronodeBaseDjangoAppApiView
from distronode_base.lib.utils.views.permissions import try_add_oauth2_scope_permission
from distronode_base.resource_registry.models import Resource, ResourceType, service_id
from distronode_base.resource_registry.registry import get_registry
from distronode_base.resource_registry.serializers import ResourceListSerializer, ResourceSerializer, ResourceTypeSerializer, UserAuthenticationSerializer
from distronode_base.resource_registry.utils.auth_code import get_user_auth_code
from distronode_base.rest_filters.rest_framework.field_lookup_backend import FieldLookupBackend
from distronode_base.rest_filters.rest_framework.order_backend import OrderByBackend
from distronode_base.rest_filters.rest_framework.type_filter_backend import TypeFilterBackend

logger = logging.getLogger('distronode_base.resource_registry.views')
...
class ResourceViewSet(
    ...
    GenericViewSet,
    DistronodeBaseDjangoAppApiView,
):
    """
...
class ResourceTypeViewSet(
    ...
    def serialize_resources_hashes(self, resources_qs, serializer_class):
        ...
        yield ("distronode_id", "resource_hash")
        for resource in resources_qs:
            yield (resource.distronode_id, serializer_class(resource.content_object).get_hash())
...
class ServiceIndexRootView(DistronodeBaseDjangoAppApiView):
...
class ValidateLocalUserView(DistronodeBaseDjangoAppApiView):
    """
    ...
    def post(self, request, **kwargs):
        ...
        response = {
            "distronode_id": Resource.get_resource_for_object(user).distronode_id,
            "auth_code": auth_code,
        }
