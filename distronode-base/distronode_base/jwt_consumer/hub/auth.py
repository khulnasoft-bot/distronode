...
logger = logging.getLogger('distronode_base.jwt_consumer.hub.auth')
        ...
        for role_name in self.common_auth.token.get('object_roles', {}).keys():
            if role_name.startswith('Team'):
                for object_index in self.common_auth.token['object_roles'][role_name]['objects']:
                    ...
                    distronode_id = team_data['distronode_id']
                    try:
                        team = Resource.objects.get(distronode_id=distronode_id).content_object
