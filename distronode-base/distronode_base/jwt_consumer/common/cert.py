...
logger = logging.getLogger('distronode_base.jwt_consumer.common.cert')
...
class JWTCert:
    key_name = 'DISTRONODE_BASE_JWT_KEY'

    def __init__(self):
        self.cached = None
        self.key = None
        # Attempt to locate the cert using DISTRONODE_BASE_JWT_KEY.  If we are running on a service that houses the JWT key
    ...
    def _get_decryption_key_from_url(self) -> None:
        ...
        validate_certs = get_setting("DISTRONODE_BASE_JWT_VALIDATE_CERT", True)
        timeout = get_setting("DISTRONODE_BASE_JWT_URL_TIMEOUT", 30)
