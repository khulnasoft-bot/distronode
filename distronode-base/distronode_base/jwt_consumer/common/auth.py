...
from distronode_base.jwt_consumer.common.cache import JWTCache
from distronode_base.jwt_consumer.common.cert import JWTCert, JWTCertException
from distronode_base.lib.logging.runtime import log_excess_runtime
...
from distronode_base.resource_registry.models import Resource, ResourceType
from distronode_base.resource_registry.signals.handlers import no_reverse_sync

logger = logging.getLogger("distronode_base.jwt_consumer.common.auth")
...
def permission_registry():
    ...
    if not _permission_registry:
        from distronode_base.rbac.permission_registry import permission_registry as permission_registry_singleton
...
class JWTCommonAuth:
    ...
    def parse_jwt_token(self, request):
        ...
        if is_cached:
            try:
                self.user = get_user_by_distronode_id(self.token['sub'])
        ...
        if not self.user:
            ...
            resource_kwargs = {}
            for resource_key, token_key in (('resource_data', 'user_data'), ('distronode_id', 'sub'), ('service_id', 'service_id')):
    ...
    def validate_token(self, unencrypted_token, decryption_key):
        ...
        try:
            ...
            validated_body = jwt.decode(
                unencrypted_token,
                decryption_key,
                audience="distronode-services",
                options={"require": local_required_field},
                issuer="distronode-issuer",
                algorithms=["RS256"],
            )
    ...
    def process_rbac_permissions(self):
        ...
        from distronode_base.rbac.models import RoleUserAssignment

        role_diff = RoleUserAssignment.objects.filter(user=self.user, role_definition__name__in=settings.DISTRONODE_BASE_JWT_MANAGED_ROLES)
        ...
        for system_role_name in self.token.get("global_roles", []):
            ...
            if rd:
                if rd.name in settings.DISTRONODE_BASE_JWT_MANAGED_ROLES:
        ...
        for object_role_name in self.token.get('object_roles', {}).keys():
            ...
            if rd is None:
                ...
                continue
            elif rd.name not in settings.DISTRONODE_BASE_JWT_MANAGED_ROLES:
            ...
            for index in object_indexes:
                ...
                if resource is not None:
                    ...
                    role_diff = role_diff.exclude(pk=assignment.pk)
                    logger.info(f"Granted user {self.user.username} role {object_role_name} to object {obj.name} with distronode_id {object_data['distronode_id']}")
    ...
    def get_or_create_resource(self, content_type: str, data: dict) -> Tuple[Optional[Resource], Optional[Model]]:
        ...
        """
        object_distronode_id = data['distronode_id']
        try:
            resource = Resource.objects.get(distronode_id=object_distronode_id)
            logger.debug(f"Resource {object_distronode_id} already exists")
        ...
        if content_type == 'team':
            ...
            resource = Resource.create_resource(
                ...
                {"name": data["name"], "organization": org_resource.distronode_id},
                distronode_id=data["distronode_id"],
            )
