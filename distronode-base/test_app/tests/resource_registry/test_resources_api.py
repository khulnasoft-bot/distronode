...
from distronode_base.authentication.models import AuthenticatorUser
...
from distronode_base.resource_registry.utils.resource_type_processor import ResourceTypeProcessor
...
def test_resources_delete_api(admin_api_client, django_user_model):
    ...
    distronode_id = Resource.objects.get(object_id=user.pk, content_type=c_type.pk).distronode_id
    admin_api_client.delete(get_relative_url("resource-detail", kwargs={"distronode_id": distronode_id}))
...
def test_resources_api_invalid_delete(admin_api_client, local_authenticator):
    ...
    distronode_id = Resource.get_resource_for_object(local_authenticator).distronode_id
    resp = admin_api_client.delete(get_relative_url("resource-detail", kwargs={"distronode_id": distronode_id}))
...
def test_resource_update(
    ...
    url = get_relative_url("resource-detail", kwargs={"distronode_id": resource.distronode_id})
...
def test_resource_update_distronode_id(admin_api_client, user):
    """Test that the distronode ID of a resource can be updated."""
    c_type = ContentType.objects.get_for_model(user)
    distronode_id = Resource.objects.get(object_id=user.pk, content_type=c_type.pk).distronode_id
    new_distronode_id = str(uuid.uuid4())

    url = get_relative_url("resource-detail", kwargs={"distronode_id": distronode_id})

    data = {"distronode_id": new_distronode_id}
    resp = admin_api_client.patch(url, data, format="json")
    assert resp.status_code == 200
    assert resp.data["distronode_id"] == new_distronode_id

    assert admin_api_client.get(url).status_code == 404

    resp = admin_api_client.get(get_relative_url("resource-detail", kwargs={"distronode_id": new_distronode_id}))
...
@pytest.mark.parametrize(
    'resource',
    [
        {"distronode_id": "a0057c59-776d-48f8-97f1-8f8033e68d93", "resource_type": "shared.organization", "resource_data": {"name": "foo"}},
        {"resource_type": "shared.organization", "resource_data": {"name": "my super cool org"}},
        {"distronode_id": "a0057c59-776d-48f8-97f1-8f8033e68d93", "resource_type": "shared.user", "resource_data": {"username": "foo"}},
        {
            "distronode_id": "a0057c59-776d-48f8-97f1-8f8033e68d93",
...
def test_resources_api_crd(admin_api_client, resource):
    ...
    if "distronode_id" in resource:
        assert response.data["distronode_id"] == resource["distronode_id"]
    ...
    # read resource
    detail_url = get_relative_url("resource-detail", kwargs={"distronode_id": response.data["distronode_id"]})
    detail_response = admin_api_client.get(detail_url)
    assert detail_response.status_code == 200
    assert detail_response.data["distronode_id"] == response.data["distronode_id"]
...
@pytest.mark.parametrize(
    'resource',
    [
        {"data": {"distronode_id": "bogus", "resource_type": "shared.user", "resource_data": {"name": "foo"}}, "field_name": "distronode_id"},
        {
            "data": {
                "distronode_id": "a0057c59-776d-48f8-97f1-8f8033e68d91",
                "service_id": "bogus",
                "resource_type": "shared.user",
        ...
        },
        {
            ...
            "field_name": "distronode_id",
        },
        {"data": {"distronode_id": "null", "resource_type": "shared.team", "resource_data": {"name": "foo"}}, "field_name": "distronode_id"},
        {
            "data": {"distronode_id": "123-a0057c59-776d-48f8-97f1-8f8033e68d91", "resource_type": "shared.team", "resource_data": {"name": "foo"}},
            "field_name": "distronode_id",
        },
        {
            "data": {"distronode_id": "a0057c59033e68d91", "resource_type": "shared.team", "resource_data": {"name": "foo"}},
            "field_name": "distronode_id",
        },
...
def test_resource_summary_fields(
    ...
    assert data["summary_fields"]["resource"]["distronode_id"] == resource.distronode_id
...
def test_team_organization_field(admin_api_client, organization, organization_1, team):
    team_id = str(team.resource.distronode_id)
    org0_id = str(organization.resource.distronode_id)
    org1_id = str(organization_1.resource.distronode_id)

    url = get_relative_url("resource-detail", kwargs={"distronode_id": team_id})
...
def test_processor_pre_serialize(admin_api_client, organization):
    ...
    url = get_relative_url("resource-detail", kwargs={"distronode_id": str(organization.resource.distronode_id)})
...
def test_processor_save(admin_api_client):
    ...
    with patch("test_app.resource_api.APIConfig", PatchedConfig):
        ...
        url = get_relative_url("resource-detail", kwargs={"distronode_id": resp.data["distronode_id"]})
...
def test_user_social_auth_field(admin_api_client, django_user_model, request, authenticator_fixture, server):
    ...
    distronode_id = str(Resource.get_resource_for_object(user).distronode_id)
...
def test_user_social_auth_no_social_core(admin_api_client, django_user_model, saml_authenticator, saml_configuration, expected_log):
    ...
    with patch("distronode_base.resource_registry.utils.sso_provider.load_strategy", None):
        with expected_log('distronode_base.resource_registry.utils.sso_provider.logger', 'debug', "'social_core' is not installed"):
            resp = admin_api_client.get(url)
            assert resp.status_code == 200
...
def test_user_social_auth_exception(admin_api_client, django_user_model, saml_authenticator, saml_configuration, expected_log):
    ...
    with patch("distronode_base.resource_registry.utils.sso_provider.load_strategy", side_effect=RuntimeError("broken")):
        with expected_log('distronode_base.resource_registry.utils.sso_provider.logger', 'warning', "Failed to parse server url from"):
            resp = admin_api_client.get(url)
            assert resp.status_code == 200
