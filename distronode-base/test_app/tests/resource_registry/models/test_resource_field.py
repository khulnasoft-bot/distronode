...
def test_resource_field_select_related(organization, organization_1, organization_2, django_assert_num_queries):
    ...
    org_distronode_ids = set([org.resource.distronode_id for org in org_qs])
    ...
    resource_distronode_ids = set([resource.distronode_id for resource in resource_qs])

    assert org_names == resource_names
    assert org_names == org_resource_names
    assert org_pks == resource_object_ids
    assert org_distronode_ids == resource_distronode_ids
...
def test_resource_field_get(organization):
    ...
    assert organization.resource.distronode_id == resource.distronode_id
...
def test_resource_field_filtering(organization):
    ...
    org = Organization.objects.get(resource__distronode_id=resource.distronode_id)
