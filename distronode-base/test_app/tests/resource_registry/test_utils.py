...
from distronode_base.resource_registry import apps
from distronode_base.resource_registry.signals.handlers import no_reverse_sync
from distronode_base.resource_registry.utils.sync_to_resource_server import sync_to_resource_server
from test_app.models import Organization

handlers_path = 'distronode_base.resource_registry.signals.handlers'
utils_path = 'distronode_base.resource_registry.utils.sync_to_resource_server'
...
class TestReverseResourceSync:
    ...
    @pytest.mark.parametrize(
        'distronode_reverse_resource_sync, should_sync',
        [("true", True), ("True", True), ("false", False), ("False", False), (None, True)],
    )
    def test_sync_to_resource_server_env_var_override(self, user, enable_reverse_sync, distronode_reverse_resource_sync, should_sync, expected_log):
        """
        We can override/disable syncing with the env var DISTRONODE_REVERSE_RESOURCE_SYNC.
        """
        cm = nullcontext()
        if distronode_reverse_resource_sync is not None:
            cm = mock.patch.dict(os.environ, DISTRONODE_REVERSE_RESOURCE_SYNC=distronode_reverse_resource_sync)

        logger_cm = nullcontext()
        if not should_sync:
            logger_cm = expected_log(f'{utils_path}.logger', 'info', "Skipping sync of resource Hello because $DISTRONODE_REVERSE_RESOURCE_SYNC is 'false'")
    ...
    @pytest.mark.django_db(transaction=True)
    def test_sync_to_resource_server_delete_and_no_distronode_id_given(self, organization, enable_reverse_sync):
        """
        sync_to_resource_server() always requires an distronode_id kwarg for delete.
        """
        with enable_reverse_sync():
            with pytest.raises(Exception, match="distronode_id should be provided for delete actions"):
                sync_to_resource_server(organization, 'delete')

    @pytest.mark.django_db(transaction=True)
    def test_sync_to_resource_server_create_update_and_distronode_id_given(self, organization, enable_reverse_sync):
        """
        sync_to_resource_server() should raise an exception if distronode_id is provided for create/update.
        """
        with enable_reverse_sync():
            with pytest.raises(Exception, match="distronode_id should not be provided for create/update actions"):
                sync_to_resource_server(organization, 'create', distronode_id='foo')

            with pytest.raises(Exception, match="distronode_id should not be provided for create/update actions"):
                sync_to_resource_server(organization, 'update', distronode_id='foo')
