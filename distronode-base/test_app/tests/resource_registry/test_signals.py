...
from distronode_base.resource_registry.signals import handlers
...
def test_unregistered_model_triggers_no_signals():
    with mock.patch('distronode_base.resource_registry.models.resource.init_resource_from_object') as mck:
        obj = EncryptionModel.objects.create()
    mck.assert_not_called()

    with mock.patch('distronode_base.resource_registry.models.Resource.update_from_content_object') as mck:
        obj.a = 'foobar'
        obj.save()
    mck.assert_not_called()

    with mock.patch('distronode_base.resource_registry.models.Resource.delete') as mck:
        obj.delete()
    mck.assert_not_called()
...
def test_registered_model_triggers_signals(model, system_user):
    with mock.patch('distronode_base.resource_registry.signals.handlers.init_resource_from_object', wraps=handlers.init_resource_from_object) as mck:
