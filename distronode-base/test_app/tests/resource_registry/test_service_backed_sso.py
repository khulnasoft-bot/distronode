...
from distronode_base.resource_registry.utils.service_backed_sso_pipeline import redirect_to_resource_server
...
def _validate_auth_code(auth_code, user):
    ...
    assert data["sub"] == str(user.resource.distronode_id)
...
def patched_load_strategy():
    ...
    with mock.patch("distronode_base.resource_registry.utils.sso_provider.load_strategy", _get_strat) as get_strat:
        yield get_strat
