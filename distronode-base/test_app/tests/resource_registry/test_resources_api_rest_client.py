...
from distronode_base.resource_registry.rest_client import ResourceAPIClient, ResourceRequestBody
...
def resource_client(system_user, admin_user, live_server, local_authenticator, transactional_db) -> ResourceAPIClient:
    ...
    """
    return ResourceAPIClient(live_server.url, "/api/v1/service-index/", jwt_user_id=admin_user.resource.distronode_id)
...
def test_create_resource(resource_client):
    ...
    data = ResourceRequestBody(
        distronode_id=new_distronode_id,
    ...
    assert resp.json()["distronode_id"] == new_distronode_id
...
def test_get_resource(resource_client, organization):
    distronode_id = str(Resource.get_resource_for_object(organization).distronode_id)
    resp = resource_client.get_resource(distronode_id)
...
def test_update_resource(resource_client, organization):
    ...
    resp = resource_client.update_resource(distronode_id, data)

    assert resp.status_code == 200
    assert resp.json()["name"] == "my_new_org"

    data = ResourceRequestBody(resource_data={"name": "my_new_org2"})
    resp = resource_client.update_resource(distronode_id, data, partial=True)
    ...
    data = ResourceRequestBody(distronode_id=new_distronode_id, service_id=new_service_id)
    resp = resource_client.update_resource(distronode_id, data, partial=True)
    ...
    resp = resource_client.update_resource(new_distronode_id, data, partial=True)
...
def test_delete_resource(resource_client, organization):
    distronode_id = str(Resource.get_resource_for_object(organization).distronode_id)
    resp = resource_client.delete_resource(distronode_id)
...
def test_list_resources(resource_client, organization):
    ...
    resp = resource_client.list_resources(filters={"distronode_id": distronode_id})

    assert resp.status_code == 200
    assert resp.json()["count"] == 1
    assert resp.json()["results"][0]["distronode_id"] == distronode_id
...
def test_validate_local_user(resource_client, admin_user, member_rd):
    ...
    assert resp.status_code == 200
    json = resp.json()
    json["distronode_id"] == str(admin_user.resource.distronode_id)
