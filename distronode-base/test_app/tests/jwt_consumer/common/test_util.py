...
from distronode_base.jwt_consumer.common.util import generate_x_trusted_proxy_header, validate_x_trusted_proxy_header
...
class TestValidateTrustedProxy:
    def test_validate_trusted_proxy_header_bad_cached_key_but_correct_setting(self, rsa_keypair, random_public_key, create_mock_method):
        ...
        with override_settings(DISTRONODE_BASE_JWT_KEY=rsa_keypair.public):
            with mock.patch("distronode_base.jwt_consumer.common.util.JWTCert.get_decryption_key", create_mock_method(field_dicts)):
    ...
    def test_validate_trusted_proxy_header_no_key(self, caplog):
        with override_settings(DISTRONODE_BASE_JWT_KEY=None):
    ...
    def test_header_timeout(self, expected_log, rsa_keypair):
        ...
        with override_settings(DISTRONODE_BASE_JWT_KEY=rsa_keypair.public):
            ...
            time.sleep(0.5)
            with expected_log(
                'distronode_base.jwt_consumer.common.util.logger',
                'warning',
    ...
    def test_validate_x_trusted_proxy_header_invalid_signature(self, random_public_key, expected_log, rsa_keypair):
        with override_settings(DISTRONODE_BASE_JWT_KEY=random_public_key):
            ...
            # with mock.patch('distronode_base.jwt_consumer.common.util.validate_x_trusted_proxy_header.bytes.fromhex', side_effect=ValueError()):
            header = generate_x_trusted_proxy_header(rsa_keypair.private)
            with expected_log(
                'distronode_base.jwt_consumer.common.util.logger',
                'warning',
