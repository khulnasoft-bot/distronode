...
from distronode_base.jwt_consumer.common.auth import JWTAuthentication, JWTCommonAuth, default_mapped_user_fields
from distronode_base.jwt_consumer.common.cert import JWTCert, JWTCertException
from distronode_base.lib.utils.translations import translatableConditionally as _
from distronode_base.rbac.models import RoleDefinition, RoleUserAssignment
...
default_logger = 'distronode_base.jwt_consumer.common.auth.logger'
...
class TestJWTCommonAuth:
    ...
    def test_parse_jwt_happy_path(self, mocked_http, test_encryption_public_key, shut_up_logging, jwt_token):
        with override_settings(DISTRONODE_BASE_JWT_KEY=test_encryption_public_key):
            my_auth = JWTCommonAuth()
    ...
    def test_parse_jwt_no_jwt_key(self, mocked_http, caplog):
        ...
        with caplog.at_level(logging.INFO):
            ...
            assert 'Failed to get the setting DISTRONODE_BASE_JWT_KEY' in caplog.text
    ...
    def test_log_exception_no_expansion(self, expected_log):
        ...
        with mock.patch('distronode_base.lib.utils.translations.translatableConditionally.translated', return_value=translated_message):
    ...
    @pytest.mark.django_db
    @mock.patch('distronode_base.jwt_consumer.common.auth.JWTCert.get_decryption_key', side_effect=JWTCertException('testing'))
    ...
    @pytest.mark.django_db
    @mock.patch('distronode_base.jwt_consumer.common.cache.cache_key', 'test_cert_fails_decryption_from_uncached_key')
    def test_cert_fails_decryption_from_uncached_key(self, mocked_http, expected_log, test_encryption_public_key):
        with override_settings(DISTRONODE_BASE_JWT_KEY=test_encryption_public_key):
            with mock.patch('distronode_base.jwt_consumer.common.auth.JWTCommonAuth.validate_token', side_effect=DecodeError('validation always fails')):
                common_auth = JWTCommonAuth()
                request = mocked_http.mocked_parse_jwt_token_get_request('with_headers')
                expected_log = partial(expected_log, "distronode_base.jwt_consumer.common.auth.logger")
    ...
    def test_cert_fails_decryption_first_key_bad_second_key_good(self, mocked_http, expected_log, test_encryption_public_key, random_public_key):
        with override_settings(DISTRONODE_BASE_JWT_KEY=random_public_key):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ({"Organization Admin": {'content_type': 'organization', 'objects': [0]}}, "info", "with distronode_id"),
    ...
    def test_process_rbac_permissions_object_role_exists_object_exists(
        ...
        authentication.token = {
            'objects': {'organization': [{'distronode_id': organization.resource.distronode_id, 'name': organization.name}]},
    ...
    def test_get_or_create_resource_invalid_content_type(self):
        authentication = JWTCommonAuth()
        assert authentication.get_or_create_resource('junk', {'distronode_id': uuid4()}) == (None, None)

    @pytest.mark.django_db
    def test_get_or_create_resource_organization(self):
        authentication = JWTCommonAuth()
        data = {'distronode_id': uuid4(), 'name': 'Test Organization'}
        assert not Organization.objects.filter(name=data['name']).exists()
        assert not Resource.objects.filter(distronode_id=data['distronode_id']).exists()
        ...
        assert Resource.objects.filter(distronode_id=data['distronode_id']).exists()
...
class TestJWTAuthentication:
    ...
    def test_authenticate_no_user(self, user):
        with mock.patch('distronode_base.jwt_consumer.common.auth.JWTCommonAuth.parse_jwt_token') as mock_parse:
    ...
    def test_process_user_data(self):
        with mock.patch("distronode_base.jwt_consumer.common.auth.JWTCommonAuth.map_user_fields") as mock_inspect:
            jwt_auth = JWTAuthentication()
            jwt_auth.process_user_data()
    ...
    def test_raise_an_exception_if_the_key_is_not_cached(self, random_public_key, mocked_http, create_mock_method):
        ...
        with override_settings(DISTRONODE_BASE_JWT_KEY=random_public_key):
            with mock.patch('distronode_base.jwt_consumer.common.auth.JWTCert.get_decryption_key', create_mock_method(mock_field_dicts)):
                request = mocked_http.mocked_parse_jwt_token_get_request('with_headers')
                jwt_auth = JWTAuthentication()
    ...
    def test_raise_an_exception_if_the_key_is_cached_but_the_new_key_is_the_same(self, random_public_key, mocked_http, create_mock_method):
        ...
        with override_settings(DISTRONODE_BASE_JWT_KEY=url):
    ...
    def test_correctly_authenticate_if_the_cached_key_is_invalid_but_the_new_key_is_correct(
        ...
        with override_settings(DISTRONODE_BASE_JWT_KEY=url):
            ...
            with mock.patch('distronode_base.jwt_consumer.common.auth.JWTCert.get_decryption_key', create_mock_method(jwt_cert_field_changes)):
                request = mocked_http.mocked_parse_jwt_token_get_request('with_headers')
                jwt_auth = JWTAuthentication()
                if not new_user:
                ...
                if not new_user:
                    ...
                    assert str(created_user.resource.distronode_id) == jwt_token.unencrypted_token['sub']
    ...
    def test_failure_when_cached_key_incorrect_and_actual_key_raises_exception(self, random_public_key, mocked_http, django_user_model, jwt_token, caplog):
        with override_settings(DISTRONODE_BASE_JWT_KEY=random_public_key):
            ...
            with mock.patch('distronode_base.jwt_consumer.common.auth.JWTCert.get_decryption_key', change_cert_key_value):
                with pytest.raises(AuthenticationFailed):
                    request = mocked_http.mocked_parse_jwt_token_get_request('with_headers')
                    jwt_auth = JWTAuthentication()
                    jwt_auth.authenticate(request)
    ...
    def test_jwt_authentication_with_common_auth_processing(self):
        with mock.patch('distronode_base.jwt_consumer.common.auth.JWTCommonAuth.process_rbac_permissions') as mp:
