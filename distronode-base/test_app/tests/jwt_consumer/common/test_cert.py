...
class TestJWTCert:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("ftp://www.distronode.com", 'Unable to determine how to handle ftp://www.distronode.com to get key'),
    ...
    def test_get_decryption_key_random_settings(self, setting, string):
        with override_settings(DISTRONODE_BASE_JWT_KEY=setting):
            cert = JWTCert()
    ...
    @override_settings(DISTRONODE_BASE_JWT_KEY='file:///gibberish')
    def test_get_decryption_key_file_exceptions(self, error, exception_class, tmp_path_factory):
        cert = JWTCert()
        url_parts = urlparse(settings.DISTRONODE_BASE_JWT_KEY)
    ...
    def test_get_decryption_key_file_read(self, remove_newlines, tmp_path_factory, test_encryption_public_key):
        temp_dir = tmp_path_factory.mktemp("distronode_base.jwt_consumer.common.auth")
        ...
        with open(temp_file_name, "w") as f:
            f.write(cert_data)
        with override_settings(DISTRONODE_BASE_JWT_KEY=f'file:{temp_file_name}'):
            cert = JWTCert()
            cert.get_decryption_key(ignore_cache=True)
    ...
    @override_settings(DISTRONODE_BASE_JWT_KEY="http://dne.cuz.junk.redhat.com")
    def test_get_decryption_key_connection_error(self):
        cert = JWTCert()
        with pytest.raises(JWTCertException, match=rf"Failed to connect to {settings.DISTRONODE_BASE_JWT_KEY}.*"):
            cert.get_decryption_key(ignore_cache=True)
            assert cert.key is None and cert.cached is None

    @mock.patch('requests.get', mock.Mock(side_effect=requests.exceptions.Timeout))
    @override_settings(DISTRONODE_BASE_JWT_KEY="http://dne.cuz.junk.redhat.com", DISTRONODE_BASE_JWT_URL_TIMEOUT=1)
    def test_get_decryption_key_url_timeout(self):
        cert = JWTCert()
        with pytest.raises(
            JWTCertException, match=rf"Timed out after {settings.DISTRONODE_BASE_JWT_URL_TIMEOUT} secs when connecting to {settings.DISTRONODE_BASE_JWT_KEY}.*"
        ):
            cert.get_decryption_key(ignore_cache=True)
            assert cert.key is None and cert.cached is None
    ...
    def test_get_decryption_key_url_bad_status_codes(self, status_code, mocked_http):
        with mock.patch('requests.get') as requests_get:
            ...
            with override_settings(DISTRONODE_BASE_JWT_KEY=f"http://someotherurl.com/{status_code}"):
                cert = JWTCert()
                with pytest.raises(JWTCertException, match=f"Failed to get 200 response from the issuer: {status_code}"):
                    cert.get_decryption_key(ignore_cache=True)
                    assert cert.key is None and cert.cached is None

    @override_settings(DISTRONODE_BASE_JWT_KEY="http://someotherurl.com/200_junk/")
    ...
    @override_settings(DISTRONODE_BASE_JWT_KEY="http://someotherurl.com/200_good")
    ...
    @mock.patch('distronode_base.jwt_consumer.common.cache.cache_key', 'expiration_test_jwt_key')
    @override_settings(DISTRONODE_BASE_JWT_KEY="http://someotherurl.com/200_good")
    def test_cache_expiring(self, mocked_http, test_encryption_public_key):
        with mock.patch('requests.get') as requests_get:
            ...
            with override_settings(DISTRONODE_BASE_JWT_CACHE_TIMEOUT_SECONDS=0):
                cert = JWTCert()
                requests_get.side_effect = mocked_http.mocked_get_decryption_key_get_request
                try:
                    cert.get_decryption_key()
