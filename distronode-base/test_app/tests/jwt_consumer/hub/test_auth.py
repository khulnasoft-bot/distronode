...
from distronode_base.jwt_consumer.common.exceptions import InvalidService
from distronode_base.jwt_consumer.hub.auth import HubJWTAuth
...
#    sys.modules['distronode_base.resource_registry.models.resource'] = mocked_Resource
#
#    mock_team = MagicMock()
...
@patch('distronode_base.jwt_consumer.hub.auth.Resource')
@patch('distronode_base.jwt_consumer.hub.auth.ContentType')
def test_hub_jwt_orgs_teams_groups_memberships(mock_contenttype, mock_resource):
    ...
    def get_resource_content(*args, **kwargs):
        for obj in [testuser, testorg, testteam]:
            if str(obj.resource.distronode_id) == kwargs['distronode_id']:
                resource = MagicMock()
    ...
    auth.common_auth.token = {
        ...
        "objects": {
            "organization": [
                {
                    "distronode_id": str(testorg.resource.distronode_id),
                    "id": testorg.id,
                    "name": testorg.name,
                }
            ],
            "team": [
                {
                    "distronode_id": str(testteam.resource.distronode_id),
                    "name": testteam.name,
                    "org": 0,
                }
            ],
