...
from distronode_base.authentication.models import Authenticator, AuthenticatorMap
from distronode_base.authentication.views import AuthenticatorViewSet
...
def test_app_ignore_field(admin_api_client):
    base_list = tuple(settings.DISTRONODE_BASE_REST_FILTERS_RESERVED_NAMES)
    with override_settings(DISTRONODE_BASE_REST_FILTERS_RESERVED_NAMES=base_list + ('foofield',)):
        url = get_relative_url('organization-list')
        response = admin_api_client.get(url, data={'foofield': 'bar'})
