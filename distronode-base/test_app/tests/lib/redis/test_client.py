...
from distronode_base.lib.constants import STATUS_DEGRADED, STATUS_FAILED, STATUS_GOOD
from distronode_base.lib.redis.client import (
...
def test_redis_cluster_mget_success():
    ...
    with mock.patch('redis.cluster.RedisCluster.__init__', return_value=None):
        with mock.patch('distronode_base.lib.redis.client.RedisCluster.mget') as mget_mock:
            with mock.patch('distronode_base.lib.redis.client.RedisCluster.mget_nonatomic') as mget_nonatomic_mock:
...
def test_redis_cluster_mget_raises_random_exception():
    # We have to mock init because the redis cluster will attempt to connect by default
    with mock.patch('redis.cluster.RedisCluster.__init__', return_value=None):
        with mock.patch('distronode_base.lib.redis.client.RedisCluster.mget', side_effect=Exception('This is not the exception you are looking for')) as mget_mock:
            with mock.patch('distronode_base.lib.redis.client.RedisCluster.mget_nonatomic') as mget_nonatomic_mock:
                cluster = DABRedisCluster()
...
def test_redis_cluster_mget_raises_expected_exception_with_wrong_message():
    ...
    with mock.patch('redis.cluster.RedisCluster.__init__', return_value=None):
        with mock.patch(
            'distronode_base.lib.redis.client.RedisCluster.mget', side_effect=RedisClusterException('This is not the exception you are looking for')
        ) as mget_mock:
            with mock.patch('distronode_base.lib.redis.client.RedisCluster.mget_nonatomic') as mget_nonatomic_mock:
                cluster = DABRedisCluster()
                with pytest.raises(Exception):
...
def test_redis_cluster_mget_raises_expected_exception():
    ...
    with mock.patch('redis.cluster.RedisCluster.__init__', return_value=None):
        with mock.patch(
            'distronode_base.lib.redis.client.RedisCluster.mget', side_effect=RedisClusterException('MGET - all keys must map to the same key slot')
        ) as mget_mock:
            with mock.patch('distronode_base.lib.redis.client.RedisCluster.mget_nonatomic') as mget_nonatomic_mock:
                cluster = DABRedisCluster()
                cluster.mget('a')
...
def test_parse_url_short_circuit():
    with mock.patch('distronode_base.lib.redis.client.urlparse') as m:
        get_redis_client()
        m.assert_not_called()
...
def test_redis_get_status_timeouts(socket_timeout, socket_connect_timeout):
    with mock.patch('distronode_base.lib.redis.client.RedisClientGetter.get_client', side_effect=Exception('stop here')) as m:
        kwargs = {}
        if socket_timeout:
...
def test_get_redis_status_standalone():
    with mock.patch('distronode_base.lib.redis.client.DABRedis.execute_command', return_value='PONG'):
...
@mock.patch('distronode_base.lib.redis.client.DABRedisCluster.cluster_nodes', return_value='there')
...
def test_get_redis_status_cluster(cluster_nodes, nm_initialize, cp_initialize, cluster_state, expected_status, node_status):
    with mock.patch('distronode_base.lib.redis.client.DABRedisCluster.cluster_info', return_value={'cluster_state': cluster_state}):
        with mock.patch('distronode_base.lib.redis.client.determine_cluster_node_status', return_value=node_status):
...
def test_get_redis_status_exception():
    ...
    with mock.patch('distronode_base.lib.redis.client.DABRedis.execute_command', side_effect=Exception(exception_message)):
        response = get_redis_status('redis://localhost', **{'mode': 'standalone'})
...
def test_redis_timeout():
    with mock.patch('distronode_base.lib.redis.client.get_redis_client', side_effect=Exception("raised")):
...
def test_redis_standalone_removes_cluster_settings():
    ...
    with mock.patch('redis.Redis.__init__', return_value=None) as rm:
        from distronode_base.lib.redis.client import RedisClientGetter
...
def test_flushdb_warns_about_async(expected_log):
    ...
    with mock.patch('redis.Redis.__init__', return_value=None):
        ...
        with expected_log('distronode_base.lib.redis.client.logger', 'warning', "DABRedis clients implement an ACL friendly FLUSHDB which can not be async"):
