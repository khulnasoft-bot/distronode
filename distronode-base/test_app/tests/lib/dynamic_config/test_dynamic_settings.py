...
def test_swagger_enabled():
    additional_settings = dedent(
        '''
        INSTALLED_APPS = ['distronode_base.api_documentation']
    '''
    )
    updated_settings = get_updated_settings(additional_settings)
...
def test_authentication_with_backends():
    additional_config = dedent(
        ...
        INSTALLED_APPS = ['distronode_base.authentication']
    '''
    )
    updated_settings = get_updated_settings(additional_config)

    # Ensure that we add our AUTHENTICATION_BACKEND
    assert 'distronode_base.authentication.backend.DistronodeBaseAuth' in updated_settings['AUTHENTICATION_BACKENDS']
    # This also tests some other "happy paths"
    assert 'distronode_base.authentication.middleware.AuthenticatorBackendMiddleware' in updated_settings['MIDDLEWARE']
    assert 'distronode_base.authentication.session.SessionAuthentication' in updated_settings['REST_FRAMEWORK']['DEFAULT_AUTHENTICATION_CLASSES']
    assert 'distronode_base.authentication.authenticator_plugins' in updated_settings['ANSIBLE_BASE_AUTHENTICATOR_CLASS_PREFIXES']
...
def test_append_middleware():
    ...
    updated_settings = get_updated_settings(additional_config)
    assert 'distronode_base.authentication.middleware.AuthenticatorBackendMiddleware' == updated_settings['MIDDLEWARE'][-1]
...
def test_insert_middleware():
    ...
    updated_settings = get_updated_settings(additional_config)
    assert 'distronode_base.authentication.middleware.AuthenticatorBackendMiddleware' == updated_settings['MIDDLEWARE'][2]
...
def test_dont_update_class_prefixes():
    ...
    updated_settings = get_updated_settings(additional_config)
    assert 'distronode_base.authentication.authenticator_plugins' not in updated_settings['ANSIBLE_BASE_AUTHENTICATOR_CLASS_PREFIXES']
...
def test_filtering():
    ...
    additional_config = dedent(
        '''
        INSTALLED_APPS = ['distronode_base.rest_filters']
        REST_FRAMEWORK = {'something': 'else'}
    '''
    )
    updated_settings = get_updated_settings(additional_config)
    assert 'distronode_base.rest_filters.rest_framework.type_filter_backend.TypeFilterBackend' in updated_settings['REST_FRAMEWORK']['DEFAULT_FILTER_BACKENDS']
