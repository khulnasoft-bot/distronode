...
def test_save_attribution_with_system_username_set_but_nonexistent_as_false(system_user, organization, expected_log):
    expected_log = partial(expected_log, "distronode_base.lib.abstract_models.common.logger")
...
def test_related_fields_view_resolution(shut_up_logging):
    ...
    with patch('distronode_base.lib.abstract_models.common.get_relative_url') as reverse:
...
def test_related_view_log_message(debug_mode, not_called, expected_log):
    ...
    with override_settings(DEBUG=debug_mode):
        with expected_log('distronode_base.lib.abstract_models.common.logger', 'error', 'but view was missing', assert_not_called=not_called):
...
def test_related_view_ignore_m2m_relations(ignore_relation, admin_user):
    rf = RequestFactory()
    request = rf.get('/')
    with patch('distronode_base.lib.abstract_models.common.get_relative_url', return_value='https://www.example.com/user'):
        if ignore_relation:
...
def test_jsonfield_can_be_encrypted(admin_user, local_authenticator):
    ...
    from distronode_base.lib.utils.encryption import Fernet256

    encryptor = Fernet256()
    encrypted_extra_data = encryptor.encrypt_string(extra_data)

    with patch('distronode_base.lib.utils.encryption.distronode_encryption.encrypt_string', return_value=encrypted_extra_data) as m:
