...
from distronode_base.lib.cache.fallback_cache import FALLBACK_CACHE, PRIMARY_CACHE, DABCacheWithFallback
...
cache_settings = {
    'default': {
        'BACKEND': 'distronode_base.lib.cache.fallback_cache.DABCacheWithFallback',
    },
    'primary': {
...
def test_dead_primary():
    ...
    logging.getLogger('distronode_base.cache.fallback_cache').setLevel(logging.CRITICAL)
...
def test_ensure_initialization_wont_happen_twice():
    with mock.patch('distronode_base.lib.cache.fallback_cache.ThreadPoolExecutor') as tfe:
        # Remove singleton instance
        DABCacheWithFallback._instance = None
...
def test_all_methods_are_overwritten(method):
    with mock.patch('distronode_base.lib.cache.fallback_cache.DABCacheWithFallback._op_with_fallback') as owf:
        cache = DABCacheWithFallback(None, {})
        if method == 'clear':
