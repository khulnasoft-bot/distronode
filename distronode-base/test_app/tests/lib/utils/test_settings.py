...
from distronode_base.lib.utils.settings import SettingNotSetException, get_setting, is_aoc_instance
...
@mock.patch("distronode_base.lib.utils.settings.logger")
...
def test_is_aoc_instance(setting_value, expected_value, expected_log_output, expected_log):
    with override_settings(ANSIBLE_BASE_MANAGED_CLOUD_INSTALL=setting_value):
        with expected_log(
            'distronode_base.lib.utils.settings.logger',
            'error',
...
def test_fallback_cache():
    ...
    fallback_cache = {
        'default': {'BACKEND': 'distronode_base.lib.cache.fallback_cache.DABCacheWithFallback'},
