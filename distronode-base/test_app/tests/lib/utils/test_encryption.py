...
from distronode_base.lib.utils.encryption import ENCRYPTION_METHOD, Fernet256

logger = 'distronode_base.lib.utils.encryption.logger'
...
def test_fernet256_decrypt_raise_exception_on_json_decode_error(expected_log):
    ...
    with mock.patch('distronode_base.lib.utils.encryption.json.loads', side_effect=exception):
        with expected_log('distronode_base.lib.utils.encryption.logger', 'exception', 'Failed to'):
            fernet = Fernet256()
            input_string = "test"
            encrypted_string = fernet.encrypt_string(input_string)
