...
from distronode_base.authentication.views.authenticator import AuthenticatorViewSet
from distronode_base.lib.utils.views.distronode_base import DistronodeBaseView


@pytest.fixture
def view_with_headers():
    view = DistronodeBaseView()
    view.headers = {}
    return view
...
def test_distronode_base_view_with_no_settings(view_with_headers, mock_request, default_headers):
...
def test_distronode_base_view_parent_view(caplog, setting, log_message, default_parent):
    with override_settings(ANSIBLE_BASE_CUSTOM_VIEW_PARENT=setting):
        with caplog.at_level(logging.ERROR):
            import distronode_base.lib.utils.views.django_app_api

            importlib.reload(distronode_base.lib.utils.views.django_app_api)
            from distronode_base.lib.utils.views.django_app_api import DistronodeBaseDjangoAppApiView

            if default_parent:
                assert issubclass(DistronodeBaseDjangoAppApiView, DistronodeBaseView)
            else:
                assert issubclass(DistronodeBaseDjangoAppApiView, DummyView)
            assert log_message in caplog.text


def test_distronode_base_view_parent_view_exception(caplog):
    with override_settings(ANSIBLE_BASE_CUSTOM_VIEW_PARENT='does.not.exist'):
        with caplog.at_level(logging.ERROR):
            with mock.patch('distronode_base.lib.utils.settings.get_from_import', side_effect=ImportError("Test Exception")):
                import distronode_base.lib.utils.views.django_app_api
...
class DeprecatedView(DistronodeBaseView):
    deprecated = True
    headers = {}


def test_distronode_base_view_deprecated_view(view_with_headers, mock_request, default_headers):
    initial_response = HttpResponseBase()
    view = DeprecatedView()
...
def test_distronode_base_view_time_header(view_with_headers, mock_request):
    initial_response = HttpResponseBase()
...
def test_distronode_base_view_version(view_with_headers, mock_request, admin_user, setting, value, log_message, caplog):
...
def test_distronode_base_view_filter_backends():
...
def test_distronode_base_view_filter_backends_custom_settings():
...
def test_check_service_token_auth(user_type, allow_service_token, request_auth, expected_result, system_user, random_user):
    from distronode_base.lib.utils.views.permissions import check_service_token_auth
...
def test_is_superuser_has_permission(check_service_token_auth_return, user_is_superuser, expected_result, random_user):
    from distronode_base.lib.utils.views.permissions import IsSuperuser
    ...
    if user_is_superuser is None:
        ...
        random_user.save()
        request.user = random_user
    with mock.patch('distronode_base.lib.utils.views.permissions.check_service_token_auth', return_value=check_service_token_auth_return):
