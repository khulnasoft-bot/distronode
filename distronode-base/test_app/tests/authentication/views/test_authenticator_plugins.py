...
def test_plugin_authenticator_view(admin_api_client):
    ...
    assert 'distronode_base.authentication.authenticator_plugins.ldap' in auth_types
    assert 'distronode_base.authentication.authenticator_plugins.local' in auth_types
...
def test_plugin_authenticator_view_import_error(admin_api_client, shut_up_logging, settings):
    ...
    settings.DISTRONODE_BASE_AUTHENTICATOR_CLASS_PREFIXES = [
        "distronode_base.authentication.authenticator_plugins",
        fixture_module,
    ]
