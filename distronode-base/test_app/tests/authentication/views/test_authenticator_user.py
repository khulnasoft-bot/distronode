...
from distronode_base.authentication.views.authenticator_users import get_authenticator_user_view
...
def test_authenticator_user_view_get_parent_view_unset_value(expected_log):
    with override_settings(DISTRONODE_BASE_USER_VIEWSET=None):
        expected_log = partial(expected_log, "distronode_base.authentication.views.authenticator_users.logger")
        with expected_log('debug', 'DISTRONODE_BASE_USER_VIEWSET was not specified'):
...
@pytest.mark.parametrize(
    ...
    (
        ('junk.views'),
        ('distronode_base.authentication.views'),
    ),
)
def test_authenticator_user_view_get_parent_view_bad_value(setting, expected_log):
    with override_settings(DISTRONODE_BASE_USER_VIEWSET=setting):
        expected_log = partial(expected_log, "distronode_base.authentication.views.authenticator_users.logger")
        with expected_log('error', 'DISTRONODE_BASE_USER_VIEWSET was not an APIView'):
            view = get_authenticator_user_view()
            assert view is None


def test_authenticator_user_view_get_parent_view_good_value():
    with override_settings(DISTRONODE_BASE_USER_VIEWSET='distronode_base.authentication.views.AuthenticatorViewSet'):
        view = get_authenticator_user_view()
