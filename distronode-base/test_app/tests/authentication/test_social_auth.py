...
from distronode_base.authentication.social_auth import (
...
@mock.patch("distronode_base.authentication.social_auth.logger")
@override_settings(DISTRONODE_BASE_SOCIAL_AUTH_STRATEGY_SETTINGS_FUNCTION='does.not.exist')
def test_authenticator_strategy_init_fail_to_load_function(logger):
    _ = AuthenticatorStrategy(storage=AuthenticatorStorage())
    logger.error.assert_any_call(SubstringMatcher(f"Failed to run {settings.DISTRONODE_BASE_SOCIAL_AUTH_STRATEGY_SETTINGS_FUNCTION} to get additional settings"))


@mock.patch("distronode_base.authentication.social_auth.logger")
@override_settings(DISTRONODE_BASE_SOCIAL_AUTH_STRATEGY_SETTINGS_FUNCTION='test_app.tests.authentication.test_social_auth.set_settings')
def test_authenticator_strategy_init_load_function(logger):
    strategy = AuthenticatorStrategy(storage=AuthenticatorStorage())
    logger.debug.assert_any_call(f"Attempting to load social settings from {settings.DISTRONODE_BASE_SOCIAL_AUTH_STRATEGY_SETTINGS_FUNCTION}")
...
@mock.patch("distronode_base.authentication.social_auth.get_fully_qualified_url")
@mock.patch("distronode_base.authentication.social_auth.generate_authenticator_slug")
...
@mock.patch("distronode_base.authentication.utils.claims.update_user_claims")
