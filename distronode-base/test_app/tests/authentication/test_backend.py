...
import distronode_base.authentication.backend as backend
from distronode_base.authentication.models import Authenticator
from distronode_base.authentication.social_auth import SOCIAL_AUTH_PIPELINE_FAILED_STATUS
...
def test_authenticator_backends_import_error(ldap_authenticator):
    ...
    with mock.patch('distronode_base.authentication.backend.get_authenticator_plugin', side_effect=ImportError("Test Exception")):
...
def test_authenticator_backends_cache(ldap_authenticator):
    ...
    with mock.patch('distronode_base.authentication.backend.get_authenticator_plugin', side_effect=OSError("Test Exception")):
...
def test_authenticate(request, local_authenticator, github_enterprise_authenticator, random_user, random_user_1, github_auth_retval, is_active, expected):
    with mock.patch(
        "distronode_base.authentication.backend.get_authentication_backends",
        ...
        auth_return = backend.DistronodeBaseAuth().authenticate(None)
...
def test_authentication_exception(expected_log):
    ...
    with mock.patch(
        ...
        with expected_log('distronode_base.authentication.backend.logger', "exception", "Exception raised while trying to authenticate with"):
            backend.DistronodeBaseAuth().authenticate(None)
