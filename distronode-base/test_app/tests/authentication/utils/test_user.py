...
from distronode_base.authentication.utils.user import can_user_change_password
...
def test_can_user_change_password_import_error(local_authenticator, random_user):
    AuthenticatorUser.objects.get_or_create(uid=random_user.username, user=random_user, provider=local_authenticator)
    with mock.patch('distronode_base.authentication.utils.user.get_authenticator_plugin', side_effect=ImportError("Test Exception")):
