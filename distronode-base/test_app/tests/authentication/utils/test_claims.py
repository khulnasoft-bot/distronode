...
from distronode_base.authentication.models import AuthenticatorMap, AuthenticatorUser
from distronode_base.authentication.utils import claims
...
@mock.patch("distronode_base.authentication.utils.claims.logger")
...
def test_create_claims_revoke(local_authenticator_map, process_function, triggers, revoke, granted, default_rbac_roles_claims):
    ...
    with mock.patch(f"distronode_base.authentication.utils.claims.{process_function}", return_value=claims.TriggerResult.SKIP):
        res = claims.create_claims(authenticator, "username", {}, [])
