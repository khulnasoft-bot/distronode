...
from distronode_base.rbac.models import ObjectRole, RoleEvaluation
from distronode_base.resource_registry.models import Resource


@pytest.mark.django_db
def test_user_assignment_distronode_id(admin_api_client, inv_rd, rando, inventory):
    resource = Resource.objects.get(object_id=rando.pk, content_type=ContentType.objects.get_for_model(rando).pk)
    url = get_relative_url('roleuserassignment-list')
    data = dict(role_definition=inv_rd.id, content_type='aap.inventory', user_distronode_id=str(resource.distronode_id), object_id=inventory.id)
...
@pytest.mark.django_db
def test_team_assignment_distronode_id(admin_api_client, inv_rd, team, inventory, member_rd, rando):
    ...
    data = dict(role_definition=inv_rd.id, content_type='aap.inventory', team_distronode_id=str(resource.distronode_id), object_id=inventory.id)
    response = admin_api_client.post(url, data=data, format="json")
    assert response.status_code == 201, response.data
...
def test_assignment_id_validation(admin_api_client, inv_rd, team, inventory, rando, actor):
    ...
    test_fields = (actor, f'{actor}_distronode_id')
    ...
    data[f'{actor}_distronode_id'] = str(resource.distronode_id)
...
@pytest.mark.django_db
def test_object_distronode_id_user(admin_api_client, org_inv_rd, rando, inventory, organization):
    resource = Resource.objects.get(object_id=organization.pk, content_type=ContentType.objects.get_for_model(organization).pk)
    url = get_relative_url('roleuserassignment-list')
    data = dict(role_definition=org_inv_rd.id, user=rando.id, object_distronode_id=str(resource.distronode_id))
    response = admin_api_client.post(url, data=data, format="json")
    assert response.status_code == 201, response.data
    assert rando.has_obj_perm(inventory, 'change')
...
@pytest.mark.django_db
def test_invalid_distronode_id(admin_api_client, org_inv_rd, rando):
    url = get_relative_url('roleuserassignment-list')
    bad_distronode_id = f'{uuid4()}'
    data = dict(role_definition=org_inv_rd.id, user=rando.id, object_distronode_id=bad_distronode_id)
    response = admin_api_client.post(url, data=data, format="json")
    assert response.status_code == 400, response.data
    assert 'object does not exist' in response.data['object_distronode_id']
    assert bad_distronode_id in response.data['object_distronode_id']


@pytest.mark.django_db
def test_object_distronode_id_bad_type(admin_api_client, inv_rd, rando, organization):
    """If giving object_id, type is implied from role definition.

    When giving distronode_id that no longer holds true, and user can give any type.
    This expects a validation error when the object type does not match the role type.
    """
    resource = Resource.objects.get(object_id=organization.pk, content_type=ContentType.objects.get_for_model(organization).pk)
    url = get_relative_url('roleuserassignment-list')
    data = dict(role_definition=inv_rd.id, user=rando.id, object_distronode_id=str(resource.distronode_id))
    response = admin_api_client.post(url, data=data, format="json")
    assert response.status_code == 400, response.data
    assert 'organization does not match role type of inventory' in str(response.data['object_distronode_id'])
