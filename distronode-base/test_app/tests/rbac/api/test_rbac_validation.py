...
from distronode_base.lib.utils.auth import get_team_model
...
def test_unregistered_model_type_for_assignment(admin_api_client, inventory, inv_rd, user):
    ...
    # Temporarily abusing user model to test this via distronode_id reference, this testing method may not work forever
    inv_rd.content_type = permission_registry.content_type_model.objects.get_for_model(user)
    inv_rd.save(update_fields=['content_type'])
    r = admin_api_client.post(url, data={'object_distronode_id': str(user.resource.distronode_id), 'user': user.pk, 'role_definition': inv_rd.pk})
    assert r.status_code == 400
