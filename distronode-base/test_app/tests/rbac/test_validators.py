...
@override_settings(DISTRONODE_BASE_ALLOW_CUSTOM_ROLES=False)
...
class TestProhibitedAssignments:
    @override_settings(DISTRONODE_BASE_ALLOW_TEAM_PARENTS=False)
    ...
    @override_settings(DISTRONODE_BASE_ALLOW_TEAM_ORG_PERMS=False)
    ...
    @override_settings(DISTRONODE_BASE_ALLOW_TEAM_ORG_ADMIN=False)
    ...
    @override_settings(DISTRONODE_BASE_ALLOW_SINGLETON_TEAM_ROLES=False)
...
class TestProhibitedRoleDefinitions:
    ...
    @override_settings(DISTRONODE_BASE_ALLOW_CUSTOM_TEAM_ROLES=False)
    ...
    @override_settings(DISTRONODE_BASE_ALLOW_SINGLETON_ROLES_API=False)
...
def test_no_delete_permission_without_change(enabled):
    with override_settings(DISTRONODE_BASE_DELETE_REQUIRE_CHANGE=enabled):
...
def test_no_change_permission_without_view(enabled):
    with override_settings(DISTRONODE_BASE_ROLES_REQUIRE_VIEW=enabled):
        catching_context = pytest.raises(ValidationError) if enabled else contextlib.nullcontext()
        with catching_context as exc:
            RoleDefinition.objects.create_from_permissions(
                name='anything',
