...
export class SettingsManager {
  ...
  public async initialize(): Promise<void> {
    ...
    const eeSettings = vscode.workspace.getConfiguration(
      "distronode.executionEnvironment",
    );
    const lightSpeedSettings =
      vscode.workspace.getConfiguration("distronode.lightspeed");
    const playbookSettings =
      vscode.workspace.getConfiguration("distronode.playbook");
    this.settings = {
      activationScript: (await distronodeSettings.get(
        "python.activationScript",
      )) as string,
      interpreterPath: (await distronodeSettings.get(
      ...
      executionEnvironment: {
        ...
        image: eeSettings.get(
          "image",
          "ghcr.io/distronode/community-distronode-dev-tools:latest",
        ),
        pull: {
      ...
      lightSpeedService: {
        ...
        URL: lightSpeedSettings.get("URL", "https://c.ai.distronode.redhat.com"),
        suggestions: {
