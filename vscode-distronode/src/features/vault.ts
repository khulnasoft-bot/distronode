...
/* local */
import * as utilDistronodeCfg from "./utils/distronodeCfg";
...
async function askForVaultId(distronodeCfg: utilDistronodeCfg.DistronodeVaultConfig) {
  const vaultId = "default";

  const identityList = distronodeCfg.defaults.vault_identity_list
    ?.split(",")
...
function displayInvalidConfigError(): void {
  vscode.window.showErrorMessage(
    "no valid distronode vault config found, cannot de/-encrypt",
  );
}

function distronodeVaultPath(config: vscode.WorkspaceConfiguration): string {
  const path = config.get("distronode.path");
  if (typeof path === "string") {
    return `${path || "distronode"}-vault`;
  }
  return "distronode-vault";
}
...
export const toggleEncrypt = async (): Promise<void> => {
  ...
  const config = vscode.workspace.getConfiguration("distronode");
  const doc = editor.document;

  // Read `distronode.cfg` or environment variable
  const rootPath: string | undefined = utilDistronodeCfg.getRootPath(
    editor.document.uri,
  );
  const distronodeConfig = await utilDistronodeCfg.getDistronodeCfg(rootPath);
  ...
  // Extract `distronode-vault` password

  console.log(`Getting vault keyfile from ${distronodeConfig.path}`);
  vscode.window.showInformationMessage(
    `Getting vault keyfile from ${distronodeConfig.path}`,
  );

  const text = editor.document.getText(selection);

  const useVaultIDs = !!distronodeConfig.defaults.vault_identity_list;
  ...
  if (text) {
    ...
    if (type === "plaintext") {
      ...
      const vaultId: string | undefined = useVaultIDs
        ? await askForVaultId(distronodeConfig)
        : undefined;
...
const getInlineTextType = (text: string) => {
  ...
  return text.trim().startsWith("$DISTRONODE_VAULT;") ? "encrypted" : "plaintext";
};

// Returns whether the file is encrypted or in plain text.
const getTextType = (text: string) => {
  return text.indexOf("$DISTRONODE_VAULT;") === 0 ? "encrypted" : "plaintext";
};
...
const pipeTextThroughCmd = (
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code !== 0) {
        console.log(`error when running distronode-vault: ${errorText}`);
        reject(errorText);
      } else {
        resolve(outputText);
      }
...
const encryptText = (
  ...
  let cmd = `${distronodeVaultPath(config)} encrypt_string`;
  if (vaultId) {
...
const decryptText = (
  ...
  const cmd = `${distronodeVaultPath(config)} decrypt`;
  return pipeTextThroughCmd(text, rootPath, cmd);
};
...
const encryptFile = (
  ...
  const cmd = vaultId
    ...
    : `${distronodeVaultPath(config)} encrypt "${f}"`;
...
const decryptFile = (
  ...
  const cmd = `${distronodeVaultPath(config)} decrypt "${f}"`;
