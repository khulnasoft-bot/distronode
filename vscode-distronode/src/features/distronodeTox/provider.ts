...
export class DistronodeToxProvider implements vscode.TaskProvider {
  static readonly toxType = TOX_TYPE;
  static readonly distronodeToxIni = DISTRONODE_TOX_FILE_NAME;
  private toxPromise: Thenable<vscode.Task[]> | undefined = undefined;

  constructor(workspaceRoot: string) {
    const pattern = path.join(workspaceRoot, DistronodeToxProvider.distronodeToxIni);
  ...
  public resolveTask(_task: vscode.Task): vscode.Task | undefined {
    const testenv = _task.definition.distronode;
    if (testenv) {
      ...
      return new vscode.Task(
        ...
        definition.distronode,
        DistronodeToxProvider.toxType,
        new vscode.ShellExecution(
          `${DISTRONODE_TOX_RUN_COMMAND} ${definition.distronode} --distronode --conf tox-distronode.ini`,
        ),
      );
    }
    return undefined;
...
async function getToxTestTasks(): Promise<vscode.Task[]> {
  ...
  for (const workspaceFolder of workspaceFolders) {
    ...
    const toxFile = path.join(folderString, DistronodeToxProvider.distronodeToxIni);
    try {
    ...
    if (toxTestenvs !== undefined) {
      for (const toxTestenv of toxTestenvs) {
        ...
        const kind: ToxTaskDefinition = {
          type: DistronodeToxProvider.toxType,
          testenv: toxTestenv,
        };
        ...
        const task = new vscode.Task(
          ...
          DistronodeToxProvider.toxType,
          new vscode.ShellExecution(
            `${DISTRONODE_TOX_RUN_COMMAND} ${toxTestenv} --distronode --conf tox-distronode.ini`,
          ),
        );
