...
import { DISTRONODE_TOX_FILE_NAME } from "./constants";

export class DistronodeToxController {
  public controller: vscode.TestController;

  constructor() {
    this.controller = vscode.tests.createTestController(
      "distronodeToxController",
      "Distronode Tox",
    );
  }
  ...
  public create() {
    ...
    for (const document of vscode.workspace.textDocuments) {
      this.parseTestsInDistronodeToxFile(document);
    }

    // Check for tox.ini files when a new document is opened or saved.
    vscode.workspace.onDidOpenTextDocument(this.parseTestsInDistronodeToxFile);
    vscode.workspace.onDidSaveTextDocument(this.parseTestsInDistronodeToxFile);
  ...
  parseTestsInDistronodeToxFile = async (
    document: vscode.TextDocument,
    filename: string = DISTRONODE_TOX_FILE_NAME,
  ) => {
    if (
