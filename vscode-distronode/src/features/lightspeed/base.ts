...
import {
  ...
  DISTRONODE_LIGHTSPEED_AUTH_NAME,
} from "./utils/webUtils";
import { LightspeedStatusBar } from "./statusBar";
import { IVarsFileContext } from "../../interfaces/lightspeed";
import { getCustomRolePaths, getCommonRoles } from "../utils/distronode";
...
export class LightSpeedManager {
  ...
  public distronodeVarFilesCache: IVarsFileContext = {};
  public distronodeRolesCache: IWorkSpaceRolesContext = {};
  public distronodeIncludeVarsCache: IIncludeVarsContext = {};
  ...
  constructor(
    ...
    // initiate the OAuth service for Distronode Lightspeed
    ...
    // create workspace context for distronode roles
    this.setContext();
  ...
  public async reInitialize(): Promise<void> {
    const lightspeedSettings = <LightSpeedServiceSettings>(
      vscode.workspace.getConfiguration("distronode").get("lightspeed")
    );
    ...
    if (!lightspeedEnabled) {
      ...
      if (lightspeedSettings.suggestions.enabled) {
        const githubConfig = (<unknown>(
          ...
          copilot: { enable?: { distronode?: boolean } };
        };
        const copilotEnableForDistronode = githubConfig?.copilot?.enable?.distronode;
        if (copilotEnableForDistronode) {
          vscode.window.showInformationMessage(
            "Please disable GitHub Copilot for Distronode Lightspeed file types to use Distronode Lightspeed.",
          );
        }
      }
    }
  ...
  private resetContext(): void {
    this.distronodeVarFilesCache = {};
    this.distronodeRolesCache = {};
  }
  ...
  private setCustomWhenClauseContext(): void {
    ...
    vscode.commands.executeCommand(
      "setContext",
      "redhat.distronode.enableExperimentalFeatures",
      false,
    );
  }
}
