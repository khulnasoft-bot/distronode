...
import { DISTRONODE_LIGHTSPEED_API_TIMEOUT } from "../../definitions/constants";
...
export class LightSpeedAPI {
  ...
  private async getApiInstance(): Promise<AxiosInstance | undefined> {
    ...
    if (authToken === undefined) {
      console.error("Distronode Lightspeed authentication failed.");
      return;
    }
    const headers = {
  ...
  public async getData(urlPath: string): Promise<any> {
    ...
    if (axiosInstance === undefined) {
      console.error("Distronode Lightspeed instance is not initialized.");
      return;
    }
    const response = await axiosInstance.get(urlPath, {
      timeout: DISTRONODE_LIGHTSPEED_API_TIMEOUT,
    });
  ...
  public async completionRequest(
    ...
    console.log(
      `[distronode-lightspeed] Completion request sent to lightspeed: ${JSON.stringify(
        inputData,
      )}`,
    );
    ...
    try {
      ...
      const requestData = {
        ...
        metadata: {
          ...inputData.metadata,
          distronodeExtensionVersion: this._extensionVersion,
        },
      };
      ...
      if (
        ...
        vscode.window.showInformationMessage(
          "Distronode Lightspeed does not have a suggestion for this input. Try changing your prompt, or contact your administrator with Suggestion Id " +
      ...
      console.log(
        `[distronode-lightspeed] Completion response: ${JSON.stringify(
          response.data,
        )}`,
      );
      return response.data;
  ...
  public async feedbackRequest(
    ...
    const requestData = {
      ...inputData,
      metadata: { distronodeExtensionVersion: this._extensionVersion },
    };
    console.log(
      `[distronode-lightspeed] Feedback request sent to lightspeed: ${JSON.stringify(
        requestData,
      )}`,
    );
    try {
  ...
  public async contentMatchesRequest(
    ...
    if (!(await this.lightspeedAuthenticatedUser.isAuthenticated())) {
      vscode.window.showErrorMessage(
        "User not authenticated to use Distronode Lightspeed.",
      );
    ...
    try {
      ...
      console.log(
        `[distronode-lightspeed] Content Match request sent to lightspeed: ${JSON.stringify(
          requestData,
        )}`,
      );
      const response = await axiosInstance.post(
  ...
  public async explanationRequest(
    ...
    try {
      ...
      console.log(
        `[distronode-lightspeed] Explanation request sent to lightspeed: ${JSON.stringify(
          requestData,
        )}`,
      );
      const response = await axiosInstance.post(
        ...
        {
          timeout: DISTRONODE_LIGHTSPEED_API_TIMEOUT,
          signal: AbortSignal.timeout(DISTRONODE_LIGHTSPEED_API_TIMEOUT),
        },
      );
      return response.data;
    } catch (error) {
  ...
  public async roleGenerationRequest(
    ...
    try {
      ...
      console.log(
        `[distronode-lightspeed] Role Generation request sent to lightspeed: ${JSON.stringify(
          requestData,
        )}`,
      );
      const response = await axiosInstance.post(
