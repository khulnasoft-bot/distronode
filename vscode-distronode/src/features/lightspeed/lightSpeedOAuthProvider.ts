  ...
  implements AuthenticationProvider, Disposable
{
  ...
  public initialize() {
    if (this._disposable) {
      console.log(
        "[distronode-lightspeed-oauth] Auth provider already registered",
      );
      return;
    }
  ...
  public async createSession(scopes: string[]): Promise<LightspeedAuthSession> {
    try {
      ...
      if (!account) {
        throw new Error(`Distronode Lightspeed login failure`);
      }
      ...
      console.log("[distronode-lightspeed-oauth] Session created...");

      return session;
    } catch (e) {
      console.error(
        `[distronode-lightspeed-oauth] Distronode Lightspeed sign in failed: ${e}`,
      );
      throw e;
    }
  }
  ...
  public async dispose() {
    if (this._disposable) {
      ...
      console.log("[distronode-lightspeed-oauth] Disposing auth provider");
      await this._disposable.dispose();
      this._disposable = undefined;
    }
  }

  /* Log in to the Distronode Lightspeed auth service */
  private async login(scopes: string[] = []) {
    console.log("[distronode-lightspeed-oauth] Logging in...");
    ...
    if (!base_uri) {
      throw new Error(
        "Please enter the Distronode Lightspeed URL under the Distronode Lightspeed settings!",
      );
    }
    ...
    const account = await window.withProgress(
      {
        title: "Waiting for authentication redirect from Distronode Lightspeed",
        ...
        Promise.race([
          ...
          new Promise<OAuthAccount>((_, reject) => {
            setTimeout(() => {
              reject(
                new Error(
                  "Cancelling the Distronode Lightspeed login after 60s. Try again.",
                ),
              );
            }, LIGHTSPEED_SERVICE_LOGIN_TIMEOUT);
          }),
  ...
  /* Handle the redirect to VS Code (after sign in from the Distronode Lightspeed auth service) */
  private handleUriForCode: (
    ...
    () => async (uri, resolve, reject) => {
      ...
      if (!code) {
        reject(
          new Error(
            "No code received from the Distronode Lightspeed OAuth Server",
          ),
        );
        return;
      }
  ...
  private async requestOAuthAccountFromCode(
    ...
    console.log(
      "[distronode-lightspeed-oauth] Sending request for access token...",
    );
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        console.error(
          "[distronode-lightspeed-oauth] error message: ",
          error.message,
        );
        /* istanbul ignore next */
        console.error(
          "[distronode-lightspeed-oauth] error response data: ",
          error.response?.data,
        );
        throw new Error("An unexpected error occurred");
      } else {
        console.error("[distronode-lightspeed-oauth] unexpected error: ", error);
        throw new Error("An unexpected error occurred");
      }
    }
  }
  ...
  private async requestTokenAfterExpiry(
    ...
    console.log(
      "[distronode-lightspeed-oauth] Sending request for a new access token...",
    );
    ...
    const account = await window.withProgress(
      ...
      async () => {
        return axios
          ...
          .catch((error) => {
            console.error(
              `[distronode-lightspeed-oauth] Request token failed with error: ${error}`,
            );
            return;
          });
      },
  ...
  public async refreshAccessToken(session: AuthenticationSession) {
    console.log("[distronode-lightspeed-oauth] Getting access token...");
    ...
    console.log("[distronode-lightspeed-oauth] Account found");
    ...
    if (timeNow >= currentAccount["expiresAtTimestampInSeconds"] - GRACE_TIME) {
      // get new token
      console.log(
        "[distronode-lightspeed-oauth] Distronode Lightspeed token expired. Getting new token...",
      );

      const result = await this.requestTokenAfterExpiry(currentAccount);
      console.log(
        "[distronode-lightspeed-oauth] New Distronode Lightspeed token received.",
      );

      if (!result) {
        await this.removeSession(sessionId);
        const selection = await window.showWarningMessage(
          "Your Distronode Lightspeed session has expired.\n",
          "Reconnect",
        );
  ...
  private async getUserInfo(token: string) {
    console.log(
      "[distronode-lightspeed-oauth] Sending request for logged-in user info...",
    );
