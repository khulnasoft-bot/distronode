...
import { getDistronodeFileType } from "../utils/distronode";
...
const onLightspeedIsDisabled: CallbackEntry = async function (
  ...
  console.debug("[distronode-lightspeed] Distronode Lightspeed is disabled.");
...
const onLightspeedURLMisconfigured: CallbackEntry = async function (
  ...
  vscode.window.showErrorMessage(
    "Distronode Lightspeed URL is empty. Please provide a URL.",
  );
  suggestionDisplayed.reset();
  return [];
};
...
function loadFile(inlinePosition: InlinePosition): DocumentInfo {
  ...
  let parsedDistronodeDocument = undefined;
  ...
  const distronodeFileType: IDistronodeFileType = getDistronodeFileType(
    documentFilePath,
    documentContent,
  );
  ...
  try {
    ...
    vscode.window.showErrorMessage(
      `Distronode Lightspeed expects valid YAML syntax to provide inline suggestions. Error: ${err}`,
    );
    throw err;
  }

  return {
    distronodeFileType: distronodeFileType,
    documentContent: documentContent,
    documentDirPath: documentDirPath,
    documentFilePath: documentFilePath,
    documentUri: documentUri,
    parsedDistronodeDocument: parsedDistronodeDocument,
  };
}
...
async function requestInlineSuggest(
  ...
  console.log(
    `[inline-suggestions] ${getCurrentUTCDateTime().toISOString()}: Completion request sent to Distronode Lightspeed.`,
  );
  ...
  console.log(
    `[inline-suggestions] ${getCurrentUTCDateTime().toISOString()}: Completion response received from Distronode Lightspeed.`,
  );
  if (outputData.model) {
    ...
    if (userProvidedModel && userProvidedModel !== "") {
      if (outputData.model !== userProvidedModel) {
        vscode.window.showWarningMessage(
          `Distronode Lightspeed is using the model ${outputData.model} ` +
...
export async function inlineSuggestionTriggerHandler() {
  ...
  if (vscode.window.activeTextEditor?.document.languageId !== "distronode") {
    return;
  }
...
export async function inlineSuggestionReplaceMarker(position: vscode.Position) {
  ...
  // that causes distronode-lint errors
...
export async function inlineSuggestionTextDocumentChangeHandler(
  ...
  // If the user accepted a suggestion on the widget, distronode.lightspeed.inlineSuggest.accept
  ...
  if (
    ...
    insertTexts &&
    e.document.languageId === "distronode" &&
