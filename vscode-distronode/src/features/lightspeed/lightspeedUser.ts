...
export enum AuthProviderType {
  rhsso = RHSSO_AUTH_ID,
  lightspeed = DISTRONODE_LIGHTSPEED_AUTH_ID,
}
...
export class LightspeedUser {
  ...
  private logAuthProviderDebugHints() {
    ...
    this._logger.info(
      `[distronode-lightspeed-user] Initializing LightspeedUser for Lightspeed URI ${lightspeedUri} and extension host ${this._extensionHost}`,
    );
    this._logger.info(
      `[distronode-lightspeed-user] Red Hat authentication extension is${vscode.extensions.getExtension("redhat.vscode-redhat-account") ? "" : " not"} installed.`,
    );
  }
  ...
  public async getUserInfo(token: string) {
    this._logger.info(
      "[distronode-lightspeed-user] Sending request for logged-in user info...",
    );
    ...
    try {
      ...
      if (
        ...
        this._logger.error(
          `[distronode-lightspeed-user] error message: ${error.message}`,
        );
        console.error(
          "[distronode-lightspeed-user] error response data: ",
          error.response?.data,
        );
        throw new Error(error.message);
      } else {
        this._logger.error(
          `[distronode-lightspeed-user] unexpected error: ${error}`,
        );
  ...
  public async getAuthProviderOrder() {
    ...
    if (preferRHSSO) {
      this._logger.info(
        "[distronode-lightspeed-user] Try Red Hat authentication first per LIGHTSPEED_PREFER_RHSSO_AUTH.",
      );
    ...
    if (this._userType) {
      this._logger.info(
        `[distronode-lightspeed-user] Trying previous auth provider first: ${this._userType}`,
      );
    ...
    if (
      lightspeedUri === "https://c.ai.distronode.redhat.com" &&
      ...
      this._logger.info(
        `[distronode-lightspeed-user] Redirect URI ${redirectUri} is${isSupportedClient ? "" : " not"} supported by Lightspeed auth provider.`,
      );
  ...
  private async _updateUserInformation(
    ...
    try {
      ...
      this._logger.info(
        `[distronode-lightspeed-user] Request for logged-in user info failed: ${error}`,
      );
  ...
  public async rhUserHasSeat(): Promise<boolean | undefined> {
    ...
    if (userDetails === undefined) {
      this._logger.info(
        "[distronode-lightspeed-user] User authentication session not found for seat check.",
      );
      return undefined;
    } else if (userDetails.rhUserHasSeat) {
      this._logger.info(
        `[distronode-lightspeed-user] User "${userDetails.displayNameWithUserType}" has a seat.`,
      );
      return true;
    } else {
      this._logger.info(
        `[distronode-lightspeed-user] User "${userDetails.displayNameWithUserType}" does not have a seat.`,
      );
      return false;
    }
  }
  ...
  public async rhOrgHasSubscription(): Promise<boolean | undefined> {
    ...
    if (userDetails === undefined) {
      this._logger.info(
        "[distronode-lightspeed-user] User authentication session not found for subscription check.",
      );
      return undefined;
    } else if (userDetails.rhOrgHasSubscription) {
      this._logger.info(
        `[distronode-lightspeed-user] User "${userDetails.displayNameWithUserType}" has an Org with a subscription.`,
      );
      return true;
    } else {
      this._logger.info(
        `[distronode-lightspeed-user] User "${userDetails.displayNameWithUserType}" does not have an Org with a subscription.`,
      );
      return false;
    }
  }
  ...
  public async getLightspeedUserAccessToken() {
    this._logger.info("[distronode-lightspeed-user] Getting access token...");

    if (process.env.TEST_LIGHTSPEED_ACCESS_TOKEN) {
      this._logger.info("[distronode-lightspeed-user] Test access token returned");
      return process.env.TEST_LIGHTSPEED_ACCESS_TOKEN;
    }

    if (!this._session) {
      this._logger.info(
        "[distronode-lightspeed-user] Session not found. Returning...",
      );
      const selection = await vscode.window.showWarningMessage(
        "You must be logged in to use Distronode Lightspeed.\n",
        "Login",
      );
      if (selection === "Login") {
    ...
    this._logger.info(
      `[distronode-lightspeed-user] Session found for auth provider "${this._userType}" with scopes "${this._session.scopes}"`,
    );
