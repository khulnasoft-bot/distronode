...
import { watchDistronodeFile } from "./watchers";
...
export function getVarsFilesContext(
  ...
  // Check if the last play in parsedDistronodeDocument has a vars_files key
  const lastObject = parsedDistronodeDocument[parsedDistronodeDocument.length - 1];
  ...
  for (const varFile of varsFiles) {
    ...
    if (!fs.existsSync(expandedVarFilePath)) {
      ...
      if (expandedVarFilePath in lightSpeedManager.distronodeVarFilesCache) {
        delete lightSpeedManager.distronodeVarFilesCache[expandedVarFilePath];
      }
      continue;
    }
    if (expandedVarFilePath in lightSpeedManager.distronodeVarFilesCache) {
      varFilesContext[varFile] =
        lightSpeedManager.distronodeVarFilesCache[expandedVarFilePath];
    } else {
      try {
        ...
        watchDistronodeFile(lightSpeedManager, expandedVarFilePath, "vars_files");
      } catch (err) {
...
export function getIncludeVarsContext(
  ...
  if (distronodeFileType === "playbook") {
    ...
    distronodeFileType === "tasks_in_role" ||
    distronodeFileType === "tasks"
  ) {
    tasksLists = <[]>parsedDistronodeDocument;
  }
    ...
    if ("file" in includeVarsTask) {
      ...
      for (const dir of dirs) {
        ...
        if (!path.isAbsolute(dirPath)) {
          if (distronodeFileType === "tasks_in_role") {
...
export function updateRolesCache(
  ...
  if (dirPath in StandardRolePaths) {
    updateRolesContext(lightSpeedManager.distronodeRolesCache, dirPath, "common");
  } else {
