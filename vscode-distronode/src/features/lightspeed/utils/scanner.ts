...
export class CollectionFinder {
  public cache: DistronodeCollection[];
  ...
  async readCollectionMetaInformation(collectionPath: string) {
    ...
    const j = Promise.any([fromGalaxyFile, fromMANIFESTFile])
      .then((info) => {
        ...
        return new DistronodeCollection(
          collectionPath,
          info["namespace"],
          info["name"],
        );
  ...
  async searchNestedCollections() {
    ...
    const r: DistronodeCollection[] = [];
    for (let i = 0; i < a.length; i++) {
      (await Promise.all(await a[i])).forEach((entry) => {
        if (entry instanceof DistronodeCollection) {
          r.push(entry);
        }
      });
    }
  ...
  public async refreshCache() {
    const compareCollection = (a: DistronodeCollection, b: DistronodeCollection) => {
