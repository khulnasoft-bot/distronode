...
export class ContentMatchesWebview implements vscode.WebviewViewProvider {
  public static readonly viewType = "distronode.lightspeed.trainingMatchPanel";
  ...
  async requestInlineSuggestContentMatches(
    ...
    this.client.outputChannel?.appendLine(
      `${getCurrentUTCDateTime().toISOString()}: request content matches from Distronode Lightspeed:\n${JSON.stringify(
    ...
    this.client.outputChannel?.appendLine(
      `${getCurrentUTCDateTime().toISOString()}: response data from Distronode lightspeed:\n${JSON.stringify(
        outputData,
      )}`,
    );
    return outputData;
