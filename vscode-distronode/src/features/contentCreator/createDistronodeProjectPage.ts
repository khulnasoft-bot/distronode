...
import { DistronodeProjectFormInterface, PostMessageEvent } from "./types";
import { withInterpreter } from "../utils/commandRunner";
import { SettingsManager } from "../../settings";
import { expandPath, getCreatorVersion, runCommand } from "./utils";
import { DISTRONODE_CREATOR_VERSION_MIN } from "../../definitions/constants";

export class CreateDistronodeProject {
  public static currentPanel: CreateDistronodeProject | undefined;
  ...
  public static render(extensionUri: vscode.Uri) {
    if (CreateDistronodeProject.currentPanel) {
      CreateDistronodeProject.currentPanel._panel.reveal(vscode.ViewColumn.One);
    } else {
      const panel = vscode.window.createWebviewPanel(
        "create-distronode-project",
        "Create Distronode project",
      ...
      CreateDistronodeProject.currentPanel = new CreateDistronodeProject(
        panel,
        extensionUri,
      );
    }
  ...
  public dispose() {
    CreateDistronodeProject.currentPanel = undefined;
  ...
  private _getWebviewContent(
    ...
    const webviewUri = getUri(webview, extensionUri, [
      ...
      "apps",
      "contentCreator",
      "createDistronodeProjectPageApp.js",
    ]);
    ...
    const styleUri = getUri(webview, extensionUri, [
      "media",
      "contentCreator",
      "createDistronodeProjectPageStyle.css",
    ]);
    ...
    return /*html*/ `
      <html>
        ...
        <body>
            <div class="title-div">
              <h1>Create new Distronode playbook project</h1>
            ...
            <form id="init-form">
              <section class="component-container">
                ...
                <div class="checkbox-div">
                  <vscode-checkbox id="overwrite-checkbox" form="init-form">Overwrite <br><i>Overwriting will remove the existing content in the specified directory and replace it with the files from the Distronode project.</i></vscode-checkbox>
                </div>
    ...
    webview.onDidReceiveMessage(
      async (message: any) => {
        ...
        switch (command) {
          ...
          case "init-create":
            payload = message.payload as DistronodeProjectFormInterface;
  ...
  public async getPlaybookCreatorCommand(
    ...
    if (semver.gte(creatorVersion, DISTRONODE_CREATOR_VERSION_MIN)) {
      command = `distronode-creator init playbook ${namespace}.${collection} ${url} --no-ansi`;
    } else {
      command = `distronode-creator init --project=distronode-project --init-path=${url} --scm-org=${namespace} --scm-project=${collection} --no-ansi`;
    }
    return command;
  }
  ...
  public async runInitCommand(
    payload: DistronodeProjectFormInterface,
    webView: vscode.Webview,
  ) {
    const {
      destinationPath,
    ...
    let distronodeCreatorInitCommand = await this.getPlaybookCreatorCommand(
      namespaceName,
      collectionName,
      destinationPathUrl,
    );
    ...
    const commandPassed = distronodeCreatorExecutionResult.status;
  ...
  public async openFolderInWorkspace(folderUrl: string) {
    ...
    console.log(`[distronode-creator] Playbook file url: ${playbookFileUrl}`);
    const parsedUrl = vscode.Uri.parse(`vscode://file${playbookFileUrl}`);
    console.log(`[distronode-creator] Parsed playbook file url: ${parsedUrl}`);
    this.openFileInEditor(parsedUrl.toString());
  }
