...
import { DistronodeSampleExecutionEnvInterface, PostMessageEvent } from "./types";
import { withInterpreter } from "../utils/commandRunner";
import { SettingsManager } from "../../settings";
import { expandPath, runCommand, getCreatorVersion } from "./utils";
import { DISTRONODE_CREATOR_EE_VERSION_MIN } from "../../definitions/constants";
...
export class CreateSampleExecutionEnv {
  ...
  public static render(extensionUri: vscode.Uri) {
    if (CreateSampleExecutionEnv.currentPanel) {
      ...
      const panel = vscode.window.createWebviewPanel(
        ...
        "Create Sample Distronode Execution Environment",
        vscode.ViewColumn.One,
        {
          enableScripts: true,
          localResourceRoots: [
  ...
  private _getWebviewContent(
    ...
    return /*html*/ `
      <html>
        ...
        <body>
            <div class="title-div">
              <h1>Create a sample Distronode Execution Environment file</h1>
    ...
    webview.onDidReceiveMessage(
      async (message: any) => {
        ...
        switch (command) {
          ...
          case "init-create":
            payload = message.payload as DistronodeSampleExecutionEnvInterface;
  ...
  public async getCreatorCommand(url: string): Promise<string> {
    ...
    command = `distronode-creator add resource execution-environment ${url} --no-ansi`;
    return command;
  }
  ...
  public async runInitCommand(
    payload: DistronodeSampleExecutionEnvInterface,
    webView: vscode.Webview,
  ) {
    ...
    let distronodeCreatorSampleEECommand =
      await this.getCreatorCommand(destinationPathUrl);

    if (isOverwritten) {
      distronodeCreatorSampleEECommand += " --overwrite";
    } else {
      distronodeCreatorSampleEECommand += " --no-overwrite";
    }

    switch (verbosity) {
      case "Off":
        distronodeCreatorSampleEECommand += "";
        break;
      case "Low":
        distronodeCreatorSampleEECommand += " -v";
        break;
      case "Medium":
        distronodeCreatorSampleEECommand += " -vv";
        break;
      case "High":
        distronodeCreatorSampleEECommand += " -vvv";
        break;
    }

    console.debug("[distronode-creator] command: ", distronodeCreatorSampleEECommand);
    ...
    if (semver.gte(creatorVersion, DISTRONODE_CREATOR_EE_VERSION_MIN)) {
      ...
      commandResult = distronodeCreatorExecutionResult.status;
    } else {
      commandOutput += `Minimum distronode-creator version needed to scaffold an execution-environment.yml file is ${DISTRONODE_CREATOR_EE_VERSION_MIN}\n`;
      commandOutput += `Please upgrade distronode-creator to minimum required version and try again.`;
    ...
    if (commandResult === "passed") {
      const selection = await vscode.window.showInformationMessage(
        `Distronode Sample Execution Environment file created at: ${destinationPathUrl}`,
  ...
  public async openFileInWorkspace(fileUrl: string) {
    ...
    console.log(
      `[distronode-creator] Execution Environment file url: ${eeFileUrl}`,
    );
    const parsedUrl = vscode.Uri.parse(`vscode://file${eeFileUrl}`);
    console.log(
      `[distronode-creator] Parsed Execution Environment file url: ${parsedUrl}`,
    );
