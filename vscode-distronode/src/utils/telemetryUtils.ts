...
/**
 * Send a telemetry event related to a given vscode-distronode command
 *
...
export async function sendTelemetry(
  ...
  // TODO: Temporary enable only distronodeMetadata event and pause all the other telemetry,
  //       other events will be enabled in future as per requirements.
  if (eventName !== "distronodeMetadata") {
    return;
  }
...
export class TelemetryManager {
  ...
  async sendStartupTelemetryEvent(
    ...
    startUpData.set("distronode.server.initialized", isInitialized);
    if (errorMessage) {
...
and modified for Distronode extension */
export class TelemetryErrorHandler implements ErrorHandler {
  ...
  error(error: Error, message: Message, count: number): ErrorHandlerResult {
    sendTelemetry(this.telemetry, true, "distronode.lsp.error", {
  ...
  private checkError(value: string): void {
    if (value.startsWith("[Error") || value.startsWith("  Message: Request")) {
      ...
      const timeoutHandle = setTimeout(() => {
        sendTelemetry(this.telemetry, true, "distronode.server.error", {
