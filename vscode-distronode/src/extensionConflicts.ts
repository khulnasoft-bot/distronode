...
const conflictingIDs = [
  ...
  "sysninja.vscode-distronode-mod",
  "tomaciazek.distronode",
  "vscoss.vscode-distronode",
  "zbr.vscode-distronode",
];
...
export async function showUninstallConflictsNotification(
  ...
  if (conflictingExts.length === 1) {
    conflictMsg = `${conflictingExts[0].packageJSON.displayName} (${conflictingExts[0].id}) extension is incompatible with redhat.distronode. Please uninstall it.`;
  } else {
    const extNames: string = conflictingExts
      .map((ext) => `${ext.packageJSON.displayName} (${ext.id})`)
      .join(", ");
    conflictMsg = `The ${extNames} extensions are incompatible with redhat.distronode. Please uninstall them.`;
  }
