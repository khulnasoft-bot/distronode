...
1. **Test fixtures:** Settings and Distronode files against which the tests are
   run.
...
2. **End-to-end (e2e) tests:** Test the entire user flow. They open a test host
   ...
   loading Distronode files in the editor, activating the language server, using
...
`distronode-language-server` directory and then run the script:
