...
import {
  ...
  setDistronodeConfigEnv,
  unsetDistronodeConfigEnv,
} from "../helper";
...
function testModuleNames(
  ...
  const tests = [
    ...
    {
      name: "`distronode.builtin.ping` with `ping`",
      position: { line: 7, character: 8 } as Position,
      triggerCharacter: "ping",
      completion: "distronode.builtin.ping",
    },
    {
      name: "`distronode.builtin.debug` with `debu`", // cspell: ignore debu
      position: { line: 7, character: 8 } as Position,
      triggerCharacter: "debu",
      completion: "distronode.builtin.debug",
    },
    {
      name: "list for all modules under distronode namespace with `distronode.`",
      position: { line: 7, character: 8 } as Position,
      triggerCharacter: "distronode.",
      completion: "distronode.",
    },
    {
      name: "list for all the modules under distronode.builtin with `distronode.builtin.`",
      position: { line: 7, character: 8 } as Position,
      triggerCharacter: "distronode.builtin.",
      completion: "distronode.builtin.",
    },
    {
...
function testModuleOptions(
  ...
  const tests = [
    {
      name: "builtin module option (distronode.builtin.debug -> msg)",
...
function testModuleNamesWithoutFQCN(
  ...
  const tests = [
    {
      name: "`ping` with `pin` (distronode.builtin.ping)",
    ...
    tests.forEach(({ name, position, triggerCharacter, completion }) => {
      it(`should provide completion for ${name}`, async function () {
        ...
        const cachedDefaultSetting = (await docSettings).validation.lint
          .enabled;
        (await docSettings).distronode.useFullyQualifiedCollectionNames = false;

        const actualCompletion = await doCompletion(textDoc, position, context);

        // Revert back the default settings
        (await docSettings).distronode.useFullyQualifiedCollectionNames =
