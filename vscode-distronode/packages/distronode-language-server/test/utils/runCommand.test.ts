...
describe("commandRunner", () => {
  ...
  const tests = [
    ...
    {
      args: ["distronode-config", "dump"],
      rc: 0,
      stdout: "DISTRONODE_FORCE_COLOR",
      stderr: "",
      pythonInterpreterPath: "",
    },
    {
  ...
  tests.forEach(({ args, rc, stdout, stderr, pythonInterpreterPath }) => {
    it(`call ${args.join(" ")}`, async function () {
      ...
      // try to enforce distronode to output ANSI in order to check if we are
      // still able to disable it at runtime in order to keep output parseable.
      process.env.DISTRONODE_FORCE_COLOR = "1";
