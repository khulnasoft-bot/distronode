...
export const DISTRONODE_ADJACENT_COLLECTIONS__PATH = path.resolve(
  FIXTURES_BASE_PATH,
  "playbook_adjacent_collection",
  "collections",
);
export const DISTRONODE_CONFIG_FILE = path.resolve(
  FIXTURES_BASE_PATH,
  "completion",
  "distronode.cfg",
);
...
export function setFixtureDistronodeCollectionPathEnv(prePendPath?: string): void {
  if (prePendPath) {
...
export function setDistronodeConfigEnv(): void {
  process.env.DISTRONODE_CONFIG = DISTRONODE_CONFIG_FILE;
}

export function unsetDistronodeConfigEnv(): void {
  process.env.DISTRONODE_CONFIG = undefined;
}
...
export async function enableExecutionEnvironmentSettings(
  ...
  (await docSettings).executionEnvironment.volumeMounts = [
    ...
    {
      src: DISTRONODE_ADJACENT_COLLECTIONS__PATH,
      dest: DISTRONODE_ADJACENT_COLLECTIONS__PATH,
      options: "ro", // read-only option for volume mounts
    },
  ];
}
...
export function getDoc(filename: string): TextDocument {
  ...
  return TextDocument.create(docUri, "distronode", 1, file);
}
