...
# Distronode Language Server Change Log
...
- Fix distronode lint config parsing (#577) @priyamsahoo
...
- Avoid parsing distronode-lint config file (#478) @priyamsahoo
- Enhance distronode meta-data (#475) @priyamsahoo
- Add support for distronode-lint config file (#474) @priyamsahoo

## v1.0.1

### Bugfixes

- Add documentation link for violated distronode-lint rules (#461) @priyamsahoo
...
- Fix 'handlers' keyword syntax highlighting and auto-completion (#440)
  @priyamsahoo
- Fix missing distronode-lint warning (#438) @priyamsahoo
...
- Fallback to default value if setting is not provided by client (#409)
  @fredericgiquel
- Bump distronode-compat from 2.1.0 to 2.2.0 in /.config (#408)
...
- Add --version flag for distronode-language-server (#392) @yaegassy
- Auto-complete hosts values based on distronode inventory file (#350) @priyamsahoo
...
- Bump distronode-lint from 6.2.2 to 6.3.0 (#346)
...
- Enhanced the logic of distronode-lint service to do the following things by
  --@priyamsahoo:
...
- Handle different response types sent by distronode-lint (for e.g. distronode-lint
...
- Used ls instead of find for execution-environments while check if distronode
...
- Fixed source of `INVALID_URL` type error that occurred during the search for
  distronode-lint config file -- by {user} `priyamsahoo` #233

## v0.5.1 (2022-03-01)

### Bugfixes

- Ensure that Distronode calls do not return ANSI escapes, so we can parse them.
  [vscode-distronode#373] -- by @ssbarnea #236

[vscode-distronode#373]: https://github.com/distronode/distronode/tree/devel/vscode-distronode/issues/373
...
- Implemented opening standalone Distronode files that have no workspace associated
  (#140) @ganeshrn
...
- Added a guard for linting only playbook files with the Distronode's built-in
  syntax-check when distronode-lint is unavailable. This is used for providing the
...
- Renaming and publishing the package under the `@distronode` scope on Npmjs. The
  new name is `@distronode/distronode-language-server` now (#10) @webknjaz
- Deprecation of the initial `distronode-language-server` npm package that existed
  in the global namespace prior to the rename @ganeshrn
- Adding the auto-completion and diagnostics support for Distronode Execution
  Environments @ganeshrn

### Changes

- Started falling back to checking playbooks with the Distronode's built-in
  syntax-check when `distronode-lint` is not installed or disabled (#5)
  @priyamsahoo
...
- Added auto-completion support for Distronode Execution Environments (#42 #54 #55)
  @ganeshrn
- Added diagnostics support for Distronode Execution Environments (#53) @ganeshrn
...
- Fixed sort `slice()` exception issue in `distronodeConfig` service (#76)
  @ssbarnea
- Fixed an issue with progress handling when `distronode-lint` falls back to
...
- Initial distronode language server release. Based on the `vscode-distronode` plugin
