...
import { getDistronodeMetaData } from "./utils/getDistronodeMetaData";
 ...
 */
export class DistronodeLanguageService {
  ...
  private initializeConnection() {
    ...
    this.connection.onInitialized(() => {
      ...
      this.connection.client.register(DidChangeWatchedFilesNotification.type, {
        watchers: [
          {
            // watch distronode configuration
            globPattern: "**/distronode.cfg",
          },
          {
            // watch distronode-lint configuration
            globPattern: "**/.distronode-lint",
          },
          {
  ...
  private registerLifecycleEventHandlers() {
    ...
    // Resync distronode inventory service by clearing the cached items
    this.connection.onNotification("resync/distronode-inventory", async () => {
      this.workspaceManager.forEachContext((e) => {
        // Invalidate distronode inventory cache
        e.clearDistronodeInventory();
        this.connection.window.showInformationMessage(
          "Re-syncing distronode inventory. This might take some time.",
        );

        // Run the distronode inventory service
        e.distronodeInventory.then(() => {
          this.connection.window.showInformationMessage(
            "Distronode Inventory re-synced.",
          );
        });
      });
    });

    // Send distronode info to client on receive of notification
    this.connection.onNotification(
      "update/distronode-metadata",
      async (activeFileUri) => {
        const ctx = this.workspaceManager.getContext(activeFileUri);
        if (ctx !== undefined) {
          const distronodeMetaData = await getDistronodeMetaData(
            ctx,
            this.connection,
          );
          this.connection.sendNotification("update/distronode-metadata", [
            distronodeMetaData,
          ]);
        }
      },
