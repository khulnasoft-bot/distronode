...
import { HostType } from "../services/distronodeInventory";
...
let isDistronodePlaybook: boolean;
...
export async function doCompletion(
  ...
  isDistronodePlaybook = isPlaybook(document);
  ...
  const useFqcn = extensionSettings.distronode.useFullyQualifiedCollectionNames;
  ...
  if (path) {
    ...
    if (node) {
      ...
      // check for 'hosts' keyword and 'distronode_host keyword under vars' to provide inventory auto-completion
      ...
      if (keyPathForHosts) {
        ...
        const conditionForDistronodeHostKeyword =
          isScalar(keyNodeForHosts) &&
          keyNodeForHosts["value"] === "distronode_host" &&
          new AncestryBuilder(keyPathForHosts)
            .parent()
            .parent(YAMLMap)
            .getStringKey() === "vars";

        if (conditionForHostsKeyword || conditionForDistronodeHostKeyword) {
          // const nodeRange = getNodeRange(node, document);
          ...
          const hostsList = (await context.distronodeInventory).hostList;
...
export async function doCompletionResolve(
  ...
  if (completionItem.data?.moduleFqcn && completionItem.data?.documentUri) {
    ...
    if (module && module.documentation) {
      ...
      ).distronode.useFullyQualifiedCollectionNames;
      ...
      if (!useFqcn) {
        ...
        declaredCollections.push("distronode.builtin");
        ...
        if (!canUseShortName) {
          // not an Distronode built-in module, and not part of the declared
          // collections
          useFqcn = true;
        }
      }
...
export function resolveSuffix(
  ...
  if (isDocPlaybook) {
    ...
    // if doc is not a playbook (any other distronode file like task file, etc.) indentation will not
