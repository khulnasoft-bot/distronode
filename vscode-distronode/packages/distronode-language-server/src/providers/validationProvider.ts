...
export async function doValidate(
  ...
  if (quick || !context) {
    ...
    // full validation with distronode-lint or distronode syntax-check (if distronode-lint is not installed or disabled)
    ...
    // validation using distronode-lint
    if (settings.validation.lint.enabled) {
      ...
      if (lintAvailability) {
        connection?.console.log("Validating using distronode-lint");
        diagnosticsByFile = await context.distronodeLint.doValidate(textDocument);
      } else {
        connection?.window.showErrorMessage(
          "Distronode-lint is not available. Kindly check the path or disable validation using distronode-lint",
        );
      }
    }

    // validate using distronode-playbook --syntax-check
    else {
      connection?.console.log("Validating using distronode syntax-check");

      if (isPlaybook(textDocument)) {
        connection?.console.log("playbook file");
        diagnosticsByFile =
          await context.distronodePlaybook.doValidate(textDocument);
      } else {
