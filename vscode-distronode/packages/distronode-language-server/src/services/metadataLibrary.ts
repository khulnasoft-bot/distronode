...
export class MetadataLibrary {
  ...
  public get(uri: string): Thenable<IDocumentMetadata> | undefined {
    const metadataUri = this.getDistronodeMetadataUri(uri);
    if (metadataUri) {
      let metadata = this.metadata.get(metadataUri);
      if (!metadata) {
        metadata = this.readDistronodeMetadata(metadataUri);
   ...
   * metadata file.
   */
  private getDistronodeMetadataUri(uri: string): string | undefined {
    let metaPath;
  ...
  private async readDistronodeMetadata(
    metadataUri: string,
