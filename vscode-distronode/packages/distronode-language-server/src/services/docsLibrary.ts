...
export class DocsLibrary {
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      const distronodeConfig = await this.context.distronodeConfig;
      if (settings.executionEnvironment.enabled) {
        ...
        await executionEnvironment.fetchPluginDocs(distronodeConfig);
      }
      for (const modulesPath of distronodeConfig.module_locations) {
        await this.findDocumentationInModulesPath(modulesPath);
      }

      (
        await findPluginRouting(distronodeConfig.distronode_location, "builtin")
      ).forEach((r, collection) => this.pluginRouting.set(collection, r));

      for (const collectionsPath of distronodeConfig.collections_paths) {
  ...
  /**
   * Tries to find an Distronode module for a given name or FQCN.
   *
  ...
  public async findModule(
    ...
    const moduleFiles = globArray([
      `${playbookAdjacentCollectionsPath}/distronode_collections/*/*/plugins/modules/*.py`,
      `${playbookAdjacentCollectionsPath}/distronode_collections/*/*/plugins/modules/**/*.py`,
      `!${playbookAdjacentCollectionsPath}/distronode_collections/*/*/plugins/modules/_*.py`,
      `!${playbookAdjacentCollectionsPath}/distronode_collections/*/*/plugins/modules/**/_*.py`,
  ...
  private async getCandidateFqcns(
    ...
    if (searchText.split(".").length >= 3) {
      ...
      candidateFqcns.push(`distronode.builtin.${searchText}`); // try searching built-in
