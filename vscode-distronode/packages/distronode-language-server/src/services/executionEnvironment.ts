...
export class ExecutionEnvironment {
  ...
  public async fetchPluginDocs(distronodeConfig: DistronodeConfig): Promise<void> {
    ...
    try {
      ...
      const hostCacheBasePath = path.resolve(
        `${process.env.HOME}/.cache/distronode-language-server/${containerName}/${this._container_image_id}`,
      );
      ...
      if (this.isPluginDocCacheValid(hostCacheBasePath)) {
        distronodeConfig.collections_paths = this.updateCachePaths(
          distronodeConfig.collections_paths,
          hostCacheBasePath,
        );
        distronodeConfig.module_locations = this.updateCachePaths(
          distronodeConfig.module_locations,
          hostCacheBasePath,
        );

        this.connection.console.log(
          `Cached plugin paths: \n collections_paths: ${distronodeConfig.collections_paths} \n module_locations: ${distronodeConfig.module_locations}`,
        );
      } else {
        ...
        this.connection.console.log(
          `Identified plugin paths by DistronodeConfig service: \n collections_paths: ${distronodeConfig.collections_paths} \n module_locations: ${distronodeConfig.module_locations}`,
        );
        distronodeConfig.collections_paths = await this.copyPluginDocFiles(
          hostCacheBasePath,
          containerName,
          distronodeConfig.collections_paths,
        ...
        distronodeConfig.module_locations.forEach((modulePath) => {
          const pluginsPathParts = modulePath.split(path.sep).slice(0, -1);
          if (pluginsPathParts.includes("site-packages")) {
            // distronode-config returns default builtin configured module path
            // as ``<python-path>/site-packages/distronode/modules`` to copy other plugins
        ...
        distronodeConfig.module_locations = await this.copyPluginDocFiles(
          hostCacheBasePath,
          containerName,
          distronodeConfig.module_locations,
          "/",
    ...
    if (!setupDone) {
      this.connection.window.showErrorMessage(
        `Execution environment image '${this._container_image}' setup failed.
         For more details check output console logs for distronode-language-server`,
      );
      return false;
    }
    return true;
  ...
  private setContainerEngine(): boolean {
    ...
    if (!["podman", "docker"].includes(this._container_engine)) {
      this.connection.window.showErrorMessage(
        "Valid container engine not found. Install either 'podman' or 'docker' if you want to use execution environment, if not disable Distronode extension execution environment setting.",
      );
      return false;
    }
    return true;
  ...
  private runContainer(containerName: string): boolean {
    ...
    try {
      ...
      command += ` -e DISTRONODE_FORCE_COLOR=0 `; // ensure output is parseable (no ANSI)
      if (
        this.settingsContainerOptions &&
        this.settingsContainerOptions !== ""
      ) {
