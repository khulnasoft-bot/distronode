...
import { distronodeMetaDataType } from "../utils/getDistronodeMetaData";

export class DistronodeConfig {
  private connection: Connection;
  private context: WorkspaceFolderContext;
  private _collection_paths: string[] = [];
  private _module_locations: string[] = [];
  private _distronode_location = "";
  private _default_host_list: string[] = [];
  private _distronode_meta_data = {}; // ini data
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      // get Distronode configuration
      const distronodeConfigResult = await commandRunner.runCommand(
        "distronode-config",
        "dump",
        workingDirectory,
        mountPaths,
      );
      let config = ini.parse(distronodeConfigResult.stdout);
      config = _.mapKeys(
        config,
      ...
      // get Distronode basic information
      const distronodeVersionResult = await commandRunner.runCommand(
        "distronode",
        "--version",
      );

      const versionInfo = ini.parse(distronodeVersionResult.stdout);
      this._distronode_meta_data = versionInfo;
      this._module_locations = parsePythonStringArray(
        versionInfo["configured module search path"],
      );
      this._module_locations.push(
        path.resolve(versionInfo["distronode python module location"], "modules"),
      );

      this._distronode_location = versionInfo["distronode python module location"];
      ...
      if (error instanceof Error) {
        ...
        this.connection.console.error(
          `Exception in DistronodeConfig service: ${JSON.stringify(error)}`,
        );
      }
    }
  }
  ...
  public get distronode_location(): string {
    return this._distronode_location;
  }

  public get distronode_meta_data(): distronodeMetaDataType {
    return this._distronode_meta_data;
  }
}
