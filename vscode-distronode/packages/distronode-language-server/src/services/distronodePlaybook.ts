...
/**
 * Acts as an interface to distronode-playbook command.
 */
export class DistronodePlaybook {
  ...
  /**
   * Acts as an interface to distronode-playbook <file> --syntax-check command and a cache of its output.
   * distronode syntax-check may provide diagnostics for more than just the file for which
   * it was triggered, and this is reflected in the implementation.
   *
   * Perform distronode syntax-check for the given document.
   */
  public async doValidate(
    ...
    try {
      // run distronode playbook syntax-check
      ...
      if (error instanceof Error) {
        ...
        const distronodeSyntaxCheckRegex =
          /The error appears to be in '(?<filename>.*)': line (?<line>\d+), column (?<column>\d+)/;

        const filteredErrorMessage = distronodeSyntaxCheckRegex.exec(
          execError.stderr,
        );
        ...
        if (execError.stderr) {
          this.connection.console.info(
            `[distronode syntax-check] ${execError.stderr}`,
          );
        }
      } else {
        this.connection.console.error(
          `Exception in DistronodeSyntaxCheck service: ${JSON.stringify(error)}`,
        );
      }
    }
  ...
  private processReport(
    ...
    if (!result) {
      this.connection.console.warn(
        "Standard output from distronode syntax-check is suspiciously empty.",
      );
      return diagnostics;
    }
