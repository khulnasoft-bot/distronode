...
export class SettingsManager {
  ...
  readonly defaultSettingsWithDescription: ExtensionSettingsWithDescription = {
    distronode: {
      path: {
        default: "distronode",
        description: "Path to the distronode executable",
      },
    ...
    python: {
      interpreterPath: {
        default: "",
        description:
          "Path to the python/python3 executable. This settings may be used to make the extension work with distronode and distronode-lint installations in a python virtual environment",
      },
      activationScript: {
        default: "",
        description:
    ...
    executionEnvironment: {
      ...
      image: {
        default: "ghcr.io/distronode/community-distronode-dev-tools:latest",
    ...
    validation: {
      enabled: {
        default: true,
        description:
          "Toggle validation provider. If enabled and distronode-lint is disabled, validation falls back to distronode-playbook --syntax-check",
      },
      lint: {
        enabled: {
          default: true,
          description: "Toggle usage of distronode-lint",
        },
        path: {
          default: "distronode-lint",
          description: "Path to the distronode-lint executable",
        },
        arguments: {
          default: "",
          description:
            "Optional command line arguments to be appended to distronode-lint invocation",
        },
      },
    },
  };
  ...
  public async handleConfigurationChanged(
    ...
    if (this.clientSupportsConfigRequests) {
      ...
      if (params.settings.distronode) {
        this.configurationChangeHandlers.forEach((h) => {
          h();
        });
      }
      this.globalSettings = params.settings.distronode || this.defaultSettings;
    }
  }
