...
/**
 * Class to extend distronode-inventory executable as a service
 */
export class DistronodeInventory {
  ...
  public async initialize() {
    ...
    const defaultHostListPath = new Set(
      (await this.context.distronodeConfig).default_host_list,
    );

    const workingDirectory = URI.parse(this.context.workspaceFolder.uri).path;

    // Get inventory hosts
    const distronodeInventoryResult = await commandRunner.runCommand(
    ...
    try {
      inventoryHostsObject = JSON.parse(
        distronodeInventoryResult.stdout,
      ) as inventoryType;
    } catch (error) {
      this.connection.console.error(
        `Exception in DistronodeInventory service: ${JSON.stringify(error)}`,
      );
    }
...
/**
 * A utility function to parse the hosts object from distronode-inventory executable
