...
import { isTaskKeyword, playExclusiveKeywords } from "./distronode";
import { playKeywords, taskKeywords } from "../utils/distronode";
...
/**
 * Determines whether the path points at a parameter key of an Distronode task.
 */
...
/**
 * Tries to find the list of collections declared at the Distronode play/block/task level.
 */
export function getDeclaredCollections(modulePath: Node[] | null): string[] {
  ...
  let path: Node[] | null = new AncestryBuilder(modulePath)
    ...
    .getPath();
  while (true) {
    // traverse the YAML up through the Distronode blocks
...
/**
 * Heuristically determines whether the path points at an Distronode play. The
...
/**
 * Determines whether the path points at one of Distronode block parameter keys.
 */
...
/**
 * Determines whether the path points at one of Distronode role parameter keys.
 */
...
/**
 * For a given Distronode task parameter path, find the module if it has been
...
/**
 * For a given yaml file that is recognized as Distronode file, the function
...
export function isPlaybook(textDocument: TextDocument): boolean {
  ...
  //   Check if any top-level key of the distronode file is a part of filtered list
...
export function isCursorInsideJinjaBrackets(
  ...
  if (nodeObject && !nodeObject.includes("{{ ")) {
    ...
    // refer: https://docs.distronode.com/distronode/latest/playbook_guide/playbooks_variables.html#when-to-quote-variables-a-yaml-gotcha
    return false;
  }
