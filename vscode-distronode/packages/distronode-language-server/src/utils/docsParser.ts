  ...
  if (
    ...
    for (const docFragmentName of docFragmentNames) {
      ...
      const docFragment =
        ...
        docFragments.get(`distronode.builtin.${docFragmentCatalogueName}`);
      if (
        docFragment &&
    ...
    if (!this._contents) {
      ...
      while ((m = LazyModuleDocumentation.docsRegex.exec(contents)) !== null) {
        if (m && m.groups && m.groups.name && m.groups.doc && m.groups.pre) {
          ...
          // There's about 20 modules (out of ~3200) in Distronode 2.9 libs that contain YAML syntax errors
