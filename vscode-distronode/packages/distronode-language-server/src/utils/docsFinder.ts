...
export async function findDocumentation(
  ...
  switch (kind) {
    ...
    case "collection":
      files = await globArray([
        `${dir}/distronode_collections/*/*/plugins/modules/*.py`,
        `${dir}/distronode_collections/*/*/plugins/modules/**/*.py`,
        `!${dir}/distronode_collections/*/*/plugins/modules/_*.py`,
        `!${dir}/distronode_collections/*/*/plugins/modules/**/_*.py`,
      ]).filter((item) => !fs.lstatSync(item).isSymbolicLink());
      break;
    case "collection_doc_fragment":
      files = await globArray([
        `${dir}/distronode_collections/*/*/plugins/doc_fragments/*.py`,
        `!${dir}/distronode_collections/*/*/plugins/doc_fragments/_*.py`,
      ]);
      break;
  }
...
export async function findPluginRouting(
  ...
  switch (kind) {
    case "builtin":
      files = await globArray([`${dir}/config/distronode_builtin_runtime.yml`]);
      break;
    case "collection":
      files = await globArray([
        `${dir}/distronode_collections/*/*/meta/runtime.yml`,
      ]);
      break;
  }
