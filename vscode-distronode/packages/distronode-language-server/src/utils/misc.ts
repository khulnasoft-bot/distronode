...
export function withInterpreter(
  ...
  const newEnv = Object.assign({}, process.env, {
    ...
    DISTRONODE_FORCE_COLOR: "0", // ensure output is parseable (no ANSI)
    ...
    // debugging distronode- lint, or another tool we call.
  });
...
export function getUnsupportedError(): string | undefined {
  ...
  if (process.platform === "win32") {
    return "Distronode Language Server can only run inside WSL on Windows. Refer to vscode documentation for more details.";
  }
}
