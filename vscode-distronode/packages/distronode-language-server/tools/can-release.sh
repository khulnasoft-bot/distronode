...
npm add ../../@distronode-distronode-language-server-*.tgz
npm install
git checkout HEAD -- package.json
...
# VIEW=$(npm view "@distronode/distronode-language-server@${VERSION}")

if npm view "@distronode/distronode-language-server@${VERSION}" > /dev/null 2>&1; then
    ...
    npm publish --dry-run --access public @distronode-distronode-language-server-*.tgz
