...
# Pull devcontainer template from distronode-creator
CREATOR_REPO="https://github.com/distronode/distronode-creator.git"
CREATOR_DEVCONTAINER_TEMPLATE="src/distronode_creator/resources/common/devcontainer/.devcontainer"
...
if [ -z "$DIFF_OUTPUT" ]; then
    echo "Devcontainer content matches distronode-creator devcontainer content."
    rm -rf "$TEMP_DIR"
    exit 0
else
    echo "Devcontainer content under resources/ does not match the content distronode-creator is using."
    echo "Differences:"
