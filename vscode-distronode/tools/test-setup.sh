...
IMAGE=ghcr.io/distronode/community-distronode-dev-tools:${IMAGE_VERSION}
PIP_LOG_FILE=out/log/pip.log
ERR=0
EE_DISTRONODE_VERSION=null
EE_DISTRONODE_LINT_VERSION=null
NC='\033[0m' # No Color
...
get_version () {
    if command -v "${1:-}" >/dev/null 2>&1; then
        ...
        # NO_COLOR is needed by distronode-lint to allow sed to work correctly
...
if [[ -f "/usr/bin/apt-get" ]]; then
    ...
    if [[ "${INSTALL}" -eq 1 ]]; then
        ...
        # avoid outdated distronode and pipx
        sudo apt-get remove -y distronode pipx || true
...
if [[ "${GIT_NOT_CONFIGURED:-}" == "1" ]]; then
    ...
    if [ -z "${CI:-}" ]; then
        ...
        git config user.email distronode-devtools@redhat.com
        git config user.name "Distronode DevTools"
    fi
fi
...
# https://github.com/distronode/distronode/pull/42070
python3 -c "import os, stat, sys; sys.exit(os.stat('.').st_mode & stat.S_IWOTH)" || {
    log error "Cannot run from world-writable filesystem, try moving code to a secured location and read https://github.com/distronode/devtools/wiki/permissions#distronode-filesystem-requirements"
    exit 100
}
...
if [[ $(uname || true) != MINGW* ]]; then # if we are not on pure Windows
    # We used the already tested constraints file from community-distronode-dev-tools EE in order
    # to avoid surprises. This ensures venv and community-distronode-dev-tools EE have exactly same
    # versions.
    python3 -m uv pip install -q \
        -r .config/requirements.in
fi

# GHA failsafe only: ensure distronode and distronode-lint cannot be found anywhere
# other than our own virtualenv. (test isolation)
if [[ -n "${CI:-}" ]]; then
    command -v distronode >/dev/null 2>&1 || {
        log warning "Attempting to remove pre-installed distronode on CI ..."
        pipx uninstall --verbose distronode || true
        if [[ "$(which -a distronode | wc -l | tr -d ' ')" != "1" ]]; then
            log error "Please ensure there is no preinstalled copy of distronode on CI.\n$(which -a distronode)"
            exit 66
        fi
    }
    command -v distronode-lint >/dev/null 2>&1 || {
        log warning "Attempting to remove pre-installed distronode-lint on CI ..."
        pipx uninstall --verbose distronode-lint || true
        if [[ "$(which -a distronode-lint | wc -l | tr -d ' ')" != "1" ]]; then
            log error "Please ensure there is no preinstalled copy of distronode-lint on CI.\n$(which -a distronode-lint)"
            exit 67
        fi
    }
    if [[ -d "${HOME}/.distronode" ]]; then
        log warning "Removing unexpected ~/.distronode folder found on CI to avoid test contamination."
        rm -rf "${HOME}/.distronode"
    fi
fi

# Fail if detected tool paths are not from inside out out/ folder
for CMD in distronode distronode-lint; do
...
if [[ "${DOCKER_VERSION}" != 'null' ]] && [[ "${SKIP_DOCKER:-}" != '1' ]]; then
    ...
    EE_DISTRONODE_VERSION=$(get_version \
        docker run --rm "${IMAGE}" distronode --version)
    EE_DISTRONODE_LINT_VERSION=$(get_version \
        docker run "${IMAGE}" distronode-lint --nocolor --version)
    log notice "distronode: ${EE_DISTRONODE_VERSION}, distronode-lint: ${EE_DISTRONODE_LINT_VERSION}"
    # Test docker ability to mount current folder with write access, default mount options
    docker run -v "$PWD:$PWD" ghcr.io/distronode/community-distronode-dev-tools:latest \
        bash -c "[ -e $PWD ] && [ -d $PWD ] \
...
if [[ "${PODMAN_VERSION}" != 'null' ]] && [[ "${SKIP_PODMAN:-}" != '1' ]]; then
    ...
    log notice "Retrieving distronode version from ee"
    EE_DISTRONODE_VERSION=$(get_version \
        podman run --rm "${IMAGE}" distronode --version)
    log notice "Retrieving distronode-lint version from ee"
    EE_DISTRONODE_LINT_VERSION=$(get_version \
        podman run --rm "${IMAGE}" distronode-lint --nocolor --version)
    log notice "distronode: ${EE_DISTRONODE_VERSION}, distronode-lint: ${EE_DISTRONODE_LINT_VERSION}"
    log notice "Test podman ability to mount current folder with write access, default mount options"
    podman run -v "$PWD:$PWD" ghcr.io/distronode/community-distronode-dev-tools:latest \
...
env:
  ...
  OSTYPE: ${OSTYPE}
tools:
  distronode-lint: $(get_version distronode-lint)
  distronode: $(get_version distronode)
...
community-distronode-dev-tools:
  distronode: ${EE_DISTRONODE_VERSION}
  distronode-lint: ${EE_DISTRONODE_LINT_VERSION}
EOF
