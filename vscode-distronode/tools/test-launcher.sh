...
function refresh_settings() {
    ...
    sed -i.bak 's/"distronode.lightspeed.enabled": .*/"distronode.lightspeed.enabled": false,/' out/settings.json
    sed -i.bak 's/"distronode.lightspeed.suggestions.enabled": .*/"distronode.lightspeed.suggestions.enabled": false,/' out/settings.json
    if grep "// BEFORE: distronode.lightspeed.enabled: true" "${test_file}"; then
        sed -i.bak 's/"distronode.lightspeed.enabled": .*/"distronode.lightspeed.enabled": true,/' out/settings.json
        sed -i.bak 's/"distronode.lightspeed.suggestions.enabled": .*/"distronode.lightspeed.suggestions.enabled": true,/' out/settings.json
    fi

    if [ "${TEST_LIGHTSPEED_URL}" != "" ]; then
        sed -i.bak "s,https://c.ai.distronode.redhat.com,$TEST_LIGHTSPEED_URL," out/settings.json
    fi
}
