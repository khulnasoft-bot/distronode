// BEFORE: distronode.lightspeed.enabled: false
...
describe("Verify the presence of lightspeed element in the status bar and the explorer view", () => {
  ...
  it("Distronode Lightspeed status bar item absent when settings not enabled", async function () {
  ...
  it("Connect button exists in Lightspeed explorer view when settings not enabled", async function () {
    await workbenchExecuteCommand("Distronode: Focus on Distronode Lightspeed View");
    await sleep(5000);
