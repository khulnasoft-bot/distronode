...
describe("Verify the presence of lightspeed element in the status bar and the explorer view", () => {
  ...
  it("Distronode Lightspeed status bar item present when lightspeed and lightspeed suggestions are enabled (with normal color)", async () => {
  ...
  it("Distronode Lightspeed status bar item present when only lightspeed is enabled (with warning color)", async () => {
    const statusBar = new StatusBar();
    const editorView = new EditorView();
    const settingsEditor = await openSettings();
    await updateSettings(
      settingsEditor,
      "distronode.lightspeed.suggestions.enabled",
      false,
    );
    await editorView.openEditor(file);
