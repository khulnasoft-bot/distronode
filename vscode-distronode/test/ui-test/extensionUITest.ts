...
describe("Verify base assets are available after installation", () => {
  ...
  it("VSCode Distronode extension is installed", async function () {
    ...
    if (!process.env.COVERAGE) {
      ...
      const item = await section.findItem("@installed Distronode");
      expect(item, "Failed to find Distronode extension").not.undefined;
      expect(await item?.getText()).to.contain("Distronode language support");
    } else {
      this.skip();
    }
  });
