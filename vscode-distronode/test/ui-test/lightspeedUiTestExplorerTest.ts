...
describe("Test Lightspeed Explorer features", () => {
  ...
  it("Verify Distronode Lightspeed Generate Playbook button", async () => {
  ...
  it("Verify Distronode Lightspeed Explain Playbook button", async () => {
  ...
  it("Sign out using Accounts global action", async () => {
    ...
    if (menu) {
      await menu.select(
        "EXTERNAL_USERNAME (licensed) (Distronode Lightspeed)",
        "Sign Out",
      );
    }
  });
