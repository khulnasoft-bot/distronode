...
describe("Check walkthroughs, elements and associated commands", async () => {
  const walkthroughs = [
    [
      ...
      [
        "Create an Distronode playbook",
        "tag in the status bar",
        "Install the Distronode environment package",
      ],
    ],
    [
      "Discover Distronode Development Tools",
      ["Create", "Test", "Deploy", "Where do I start"],
    ],
    [
      "Start automating with your first Distronode playbook",
      [
        "Enable Distronode Lightspeed",
        "Create an Distronode playbook project",
        "Create an Distronode playbook",
  ...
  walkthroughs.forEach(([walkthroughName, steps]) => {
    it(`Open the ${walkthroughName} walkthrough and check elements`, async function () {
      ...
      const welcomeTab = await editorView.getTabByTitle("Walkthrough: Distronode");
  ...
  it("Check empty playbook command option", async function () {
    ...
    await workbench.executeCommand(
      "Distronode: Create an empty playbook or with Lightspeed (if enabled)",
    );
    await sleep(500);
