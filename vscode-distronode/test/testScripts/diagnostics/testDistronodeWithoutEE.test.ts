...
export function testDiagnosticsDistronodeWithoutEE(): void {
  describe("TEST FOR DISTRONODE DIAGNOSTICS WITHOUT EE", () => {
    const docUri1 = getDocUri("diagnostics/distronode/without_ee/playbook_1.yml");
    const docUri2 = getDocUri("diagnostics/distronode/without_ee/playbook_2.yml");

    before(async () => {
      await vscode.commands.executeCommand("workbench.action.closeAllEditors");
    });

    describe("Diagnostic test with distronode-lint", () => {
    ...
    describe("Diagnostic test with distronode-syntax-check", () => {
      before(async () => {
