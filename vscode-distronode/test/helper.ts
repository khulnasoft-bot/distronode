...
export const DISTRONODE_COLLECTIONS_FIXTURES_BASE_PATH = path.resolve(
  FIXTURES_BASE_PATH,
  "common",
  "collections",
);
...
/**
 * Activates the redhat.distronode extension
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function activate(docUri: vscode.Uri): Promise<any> {
  const extension = vscode.extensions.getExtension("redhat.distronode");
  ...
  try {
    ...
    await reinitializeDistronodeExtension();
    return activation;
  } catch (e) {
...
async function reinitializeDistronodeExtension(): Promise<void> {
  await vscode.languages.setTextDocumentLanguage(doc, "distronode");
...
export async function updateSettings(
  ...
  const distronodeConfiguration = vscode.workspace.getConfiguration(section);
  return distronodeConfiguration.update(setting, value);
}

export function deleteAlsCache(): void {
  const hostCacheBasePath = path.resolve(
    `${process.env.HOME}/.cache/distronode-language-server/`,
  );
  rmSync(hostCacheBasePath, { recursive: true, force: true });
}

export function setFixtureDistronodeCollectionPathEnv(
  prePendPath: string | undefined = undefined,
): void {
  if (prePendPath) {
    process.env.DISTRONODE_COLLECTIONS_PATH = `${prePendPath}:${DISTRONODE_COLLECTIONS_FIXTURES_BASE_PATH}`;
  } else {
    process.env.DISTRONODE_COLLECTIONS_PATH =
      DISTRONODE_COLLECTIONS_FIXTURES_BASE_PATH;
  }
}

export function unSetFixtureDistronodeCollectionPathEnv(): void {
  process.env.DISTRONODE_COLLECTIONS_PATH = undefined;
}
...
export async function enableExecutionEnvironmentSettings(): Promise<void> {
  ...
  const volumeMounts = [
    {
      src: DISTRONODE_COLLECTIONS_FIXTURES_BASE_PATH,
      dest: DISTRONODE_COLLECTIONS_FIXTURES_BASE_PATH,
...
export async function enableLightspeedSettings(): Promise<void> {
  ...
  vscode.commands.executeCommand(
    "setContext",
    "redhat.distronode.lightspeedSuggestionsEnabled",
    true,
  );

  // Open content matches panel
  await vscode.commands.executeCommand(
    "distronode.lightspeed.trainingMatchPanel.focus",
  );
}
...
export async function canRunLightspeedTests(): Promise<boolean> {
  ...
  const lightspeedBaseURL: string | undefined = vscode.workspace
    .getConfiguration("distronode")
    .get("lightspeed.URL");

  const token = process.env.TEST_LIGHTSPEED_ACCESS_TOKEN;
  const distronodeLightspeedURL = lightspeedBaseURL?.trim();

  if (!distronodeLightspeedURL) {
    console.warn(
  ...
  try {
    const { status } = await axios.get(
      `${distronodeLightspeedURL}${LIGHTSPEED_ME_AUTH_URL}`,
      {
        headers: {
...
export async function testInlineSuggestion(
  ...
  // i.e., <insert task name for distronode lightspeed suggestion here>
...
export async function waitForDiagnosisCompletion(
  ...
  let done = false;
  let elapsed = 0;
  // If either distronode-lint or distronode-playbook has started within the
  ...
  while (!done && (started || elapsed < timeout)) {
    const processes = await findProcess("name", /distronode-(?:lint|playbook)/);
