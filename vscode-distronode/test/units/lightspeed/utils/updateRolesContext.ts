...
describe("testing updateRolesContext", () => {
  ...
  it("with no root directory", () => {
    const distronodeRolesCache: IWorkSpaceRolesContext = {};
    const result = updateRolesContext(distronodeRolesCache, "", "workSpace");

    assert.strictEqual(result, undefined);
    assert.equal(Object.keys(distronodeRolesCache).length, 0);
  });
  ...
  it("with an empty roles directory", () => {
    ...
    updateRolesContext(distronodeRolesCache, `${root_dir}/roles`, "my_workSpace");
  ...
  it("with a role with just one task'", () => {
    ...
    updateRolesContext(distronodeRolesCache, `${root_dir}/roles`, "my_workSpace");
    const cacheEntry =
      distronodeRolesCache["my_workSpace"][`${root_dir}/roles/my_role`];
