...
if ! distronode-test integration "${changed_all_target}" --list-targets > /dev/null 2>&1; then
...
distronode-test integration --color -v --retry-on-error "${target}" ${COVERAGE:+"$COVERAGE"} ${CHANGED:+"$CHANGED"} ${UNSTABLE:+"$UNSTABLE"} \
