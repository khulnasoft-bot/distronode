...
options:
  path:
    description:
      ...
      - Before Distronode 2.3 this option was only usable as I(dest), I(destfile) and I(name).
    type: path
    required: yes
  ...
  newline:
    ...
    default: windows
notes:
  - As of Distronode 2.3, the I(dest) option has been changed to I(path) as default, but I(dest) still works as well.
seealso:
- module: assemble
- module: lineinfile
author:
...
EXAMPLES = r"""
# Before Distronode 2.3, option 'dest', 'destfile' or 'name' was used instead of 'path'
