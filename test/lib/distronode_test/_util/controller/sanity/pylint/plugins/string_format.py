"""Distronode specific pylint plugin for checking format string usage."""
...
MSGS = {
    'E9305': ("disabled",  # kept for backwards compatibility with inline ignores, remove after 2.14 is EOL
              "distronode-format-automatic-specification",
              "disabled"),
    'E9390': ("bytes object has no .format attribute",
              "distronode-no-format-on-bytestring",
...
class DistronodeStringFormatChecker(BaseChecker):
    ...
    def _check_new_format(self, node, func):
        ...
        if isinstance(strnode.value, bytes):
            self.add_message('distronode-no-format-on-bytestring', node=node)
            return


def register(linter):
    """required method to auto register this checker """
    linter.register_checker(DistronodeStringFormatChecker(linter))
