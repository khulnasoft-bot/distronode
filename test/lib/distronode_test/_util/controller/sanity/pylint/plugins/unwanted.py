...
DISTRONODE_TEST_MODULES_PATH = os.environ['DISTRONODE_TEST_MODULES_PATH']
DISTRONODE_TEST_MODULE_UTILS_PATH = os.environ['DISTRONODE_TEST_MODULE_UTILS_PATH']
...
class UnwantedEntry:
    ...
    def __init__(
            ...
            distronode_test_only=False,  # type: bool
    ):  # type: (...) -> None
        self.alternative = alternative
        self.modules_only = modules_only
        self.names = set(names) if names else set()
        self.ignore_paths = ignore_paths
        self.distronode_test_only = distronode_test_only
    ...
    def applies_to(self, path, name=None):  # type: (str, t.Optional[str]) -> bool
        ...
        if self.distronode_test_only and '/test/lib/distronode_test/_internal/' not in path:
            return False
...
def is_module_path(path):  # type: (str) -> bool
    ...
    return path.startswith(DISTRONODE_TEST_MODULES_PATH) or path.startswith(DISTRONODE_TEST_MODULE_UTILS_PATH)


class DistronodeUnwantedChecker(BaseChecker):
    """Checker for unwanted imports and functions."""

    name = 'unwanted'

    BAD_IMPORT = 'distronode-bad-import'
    BAD_IMPORT_FROM = 'distronode-bad-import-from'
    BAD_FUNCTION = 'distronode-bad-function'
    BAD_MODULE_IMPORT = 'distronode-bad-module-import'
    ...
    msgs = dict(
        ...
        E5104=('Import external package or distronode.module_utils not %s',
               BAD_MODULE_IMPORT,
               'Identifies imports which should not be used.'),
    )
    ...
    unwanted_imports = dict(
        ...
        # requests=UnwantedEntry('distronode.module_utils.urls', modules_only=True),
        # urllib=UnwantedEntry('distronode.module_utils.urls', modules_only=True),

        # see https://docs.python.org/2/library/urllib2.html
        urllib2=UnwantedEntry('distronode.module_utils.urls',
                              ignore_paths=(
                                  '/lib/distronode/module_utils/urls.py',
                              )),

        # see https://docs.python.org/3/library/collections.abc.html
        collections=UnwantedEntry('distronode.module_utils.six.moves.collections_abc',
                                  names=(
                                      'MappingView',
                                      'ItemsView',
                                      'KeysView',
    ...
    unwanted_functions = {
        ...
        'posix.chmod': UnwantedEntry('verified_chmod',
                                     distronode_test_only=True),

        'sys.exit': UnwantedEntry('exit_json or fail_json',
                                  ignore_paths=(
                                      '/lib/distronode/module_utils/basic.py',
                                      '/lib/distronode/modules/async_wrapper.py',
                                  ),
                                  modules_only=True),
    ...
    def _check_module_import(self, node, modname):  # type: (t.Union[astroid.node_classes.Import, astroid.node_classes.ImportFrom], str) -> None
        ...
        if modname == 'distronode.module_utils' or modname.startswith('distronode.module_utils.'):
            return

        if modname == 'distronode' or modname.startswith('distronode.'):
            self.add_message(self.BAD_MODULE_IMPORT, args=(modname,), node=node)


def register(linter):
    """required method to auto register this checker """
    linter.register_checker(DistronodeUnwantedChecker(linter))
