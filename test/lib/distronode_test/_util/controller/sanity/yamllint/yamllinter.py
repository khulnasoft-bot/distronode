"""Wrapper around yamllint that supports YAML embedded in Distronode modules."""
...
class TestConstructor(SafeConstructor):
    """Yaml Safe Constructor that knows about Distronode tags."""
...
class TestLoader(CParser, TestConstructor, Resolver):
    """Custom YAML loader that recognizes custom Distronode tags."""
...
class YamlChecker:
    ...
    def check(self, paths):  # type: (t.List[str]) -> None
        ...
        for path in paths:
            ...
            if extension in ('.yml', '.yaml'):
                ...
                if path.startswith('lib/distronode/modules/') or path.startswith('plugins/modules/'):
                    conf = module_conf
                else:
                    conf = plugin_conf
    ...
    def get_module_docs(self, path, contents):  # type: (str, str) -> t.Dict[str, t.Any]
        ...
        is_plugin = path.startswith('lib/distronode/modules/') or path.startswith('lib/distronode/plugins/') or path.startswith('plugins/')
        is_doc_fragment = path.startswith('lib/distronode/plugins/doc_fragments/') or path.startswith('plugins/doc_fragments/')
