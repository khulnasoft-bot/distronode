...
from distronode.module_utils.common.yaml import SafeLoader
from distronode.module_utils.six import string_types
from distronode.parsing.yaml.loader import DistronodeLoader


class DistronodeTextIOWrapper(TextIOWrapper):
    def write(self, s):
        super(DistronodeTextIOWrapper, self).write(to_text(s, self.encoding, errors='replace'))
...
class CaptureStd():
    ...
    def __enter__(self):
        ...
        sys.stdout = self.stdout = DistronodeTextIOWrapper(BytesIO(), encoding=self.sys_stdout.encoding)
        sys.stderr = self.stderr = DistronodeTextIOWrapper(BytesIO(), encoding=self.sys_stderr.encoding)
        return self
...
def get_module_name_from_filename(filename, collection):
    ...
    if collection:
        # collection is a relative path, example: distronode_collections/my_namespace/my_collection
        # filename is a relative path, example: plugins/modules/my_module.py
        path = os.path.join(collection, filename)
    else:
        # filename is a relative path, example: lib/distronode/modules/system/ping.py
...
def parse_yaml(value, lineno, module, name, load_all=False, distronode_loader=False):
    traces = []
    errors = []
    data = None
    ...
    if distronode_loader:
        loader = DistronodeLoader
    else:
        loader = SafeLoader
...
class NoArgsDistronodeModule(DistronodeModule):
    """DistronodeModule that does not actually load params. This is used to get access to the
    methods within DistronodeModule without having to fake a bunch of data
    """
    def _load_params(self):
        self.params = {'_distronode_selinux_special_fs': [], '_distronode_remote_tmp': '/tmp', '_distronode_keep_remote_files': False, '_distronode_check_mode': False}
