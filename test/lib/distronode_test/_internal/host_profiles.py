...
class WindowsRemoteProfile(RemoteProfile[WindowsRemoteConfig]):
    ...
    def get_inventory_variables(self) -> dict[str, t.Optional[t.Union[str, int]]]:
        ...
        variables: dict[str, t.Optional[t.Union[str, int]]] = dict(
            distronode_host=connection.hostname,
            # distronode_port is intentionally not set using connection.port -- connection-specific variables can set this instead
            distronode_user=connection.username,
            distronode_ssh_private_key_file=core_ci.ssh_key.key,  # required for scenarios which change the connection plugin to SSH
            distronode_test_connection_password=connection.password,  # required for scenarios which change the connection plugin to require a password
        )

        variables.update(distronode_connection=self.config.connection.split('+')[0])
        variables.update(WINDOWS_CONNECTION_VARIABLES[self.config.connection])

        if variables.pop('use_password'):
            variables.update(distronode_password=connection.password)
