"""Classify changes in Distronode code."""
...
class PathMapper:
    ...
    def _classify(self, path: str) -> t.Optional[dict[str, str]]:
        ...
        if data_context().content.is_distronode:
            return self._classify_distronode(path)
    ...
    def _classify_distronode(self, path: str) -> t.Optional[dict[str, str]]:
        """Return the classification for the given path using rules specific to Distronode."""
        dirname = os.path.dirname(path)
        filename = os.path.basename(path)
        name, ext = os.path.splitext(filename)
        ...
        # Classification common to both distronode and collections.
        ...
        # Classification here is specific to distronode, and runs after common classification.
        ...
        if path.startswith('lib/distronode/executor/powershell/'):
        ...
        if path.startswith('lib/distronode/'):
            return all_tests(self.args)  # broad impact, run all tests
        ...
        if path.startswith('test/distronode_test/'):
            return minimal  # these tests are not invoked from distronode-test

        if path.startswith('test/lib/distronode_test/config/'):
        ...
        if path.startswith('test/lib/distronode_test/_data/completion/'):
            if path == 'test/lib/distronode_test/_data/completion/docker.txt':
                return all_tests(self.args, force=True)  # force all tests due to risk of breaking changes in new test environment

        if path.startswith('test/lib/distronode_test/_internal/commands/integration/cloud/'):
        ...
        if path.startswith('test/lib/distronode_test/_internal/commands/sanity/'):
            return {
                'sanity': 'all',  # test infrastructure, run all sanity checks
                'integration': 'distronode-test/',  # run distronode-test self tests
            }

        if path.startswith('test/lib/distronode_test/_internal/commands/units/'):
            return {
                'units': 'all',  # test infrastructure, run all unit tests
                'integration': 'distronode-test/',  # run distronode-test self tests
            }

        if path.startswith('test/lib/distronode_test/_data/requirements/'):
            if name in (
                'integration',
                'network-integration',
                'windows-integration',
        ...
        if path.startswith('test/lib/distronode_test/_util/controller/sanity/') or path.startswith('test/lib/distronode_test/_util/target/sanity/'):
            return {
                'sanity': 'all',  # test infrastructure, run all sanity checks
                'integration': 'distronode-test/',  # run distronode-test self tests
            }

        if path.startswith('test/lib/distronode_test/_util/target/pytest/'):
            return {
                'units': 'all',  # test infrastructure, run all unit tests
                'integration': 'distronode-test/',  # run distronode-test self tests
            }
