...
# information about support containers provisioned by the current distronode-test instance
    ...
    if args.docker_network:
        ...
        if current_container_id:
            try:
                ...
                # This is needed when distronode-test is running in a container that is not connected to Docker's default network.
...
def create_hosts_entries(context: dict[str, ContainerAccess]) -> list[str]:
    ...
    for container in context.values():
        ...
        entries.append('%s %s # distronode-test %s' % (host_ip, ' '.join(container.names), unique_id))
