"""Provides compatibility with first-generation host delegation options in distronode-test."""
...
def get_legacy_host_config(
    ...
    if options.venv:
        ...
        # These changes are due to distronode-test now maintaining consistency between its own Python and that of controller Python subprocesses.
        #
        # 1) The `--python-interpreter` option (if different from sys.executable) now affects controller subprocesses and triggers re-execution of distronode-test.
        #    Previously this option was completely ignored except when used with the `--docker` or `--remote` options.
        # 2) The `--python` option now triggers re-execution of distronode-test if it differs from sys.version_info.
        #    Previously it affected Python subprocesses, but not distronode-test itself.
