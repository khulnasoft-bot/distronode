...
#          Any third-party imports occurring here will result in an error if they are vendored by distronode-core.
...
DISTRONODE_TEST_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

# assume running from install
DISTRONODE_ROOT = os.path.dirname(DISTRONODE_TEST_ROOT)
DISTRONODE_LIB_ROOT = os.path.join(DISTRONODE_ROOT, 'distronode')
DISTRONODE_SOURCE_ROOT = None

if not os.path.exists(DISTRONODE_LIB_ROOT):
    # running from source
    DISTRONODE_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(DISTRONODE_TEST_ROOT)))
    DISTRONODE_LIB_ROOT = os.path.join(DISTRONODE_ROOT, 'lib', 'distronode')
    DISTRONODE_SOURCE_ROOT = DISTRONODE_ROOT

DISTRONODE_TEST_DATA_ROOT = os.path.join(DISTRONODE_TEST_ROOT, '_data')
DISTRONODE_TEST_UTIL_ROOT = os.path.join(DISTRONODE_TEST_ROOT, '_util')
DISTRONODE_TEST_CONFIG_ROOT = os.path.join(DISTRONODE_TEST_ROOT, 'config')

DISTRONODE_TEST_CONTROLLER_ROOT = os.path.join(DISTRONODE_TEST_UTIL_ROOT, 'controller')
DISTRONODE_TEST_TARGET_ROOT = os.path.join(DISTRONODE_TEST_UTIL_ROOT, 'target')

DISTRONODE_TEST_TOOLS_ROOT = os.path.join(DISTRONODE_TEST_CONTROLLER_ROOT, 'tools')
DISTRONODE_TEST_TARGET_TOOLS_ROOT = os.path.join(DISTRONODE_TEST_TARGET_ROOT, 'tools')
...
class Architecture:
    ...
    These are the architectures supported by distronode-test, such as when provisioning remote instances.
    """
...
WINDOWS_CONNECTION_VARIABLES: dict[str, t.Any] = {
    'psrp+http': dict(
        ...
        distronode_psrp_protocol='http',
        use_password=True,
    ),
    'psrp+https': dict(
        distronode_port=5986,
        distronode_psrp_protocol='https',
        distronode_psrp_cert_validation='ignore',
        use_password=True,
    ),
    'ssh+key': dict(
        distronode_port=22,
        distronode_shell_type='powershell',
        use_password=False,
    ),
    'ssh+password': dict(
        distronode_port=22,
...
@cache
def get_distronode_version() -> str:
    """Return the Distronode version."""
    # distronode may not be in our sys.path
    # avoids a symlink to release.py since distronode placement relative to distronode-test may change during delegation
    load_module(os.path.join(DISTRONODE_LIB_ROOT, 'release.py'), 'distronode_release')

    # noinspection PyUnresolvedReferences
    from distronode_release import __version__ as distronode_version  # pylint: disable=import-error

    return distronode_version


def _enable_vendoring() -> None:
    """Enable support for loading Python packages vendored by distronode-core."""
    # Load the vendoring code by file path, since distronode may not be in our sys.path.
    # Convert warnings into errors, to avoid problems from surfacing later.

    with warnings.catch_warnings():
        warnings.filterwarnings('error')

        load_module(os.path.join(DISTRONODE_LIB_ROOT, '_vendor', '__init__.py'), 'distronode_vendor')
...
def common_environment() -> dict[str, str]:
    ...
    # It is included here so that tests can take advantage of it, rather than only distronode-test during managed pip installs.
...
def verified_chmod(path: str, mode: int) -> None:
    ...
    os.chmod(path, mode)  # pylint: disable=distronode-bad-function
...
class InternalError(Exception):
    ...
    def __init__(self, message: str) -> None:
        super().__init__(f'An internal error has occurred in distronode-test: {message}')
