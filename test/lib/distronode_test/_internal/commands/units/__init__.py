...
def command_units(args: UnitsConfig) -> None:
    ...
    for version in SUPPORTED_PYTHON_VERSIONS:
        ...
        for test_context, paths in test_context_paths.items():
            ...
            env.update(
                PYTHONPATH=get_units_distronode_python_path(args, test_context),
        ...
        if args.requirements_mode != 'skip':
            install_requirements(args, target_profile.python, distronode=controller, command=True, controller=False)  # units
    ...
    for test_context, python, paths, env in test_sets:
        ...
        # Having the cutoff at Python 3.8 also eases packaging of distronode-core since no supported controller version requires the use of mock.
        #
        # NOTE: This only affects use of pytest-mock.
        #       Collection unit tests may directly import mock, which will be provided by distronode-test when it installs requirements using pip.
        #       Although mock is available for distronode-core unit tests, they should import unittest.mock instead.
        ...
        cmd = [
            ...
            '-p', 'no:cacheprovider',
            '-c', os.path.join(DISTRONODE_TEST_DATA_ROOT, 'pytest', 'config', config_name),
        ...
        if args.coverage:
            plugins.append('distronode_pytest_coverage')

        if data_context().content.collection:
            plugins.append('distronode_pytest_collections')

        plugins.append('distronode_forked')

        if plugins:
            env['PYTHONPATH'] += ':%s' % os.path.join(DISTRONODE_TEST_TARGET_ROOT, 'pytest/plugins')
...
def get_units_distronode_python_path(args: UnitsConfig, test_context: str) -> str:
    """
    Return a directory usable for PYTHONPATH, containing only the modules and module_utils portion of the distronode package.
    The temporary directory created will be cached for the lifetime of the process and cleaned up at exit.
    """
    if test_context == TestContext.controller:
        return get_distronode_python_path(args)

    try:
        cache = get_units_distronode_python_path.cache  # type: ignore[attr-defined]
    ...
    distronode_path = os.path.join(python_path, 'distronode')
    distronode_test_path = os.path.join(python_path, 'distronode_test')

    write_text_file(os.path.join(distronode_path, '__init__.py'), '', True)
    os.symlink(os.path.join(DISTRONODE_LIB_ROOT, 'module_utils'), os.path.join(distronode_path, 'module_utils'))

    if data_context().content.collection:
        # built-in runtime configuration for the collection loader
        make_dirs(os.path.join(distronode_path, 'config'))
        os.symlink(os.path.join(DISTRONODE_LIB_ROOT, 'config', 'distronode_builtin_runtime.yml'), os.path.join(distronode_path, 'config', 'distronode_builtin_runtime.yml'))

        # current collection loader required by all python versions supported by the controller
        write_text_file(os.path.join(distronode_path, 'utils', '__init__.py'), '', True)
        os.symlink(os.path.join(DISTRONODE_LIB_ROOT, 'utils', 'collection_loader'), os.path.join(distronode_path, 'utils', 'collection_loader'))

        # legacy collection loader required by all python versions not supported by the controller
        write_text_file(os.path.join(distronode_test_path, '__init__.py'), '', True)
        write_text_file(os.path.join(distronode_test_path, '_internal', '__init__.py'), '', True)
    elif test_context == TestContext.modules:
        # only non-collection distronode module tests should have access to distronode built-in modules
        os.symlink(os.path.join(DISTRONODE_LIB_ROOT, 'modules'), os.path.join(distronode_path, 'modules'))
