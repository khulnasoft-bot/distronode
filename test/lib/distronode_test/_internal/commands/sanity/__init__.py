...
class SanityScript(SanityTest, metaclass=abc.ABCMeta):
    ...
    def test_script(self, args: SanityConfig, targets: SanityTargets, virtualenv_python: PythonConfig, python: PythonConfig) -> TestResult:
        ...
        env.update(
            ...
            DISTRONODE_TEST_TARGET_PYTHON_VERSION=python.version,
            DISTRONODE_TEST_CONTROLLER_PYTHON_VERSIONS=','.join(CONTROLLER_PYTHON_VERSIONS),
            DISTRONODE_TEST_REMOTE_ONLY_PYTHON_VERSIONS=','.join(REMOTE_ONLY_PYTHON_VERSIONS),
        )

        if self.min_max_python_only:
            min_python, max_python = self.supported_python_versions

            env.update(DISTRONODE_TEST_MIN_PYTHON=min_python)
            env.update(DISTRONODE_TEST_MAX_PYTHON=max_python)
