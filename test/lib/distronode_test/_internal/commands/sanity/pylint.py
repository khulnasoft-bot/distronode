...
class PylintTest(SanitySingleVersion):
    ...
    def test(self, args: SanityConfig, targets: SanityTargets, python: PythonConfig) -> TestResult:
        ...
        if data_context().content.collection:
            ...
            add_context(remaining_paths, 'validate-modules', filter_path('test/lib/distronode_test/_util/controller/sanity/validate-modules/'))
            add_context(remaining_paths, 'validate-modules-unit', filter_path('test/lib/distronode_test/tests/validate-modules-unit/'))
            add_context(remaining_paths, 'code-smell', filter_path('test/lib/distronode_test/_util/controller/sanity/code-smell/'))
            add_context(remaining_paths, 'distronode-test-target', filter_path('test/lib/distronode_test/_util/target/'))
            add_context(remaining_paths, 'distronode-test', filter_path('test/lib/'))
            add_context(remaining_paths, 'test', filter_path('test/'))
            add_context(remaining_paths, 'hacking', filter_path('hacking/'))
            add_context(remaining_paths, 'distronode', lambda p: True)
    ...
    def pylint(
        ...
        # plugin: deprecated (distronode-test)
        if data_context().content.collection:
        ...
        env.update(dict(('DISTRONODE_TEST_%s_PATH' % k.upper(), os.path.abspath(v) + os.path.sep) for k, v in data_context().content.plugin_paths.items()))
