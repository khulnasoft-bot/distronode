"""Sanity test for distronode-doc."""
...
class DistronodeDocTest(SanitySingleVersion):
    """Sanity test for distronode-doc."""
    ...
    def test(self, args: SanityConfig, targets: SanityTargets, python: PythonConfig) -> TestResult:
        ...
        for doc_type in MULTI_FILE_PLUGINS:
            if doc_targets.get(doc_type):
                # List plugins
                cmd = ['distronode-doc', '-l', '--json', '-t', doc_type]
                prefix = data_context().content.prefix if data_context().content.collection else 'distronode.builtin.'
                cmd.append(prefix[:-1])
                try:
                ...
                if stderr:
                    summary = 'Output on stderr from distronode-doc is considered an error.\n\n%s' % SubprocessError(cmd, stderr=stderr)
                    return SanityFailure(self.name, summary=summary)
