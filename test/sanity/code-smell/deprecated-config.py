...
def find_deprecations(obj, path=None):
    ...
    for key, value in items:
        ...
        if key != 'deprecated':
            ...
            if StrictVersion(version) <= DISTRONODE_MAJOR:
...
def main():
    ...
    base = os.path.join(os.path.dirname(distronode.config.__file__), 'base.yml')
    root_path = os.path.dirname(os.path.dirname(os.path.dirname(distronode.__file__)))
