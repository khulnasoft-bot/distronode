...
def collect_sdist_files(complete_file_list: list[str]) -> list[str]:
    ...
    sdist_files.extend(f'distronode_core.egg-info/{name}' for name in egg_info)
...
def collect_wheel_files(complete_file_list: list[str]) -> list[str]:
    ...
    for path in complete_file_list:
        ...
        if path.startswith('lib/distronode/'):
            prefix = 'lib'
        elif path.startswith('test/lib/distronode_test/'):
            prefix = 'test/lib'
        else:
            continue
    ...
    wheel_files.extend(f'distronode_core-{__version__}.dist-info/{name}' for name in dist_info)
...
def main() -> None:
    ...
    python_min = os.environ['DISTRONODE_TEST_MIN_PYTHON']
    python_max = os.environ['DISTRONODE_TEST_MAX_PYTHON']
...
def check_build(complete_file_list: list[str], use_upper_setuptools_version: bool) -> list[str]:
    ...
    # Limit visible files to those reported by distronode-test.
