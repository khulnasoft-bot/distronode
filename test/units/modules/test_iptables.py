...
from units.modules.utils import DistronodeExitJson, DistronodeFailJson, set_module_args, fail_json, exit_json
import pytest
...
def _mock_basic_commands(mocker):
    ...
    mocker.patch(
        "distronode.module_utils.basic.DistronodeModule.get_bin_path",
        return_value=IPTABLES_CMD,
    )
    mocker.patch("distronode.modules.iptables.get_iptables_version", return_value=IPTABLES_VERSION)
...
def test_flush_table_without_chain(mocker):
    ...
    run_command = mocker.patch(
        "distronode.module_utils.basic.DistronodeModule.run_command", return_value=(0, "", "")
    )
...
def test_policy_table(mocker):
    ...
    run_command = mocker.patch(
        "distronode.module_utils.basic.DistronodeModule.run_command",
...
def test_jump_tee_gateway_negative(mocker):
    ...
    mocker.patch(
        "distronode.module_utils.basic.DistronodeModule.fail_json",
        side_effect=fail_json,
    )
    jump_err_msg = "jump is TEE but all of the following are missing: gateway"
    with pytest.raises(DistronodeFailJson, match=jump_err_msg) as exc:
        iptables.main()
...
def test_chain_creation(mocker):
    ...
    mocker.patch(
        "distronode.module_utils.basic.DistronodeModule.exit_json",
        side_effect=exit_json,
    )
    with pytest.raises(DistronodeExitJson):
        iptables.main()
    ...
    with pytest.raises(DistronodeExitJson) as exc:
        iptables.main()
