...
def test_deprecate_message_only(reset):
    deprecate('Deprecation message')  # pylint: disable=distronode-deprecated-no-version
...
def test_deprecate_with_collection(reset):
    deprecate(msg='Deprecation message', collection_name='distronode.builtin')  # pylint: disable=distronode-deprecated-no-version
    assert warnings._global_deprecations == [
        {'msg': 'Deprecation message', 'version': None, 'collection_name': 'distronode.builtin'}]


def test_deprecate_with_version(reset):
    deprecate(msg='Deprecation message', version='2.14')  # pylint: disable=distronode-deprecated-version
    assert warnings._global_deprecations == [
        {'msg': 'Deprecation message', 'version': '2.14', 'collection_name': None}]


def test_deprecate_with_version_and_collection(reset):
    deprecate(msg='Deprecation message', version='2.14', collection_name='distronode.builtin')  # pylint: disable=distronode-deprecated-version
    assert warnings._global_deprecations == [
...
def test_deprecate_failure(test_case):
    with pytest.raises(TypeError, match='deprecate requires a string not a %s' % type(test_case)):
        deprecate(test_case)  # pylint: disable=distronode-deprecated-no-version
