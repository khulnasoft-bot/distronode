...
if t.TYPE_CHECKING:  # pragma: nocover
    ...
    from distronode_test._internal.diff import FileDiff
...
def get_parsed_diff(name: str) -> list[FileDiff]:
    ...
    # noinspection PyProtectedMember
    from distronode_test._internal.diff import parse_diff
...
def test_parse_rename(diffs) -> None:
    ...
    assert diffs[0].old.path == 'packaging/debian/distronode-base.dirs'
    assert diffs[0].new.path == 'packaging/debian/distronode-core.dirs'

    assert diffs[1].old.path == 'packaging/debian/distronode-base.install'
    assert diffs[1].new.path == 'packaging/debian/distronode-core.install'
