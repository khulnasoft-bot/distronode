...
export DISTRONODE_VARS_PLUGINS=./vars_plugins

# Test vars plugin without REQUIRES_ENABLED class attr and vars plugin with REQUIRES_ENABLED = False run by default
[ "$(distronode-inventory -i localhost, --list --yaml all "$@" | grep -Ec '(implicitly|explicitly)_auto_enabled')" = "2" ]

# Test vars plugin with REQUIRES_ENABLED=True only runs when enabled
[ "$(distronode-inventory -i localhost, --list --yaml all "$@" | grep -Ec 'require_enabled')" = "0" ]
export DISTRONODE_VARS_ENABLED=require_enabled
[ "$(distronode-inventory -i localhost, --list --yaml all "$@" | grep -c 'require_enabled')" = "1" ]

# Test deprecated features
export DISTRONODE_VARS_PLUGINS=./deprecation_warning
WARNING="The vars plugin v2_vars_plugin .* is relying on the deprecated entrypoints 'get_host_vars' and 'get_group_vars'"
DISTRONODE_DEPRECATION_WARNINGS=True DISTRONODE_NOCOLOR=True DISTRONODE_FORCE_COLOR=False \
        distronode-inventory -i localhost, --list all "$@" 2> err.txt
distronode localhost -m debug -a "msg={{ lookup('file', 'err.txt') | regex_replace('\n', '') }}" | grep "$WARNING"
...
DISTRONODE_DEBUG=True distronode-playbook test_task_vars.yml > out.txt
...
export DISTRONODE_VARS_ENABLED=distronode.builtin.host_group_vars
DISTRONODE_DEBUG=True distronode-playbook test_task_vars.yml > out.txt
[ "$(grep -c "Loading VarsModule 'host_group_vars'" out.txt)" -eq 1 ]
[ "$(grep -c "Loading VarsModule 'require_enabled'" out.txt)" -lt 3 ]
[ "$(grep -c "Loading VarsModule 'auto_enabled'" out.txt)" -eq 22 ]

distronode localhost -m include_role -a 'name=a' "$@"
