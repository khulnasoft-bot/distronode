...
$tmp_dir = Join-Path -Path $env:TEMP -ChildPath "distronode .ÅÑŚÌβŁÈ [$!@^&test(;)]"
try {
...
$actual = Run-Command -command "powershell.exe -ExecutionPolicy ByPass -EncodedCommand $encoded_wrapper" -stdin "Distronode"
Assert-Equal -actual $actual.stdout -expected "Distronode`n"
