#!/usr/bin/env python
"""Test suite used to verify distronode-test is able to run its containers on various container hosts."""
...
UNPRIVILEGED_USER_NAME = 'distronode-test'
...
def main() -> None:
    ...
    try:
        bootstrap_type = pathlib.Path('/etc/distronode-test.bootstrap').read_text().strip()
    except FileNotFoundError:
    ...
    if bootstrap_type != 'remote':
        display.warning('Skipping destructive test on system which is not an distronode-test remote provisioned instance.')
        return
...
def get_container_completion_entries() -> dict[str, dict[str, str]]:
    """Parse and return the distronode-test container completion entries."""
    completion_lines = pathlib.Path(os.environ['PYTHONPATH'], '../test/lib/distronode_test/_data/completion/docker.txt').read_text().splitlines()
...
def run_test(scenario: TestScenario) -> TestResult:
    ...
    integration = ['distronode-test', 'integration', 'split']
    ...
    if scenario.engine == 'podman':
        ...
        test_env.update(DISTRONODE_TEST_PREFER_PODMAN='1')

    test_env.update(common_env)

    if scenario.user_scenario.ssh:
        client_become_cmd = ['ssh', f'{scenario.user_scenario.ssh.name}@localhost']
        test_commands = [client_become_cmd + [f'cd ~/distronode; {format_env(test_env)}{sys.executable} bin/{shlex.join(command)}'] for command in commands]
    else:
...
def run_module(
    ...
    """Run the specified Distronode module and return the result."""
    return run_command('distronode', '-m', module, '-v', '-a', json.dumps(args), 'localhost')
...
class Bootstrapper(metaclass=abc.ABCMeta):
    ...
    def configure_source_trees(cls):
        ...
        current_distronode = pathlib.Path(os.environ['PYTHONPATH']).parent

        root_distronode = pathlib.Path('~').expanduser() / 'distronode'
        test_distronode = pathlib.Path(f'~{UNPRIVILEGED_USER_NAME}').expanduser() / 'distronode'

        if current_distronode != root_distronode:
            display.info(f'copying {current_distronode} -> {root_distronode} ...')
            rmtree(root_distronode)
            shutil.copytree(current_distronode, root_distronode)
            run_command('chown', '-R', 'root:root', str(root_distronode))

        display.info(f'copying {current_distronode} -> {test_distronode} ...')
        rmtree(test_distronode)
        shutil.copytree(current_distronode, test_distronode)
        run_command('chown', '-R', f'{UNPRIVILEGED_USER_NAME}:{UNPRIVILEGED_USER_NAME}', str(test_distronode))

        paths = [pathlib.Path(test_distronode)]

        for root, dir_names, file_names in os.walk(test_distronode):
