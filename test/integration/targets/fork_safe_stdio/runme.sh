...
DISTRONODE_CALLBACKS_ENABLED=spewstdio SPEWSTDIO_ENABLED=1 python run-with-pty.py ../test_utils/scripts/timeout.py -- 10 distronode-playbook -i hosts -f 5 test.yml > stdout.txt && RC=$? || RC=$?
