...
run_test() {
	...
	{ distronode-playbook -i inventory "$playbook" "${@:3}" \
	...
	sed -i -e "s#${DISTRONODE_PLAYBOOK_DIR}#TEST_PATH#g" "${OUTFILE}.${testname}.stdout" "${OUTFILE}.${testname}.stderr"
...
run_test_dryrun() {
	...
	cmd="distronode-playbook -i inventory ${chk} test_dryrun.yml"
...
adjust_tty_cols() {
	if [[ -t 1 ]]; then
		...
		# Override TTY cols to make comparing distronode-playbook output easier
...
export DISTRONODE_STDOUT_CALLBACK=default
# Disable color in output for consistency
export DISTRONODE_FORCE_COLOR=0
export DISTRONODE_NOCOLOR=1

# Default settings
export DISTRONODE_DISPLAY_SKIPPED_HOSTS=1
export DISTRONODE_DISPLAY_OK_HOSTS=1
export DISTRONODE_DISPLAY_FAILED_STDERR=0
export DISTRONODE_CHECK_MODE_MARKERS=0

run_test default test.yml

set +e
DISTRONODE_CALLBACKS_ENABLED=default run_test include_role_fails test_include_role_fails.yml
set -e

# Check for async output
# NOTE: regex to match 1 or more digits works for both BSD and GNU grep
distronode-playbook -i inventory test_async.yml 2>&1 | tee async_test.out
grep "ASYNC OK .* jid=j[0-9]\{1,\}" async_test.out
grep "ASYNC FAILED .* jid=j[0-9]\{1,\}" async_test.out
rm -f async_test.out

# Hide skipped
export DISTRONODE_DISPLAY_SKIPPED_HOSTS=0

run_test hide_skipped test.yml

# Hide skipped/ok
export DISTRONODE_DISPLAY_SKIPPED_HOSTS=0
export DISTRONODE_DISPLAY_OK_HOSTS=0
...
export DISTRONODE_DISPLAY_SKIPPED_HOSTS=1
export DISTRONODE_DISPLAY_OK_HOSTS=1
export DISTRONODE_DISPLAY_FAILED_STDERR=1
...
export DISTRONODE_SHOW_TASK_PATH_ON_FAILURE=1
run_test display_path_on_failure test.yml
export DISTRONODE_SHOW_TASK_PATH_ON_FAILURE=0
...
set +e
distronode-playbook -i inventory test_2.yml > >(set +x; tee "${BASEFILE}.unreachable.stdout";) 2> >(set +x; tee "${BASEFILE}.unreachable.stderr" >&2) || true
set -e
...
export DISTRONODE_CALLBACK_RESULT_FORMAT=yaml
run_test result_format_yaml test.yml
export DISTRONODE_CALLBACK_RESULT_FORMAT=json

export DISTRONODE_CALLBACK_RESULT_FORMAT=yaml
export DISTRONODE_CALLBACK_FORMAT_PRETTY=1
run_test result_format_yaml_lossy_verbose test.yml -v
run_test yaml_result_format_yaml_verbose test_yaml.yml -v
export DISTRONODE_CALLBACK_RESULT_FORMAT=json
unset DISTRONODE_CALLBACK_FORMAT_PRETTY

export DISTRONODE_CALLBACK_RESULT_FORMAT=yaml
export DISTRONODE_CALLBACK_FORMAT_PRETTY=0
...
export DISTRONODE_CHECK_MODE_MARKERS=1
...
distronode-playbook -i host1,host2 no_implicit_meta_banners.yml > meta_test.out
cat meta_test.out
[ "$(grep -c 'TASK \[meta\]' meta_test.out)" -eq 0 ]
rm -f meta_test.out

# Ensure free/host_pinned non-lockstep strategies display correctly
diff -u callback_default.out.free.stdout <(DISTRONODE_STRATEGY=free distronode-playbook -i inventory test_non_lockstep.yml 2>/dev/null)
diff -u callback_default.out.fqcn_free.stdout <(DISTRONODE_STRATEGY=distronode.builtin.free distronode-playbook -i inventory test_non_lockstep.yml 2>/dev/null)
diff -u callback_default.out.host_pinned.stdout <(DISTRONODE_STRATEGY=host_pinned distronode-playbook -i inventory test_non_lockstep.yml 2>/dev/null)
