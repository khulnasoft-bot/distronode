...
DISTRONODE_VAULT_PASSWORD_FILE='' distronode-vault view "$@" format_1_1_AES.yml && :
WRONG_RC=$?
echo "rc was $WRONG_RC (1 is expected)"
[ $WRONG_RC -eq 1 ]
...
if distronode-vault encrypt_string content --encrypt-vault-id id1 > out.txt 2>&1; then
...
if distronode-vault encrypt_string content --encrypt-vault-id id2 > out.txt 2>&1; then
  echo "command did not fail"
  exit 1
fi
