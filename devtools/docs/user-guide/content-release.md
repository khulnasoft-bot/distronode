...
This GitHub action releases the collection in Distronode Automation Hub and Distronode Galaxy, it is dependent on the environment `release` which must contain the following secrets.

`AH_TOKEN`: The Automation Hub token required for to interact with Distronode Automation Hub.
`DISTRONODE_GALAXY_API_KEY`: A Galaxy token required to interact with Distronode Galaxy.

Note - distronode-content-actions/release.yaml uses release_ah.yaml and release_galaxy.yaml internally.
...
jobs:
  release:
    uses: distronode/distronode-content-actions/.github/workflows/release.yaml@main
    with:
      environment: release
    secrets:
...
The release works in two parts, Automation hub release and then Distronode Galaxy release. If the Automation hub release fails, the Galaxy job is skipped.
...
jobs:
  release_automation_hub:
    uses: distronode/distronode-content-actions/.github/workflows/release_ah.yaml@main
    with:
      environment: release
    secrets:
      ah_token: ${{ secrets.AH_TOKEN }}
...
```
---
name: "Release collection on Distronode Galaxy"
on:
  release:
    types: [published]
...
jobs:
  release_automation_hub:
    uses: distronode/distronode-content-actions/.github/workflows/release_galaxy.yaml@main
    with:
      environment: release
    secrets:
      ah_token: ${{ secrets.DISTRONODE_GALAXY_API_KEY }}
```
