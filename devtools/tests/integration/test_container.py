...
from distronode_devtools.version_builder import PKGS
...
def test_cinc(
    ...
    podman_run_container = exec_container(
        "podman run --pull=never -i --rm -d -e DISTRONODE_DEVTOOLS_CONTAINER=1"
        " -e DISTRONODE_FORCE_COLOR=0 --name test_cinc"
        ...
        shlex.join(
            [
                ...
                (
                    ...
                    " pathlib.Path(f'/usr/local/lib/python{sys.version_info[0]}.{sys.version_info[1]}/site-packages/distronode/plugins/').exists()"
                    " else 1);"
                ),
            ],
        ),
...
def test_navigator_2_c_in_c(
    ...
    """Test distronode-navigator run against a simple playbook within the container.
    ...
    result = exec_container(
        f"distronode-navigator run {playbook}"
...
def test_navigator_1(
    ...
    """Test distronode-navigator run using the container as an ee.
...
def test_nav_collections(
    ...
    """Test distronode-navigator collections.
    ...
    """
    cmd = (
        f"distronode-navigator collections --lf {tmp_path}/navigator.log"
        f" --pp never --eei {infrastructure.navigator_ee}"
    )
    stdout = container_tmux.send_and_wait(cmd=cmd, wait_for=":help help", timeout=30)
    assert any("distronode.builtin" in line for line in stdout)
    assert any("distronode.posix" in line for line in stdout)
    cmd = ":0"
...
def test_nav_images(
    ...
    """Test distronode-navigator images.
    ...
    """
    cmd = (
        f"distronode-navigator images --lf {tmp_path}/nav.log"
        f" --pp never --eei {infrastructure.navigator_ee}"
    )
...
def test_nav_playbook(
    ...
    """Test distronode-navigator run using a creator created playbook.
    ...
    Args:
        ...
        infrastructure: The testing infrastructure
    """
    cmd = f"distronode-creator init playbook test_ns.test_name {tmp_path}"
    ...
    cmd = (
        f"cd {tmp_path} &&"
        " distronode-navigator run site.yml"
        f" --pp never --eei {infrastructure.navigator_ee}"
    )
...
def test_nav_collection(container_tmux: ContainerTmux, tmp_path: Path) -> None:
    """Test distronode-navigator run using a creator created collection.
    ...
    namespace = "test_ns"
    name = "test_name"
    collection_path = tmp_path / "collections" / "distronode_collections" / namespace / name
    cmd = f"distronode-creator init collection test_ns.test_name {collection_path}"
    stdout = container_tmux.send_and_wait(cmd=cmd, wait_for="created", timeout=10)
    output = "Note: collection project created"
    assert any(output in line for line in stdout)
    cmd = (
        f"DISTRONODE_COLLECTIONS_PATH={tmp_path}/collections distronode-navigator collections --ee false"
    )
    stdout = container_tmux.send_and_wait(cmd=cmd, wait_for=":help help", timeout=10)
...
def test_builder(
    ...
    """Test building an execution environment with distronode-builder.
    ...
    result = exec_container(f"DISTRONODE_NOCOLOR=1 distronode-builder build -f {ee_file} -c {tmp_path}")
