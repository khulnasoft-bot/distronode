...
def test_playbook_v2(server_url: str, tmp_path: Path) -> None:
    ...
    assert response.headers["Content-Disposition"] == 'attachment; filename="distronode-devops.tar.gz"'
    assert response.headers["Content-Type"] == "application/tar+gzip"
    dest_file = tmp_path / "distronode-devops.tar.gz"
    with dest_file.open(mode="wb") as tar_file:
        tar_file.write(response.content)
    with tarfile.open(dest_file) as file:
        assert (
            "./collections/distronode_collections/distronode/devops/roles/run/README.md"
            in file.getnames()
        )
