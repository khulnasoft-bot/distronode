...
from distronode_devtools.arg_parser import parse
from distronode_devtools.utils import Colors
...
class Cli:
    ...
    def _run_subcommand(self, subcommand: str) -> None:
        ...
        """
        subcommand_module = f"distronode_devtools.subcommands.{subcommand}"
        ...
        if subcommand == "server":
            try:
                ...
                err = (
                    ...
                    " `pip install distronode-devtools[server]`"
                )
...
def main() -> None:
    """Entry point for distronode-creator CLI."""
    cli = Cli()
    cli.parse_args()
    cli.run()
