...
TAG_BASE=community-distronode-devtools-base:latest
...
IMAGE_NAME=localhost/community-distronode-devtools:test
...
if [ "--publish" == "${1:-}" ]; then
    ...
    ${ADT_CONTAINER_ENGINE} pull -q "ghcr.io/distronode/community-distronode-devtools-tmp:${GITHUB_SHA:-}-arm64"
    ${ADT_CONTAINER_ENGINE} pull -q "ghcr.io/distronode/community-distronode-devtools-tmp:${GITHUB_SHA:-}-amd64"

    for TAG in ghcr.io/distronode/community-distronode-devtools:${2:-} ghcr.io/distronode/community-distronode-devtools:latest; do
        ${ADT_CONTAINER_ENGINE} manifest create "$TAG" --amend "ghcr.io/distronode/community-distronode-devtools-tmp:${GITHUB_SHA:-}-amd64" --amend "ghcr.io/distronode/community-distronode-devtools-tmp:${GITHUB_SHA:-}-arm64"
        ${ADT_CONTAINER_ENGINE} manifest annotate --arch arm64 "$TAG" "ghcr.io/distronode/community-distronode-devtools-tmp:${GITHUB_SHA:-}-arm64"
...
distronode-builder create -f execution-environment.yml --output-filename Containerfile -v3
...
if [[ -n "${GITHUB_SHA:-}" && "${GITHUB_EVENT_NAME:-}" != "pull_request" ]]; then
    FQ_IMAGE_NAME="ghcr.io/distronode/community-distronode-devtools-tmp:${GITHUB_SHA}-$ARCH"
