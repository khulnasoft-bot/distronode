...
from distronode.errors import DistronodeError, DistronodeOptionsError, DistronodeRequiredOptionError
...
def _distronode_env_vars(varname):
    """ return true or false depending if variable name is possibly a 'configurable' distronode env variable """
    return all(
        [
            varname.startswith("DISTRONODE_"),
            not varname.startswith(("DISTRONODE_TEST_", "DISTRONODE_LINT_")),
            varname not in ("DISTRONODE_CONFIG", "DISTRONODE_DEV_HOME"),
        ]
    )
...
class ConfigCLI(CLI):
    ...
    def _get_plugin_configs(self, ptype, plugins):
        ...
        for plugin in plugin_cs:
            ...
            for setting in config_entries[finalname].keys():
                ...
                except DistronodeRequiredOptionError:
                    v = None
                    o = 'REQUIRED'
    ...
    def execute_validate(self):
        ...
        if context.CLIARGS['format'] == 'ini':
            ...
            # validate any 'DISTRONODE_' env vars found
            evars = [varname for varname in os.environ.keys() if _distronode_env_vars(varname)]
            if evars:
