...
class RoleMixin(object):
    ...
    def _load_role_data(self, root, files, role_name, collection):
        ...
        try:
            ...
            raise DistronodeParserError("Could not read the role '%s' (at %s)" % (role_name, path), orig_exc=e)
...
class DocCLI(CLI, RoleMixin):
    ...
    def _display_role_doc(self, role_json):
        ...
        for role in roles:
            try:
                ...
                raise DistronodeParserError("Role '%s" % (role), orig_exc=e)
    ...
    def format_plugin_doc(plugin, plugin_type, doc, plainexamples, returndocs, metadata):
        ...
        try:
            ...
            display.vvv(traceback.format_exc())
            raise DistronodeError("Unable to retrieve documentation from '%s'" % (plugin), orig_exc=e)
                    ...
                    for ignore in DocCLI.IGNORE:
                        for item in conf[config]:
                            if display.verbosity > 0 and 'version_added' in item:
                                item['added_in'] = DocCLI._format_version_added(item['version_added'], item.get('version_added_colleciton', 'distronode-core'))
                            if ignore in item:
                                del item[ignore]
    ...
    def get_role_man_text(self, role, role_json):
        ...
        for entry_point in role_json['entry_points']:
            ...
            if doc.get('options'):
                text.append(_format("Options", 'bold') + " (%s indicates it is required):" % ("=" if C.DISTRONODE_NOCOLOR else 'red'))
                DocCLI.add_fields(text, doc.pop('options'), limit, opt_indent)

            if doc.get('attributes', False):
                display.deprecated(
                    f'The role {role}\'s argument spec {entry_point} contains the key "attributes", '
                    'which will not be displayed by distronode-doc in the future. '
    ...
    def get_man_text(doc, collection_name='', plugin_type=''):
        ...
        if display.verbosity > 0:
            doc['added_in'] = DocCLI._format_version_added(doc.pop('version_added', 'historical'), doc.pop('version_added_collection', 'distronode-core'))
        ...
        if doc.get('deprecated', False):
            ...
            if isinstance(doc['deprecated'], dict):
                ...
                try:
                    ...
                    raise DistronodeError("Invalid deprecation documentation structure", orig_exc=e)
            else:
        ...
        if doc.get('seealso', False):
            ...
            for item in doc['seealso']:
                if 'module' in item:
                    ...
                    text.append(DocCLI.warp_fill(DocCLI.tty_ify('Distronode documentation [%s]' % item['ref']),
                                limit - 6, initial_indent=opt_indent[:-2] + "* ", subsequent_indent=opt_indent))
                    text.append(DocCLI.warp_fill(DocCLI.tty_ify(item['description']),
                                limit - 6, initial_indent=opt_indent + '   ', subsequent_indent=opt_indent + '   '))
