...
def get_encrypted_password(password, hashtype='sha512', salt=None, salt_size=None, rounds=None, ident=None):
    ...
    if PASSLIB_AVAILABLE and hashtype not in passlib_mapping and hashtype not in passlib_mapping.values():
        raise DistronodeFilterError(f"{hashtype} is not in the list of supported passlib algorithms: {', '.join(passlib_mapping)}")
    ...
    try:
        ...
        raise DistronodeFilterError(f"Failed to encrypt the password due to: {e}")
...
def commonpath(paths):
    ...
    if not is_sequence(paths):
        raise DistronodeFilterTypeError("|commonpath expects sequence, got %s instead." % type(paths))
