...
- name: Before 2.13, how to set default value if the variable is not defined
  distronode.builtin.debug:
    msg: "Hello {{ lookup('distronode.builtin.env', 'UNDEFINED_VARIABLE') | default('World', True) }}"

- name: Example how to set default value if the variable is not defined
  distronode.builtin.debug:
    msg: "Hello {{ lookup('distronode.builtin.env', 'UNDEFINED_VARIABLE', default='World') }}"

- name: Fail if the variable is not defined by setting default value to 'Undefined'
  distronode.builtin.debug:
    msg: "Hello {{ lookup('distronode.builtin.env', 'UNDEFINED_VARIABLE', default=Undefined) }}"

- name: Fail if the variable is not defined by setting default value to 'undef()'
  distronode.builtin.debug:
    msg: "Hello {{ lookup('distronode.builtin.env', 'UNDEFINED_VARIABLE', default=undef()) }}"
"""
