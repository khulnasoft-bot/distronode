...
DOCUMENTATION = """
    ...
    seealso:
    - plugin_type: lookup
      plugin: distronode.builtin.varnames
...
- name: Produce an error since i dont have 'variablnotename'
  distronode.builtin.debug: msg="{{ q('vars', 'variabl' + myvar) }}"
  ignore_errors: True
  vars:
    variablename: hello
    myvar: notename
...
- name: find several related variables
  distronode.builtin.debug: msg="{{ query('distronode.builtin.vars', 'distronode_play_hosts', 'distronode_play_batch', 'distronode_play_hosts_all') }}"

- name: show values from variables found via varnames (note "*" is used to dereference the list to a 'list of arguments')
  debug: msg="{{ q('vars', *q('varnames', 'distronode_play_.+')) }}"
