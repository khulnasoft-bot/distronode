...
### simple bash script
   ...
   EOF
     ...
     # this should not normally be called by Distronode as we return _meta above
...
### python example with ini config
    ...
    def load_config() -> ConfigParser:
        cp = ConfigParser()
        config_file = os.path.expanduser("~/.config/distronode_inventory_script.cfg")
        cp.read(config_file)
    ...
    def get_api_data(namespace: str, pretty=False) -> str:
        ...
        for host_data in found_data['hosts']:
            for name in host_data.items():
                ...
                # set distronode_host if possible
                if 'address' in found_data[name]:
                    data[name]['_meta']['distronode_host'] = found_data[name]['address']
