...
from distronode.errors import DistronodeOptionsError, DistronodeError, DistronodeRequiredOptionError
                    ...
                    try:
                        for entry in defs[config][ftype]:
                            ...
                            if ftype == 'ini':
                                ...
                                raise DistronodeError('YAML configuration type has not been implemented yet')
                            else:
                                raise DistronodeError('Invalid configuration file type: %s' % ftype)
                            ...
                            if temp_value is not None:
                                ...
                                if 'deprecated' in entry:
                                    if ftype == 'ini':
                                        ...
                                        raise DistronodeError('Unimplemented file type: %s' % ftype)
            ...
            if value is None:
                if defs[config].get('required', False):
                    if not plugin_type or config not in INTERNAL_DEFS.get(plugin_type, {}):
                        raise DistronodeRequiredOptionError("No setting was provided for required configuration %s" %
    ...
    def get_deprecated_msg_from_config(dep_docs, include_removal=False, collection_name=None):
        ...
        if include_removal:
            if 'removed_at_date' in dep_docs:
                ...
                removal = f"Will be removed in: Distronode {dep_docs['removed_in']}\n\t"
