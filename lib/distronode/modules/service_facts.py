...
- name: show names of existing systemd services, sometimes systemd knows about services that were never installed
  ...
  vars:
     known_systemd_services: "{{ distronode_facts['services'].values() | selectattr('source', 'equalto', 'systemd') }}"
...
- name: restart systemd service if it exists
  service:
    state: restarted
    name: ntpd.service
  when: distronode_facts['services']['ntpd.service']['status'] | default('not-found') != 'not-found'
