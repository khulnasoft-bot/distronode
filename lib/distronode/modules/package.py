...
description:
    - This modules manages packages on a target without specifying a package manager module (like M(distronode.builtin.dnf), M(distronode.builtin.apt), ...).
...
options:
  ...
  use:
    description:
      ...
      - Since version 2.17 you can use the C(distronode_package_use) variable to override the automatic detection, but this option still takes precedence.
    default: auto
requirements:
