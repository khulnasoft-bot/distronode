...
options:
  ...
  include_aggregate_mounts:
    description:
      - Whether or not the module should return the C(aggregate_mounts) list in C(distronode_facts).
...
- name: Get NFS mounts during gather_facts with timeout
  ...
  vars:
    distronode_facts_modules:
      - distronode.builtin.mount_facts
  module_default:
    distronode.builtin.mount_facts:
      timeout: 10
      fstypes:
...
distronode_facts:
    description:
      - An distronode_facts dictionary containing a dictionary of C(mount_points) and list of C(aggregate_mounts) when enabled.
      - Each key in C(mount_points) is a mount point, and the value contains mount information (similar to C(distronode_facts["mounts"])).
        Each value also contains the key C(distronode_context), with details about the source and line(s) corresponding to the parsed mount point.
...
from distronode.module_utils.facts import timeout as _timeout
from distronode.module_utils.facts.utils import get_mount_size, get_file_content
...
def get_device_by_uuid(module: DistronodeModule, uuid : str) -> str | None:
...
@functools.lru_cache(maxsize=None)
def run_lsblk(module : DistronodeModule) -> list[list[str]]:
...
@functools.lru_cache(maxsize=None)
def get_udevadm_device_uuid(module : DistronodeModule, device : str) -> str | None:
...
def get_partition_uuid(module: DistronodeModule, partname : str) -> str | None:
...
def run_mount_bin(module: DistronodeModule, mount_bin: str) -> str:  # type: ignore # Missing return statement
...
def get_sources(module: DistronodeModule) -> list[str]:
...
def gen_mounts_by_source(module: DistronodeModule):
...
def get_mount_facts(module: DistronodeModule):
    ...
    for source, mount, origin, fields in gen_mounts_by_source(module):
        ...
        fields.update({"distronode_context": {"source": source, "source_data": origin}, "uuid": uuid})
...
def handle_deduplication(module, mounts):
    ...
    for mount in mounts:
        ...
        source = mount["distronode_context"]["source"]
...
def main():
    ...
    module.exit_json(distronode_facts={"mount_points": mount_points, "aggregate_mounts": aggregate_mounts})
