...
description:
  ...
  - 'When crontab jobs are managed: the module includes one line with the description of the
    crontab entry C("#Distronode: <name>") corresponding to the O(name) passed to the module,
    which is used by future distronode/module calls to find/check the state. The O(name)
...
from distronode.module_utils.common.file import S_IRWU_RWG_RWO
