...
notes:
    ...
    - The M(distronode.builtin.debconf) module does not reconfigure packages, it just updates the debconf database.
...
options:
  ...
  vtype:
    description:
      ...
      - After Distronode 2.17, user can specify C(value) as a list, if C(vtype) is set as V(multiselect).
    type: str
    choices: [ boolean, error, multiselect, note, password, seen, select, string, text, title ]
  value:
    description:
      - Value to set the configuration to.
      - After Distronode 2.17, C(value) is of type C(raw).
    type: raw
    aliases: [ answer ]
  unseen:
    description:
