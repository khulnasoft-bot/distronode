  ...
  validate_certs:
    description:
      - This is effectively a no-op in the dnf5 module as dnf5 itself handles downloading a https url as the source of the rpm,
        but is an accepted parameter for feature parity/compatibility with the M(distronode.builtin.dnf) module.
    type: bool
    default: "yes"
  sslverify:
    description:
...
attributes:
    action:
        details: dnf5 has 2 action plugins that use it under the hood, M(distronode.builtin.dnf) and M(distronode.builtin.package).
        support: partial
    async:
        support: none
    bypass_host_loop:
...
class Dnf5Module(YumDnf):
    ...
    def _ensure_dnf(self):
        ...
        self.module.fail_json(
            ...
            f"or that you have specified the correct distronode_python_interpreter. (attempted {system_interpreters}).",
            failures=[],
        )
