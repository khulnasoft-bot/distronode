...
options:
  ...
  fail_msg:
    description:
      ...
      - This argument was called O(msg) before Distronode 2.7, now it is renamed to O(fail_msg) with alias O(msg).
    type: str
    aliases: [ msg ]
    version_added: "2.7"
  success_msg:
...
- name: A single condition can be supplied as string instead of list
  distronode.builtin.assert:
    that: "distronode_os_family != 'RedHat'"
...
- name: Please use O(msg) when distronode version is smaller than 2.7
  distronode.builtin.assert:
    that:
      - my_param <= 100
      - my_param >= 0
