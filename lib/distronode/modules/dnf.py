...
options:
  use_backend:
    ...
    default: "auto"
    choices:
        auto: Automatically select the backend based on the C(distronode_facts.pkg_mgr) fact.
        yum: Alias for V(auto) (see Notes)
        dnf: M(distronode.builtin.dnf)
        yum4: Alias for V(dnf)
        dnf4: Alias for V(dnf)
        dnf5: M(distronode.builtin.dnf5)
    type: str
    version_added: 2.15
  name:
    description:
  ...
  install_repoquery:
    description:
      ...
      - This option is deprecated and will be removed in distronode-core 2.20.
    type: bool
    default: "yes"
    version_added: "2.7"
  download_only:
  ...
  nobest:
    description:
      ...
      - Since distronode-core 2.17 the default value is set by the operating system distribution.
    required: false
    type: bool
    version_added: "2.11"
  best:
...
attributes:
    action:
        details: dnf has 2 action plugins that use it under the hood, M(distronode.builtin.dnf) and M(distronode.builtin.package).
        support: partial
    async:
        support: none
    bypass_host_loop:
...
notes:
  ...
  - While O(use_backend=yum) and the ability to call the action plugin as
    M(distronode.builtin.yum) are provided for syntax compatibility, the YUM
    backend was removed in distronode-core 2.17 because the required libraries are
    not available for any supported version of Python. If you rely on this
    functionality, use an older version of Distronode.
requirements:
  - python3-dnf
author:
