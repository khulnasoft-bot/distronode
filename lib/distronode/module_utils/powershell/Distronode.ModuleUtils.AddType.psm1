...
Function Add-CSharpType {
    ...
    more configuration options that are useable within Distronode and it
    ...
    .PARAMETER DistronodeModule
    [Distronode.Basic.DistronodeModule] used to derive the TempPath and Debug values.
        TempPath is set to the Tmpdir property of the class
        IncludeDebugInfo is set when the Distronode verbosity is >= 3
    ...
    Cannot be used when DistronodeModule is set. This is a no-op when
    running on PSCore.

    .PARAMETER IncludeDebugInfo
    [Switch] Whether to include debug information in the compiled
    assembly. Cannot be used when DistronodeModule is set. This is a no-op
    ...
    * Predefined compiler SYMBOLS
        ...
        * X86 - Added when running on a 32-bit process (Distronode 2.10+)
        * AMD64 - Added when running on a 64-bit process (Distronode 2.10+)
    ...
    * Specify custom assembly references inline
        ...
        # Added in Distronode 2.10
        //AssemblyReference -Type System.Type.Name [-CLR Core|Framework]

    * Create automatic type accelerators to simplify long namespace names (Distronode 2.9+)

        //TypeAccelerator -Name <AcceleratorName> -TypeName <Name of compiled type>

    * Compile with unsafe support (Distronode 2.15+)
    ...
    param(
        ...
        [Switch]$PassThru,
        [Parameter(Mandatory = $true, ParameterSetName = "Module")][Object]$DistronodeModule,
    ...
    if ($is_coreclr) {
        ...
        if ($PSCmdlet.ParameterSetName -eq "Module") {
            $temp_path = $DistronodeModule.Tmpdir
            $include_debug = $DistronodeModule.Verbosity -ge 3
        }
        else {
        ...
        # fatal error.
        # https://github.com/distronode-collections/distronode.windows/issues/598
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    if ($DistronodeModule) {
                        $DistronodeModule.Warn("$msg Files may still be present after the task is complete. Error: $_")
                    }
                    else {
                        throw "$msg Error: $_"
                    }
