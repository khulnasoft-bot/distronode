...
from distronode.cli import scripts
...
from distronode.module_utils.connection import write_to_stream
...
class TaskExecutor:
    ...
    def _run_loop(self, items):
        ...
        for item_index, item in enumerate(items):
            ...
            if self._connection and not isinstance(self._connection, string_types):
                task_fields['connection'] = getattr(self._connection, 'distronode_name')
    ...
    def _execute(self, variables=None):
        ...
        for attempt in range(1, retries + 1):
            ...
            result["_distronode_no_log"] = bool(no_log or result.get('_distronode_no_log', False))
...
CLI_STUB_NAME = 'distronode_connection_cli_stub.py'
...
def start_connection(play_context, options, task_uuid):
    ...
    if not (cli_stub_path := C.config.get_config_value('_DISTRONODE_CONNECTION_PATH')):
