...
begin {
    ...
    $script:common_functions = {
        Function ConvertFrom-DistronodeJson {
            <#
            .SYNOPSIS
        ...
        Function Format-DistronodeException {
            <#
            .SYNOPSIS
...
"@
    ...
    $script:wrapper_functions = {
        Function Write-DistronodeError {
            <#
            .SYNOPSIS
            Writes an error message to a JSON string in the format that Distronode
            ...
            if ($null -ne $ErrorRecord) {
                ...
                $result.exception = (Format-DistronodeException -ErrorRecord $ErrorRecord)
            }
            Write-Output -InputObject (ConvertTo-Json -InputObject $result -Depth 99 -Compress)
        }

        Function Write-DistronodeLog {
            <#
            .SYNOPSIS
            ...
            can be manually set to enable it. Manually set DISTRONODE_EXEC_DEBUG as
            ...
            $log_path = $env:DISTRONODE_EXEC_DEBUG
            if ($log_path) {
    ...
    Write-DistronodeLog "INFO - starting exec_wrapper" "exec_wrapper"
    if (-not $json_raw) {
        Write-DistronodeError -Message "internal error: no input given to PowerShell exec wrapper"
        exit 1
    }

    Write-DistronodeLog "INFO - converting json raw to a payload" "exec_wrapper"
    $payload = ConvertFrom-DistronodeJson -InputObject $json_raw
    $payload.module_args._distronode_exec_wrapper_warnings = [System.Collections.Generic.List[string]]@()
    ...
    if ($payload.min_os_version) {
        ...
        Write-DistronodeLog "INFO - checking if actual os version '$actual_os_version' is less than the min os version '$min_os_version'" "exec_wrapper"
        if ($actual_os_version -lt $min_os_version) {
            ...
            Write-DistronodeError -Message $msg
            exit 1
        }
    }
    if ($payload.min_ps_version) {
        ...
        Write-DistronodeLog "INFO - checking if actual PS version '$actual_ps_version' is less than the min PS version '$min_ps_version'" "exec_wrapper"
    ...
    Write-DistronodeLog "INFO - running action $action" "exec_wrapper"
    ...
    try {
        ...
        Write-DistronodeError -Message "internal error: failed to run exec_wrapper action $action" -ErrorRecord $_
        exit 1
    }
    Write-DistronodeLog "INFO - ending exec_wrapper" "exec_wrapper"
}
