...
#DistronodeRequires -Wrapper module_wrapper

$ErrorActionPreference = "Stop"

Write-DistronodeLog "INFO - starting module_powershell_wrapper" "module_powershell_wrapper"

$module_name = $Payload.module_args["_distronode_module_name"]
Write-DistronodeLog "INFO - building module payload for '$module_name'" "module_powershell_wrapper"
...
foreach ($csharp_util in $Payload.csharp_utils_module) {
    Write-DistronodeLog "INFO - adding $csharp_util to list of C# references to compile" "module_powershell_wrapper"
...
if ($csharp_utils.Count -gt 0) {
    ...
    $warning_func = New-Object -TypeName System.Management.Automation.PSScriptMethod -ArgumentList Warn, {
        param($message)
        $Payload.module_args._distronode_exec_wrapper_warnings.Add($message)
    }
    $fake_module.PSObject.Members.Add($warning_func)
    Add-CSharpType -References $csharp_utils -DistronodeModule $fake_module
}
...
try {
    ...
    # output a pretty error for Distronode to parse
    $result = @{
        msg = "Failed to invoke PowerShell module: $($_.Exception.Message)"
        failed = $true
        exception = (Format-DistronodeException -ErrorRecord $_)
    }
    Write-Output -InputObject (ConvertTo-Json -InputObject $result -Depth 99 -Compress)
    $host.SetShouldExit(1)
}

Write-DistronodeLog "INFO - ending module_powershell_wrapper" "module_powershell_wrapper"
