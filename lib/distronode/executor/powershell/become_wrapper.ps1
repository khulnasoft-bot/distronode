...
#Requires -Module Distronode.ModuleUtils.AddType
#DistronodeRequires -CSharpUtil Distronode.AccessToken
#DistronodeRequires -CSharpUtil Distronode.Become

$ErrorActionPreference = "Stop"

Write-DistronodeLog "INFO - starting become_wrapper" "become_wrapper"
...
Function Get-BecomeFlag($flags) {
    $logon_type = [Distronode.AccessToken.LogonType]::Interactive
    $logon_flags = [Distronode.Become.LogonFlags]::WithProfile
    ...
    foreach ($flag in $flag_split) {
        ...
        if ($flag_key -eq "logon_type") {
            $enum_details = @{
                enum = [Distronode.AccessToken.LogonType]
                flag_type = $flag_key
                value = $flag_value
            }
        ...
        elseif ($flag_key -eq "logon_flags") {
            ...
            $logon_flags = 0 -as [Distronode.Become.LogonFlags]
            foreach ($logon_flag_value in $logon_flag_values) {
                if ($logon_flag_value -eq "") {
                    continue
                }
                $enum_details = @{
                    enum = [Distronode.Become.LogonFlags]
                    flag_type = $flag_key
    ...
    return $logon_type, [Distronode.Become.LogonFlags]$logon_flags
}

Write-DistronodeLog "INFO - loading C# become code" "become_wrapper"
$add_type_b64 = $Payload.powershell_modules["Distronode.ModuleUtils.AddType"]
$add_type = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($add_type_b64))
New-Module -Name Distronode.ModuleUtils.AddType -ScriptBlock ([ScriptBlock]::Create($add_type)) | Import-Module > $null

$new_tmp = [System.Environment]::ExpandEnvironmentVariables($Payload.module_args["_distronode_remote_tmp"])
$access_def = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($Payload.csharp_utils["Distronode.AccessToken"]))
$become_def = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($Payload.csharp_utils["Distronode.Become"]))
$process_def = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($Payload.csharp_utils["Distronode.Process"]))
...
try {
    ...
    Write-DistronodeError -Message "internal error: failed to parse become_flags '$($Payload.become_flags)'" -ErrorRecord $_
    $host.SetShouldExit(1)
    return
}
Write-DistronodeLog "INFO - parsed become input, user: '$username', type: '$logon_type', flags: '$logon_flags'" "become_wrapper"
...
try {
    Write-DistronodeLog "INFO - starting become process '$lp_command_line'" "become_wrapper"
    $result = [Distronode.Become.BecomeUtil]::CreateProcessAsUser($username, $password, $logon_flags, $logon_type,
        $null, $lp_command_line, $lp_current_directory, $null, $exec_wrapper)
    Write-DistronodeLog "INFO - become process complete with rc: $($result.ExitCode)" "become_wrapper"
    $stdout = $result.StandardOut
    try {
        $stdout = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($stdout))
    }
    catch [FormatException] {
        # output wasn't Base64, ignore as it may contain an error message we want to pass to Distronode
        Write-DistronodeLog "WARN - become process stdout was not base64 encoded as expected: $stdout"
    }
    ...
    Write-DistronodeError -Message "internal error: failed to become user '$username'" -ErrorRecord $_
    $host.SetShouldExit(1)
}

Write-DistronodeLog "INFO - ending become_wrapper" "become_wrapper"
